...
def build_fund_basic_rows() -> List[Dict[str, str]]:
    # fund_basic 文档 doc_id = 43（基金基础信息，可用于筛选 ETF）
    fields = tcache.get_interface_fields("fund_basic", doc_id=43, force_refresh=False)
...
def append_to_fields_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 fund_basic 行
    existing = [r for r in existing if r.get("数据集") != "fund_basic"]
    ...
    print(f"追加 {len(new_rows)} 条 fund_basic 规范字段到 {FIELDS_CSV}")
...
def append_to_mapping_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 fund_basic + tushare_fund_basic 行
    existing = [
        r
        for r in existing
        if not (
            r.get("数据集") == "fund_basic" and r.get("来源系统") == "tushare_fund_basic"
        )
    ]
    ...
    cache = tcache.load_cache().get("fund_basic", {})
    src_fields = cache.get("fields", [])

    new_rows: List[Dict[str, str]] = []
    for r in rows:
        key = (r["数据集"], r["规范字段名"], "tushare_fund_basic")
    ...
    print(f"追加 {len(new_rows)} 条 fund_basic 映射到 {MAPPING_CSV}")


def main() -> None:
    rows = build_fund_basic_rows()
