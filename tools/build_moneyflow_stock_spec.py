...
def build_moneyflow_stock_rows() -> List[Dict[str, str]]:
    # moneyflow 文档 doc_id = 170（个股资金流向）
    fields = tcache.get_interface_fields("moneyflow", doc_id=170, force_refresh=False)
...
def append_to_fields_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 moneyflow_stock 行
    existing = [r for r in existing if r.get("数据集") != "moneyflow_stock"]
    ...
    print(f"追加 {len(new_rows)} 条 moneyflow_stock 规范字段到 {FIELDS_CSV}")
...
def append_to_mapping_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 moneyflow_stock + tushare_moneyflow_stock 行
    existing = [
        r
        for r in existing
        if not (
            r.get("数据集") == "moneyflow_stock" and r.get("来源系统") == "tushare_moneyflow_stock"
        )
    ]
    ...
    cache = tcache.load_cache().get("moneyflow", {})
    src_fields = cache.get("fields", [])

    new_rows: List[Dict[str, str]] = []
    for r in rows:
        key = (r["数据集"], r["规范字段名"], "tushare_moneyflow_stock")
    ...
    print(f"追加 {len(new_rows)} 条 moneyflow_stock 映射到 {MAPPING_CSV}")


def main() -> None:
    rows = build_moneyflow_stock_rows()
