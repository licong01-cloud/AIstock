...
def build_moneyflow_ind_dc_rows() -> List[Dict[str, str]]:
    # moneyflow_ind_dc 文档 doc_id = 344
    fields = tcache.get_interface_fields("moneyflow_ind_dc", doc_id=344, force_refresh=False)
...
def append_to_fields_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 moneyflow_ind_dc 行
    existing = [r for r in existing if r.get("数据集") != "moneyflow_ind_dc"]
    ...
    print(f"追加 {len(new_rows)} 条 moneyflow_ind_dc 规范字段到 {FIELDS_CSV}")
...
def append_to_mapping_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 moneyflow_ind_dc + tushare_moneyflow_ind_dc 行
    existing = [
        r
        for r in existing
        if not (
            r.get("数据集") == "moneyflow_ind_dc" and r.get("来源系统") == "tushare_moneyflow_ind_dc"
        )
    ]
    ...
    cache = tcache.load_cache().get("moneyflow_ind_dc", {})
    src_fields = cache.get("fields", [])

    new_rows: List[Dict[str, str]] = []
    for r in rows:
        key = (r["数据集"], r["规范字段名"], "tushare_moneyflow_ind_dc")
        ...
        new_rows.append(
            {
                "数据集": r["数据集"],
                "规范字段名": r["规范字段名"],
                "来源系统": "tushare_moneyflow_ind_dc",
                "来源接口": "moneyflow_ind_dc",
                "来源字段名": r["规范字段名"],
                "来源数据类型": r["数据类型"],
                "来源单位": "无",
                "转换系数": "1.0",
    ...
    print(f"追加 {len(new_rows)} 条 moneyflow_ind_dc 映射到 {MAPPING_CSV}")


def main() -> None:
    rows = build_moneyflow_ind_dc_rows()
