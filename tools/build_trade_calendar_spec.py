...
# trade_cal 文档 doc_id = 26（见 tushare_atomic_api）
TRADE_CAL_DOC_ID = 26


def build_trade_calendar_rows() -> List[Dict[str, str]]:
    fields = tcache.get_interface_fields("trade_cal", doc_id=TRADE_CAL_DOC_ID, force_refresh=False)
    ...
    for f in fields:
        ...
        fmt = ""
        # 简单类型与格式约定
        if name in {"cal_date", "pretrade_date"}:
            fmt = "YYYY-MM-DD"
        if name == "is_open":
            ftype = ftype or "int"
        row: Dict[str, str] = {
...
def append_to_fields_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 trade_calendar 行
    existing = [r for r in existing if r.get("数据集") != "trade_calendar"]
    ...
    print(f"追加 {len(new_rows)} 条 trade_calendar 规范字段到 {FIELDS_CSV}")
...
def append_to_mapping_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 trade_calendar + tushare_trade_cal 行
    existing = [
        r
        for r in existing
        if not (r.get("数据集") == "trade_calendar" and r.get("来源系统") == "tushare_trade_cal")
    ]
    ...
    cache = tcache.load_cache().get("trade_cal", {})
    src_fields = cache.get("fields", [])

    new_rows: List[Dict[str, str]] = []
    for r in rows:
        key = (r["数据集"], r["规范字段名"], "tushare_trade_cal")
    ...
    print(f"追加 {len(new_rows)} 条 trade_calendar 映射到 {MAPPING_CSV}")


def main() -> None:
    rows = build_trade_calendar_rows()
