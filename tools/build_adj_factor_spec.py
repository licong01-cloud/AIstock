...
# adj_factor 文档 doc_id = 28
ADJ_FACTOR_DOC_ID = 28


def build_adj_factor_rows() -> List[Dict[str, str]]:
    fields = tcache.get_interface_fields("adj_factor", doc_id=ADJ_FACTOR_DOC_ID, force_refresh=False)
    ...
    for f in fields:
        ...
        if name == "trade_date":
            fmt = "YYYY-MM-DD"
        if name == "adj_factor" and ftype in {"float", "double"}:
            precision = "6"
            fmt = "%.6f"
        row: Dict[str, str] = {
            "数据集": "adj_factor_daily",
            "规范字段名": name,
            "中文字段名": cn,
            "数据类型": ftype,
            "单位": unit,
...
def append_to_fields_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 adj_factor_daily 行
    existing = [r for r in existing if r.get("数据集") != "adj_factor_daily"]
    ...
    print(f"追加 {len(new_rows)} 条 adj_factor_daily 规范字段到 {FIELDS_CSV}")
...
def append_to_mapping_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 adj_factor_daily + tushare_adj_factor 行
    existing = [
        r
        for r in existing
        if not (
            r.get("数据集") == "adj_factor_daily" and r.get("来源系统") == "tushare_adj_factor"
        )
    ]
    ...
    cache = tcache.load_cache().get("adj_factor", {})
    src_fields = cache.get("fields", [])

    new_rows: List[Dict[str, str]] = []
    for r in rows:
        key = (r["数据集"], r["规范字段名"], "tushare_adj_factor")
    ...
    print(f"追加 {len(new_rows)} 条 adj_factor_daily 映射到 {MAPPING_CSV}")


def main() -> None:
    rows = build_adj_factor_rows()
