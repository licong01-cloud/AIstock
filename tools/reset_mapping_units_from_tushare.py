from __future__ import annotations

import csv
import json
from pathlib import Path
from typing import Dict, Tuple

PROJECT_ROOT = Path(__file__).resolve().parents[2]
DOCS_DIR = PROJECT_ROOT / "docs"
MAPPING_CSV = DOCS_DIR / "data_schema_source_mapping.csv"
FIELDS_CACHE = DOCS_DIR / "tushare_fields_cache.json"


def load_fields_cache() -> Dict[str, Dict]:
    with FIELDS_CACHE.open("r", encoding="utf-8") as f:
        return json.load(f)


def infer_unit_from_text(text: str) -> str:
    if not text:
        return "无"
    t = text.replace(" ", "")
    if "%" in t or "百分比" in t or "涨跌幅" in t or "股息率" in t:
        return "%"
    if "千元" in t:
        return "千元"
    if "万元" in t:
        return "万元"
    if "元CNY" in t or "元cny" in t or "元Cny" in t:
        return "元"
    if "(元" in t or "（元" in t or "元)" in t or "元）" in t:
        return "元"
    if "万股" in t:
        return "万股"
    if "单位：手" in t or "(手" in t or "（手" in t or "手)" in t or "手）" in t:
        return "手"
    return "无"


def build_ts_index(cache: Dict[str, Dict]) -> Dict[Tuple[str, str], Dict]:
    idx: Dict[Tuple[str, str], Dict] = {}
    for api_name, meta in cache.items():
        for f in meta.get("fields", []) or []:
            name = f.get("name")
            if not name:
                continue
            idx[(api_name, name)] = f
    return idx


def main() -> None:
    cache = load_fields_cache()
    ts_index = build_ts_index(cache)

    with MAPPING_CSV.open("r", encoding="utf-8-sig", newline="") as f:
        rows = list(csv.DictReader(f))

    updated = 0
    for r in rows:
        src_system = (r.get("来源系统") or "").strip()
        src_api = (r.get("来源接口") or "").strip()
        src_field = (r.get("来源字段名") or "").strip()
        if src_system.startswith("tushare_"):
            api_name = src_system[len("tushare_") :]
        else:
            api_name = src_api
        if not api_name or not src_field:
            continue
        meta = ts_index.get((api_name, src_field))
        if not meta:
            continue
        desc = meta.get("desc") or meta.get("cn") or ""
        unit = infer_unit_from_text(desc)
        # 直接用 Tushare 推断结果覆盖来源单位（回退掉规范表同步的影响）
        if unit != (r.get("来源单位") or "").strip():
            r["来源单位"] = unit
            updated += 1

    fieldnames = list(rows[0].keys()) if rows else []
    with MAPPING_CSV.open("w", encoding="utf-8-sig", newline="") as f:
        w = csv.DictWriter(f, fieldnames=fieldnames)
        w.writeheader()
        for r in rows:
            w.writerow({k: r.get(k, "") for k in fieldnames})

    print(f"reset 来源单位 from Tushare meta for {updated} rows")


if __name__ == "__main__":
    main()
