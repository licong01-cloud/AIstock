...
def build_daily_basic_rows() -> List[Dict[str, str]]:
    # 不强制 doc_id，走缓存里的元数据
    fields = tcache.get_interface_fields("daily_basic", doc_id=None, force_refresh=False)
...
def append_to_fields_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 daily_basic 行
    existing = [r for r in existing if r.get("数据集") != "daily_basic"]
    ...
    print(f"追加 {len(new_rows)} 条 daily_basic 规范字段到 {FIELDS_CSV}")
...
def append_to_mapping_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 daily_basic + tushare_daily_basic 行
    existing = [
        r
        for r in existing
        if not (r.get("数据集") == "daily_basic" and r.get("来源系统") == "tushare_daily_basic")
    ]
    ...
    cache = tcache.load_cache().get("daily_basic", {})
    src_fields = cache.get("fields", [])

    new_rows: List[Dict[str, str]] = []
    for r in rows:
        key = (r["数据集"], r["规范字段名"], "tushare_daily_basic")
    ...
    print(f"追加 {len(new_rows)} 条 daily_basic 映射到 {MAPPING_CSV}")


def main() -> None:
    rows = build_daily_basic_rows()
