...
# tdx_member 文档 doc_id = 377
TDX_MEMBER_DOC_ID = 377


def build_tdx_member_rows() -> List[Dict[str, str]]:
    fields = tcache.get_interface_fields("tdx_member", doc_id=TDX_MEMBER_DOC_ID, force_refresh=False)
...
def append_to_fields_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 tdx_member 行
    existing = [r for r in existing if r.get("数据集") != "tdx_member"]
    ...
    print(f"追加 {len(new_rows)} 条 tdx_member 规范字段到 {FIELDS_CSV}")
...
def append_to_mapping_csv(rows: List[Dict[str, str]]) -> None:
    ...
    # 删除旧的 tdx_member + tushare_tdx_member 行
    existing = [
        r
        for r in existing
        if not (r.get("数据集") == "tdx_member" and r.get("来源系统") == "tushare_tdx_member")
    ]
    ...
    cache = tcache.load_cache().get("tdx_member", {})
    src_fields = cache.get("fields", [])

    new_rows: List[Dict[str, str]] = []
    for r in rows:
        key = (r["数据集"], r["规范字段名"], "tushare_tdx_member")
    ...
    print(f"追加 {len(new_rows)} 条 tdx_member 映射到 {MAPPING_CSV}")


def main() -> None:
    rows = build_tdx_member_rows()
