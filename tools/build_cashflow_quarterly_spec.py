...
# cashflow 文档 doc_id = 44
CASHFLOW_DOC_ID = 44


def build_cashflow_rows() -> List[Dict[str, str]]:
    fields = tcache.get_interface_fields("cashflow", doc_id=CASHFLOW_DOC_ID, force_refresh=False)
...
def main() -> None:
    rows = build_cashflow_rows()
    _append_to_fields_csv("cashflow_quarterly", rows)
    _append_to_mapping_csv("cashflow_quarterly", "tushare_cashflow", "cashflow", rows, "现金流量表季度")
