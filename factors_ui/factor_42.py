...
def calculate_PVF_Sync_10D():
    ...
    # 按 instrument 分组计算三个相关系数
    corr_r_delta_v = temp_df.groupby(level="instrument").apply(lambda x: rolling_corr_pair(x, "r_t", "delta_v_t", window=10))
    corr_r_mf = temp_df.groupby(level="instrument").apply(lambda x: rolling_corr_pair(x, "r_t", "mf_main_net_amt_ratio_t", window=10))
    corr_delta_v_mf = temp_df.groupby(level="instrument").apply(lambda x: rolling_corr_pair(x, "delta_v_t", "mf_main_net_amt_ratio_t", window=10))
