...
def calculate_mf_elg_net_amt_ratio_stability_5D():
    ...
    if missing:
        ...
        # 计算5日滚动均值和标准差，min_periods=3 以提高覆盖率
        rolling_mean = mf_elg_net_amt_ratio.groupby(level="instrument").rolling(window=5, min_periods=3).mean()
        rolling_std = mf_elg_net_amt_ratio.groupby(level="instrument").rolling(window=5, min_periods=3).std()
        # 重置索引以对齐原始df
        rolling_mean = rolling_mean.reset_index(level=0, drop=True)
        rolling_std = rolling_std.reset_index(level=0, drop=True)
        
        # 计算变异系数倒数：|mean| / std，显式避免除零
