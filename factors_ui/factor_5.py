...
def calculate_mf_main_net_amt_ratio_5d():
    ...
    # 检查资金流列是否存在，用于计算 mf_main_net_amt
    ...
    if missing_flow:
        ...
        window = 5
        main_net_sum = daily_main_net_amt.groupby(level="instrument").rolling(window=window, min_periods=window).sum()
        ...
        # 计算比率，分母为零时设为 NaN
        series = main_net_sum.div(amount_sum).where(amount_sum != 0, np.nan)
