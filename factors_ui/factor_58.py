...
def calculate_SizeAdjElgNet_5D():
    ...
    # 当成交额为 0 或 NaN 时，设置因子值为 NaN
    elg_net_ratio_t = np.where(denominator == 0, np.nan, numerator / denominator)
    elg_net_ratio_t = np.where(pd.isna(denominator), np.nan, elg_net_ratio_t)
    elg_net_ratio_t = pd.Series(elg_net_ratio_t, index=df.index, dtype="float64")

    # 计算过去5个交易日的滚动均值，min_periods=5 确保信号稳定性
    mean_5D = elg_net_ratio_t.groupby(level="instrument").rolling(window=5, min_periods=5).mean()
