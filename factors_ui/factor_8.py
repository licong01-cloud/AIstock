...
def calculate_mf_main_net_amt_ratio_5d():
    ...
    if missing_flow:
        ...
        # 使用 groupby + rolling 计算 5 日滚动和，min_periods=5 确保有足够数据点
        window = 5
        ...
        amount_5d = df["amount"].groupby(level="instrument").rolling(window=window, min_periods=window).sum()
        amount_5d = amount_5d.reset_index(level=0, drop=True)  # 恢复索引对齐
        
        # 计算 5 日主力净流入强度，分母为零时返回 NaN
        series = net_amt_5d / amount_5d
        series = series.where(amount_5d != 0, np.nan)
