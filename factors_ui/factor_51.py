...
def calculate_PVF_Sync_10D():
    ...
    if all(col in df.columns for col in mf_cols):
        ...
        # 安全除法，避免除零或缺失值
        mf_main_net_amt_ratio_t = np.where(df["amount"] > 0, mf_main_net_amt / df["amount"], np.nan)
        mf_available = True
    else:
    ...
    # 计算三个滚动相关系数
    # 1. 价格与成交量的相关系数
    corr_r_delta_v = temp_df.groupby(level="instrument", group_keys=False).apply(
        lambda x: rolling_corr_pair(x, "r_t", "delta_v_t")
    )
    
    # 2. 价格与主力净流入强度的相关系数（仅在资金流可用时计算）
    if mf_available:
        corr_r_mf = temp_df.groupby(level="instrument", group_keys=False).apply(
            lambda x: rolling_corr_pair(x, "r_t", "mf_main_net_amt_ratio_t")
        )
    else:
    ...
    if mf_available:
        corr_delta_v_mf = temp_df.groupby(level="instrument", group_keys=False).apply(
            lambda x: rolling_corr_pair(x, "delta_v_t", "mf_main_net_amt_ratio_t")
        )
    else:
