...
def calculate_mf_elg_net_amt_ratio_stability_5D():
    ...
    if missing:
        raise ValueError(f"Missing columns: {missing}. Please ensure static_factors.parquet contains mf_elg_net_amt_ratio.")
    ...
    # 当标准差为零时：若均值也为零，则返回NaN；否则返回一个大的有限值（例如1e6）表示极端稳定
    series = np.where(rolling_std > 0, abs_mean / rolling_std,
                     np.where(abs_mean > 0, 1e6, np.nan))
