...
def calculate_mf_elg_net_amt_ratio_stability_5D():
    ...
    if missing:
        ...
        # 计算变异系数倒数：|mean| / std，正确处理边界情况
        abs_mean = np.abs(rolling_mean)
        # 当标准差为零时：若均值也为零，则返回NaN；否则返回inf表示极端稳定
        series = np.where(rolling_std > 0, abs_mean / rolling_std,
                         np.where(abs_mean > 0, np.inf, np.nan))
