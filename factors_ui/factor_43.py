...
def calculate_SizeAdjElgNet_5D():
    ...
    # 处理分母为零或NaN的情况，设为NaN
    mf_elg_net_amt_ratio_t = np.where(denominator == 0, np.nan, numerator / denominator)
    mf_elg_net_amt_ratio_t = np.where(pd.isna(denominator), np.nan, mf_elg_net_amt_ratio_t)
    mf_elg_net_amt_ratio_t = pd.Series(mf_elg_net_amt_ratio_t, index=df.index, dtype="float64")

    # 计算过去5个交易日的滚动均值
    mean_5D = mf_elg_net_amt_ratio_t.groupby(level="instrument").rolling(window=5, min_periods=5).mean()
