import pandas as pd
import numpy as np

def calculate_mf_elg_share_stability_5D():
    """根据给定因子定义计算因子值，并写入 result.h5"""

    # 1. 读取数据并按索引排序（索引应为 MultiIndex(datetime, instrument)）
    df = pd.read_hdf("daily_pv.h5", key="data").sort_index()

    # 2. 再次确保索引按 (datetime, instrument) 排序
    df = df.sort_index()

    # 3. 读取静态因子数据并合并
    try:
        static_df = pd.read_parquet("static_factors.parquet").sort_index()
        df = df.join(static_df, how="left")
    except FileNotFoundError:
        # 如果 static_factors.parquet 文件不存在，降级处理
        print("Warning: static_factors.parquet not found. Outputting all NaN values.")
        series = pd.Series(np.nan, index=df.index, dtype="float64")
        result_df = pd.DataFrame(index=df.index)
        result_df["mf_elg_share_stability_5D"] = series.astype("float64")
        result_df.index.names = df.index.names
        result_df = result_df.sort_index()
        result_df.to_hdf("result.h5", key="data", mode="w")
        return result_df
    except Exception as e:
        # 其他异常情况，降级处理
        print(f"Warning: Error reading static factors: {e}. Outputting all NaN values.")
        series = pd.Series(np.nan, index=df.index, dtype="float64")
        result_df = pd.DataFrame(index=df.index)
        result_df["mf_elg_share_stability_5D"] = series.astype("float64")
        result_df.index.names = df.index.names
        result_df = result_df.sort_index()
        result_df.to_hdf("result.h5", key="data", mode="w")
        return result_df

    # 4. ==== BEGIN FACTOR COMPUTATION AREA ====
    # 检查所需原始字段是否存在
    required_cols = ["mf_elg_buy_amt", "mf_elg_sell_amt", "mf_lg_buy_amt", "mf_lg_sell_amt"]
    missing = [c for c in required_cols if c not in df.columns]
    if missing:
        # 如果静态字段缺失，输出全 NaN 并打印警告
        print(f"Warning: Missing columns: {missing}. Outputting all NaN values.")
        series = pd.Series(np.nan, index=df.index, dtype="float64")
    else:
        # 计算特大单净流入占主力净流入比例 mf_elg_share
        numerator = df["mf_elg_buy_amt"] - df["mf_elg_sell_amt"]
        denominator = (df["mf_lg_buy_amt"] + df["mf_elg_buy_amt"]) - (df["mf_lg_sell_amt"] + df["mf_elg_sell_amt"])
        mf_elg_share = numerator / denominator
        # 处理分母为0或缺失的情况
        mf_elg_share = mf_elg_share.replace([np.inf, -np.inf], np.nan)
        
        # 计算5日滚动均值和标准差
        rolling_mean = mf_elg_share.groupby(level="instrument").rolling(window=5, min_periods=5).mean()
        rolling_std = mf_elg_share.groupby(level="instrument").rolling(window=5, min_periods=5).std()
        # 重置索引以对齐
        rolling_mean = rolling_mean.reset_index(level=0, drop=True)
        rolling_std = rolling_std.reset_index(level=0, drop=True)
        
        # 计算稳定性因子：1 - (标准差 / 绝对均值)
        # 避免除零，当绝对均值为0时设为NaN
        abs_mean = rolling_mean.abs()
        cv = rolling_std / abs_mean
        cv = cv.replace([np.inf, -np.inf], np.nan)  # 处理绝对均值为0的情况
        series = 1 - cv
    # ==== END FACTOR COMPUTATION AREA ====

    # 5. 构造结果 DataFrame：索引必须与 df.index 完全一致
    result_df = pd.DataFrame(index=df.index)
    result_df["mf_elg_share_stability_5D"] = series.astype("float64")

    # 6. 索引名称必须直接继承 df.index.names，禁止手写 ["datetime", "instrument"]
    result_df.index.names = df.index.names

    # 7. 按索引排序并写入 result.h5
    result_df = result_df.sort_index()
    result_df.to_hdf("result.h5", key="data", mode="w")

    return result_df

if __name__ == "__main__":
    calculate_mf_elg_share_stability_5D()