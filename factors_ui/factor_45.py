...
def calculate_PVF_Sync_10D():
    ...
    corr_r_delta_v = temp_df.groupby(level="instrument").apply(
        lambda g: g["r_t"].rolling(window=window, min_periods=window).corr(g["delta_v_t"])
    )
    # 清理索引：移除多级索引中的 instrument 级别
    corr_r_delta_v = corr_r_delta_v.droplevel(level=0)
    
    # 2. 价格与主力净流入强度的相关系数（仅在资金流可用时计算）
    if mf_available:
        corr_r_mf = temp_df.groupby(level="instrument").apply(
            lambda g: g["r_t"].rolling(window=window, min_periods=window).corr(g["mf_main_net_amt_ratio_t"])
        )
        corr_r_mf = corr_r_mf.droplevel(level=0)
    else:
        # 如果资金流不可用，设为 NaN
    ...
    if mf_available:
        corr_delta_v_mf = temp_df.groupby(level="instrument").apply(
            lambda g: g["delta_v_t"].rolling(window=window, min_periods=window).corr(g["mf_main_net_amt_ratio_t"])
        )
        corr_delta_v_mf = corr_delta_v_mf.droplevel(level=0)
    else:
    ...
    # 计算三个相关系数的平均值
    # 如果资金流不可用，则只使用价格和成交量的相关系数（双因子版本）
    if mf_available:
        ...
        # 可选：打印警告信息（不影响输出）
        print("Warning: Capital flow data not available. Using price-volume correlation only.")
