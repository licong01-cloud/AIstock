...
def calculate_SizeAdjElgNet_5D():
    ...
    # 检查所需列是否存在
    required_cols = ["mf_elg_net_amt_ratio", "db_circ_mv"]
    missing = [c for c in required_cols if c not in df.columns]
    if missing:
        raise ValueError(f"Missing columns: {missing}. Please redesign factor using available fields.")

    # 使用预计算的 mf_elg_net_amt_ratio 作为特大单净流入强度
    elg_net_ratio = df["mf_elg_net_amt_ratio"].copy()
    
    # 计算过去5个交易日的滚动均值，min_periods设为1以降低稀疏度
    mean_5D = elg_net_ratio.groupby(level="instrument").rolling(window=5, min_periods=1).mean()
