...
def test_simulation_stable_profile_calls_stable_backend_facades() -> None:
    ...
    async def exercise() -> None:
        mcp, _registry = create_gateway(profile="simulation_stable", transport=httpx.MockTransport(handler))
        ...
        await mcp.call_tool("simulation_runtime_monitoring_list_accounts", {"payload": {"limit": 3}})
    ...
    assert seen == [
        ...
        (
            "GET",
            "http://127.0.0.1:8001/api/v1/simulation-runtime/localsim/accounts?limit=3",
            None,
        ),
...
def test_simulation_confirmed_tools_require_confirmation_before_http() -> None:
    seen: list[tuple[str, str, dict | None]] = []
