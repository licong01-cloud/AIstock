...
- First incremental sync for `cyq_perf` without audit/table cursor must use the declared bootstrap start date and catch up to the target date, not silently fetch only today.
...
| L1 | `cyq_perf` engine path | `DATASET_REGISTRY["cyq_perf"]` exists, is `BY_DATE`, uses pagination, reads incremental cursor from audit, bootstraps from declared start date when no cursor exists, validates returned trade_date, and writes `dataset_date_refresh_audit`. |
| L1 | Scheduler | `cyq_perf` routes to `_run_tushare_engine_sync`; `cyq_chips` remains on legacy script; `_data_freshness_check` does not call alerter flush; delayed retries persist due targets. |
