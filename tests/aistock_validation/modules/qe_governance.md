...
| QE-GOV-L2-003A protected asset ledger | `python -m pytest backend/tests/strategy_package/test_package_asset_ledger.py -q` | StrategyPackage protected asset references are recorded as metadata without modifying frozen manifests or asset files. | Ledger only; no file copy/delete and no protected asset writes. |
...
| QE-GOV-L2-004A Paper original retest gate | `python -m pytest backend/tests/strategy_package/test_repository_service.py -q -k "enable_paper"` | Paper enablement fails fast until an `original_fixed_weight` validation run has `PASSED` for the current manifest. | Does not execute Paper v2; gate is repository/service-level only. |
...
| QE-GOV-L4-003B Phase 7 stability scoring | `python -m pytest backend/tests/strategy_package/test_validation_stability.py -q` | Seed/regime stability summaries are computed read-only from validation-run evidence and explicitly report insufficient evidence. | Does not mutate validation rows, StrategyPackage manifests, Paper state, or production DB. |
...
| Phase 2 asset ledger foundation | Phase 0 gates plus QE-GOV-L2-003/003A and protected asset scan. | StrategyPackage asset refs are governed metadata rows; recording them does not mutate manifests or asset files. |
| Phase 3 original retest Paper gate | Phase 0 gates plus QE-GOV-L2-004/004A and QE-GOV-L4-003 evidence. | StrategyPackage cannot be Paper-enabled until original fixed-weight retest has passed for the current manifest. |
...
| Phase 7 latest/rolling validation | Phase 0 gates plus QE-GOV-L2-004, QE-GOV-L4-003/003A/003B, and migration comment checks. | Latest-data and rolling validation evidence is appended as `package_validation_run` rows; stability scoring is read-only and never overwrites original QE metrics or frozen StrategyPackage manifests. |
