...
Windows runner 的 workflow shell 必须显式解析到 Git Bash（禁止落到 WSL `System32\\bash.exe`）。每个使用远端 `actions/checkout` 的 self-hosted job 必须先清空 wrapper 遗留的 alternate 环境值，再在 checkout 前验证只读本地 Git object mirror 的 manifest、仓库 identity、bare 状态与 main commit；workspace 是否尚未初始化必须通过 `.git/HEAD` 文件存在性判断，禁止在 GitHub Actions 的 fail-fast PowerShell 中用预期失败的 `git rev-parse` 作为空目录探针。发现 workspace 为空仓库时，先从该 mirror 本地播种 main，再通过 job 级 `GIT_ALTERNATE_OBJECT_DIRECTORIES` 让 checkout 及其后的 base/changed-file Git 命令共享同一已验证对象视图。mirror 缺失或校验失败时必须把该变量明确置空、保留可见 warning、以成功的 preflight 状态降级为有界远程 checkout，不新增 PR 门禁；mirror 由单一维护 helper 从干净且 `main == origin/main` 的 canonical root 在互斥锁内离线更新，只写 runner prebuilt 根下的精确 bare repo 与 manifest，不访问网络、不控制 runner 或业务进程。checkout 仍使用浅深度和当前 PR base SHA；仅在 mirror 缺少对象时远程补取。所有 self-hosted workflow 必须设置 `GIT_HTTP_LOW_SPEED_LIMIT=524288`、`GIT_HTTP_LOW_SPEED_TIME=30`，并通过 `GIT_CONFIG_COUNT=1`、`GIT_CONFIG_KEY_0=http.version`、`GIT_CONFIG_VALUE_0=HTTP/1.1` 向 Git 子进程注入 HTTP/1.1：连续 30 秒低于 512 KiB/s 的停滞或不可用慢速传输必须终止当前 Git HTTP 请求，并只交由 checkout 的既有有限重试处理。每个 `actions/checkout` step 必须设置 `timeout-minutes: 5`；随后以 `always()` 运行受 workspace 根目录约束的轻量清理，只枚举 `.git/objects/pack` 下的 `tmp_pack_*`，逐个通过 `Remove-Item -LiteralPath` 回收中断传输片段并报告数量/字节。该清理必须设置 `continue-on-error: true`：删除失败由 step 日志明确暴露并在下次 runner 分配时重试，不得覆盖业务测试结论或新增合入门禁。GitHub-hosted close-sync 不执行本机 pack 清理。禁止让冷克隆、TLS/EOF、握手停滞或残缺 pack 占用整个 job 超时或持续积累；该设置不是新的 PR 门禁，基础设施失败不得归因于业务测试。正常传输在阈值以上继续完成，连续失败快速释放 runner并保留原始基础设施证据，禁止通过无限重试、完整历史 checkout 或重复依赖安装恢复。mirror、shell、Schannel 和代理异常属于 runner 基础设施诊断。
