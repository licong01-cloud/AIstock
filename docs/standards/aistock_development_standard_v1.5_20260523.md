...
Windows runner 的 workflow shell 必须显式解析到 Git Bash（禁止落到 WSL `System32\\bash.exe`）；runner wrapper 可配置只读本地 Git object mirror，并通过 `GIT_ALTERNATE_OBJECT_DIRECTORIES` 复用已验证对象。checkout 仍使用浅深度和当前 PR base SHA；仅在 mirror 缺少对象时远程补取。所有 self-hosted workflow 必须设置 `GIT_HTTP_LOW_SPEED_LIMIT=1` 与 `GIT_HTTP_LOW_SPEED_TIME=60`：连续 60 秒无有效传输时终止该次 Git HTTP 请求并交由 checkout 的既有有限重试处理，禁止让断链 fetch 占用整个 job 超时；该设置不是新的 PR 门禁，也不得把正常慢速传输、单次 TLS/EOF 或 GitHub 波动归因于业务测试。mirror、shell 和代理异常属于 runner 基础设施诊断，禁止转化为业务门禁或重复依赖安装。
