...
source/mixed PR merge tree 的 changed-file 分类、静态 L0/catalog、被选中的 backend/frontend/Go/prompt/workflow 验证、workflow policy 与最终 `CI verdict` 必须复用一个 job、一次 checkout、一次 base-ref 准备和一次 AIstock-CI runner allocation；在只有一个普通 runner 的当前配置下，各选中 lane 在该 job 内顺序执行并以 step outcome fail-closed 汇总，禁止拆成会释放后重新争抢同一 runner 的 classifier、matrix/module 或 verdict job。docs-lite 记录同样在该 job 内生成。官方 close-sync 分支在同一动态 job 中切换到 `ubuntu-latest`，只使用系统 Python 标准库、一次 checkout/base-ref 准备和 `bug_registry_metadata_check.py --close-sync-only`，不得获取普通或安全 self-hosted runner，也不得执行环境/依赖安装。任一越界文件或任一选中 lane 失败都必须使稳定 `CI verdict` 失败；DEV DB 计划继续仅报告外部 DEV 验证要求，不在 CI 执行 DDL/DML。
...
多个已经独立合入、且无需 backend restart 或 post-restart receipt 的 BUG，可使用 `close-sync-aggregate` 在一个 registry-only PR 中持久化善后元数据；这不是 source batch，也不得把一个 BUG 的源身份或验证收据投射到其他 BUG。`--apply` 必须在创建聚合工作树、修改 BUG JSON 或同步外部 Issue 状态前，逐 BUG 验证 GitHub Issue linkage、独立且已合入的 source PR、该 PR 的 merge commit、BUG JSON 内已持久化的 validation evidence 和有效 runtime contract；任一项失败则整组 fail-closed。聚合写入必须逐 BUG 保留各自的 `pr_url`、`fix_commit`、验证证据、生产门禁和 Issue 同步结果，禁止使用一个全局 `--pr-url`、全局 merge commit 或全局 validation receipt 覆盖整组。共享 source PR 的兼容 BUG 继续使用 `close-sync-batch`；runtime/restart-pending BUG 继续单独完成运行态验证与 close-sync，不得进入 aggregate。聚合只减少重复的 metadata PR、worktree、CI 和 cleanup 周期，不改变 source PR 边界、模块所有权、验证要求或生产授权。
