...
> 版本：1.2
> 更新日期：2026-05-19
> 状态：人类可读规范源；本版在 v1.1 基础上补充详细设计方案交付、测试方案、验证证据和 Main 合入治理规则
> 当前文件：`docs/standards/aistock_development_standard_v1.2_20260519.md`
> 同步机器版本：`docs/standards/aistock_development_standard_v1.2_20260519.yaml`
...
| 文件 | 角色 | 权威性 | 说明 |
|---|---|---:|---|
| `docs/standards/aistock_development_standard_v1.2_20260519.md` | 人类可读规范源 | 最高 | 定义必须遵守的项目开发、量化、测试和提交流程。 |
| `docs/standards/aistock_development_standard_v1.2_20260519.yaml` | 机器可读同步版本 | 派生 | 由本文抽取规则，供 guardrail scanner、nox、Validation Center、Codex/Claude Code 使用。 |
...
11. 不得污染项目根目录；一次性测试、诊断、排查脚本必须放在 `debug_tools/`，不得放根目录或正式 `scripts/`。
12. 正式业务脚本和一次性诊断脚本必须分离；`debug_tools/` 不得被生产服务、定时任务或正式 API 依赖。
13. 所有脚本、诊断工具、数据处理流程都不得静默报错；即使是一次性脚本，也必须输出错误上下文并用非零退出码表示失败。
...
本节中的规则 ID 必须与 `docs/standards/aistock_development_standard_v1.2_20260519.yaml` 保持一致。
...
- 根目录新增文件必须说明为什么不能放入现有目录，并经过 review。
...
### 15.3 [DESIGN-MAIN-001] 详细设计方案交付和 Main 合入规则
