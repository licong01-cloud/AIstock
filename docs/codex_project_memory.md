...
- F-014 current boundary: PR #2875 third review closed the v1.12 Loop as-of/family evidence/coverage/censoring, Archive metric/horizon and execution-quality, catalog/3012, and CI dependency blockers. Source head `bcced35a855df2b7e6cd369eaef04a778509c254` passed Phase 2/3, mock UI, catalog/module ownership, F2/L0 and PR CI; execution status and execution evidence level remain separate filters. The current `8001` OpenAPI still lacks `metric_key` and entry/exit evidence-level parameters, so runtime activation is not verified and old receipts without the new summary keys do not match those filters.
