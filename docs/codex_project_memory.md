...
This is the compact active-work snapshot as of 2026-08-10. Detailed evidence remains in the authoritative blueprint; this section records only the current boundary and next order.

- Strategy evolution is the highest priority. The active blueprint is `docs/analysis/sector_rotation_factors_develop_spec_20260710.md` v6.1. MA-E07 four-leg soft de-risk and MA-E10 drop-TCN three-leg remain the full-window Pareto anchors; MA-E14 ended the current H95/C85 threshold-overlay branch; MA-E15 completed 9/9 but CE3/CE4-B changed the observation panel, so breadth attribution is invalid until a matched rerun.
- MA-E15 signal evidence (prediction, IC/RankIC and Top-K panel diagnostics) remains usable. Its portfolio CAGR/Sharpe/MDD/Calmar/turnover used V25_1 minute execution and is marked `EXECUTION_TRUTH_LIMITED_BY_BUG852`; BUG-852 remains open and is not repaired or closed by the current work.
- BUG-1006 is the current matched-attribution blocker: `qe_observation_panel_v1` must stay in the generated data-loader contract, never strategy kwargs; matched trials fail closed on missing configured results/columns/empty panels, while historical no-contract tasks retain successful-factor partial concat. Source merge, user-owned backend restart and runtime activation remain separate states.
- After BUG-1006 runtime activation, MA-E16 is one 12-Loop matched trial: CE3/CE4-B × LGBM/LSTM × seeds 123/314/2718 on the same 2026-06-30 frozen file snapshot and CE3 reference panel. All arms use daily `CLOSE_PRICE` (`runtime_mode=day`, `backtest_freq=day`), Top50/n_drop1 and identical costs; remote LGBM runs at parallelism 3 and WSL LSTM serially at 1, with no other concurrent QE task. This bypasses but does not fix BUG-852.
- After MA-E16, the order is conditional fixed-weight blend/LOO, participation-gap and leadership-exhaustion experts, multi-model disagreement weighting, dynamic-relation drawdown hazard, then PIT fundamental diffusion and right-tail multitask research. Order-size buckets are proxies and do not identify investor account types.
