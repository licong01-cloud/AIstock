...
- F-014 current boundary: the same request replay kept evaluation id, row version `42`, update timestamp, row identities/digests and task evaluation count unchanged, with `ready_for_node=false`; no training, backtest or worker submission occurred. PR #2875 has a Loop long-trend tab with one idempotent action, DB-restored state and a same-outcome-snapshot Archive comparison; mock UI flows passed 2/2 and a read-only live Archive smoke passed. Formal review on 2026-07-29 nevertheless marked the PR changes-required: Loop as-of/family reason/coverage/censoring, Archive execution-quality/horizon pushdown and validation catalog command/DEV-port compliance are still open, and CI is blocked. The UI source must not be called verified until those items close.
- Next order: first repair and re-review PR #2875 against F2 design v1.12, then complete the Phase 4 task-create immutable profile switch and historical-input availability preview, followed by Phase 5 interruption/recovery and non-QE zero-impact E2E. The other five R8B CAS evaluations remain unmaterialized; materializing them or any wider historical set is a separate batch-write authorization. Source merge, close-sync, cleanup, root sync, DDL, service restart and runtime activation remain separate facts.
