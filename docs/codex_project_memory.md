...
This is the compact active-work snapshot as of 2026-07-31. Detailed implementation and acceptance evidence remains in the authoritative designs; this section records only the current boundary and next order.
...
- F-014 current boundary: PR #2875 remains the merged/runtime-verified Loop/Archive UI slice. The remaining Phase 4 task-create immutable profile switch and historical-input availability preview merged through PR #2906 as `bba48911342a3bee51e4339d597d9b2a5b85d71a`; production task-profile DDL is applied and verified. The currently listening backend/frontend processes both started at 2026-07-31 01:55 +08:00 after a restart performed outside this documentation task; this task only observed the processes and did not start, stop or restart them. The current 8001 OpenAPI exposes `long_trend_profile_id` and `/api/v1/quantevolver/evolution/tasks/{task_id}/loops/{loop_index}/long-trend-input-preview`, proving the F-014 contract is loaded without claiming full runtime/root HEAD identity. A fresh read-only GET for fixed task `qe_20260715_104922_001d` Loop 4 again returned 8/11 inputs available and explicit gaps for archived feature snapshot identity, orders and trades; `ready_for_node=false`, `technical_readiness_only=true`, `research_gate=false` and all seven side-effect flags were false. The previously verified production DB baseline remains 17 evaluations / 2,004 metrics / 6 artifacts; this documentation correction did not write the database. Frontend `/qe-archive` HTTP is 200, but no browser runtime instance was available, so per-control visual acceptance remains pending. The PR #2906 source worktree/local/remote branch and PR #2935/#2946 docs worktrees/local/remote branches remain retained pending separate cleanup authorization.
- Next order: complete browser per-control visual acceptance when a browser runtime is available, then execute Phase 5 normal-Loop activation smoke, interruption/duplicate-callback/CAS-DB recovery and non-QE zero-impact E2E. The other five R8B CAS evaluations remain unmaterialized and require separate batch-write authorization. Source merge, cleanup, DDL, process restart, runtime-contract evidence, normal-Loop execution and historical materialization remain separate facts.
