...
- F-014 current boundary: the same request replay kept evaluation id, row version `42`, update timestamp, row identities/digests and task evaluation count unchanged, with `ready_for_node=false`; no training, backtest or worker submission occurred. This proves one canary CAS→DB→API/MCP path, not full R8B or Phase 4–5 delivery. The other five R8B CAS evaluations remain unmaterialized, Phase 4 UI/historical operations and Phase 5 restart/failure/backfill E2E remain open.
- Next order: implement the Phase 4 single-action UI and DB-restored progress against the verified canary, then perform Phase 5 interruption/recovery and non-QE zero-impact E2E. Materializing the other five R8B receipts or any wider historical set is a separate batch-write authorization. Source merge, close-sync, cleanup, root sync, DDL, service restart and runtime activation remain separate facts.
