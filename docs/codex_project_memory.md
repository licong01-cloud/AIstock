...
- Development standards and guardrail framework document: `docs/architecture/aistock_development_standards_and_guardrails_20260504.md`. It defines P0/P1/P2/P3 severity, policy-as-code, full-repo baseline scan, changed-files blocking, exception expiry, agent-first repair context, and optional future MCP resources/tools/prompts.
- Required implementation path: first create `tests/aistock_validation/catalog/development_guardrails.yaml`, extend or add a guardrail scanner, run a read-only full-repo baseline scan, save the human summary to `docs/analysis/aistock_guardrail_baseline_YYYYMMDD.md`, then make `nox -s l0` block new/changed P0/P1 violations.
