...
- F-014 current boundary: the same request replay kept evaluation id, row version `42`, update timestamp, row identities/digests and task evaluation count unchanged, with `ready_for_node=false`; no training, backtest or worker submission occurred. Phase 4 now has a Loop long-trend tab with one idempotent action, DB-restored family/platform/metric/artifact state, plus a QE Archive same-outcome-snapshot comparison across run/model/seed/factor-set. Mock UI flows passed 2/2 and a read-only live Archive UI smoke against the current 8001 canary passed; the live smoke did not issue POST or DML.
- Next order: complete the remaining Phase 4 task-create immutable profile switch and historical-input availability preview, then perform Phase 5 interruption/recovery and non-QE zero-impact E2E. The other five R8B CAS evaluations remain unmaterialized; materializing them or any wider historical set is a separate batch-write authorization. Source merge, close-sync, cleanup, root sync, DDL, service restart and runtime activation remain separate facts.
