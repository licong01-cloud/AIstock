...
- UI design standard confirmed on 2026-05-24: new AIstock user/operator-facing UI must default to `shadcn/ui Blocks` visual language and components. Research Assistant uses `assistant-ui` for conversation interaction but must use shadcn-compatible theme/tokens, not Paper v2 colors or `pv2-*` shell. Paper v2 `paper-v2.css` and `components/paper-v2/*` are legacy local implementation details and must not spread to new modules.
...
- Codex issue-fix skill standard: short prompts such as "闁圭顦抽～澶愭嚑閸愌勫弿濠?BUG-XXX" must use `.codex/skills/fix-aistock-issue` and `scripts/aistock_issue_workflow.py start/finish/triage-p0` before manual exploration, so later Codex windows get compact context packs, explicit scope, selected validation, and production gates automatically.
...
- UI/manual page闂備浇澹堟ご绋课涙笟鈧幆?remains deferred per user direction; use a non-8001 temporary backend port for API validation.
...
- User requires all future design闂佸搫鍊介～澶愩€?/ architecture design / implementation design documents produced by Codex in this repository to be stored under `docs/architecture` (`F:\Dev\AIstock\docs\architecture`). Do not place new design documents elsewhere unless the user explicitly requests a different path.
...
- Added `backend/services/qe_archive/backfill_service.py` so historical experiment/loop闁荤偞绋忛崕杈╃礊?can be triggered through API instead of hand-running `scripts/qe_archive_backfill.py`. API write mode requires `confirm_write=QE_ARCHIVE_WRITE` and can validate minimum metric/curve/factor/account row counts after writing.
...
- The `/qe-archive` UI now shows selectable pending candidates, "select all pending", dry-run preview, confirmed "闂佸憡鍔栭悷銉╁矗閸℃稑鏋佸Λ棰佽兌濞?, and a fixed quality gate explanation. Minimum metrics/curves/factors are post-write completeness checks, not data collection filters.
