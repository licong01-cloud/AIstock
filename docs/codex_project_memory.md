...
- F-014 authority: `docs/architecture/qe_long_trend_evaluation_f2_design_20260714.md`. Phase 1 compute and Phase 2 control/CAS canary are complete. Phase 3 source is merged through PR #2835 and fixes #2845/#2847/#2855; the current `8001` runtime exposes four long-trend paths and validates the `qe_archive_v5_20260728` result schema on its connected database.
- F-014 remaining gap: the 2026-07-29 read-only quality query returned zero rows. Therefore CAS-to-result-table materialization, DB/API/MCP readback with non-empty evidence, Phase 4 UI/historical operations, and Phase 5 E2E/backfill remain open. The R8B 6/6 `partial + CAS published` receipt remains valid research evidence but is not proof that Phase 3 result rows were materialized.
- Next order: close BUG-904 documentation drift; close-sync stale BUG/GitHub metadata separately; then, only under a separate write authorization, run one existing-CAS idempotent materialization and verify DB/API/MCP readback before UI, historical backfill, or broader E2E work. Source merge, close-sync, cleanup, root sync, DDL, service restart and runtime activation remain separate facts.
