...
This is the compact active-work snapshot as of 2026-07-29. Detailed implementation and acceptance evidence remains in the authoritative designs; this section records only the current boundary and next order.
...
- F-014 current boundary: PR #2875 third review closed the v1.12 Loop as-of/family evidence/coverage/censoring, Archive metric/horizon and execution-quality, catalog/3012, and CI dependency blockers. It merged as `5413d799c59d16b8bf53689cca856f669f283fcf`; root sync and source branch/worktree cleanup are complete. After the user's 2026-07-29 restart, `8001` health and `3000/qe-archive` HTTP returned 200, OpenAPI exposed `metric_key`, `horizon`, and entry/exit evidence-level parameters, and the fixed R8B Loop4 evaluation passed list/detail/Archive readback; `rank_ic + horizon=60` returned three rows. Old receipts without the new summary keys correctly return no evidence-level matches. Frontend per-control visual acceptance remains pending because no browser runtime instance was available; HTTP liveness and mock Playwright are not treated as that evidence.
- Next order: complete the Phase 4 task-create immutable profile switch and historical-input availability preview, then Phase 5 interruption/recovery and non-QE zero-impact E2E. The other five R8B CAS evaluations remain unmaterialized and require separate batch-write authorization. Source merge, close-sync, cleanup, root sync, DDL, service restart and runtime activation remain separate facts.
