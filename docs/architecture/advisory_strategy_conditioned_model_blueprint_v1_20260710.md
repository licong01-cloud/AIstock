...
- Phase 1F.1 forward修正代码已完成并在 disposable PostgreSQL L2通过：它采用全局identity、月分区payload和只读compatibility view，保持snapshot读取结果不变；DEV/production DDL均未执行，不新增审批、角色或备份要求。
