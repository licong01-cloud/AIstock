...
| P0-E outcome-weighted meta-label | request `advmetareq_49042f2c766a6d32bb801439`；bundle `a222a1a4999e465b13d48000e024f885b6276801ac92d34ca34d497841daa34e` | 2 families × 3 seeds × 28 paths = 168；winner `FAMILY_CORE_HMM/20260817` | 363.412秒；RSS 2.84GB；exact retry 4.7秒 | `18.9626 bps` vs P0-D `19.4357 bps`，lift `-0.4731 bps`，path win `35.71%`，PBO `0.8143` | `research_improvement=false`；负面实验，不激活、不追加调参 |
| P0-E HISTORICAL_REPLAY | artifact `c7f6ec1388adf8e8b7b13aa20aff35047919558b4d833091f1bf1311c9067934` | 同24决策日+20日tail；24/24 resolved；证据已消费 | 冷日约15–20秒；exact retry同hash | hit `23.08%`；累计 `-13.39%` vs Selection `-16.90%` / P0-D `-19.45%`；回撤 `-14.92%`；换手 `29.71%` | 幅度结构在该窗口改善但未被CPCV确认，只作诊断，不晋级 |
...
M0-M5C 的代码、真实 WSL 实验和固定日期推理已形成当前基线；M5A/M5B/M5C 均不激活。P0-A/P0-B 已在两个 ENABLED Program 上连续产生真实 `PUBLISHED` run。P0-C 已通过 PR #3367 合入；P0-D 已通过 PR #3368 合入并完成 exact descriptor、安全rotation、真实在线shadow和maturity修复，forward evaluation 已通过 PR #3697 合入。v6 44日 A/B/C/C5 对照已冻结；P0-D 历史虚拟前向已完成24个成熟决策日验证，能力可用但结果不支持模型激活。

下一工作严格按以下顺序执行；模型/前向与 H0 历史执行是互不阻断的两条线：

1. **合入 P0-D 历史虚拟前向能力**：完成 F2 验收、代码审核和 PR；无 DDL/DML、模型激活或后端重启。权威24日结果与自然 future OOS 分开保存。
2. **训练下一代 P0-D challenger**：优先分析“命中率提高但亏损幅度、回撤和换手恶化”的原因，调整 policy-aligned 标签、损失或候选约束；先在 train/validation 选择，再用未消费 OOT 窗口验证。若只能复用本次窗口，必须标为 `HISTORICAL_REPLAY` 并避免将其作为独立泛化证据。
