# AIstock 荐股策略条件化模型体系 F2 架构蓝图 v3.60

> 初始日期：2026-07-10
> 修订日期：2026-09-15
> 文档类型：F2 顶层架构蓝图，`docs-fast-update`
> 当前状态：`P0_FAMILY_FROZEN_ADVISORY_ALPHA_EXPERIMENT_LINE_CLOSED_DAILY_PRICE_ENVELOPE_FRESH_CONFIRMATION_REQUIRED_NOT_ACTIVATED_QE_EXPERIMENTS_EXTERNALLY_OWNED`
...
> 当前能力基线：Top5、收益/周期、价格范围和页面/API 均有真实实现与独立验证，但尚无当前同时提供四类输出的组合bundle。`AdvisoryDailyPriceEnvelopeV1`源码、v3三头真实训练 artifact `30e8a75b...` 和 v4 validation-only 校准 artifact `508fedfe...` 均已完成；validation/test 覆盖率分别为 `0.811702/0.733125`，校准扩张量为 0，结论为 `FRESH_CONFIRMATION_REQUIRED`、`activation_recommended=false`。因此当前零新 binding、零运行时激活，P0-D exact descriptor 仍只绑定 meta-label shadow，M3/M4 child typed unavailable。两 ENABLED Program 已形成真实 PUBLISHED/settlement/episode，最新复评日`2026-09-11`均为`SUCCEEDED`；2026-09-14零点对当日目标的`WAITING_DATA/TARGET_OPEN_SETTLE`发生在开盘数据可用前，不构成系统失败。历史虚拟前向可用于快速开发验证，不需每次等待20个自然交易日；但该已消费 test 只是开发诊断，自然成熟或一次性新鲜 holdout 证据独立积累。
...
截至 2026-09-14（Advisory最新只读快照；QE研究条目保留其各自已记录的事实日期）：
...
| 已完成源码 / PRICE-ENVELOPE-SOURCE | `advisory_daily_price_envelope_v1_f2_design_20260914.md` v1.1及Advisory源码已实现严格日级信封、legacy兼容、三头v3训练、v4校准容器、runtime loader和UI；近单类binary已从新合同/UI退役 | 本地合同、PIT、boundary、training、bundle、calibration、runtime、TypeScript及UI定向验证通过；源码合入不等于模型效果、binding或运行时激活，后端重启由用户执行 |
| P1 / M4-DAILY-MODEL-RUN | 在确认没有QE实验运行后，用既有exact package/parent/outcome/103特征和冻结M3 split创建全新v2 request，运行v3三头日级价格模型；随后只用validation拟合v4 CQR并在已消费test作开发诊断 | 生成不可变request/bundle/calibration/evaluation receipt并exact retry；报告coverage、宽度、上下越界、pinball、crossing、裁剪和typed unavailable。未通过则保持`EXPERIMENTAL_SHADOW/UNCALIBRATED`或不发布binding；不读取分钟Bin、不调已消费test、不与QE实验并行 |
| P2 / PRICE-ROLE-BINDING | 仅将证据合格且exact compatible的v3/v4日级价格bundle作为独立`ENTRY_PRICE` shadow角色绑定，不允许Ranking旋转覆盖；API/UI显示范围、目标日、价格基准、校准、身份和不可用原因 | 当前P0-D的M3/M4 unavailable先保持不变；模型不合格即终止本阶段，不发布binding。源码、bundle、binding、用户重启和运行时readback分别报告 |
