# AIstock 荐股策略条件化模型体系 F2 架构蓝图 v3.55

> 初始日期：2026-07-10
> 修订日期：2026-09-13
> 文档类型：F2 顶层架构蓝图，`docs-fast-update`
> 当前状态：`P0_FAMILY_FROZEN_ADVISORY_EXPERIMENT_LINE_CLOSED_QE_SINGLE_EXPERIMENT_OWNER_INDEX_UNIVERSE_AND_DAILY_DB_ONLY_RUNTIME_VERIFIED_QE_DELIVERY_PREFLIGHT_SOURCE_VERIFIED_PR_PENDING`
...
| F-254 | `advisory_program.py::_with_advisory_date_context`；`selection_center/result_enrichment.py`；日频DB-only F1详细设计 | DB-only未来目标日、正常停牌last-close保留、无历史价格、查询失败、非法mode/D-T、Advisory强制模式及AUTO兼容测试；F1 validator | IMPLEMENTED_VERIFIED | 仅Advisory正式D/T context启用；不改Selection评分/排序/候选，零QE、实验、DDL/DML与进程控制；运行时readback等待用户重启 |
| F-255 | `advisory_qe_delivery_consume_preflight_f2_detailed_design_20260913.md`；`backend/services/advisory_delivery_preflight.py`、Advisory API/UI | backend service/API 53项定向组合通过、frontend Playwright 17项通过、type/lint、F2 validator、changed-file边界与DESIGN-COMPLIANCE-001 | IMPLEMENTED_LOCAL_VERIFIED_PR_PENDING | 现有旧包普遍没有冻结股票池声明，只能返回legacy/filter-only兼容；future exact正路径以合同夹具覆盖，正式包交付后再做运行时readback |
...
| P0 / ADV-DAILY-DB | 完成正式日频荐股价格源合同：D日数据库收盘后生成T日推荐，Selection补充层不得因T为今天/未来而读取TDX | `DAILY_DB_ONLY`强制模式、逐股D日前last-close来源、正常停牌保留、零quote调用、无历史价/查询异常fail closed、AUTO兼容及多轮审核通过；合入后由用户重启并做真实Program readback |
| P1 / QE-DELIVERY-CONSUME | 实现StrategyPackage绑定前只读兼容预检：核对package/asset、冻结股票池证据与目标universe、descriptor阶段，区分exact、后置过滤兼容、旧包未声明、identity mismatch和交付合同不完整；通过后再创建独立Program binding并执行历史回放/自然前向业务验证 | API/UI在create与binding apply前给出可审计结论；硬身份错误fail closed，旧包保留baseline兼容但不得标exact，descriptor缺失typed baseline-only；不从QE收益直接外推荐股，不由Advisory补跑seed、LOO、组合或新因子实验 |
