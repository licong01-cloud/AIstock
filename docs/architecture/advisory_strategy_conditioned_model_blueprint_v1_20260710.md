...
- Phase 1F 已形成唯一实施级详细设计 `docs/architecture/advisory_phase1f_release_schema_verification_f2_design_20260714.md`：冻结完整 migration order/hash 和 PostgreSQL catalog contract，按 Phase 1D 显式历史日期范围生成月分区，分离 DEV/production apply 与只读 runtime verify；不执行业务 DML，不依赖 Phase 1E persistent L4、capacity `MEASURED`、Parquet 或模型状态，不新增角色、审批、授权或每次 DDL 前全库备份要求。当前状态为 `design_ready`，代码尚未开始。
