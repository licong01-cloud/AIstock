# AIstock 荐股策略条件化模型体系 F2 架构蓝图 v3.61
...
| P1 / PRICE-PROSPECTIVE-PREDICTION | 对冻结v4 bundle建立不绑定生产descriptor的日频自然前向预测收集：每个交易日只消费D时点可见候选、日线与103特征，发布不可变D/T、package/model/policy及输入覆盖receipt；不得回填模型冻结日前的预测，不读取T日结果或在sealed holdout内滚动看coverage | 先以详细设计和最小实现证明当前Program候选与v4 exact身份/特征可产；成功则从首个真实未来交易日开始只写预测，失败则返回精确typed blocker。预测收集不改变Top5、排序、推荐、binding、数据库或运行时，不与QE实验并行 |
