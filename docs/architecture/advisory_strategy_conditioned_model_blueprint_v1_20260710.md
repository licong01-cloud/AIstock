# AIstock 荐股策略条件化模型体系 F2 架构蓝图 v3.17

> 初始日期：2026-07-10
> 修订日期：2026-08-31
> 文档类型：F2 顶层架构蓝图，`docs-fast-update`
> 当前状态：`P0_RESEARCH_FAMILY_FROZEN_N1_FORMAL_COMPLETE_N2A_MERGED_N2B_V1_4_THROUGHPUT_FIX_IN_PROGRESS`
...
- N2-B权威详细设计已修订为`docs/architecture/advisory_independent_strategy_package_alpha_audit_f2_detailed_design_20260831.md` v1.4。v1.3已完成rolling PIT窗口、in-memory I/O与两个真实closure的file-backed parity，但稳定阶段仍约4分钟/日。v1.4不改变386个T、772次exact closure、三模型、PIT、窗口或输出；只把factor input改为COW隔离视图，把两个closure间28个同名同source-SHA因子的当日target series精确复用，并把临时计算迁到WSL local ephemeral filesystem。首日两个closure仍各自完整执行file-backed reference且不消费缓存；配对hash、精确计算/复用计数和8小时wall均fail closed。结果仍为0-trial `ORACLE_DIAGNOSTIC/NAVIGATION_ONLY`。
...
4. **扩展独立旧包的同窗审计（当前主任务）**：F2详细设计v1.4正在完成8小时内吞吐纠偏。按`pkg_378eb9...`→`pkg_5a5ccb...`→`pkg_b668f8...`顺序，一次读取完整区间，在单一批任务内按386个T的PIT股票池和各closure冻结滚动窗口各执行两个exact closure，每个冻结模型只加载一次。允许的复用仅限同T/同窗口/同factor name/同source SHA，当前固定28个；COW与WSL local temp不得改变数值，并须由真实file-backed reference逐值证明。生成Prediction Store后复用相同PIT/H20/cost审计；禁止union-wide后置PIT、直接横比原生指标、引入12个RETIRED包、结果后选包或窗口。
