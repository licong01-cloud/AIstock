# AIstock 荐股策略条件化模型体系 F2 架构蓝图 v3.30

> 初始日期：2026-07-10
> 修订日期：2026-09-03
> 文档类型：F2 顶层架构蓝图，`docs-fast-update`
> 当前状态：`P0_FAMILY_FROZEN_N1_N2_COMPLETE_N3_MINUTE_SELECTED_ZERO_QE_ALPHA_GENERATOR_MVE_DESIGN_ACTIVE`
...
> 当前唯一主动目标：按正式route设计 `N3_QE_ALPHA_GENERATOR_MVE` 的最小可行实验，使用新的自动化上游Alpha生成假设，不重跑已关闭的24-proposal归档screen，不回到N2-A三腿/分钟feature/loss/model搜索。先冻结生成预算、表达式原创性/防衰减约束、PIT信息集、父包增量评价与一次性frontier，再实现和运行；仍不得读取sealed holdout、写因子库/StrategyPackage或激活运行时。自然future OOS只被动积累；H0仅在业务正确性或资源问题直接阻塞该主线时执行最小修复
...
| 新模型演进路线 | `N3_MINUTE_MVE_FORMAL_SELECTED_ZERO__QE_ALPHA_GENERATOR_DESIGN_ACTIVE` | N0/N1/N2、N3单信号、父overlay、腿间及分钟MVE均已正式完成；四个N3 frontier均`selected=0`且exact retry稳定。当前只允许设计新的QE Alpha generator MVE，不重跑旧24-proposal screen，不改生产权重、不读sealed holdout |
...
状态：`MINUTE_INFORMATION_SET_FORMAL_COMPLETE_SELECTED_ZERO__QE_ALPHA_GENERATOR_MVE_DESIGN_ACTIVE`。
...
6. **QE Alpha generator MVE设计（当前唯一主动步骤）**：基于现有QE/RD-Agent资产设计新的自动化因子生成MVE，冻结生成预算、表达式与数据边界、原创性/已知效应重叠、防衰减约束、父包增量指标、资源和一次性frontier。它不得重跑旧24-proposal集合，也不得在设计前生成研究结果。
7. **QE Alpha generator源码与正式实验**：详细设计通过后实现最小生成/评价闭环，经重复审核合入，再从clean main运行；实验可直接执行但仍只可navigation，不写因子库或StrategyPackage。selected=0时根据预注册route停止或转新的上游数据假设；selected=1只进入独立confirmation。
