...
> 当前状态：`P0F_STAGE_A_NEGATIVE_STOP_P0G_LOCAL_IMPLEMENTATION_VALIDATED_STAGE_A_PENDING`
...
> 演进方向：P0-D 历史虚拟前向已证明二分类概率不能稳定表达收益幅度；P0-E outcome weighting同样负向停止。P0-F v2已完成真实28-path训练，bundle `ff336eadb131cb6a3d431a846de4e9949ad984da1dcc4d9c231aa313886ebc10` 相对exact P0-D提高 `2.612087 bps`、path win `53.57%`、回撤改善，但平均换手增加 `0.007419`，因此按预注册合同 `NEGATIVE_STOP_NOT_ADVANCED`，禁止Stage B/replay/runtime和结果后调参。下一模型固定为P0-G：只在outer-train用真实持有期估计每日换手负担和最小可行换手影子价格，训练turnover-constrained continuous utility；详细设计见 `advisory_p0g_turnover_constrained_utility_f2_design_20260825.md`
...
7. P0-G 只改变训练label：用成熟episode持有期构造与shared evaluator同单位的每日换手负担，在每条outer path的train blocks内选择最小可行换手影子价格，再训练固定Huber utility；validation、policy和成本不参与调参或改变。
...
完成判定：F2设计和验收索引通过后，实现完整Stage A、真实WSL 168 trial-path、不可变bundle、exact retry、资源/PBO/constraint/paired/advancement receipts；无论结果如何均不自动激活。
