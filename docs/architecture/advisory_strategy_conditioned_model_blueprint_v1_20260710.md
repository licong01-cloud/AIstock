...
- Phase 1G开工一致性复核发现Phase 1F.1 outbox唯一键及capture-gap identity缺少scope，同一Selection证据无法被多个独立Program合法消费或形成独立失败证据。Phase 1G详细设计已把Phase 1F.2 scope-aware trace identity migration/registry/verifier/DEV release冻结为G0技术前置；下一项任务是形成独立Phase 1F.2 F2详细设计，当前尚未实现或应用，不是审批或人工门禁。
