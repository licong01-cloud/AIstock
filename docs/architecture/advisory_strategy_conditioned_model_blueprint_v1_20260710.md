# AIstock 荐股策略条件化模型体系 F2 架构蓝图 v3.59

> 初始日期：2026-07-10
> 修订日期：2026-09-14
> 文档类型：F2 顶层架构蓝图，`docs-fast-update`
> 当前状态：`P0_FAMILY_FROZEN_ADVISORY_ALPHA_EXPERIMENT_LINE_CLOSED_DAILY_PRICE_ENVELOPE_SOURCE_READY_MODEL_RUN_PENDING_QE_EXPERIMENTS_EXTERNALLY_OWNED`
...
> 当前能力基线：Top5、收益/周期、价格范围和页面/API 均有真实实现与独立验证，但尚无当前同时提供四类输出的组合bundle。`AdvisoryDailyPriceEnvelopeV1`源码已实现严格D/T、package/model/policy identity、候选级typed unavailable和无分钟/订单字段；legacy v1/v2只消费三个quantile，新v3三头训练及v4校准容器源码已本地验证，但尚未产生真实v3/v4模型artifact、binding或业务效果证据。两 ENABLED Program 已形成真实 PUBLISHED/settlement/episode，最新复评日`2026-09-11`均为`SUCCEEDED`；2026-09-14零点对当日目标的`WAITING_DATA/TARGET_OPEN_SETTLE`发生在开盘数据可用前，不构成系统失败。P0-D exact descriptor 仅绑定 meta-label shadow，M3/M4 child typed unavailable。历史虚拟前向可用于快速开发验证，不需每次等待20个自然交易日；自然成熟证据独立积累。
