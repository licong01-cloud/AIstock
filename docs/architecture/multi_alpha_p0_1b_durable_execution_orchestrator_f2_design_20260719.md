...
- 日期：2026-07-19
- 状态：`DESIGN_REVIEWED_READY_CODE_PENDING`
- 审计修订：已补齐 QE Workspace 服务端幂等、跨来源容量 reservation、task identity/场景参数分离、终态后 Archive 可见性和 timeout/wait 语义；代码与配套 DDL 仍未实现
...
4. reservation 在远端 POST 前已经包含 node、expected `qe_task_id/qe_loop_id` 和 `submission_intent_hash`；唯一约束拒绝同一 source execution 或同一 remote identity 的重复占位。
5. 容量不足时不插入 active reservation，source 保持 `queued`，写入 `waiting_capacity` event/progress 后返回无任务。
...
- retry lineage 只通过 `retry_of_run_id` 表达；P0-2 再开放正式 retry 操作。
...
1. `LoopRunRequest` 增加必填 `submission_intent_hash`；AIstock `QEWorkspaceClient` 不得在 durable/coordinated submit 中省略该字段。
2. 服务端以 `(task_id, loop_id)` 为 identity，在对应 loop workspace 下使用跨进程文件锁或等价原子原语创建 submission receipt；receipt 持久化 intent hash、canonical request digest、状态和时间戳。Request digest 覆盖 `loop_index/config/experiment_files` 内容 hash、`wsl_command/model_source`，不把仅用于通知的 `callback_url` 当作执行 identity。
3. 第一次请求先原子持久化 `reserved` receipt，再注册一次 background execution；相同 identity/hash 的重复 POST 返回原 loop/receipt，`duplicate_replay=true`，不得再次调用 `background_tasks.add_task()`。
4. 相同 identity 但 intent hash 或 canonical request digest 不同，返回 HTTP 409 与 `qe_workspace_submission_identity_conflict`。
5. 服务在 receipt 已为 `reserved`、但还没有 started evidence 时也不得自动注册第二次 execution；状态 API/receipt API 返回 `reserved_not_started`，AIstock 保持 reconciling。P0-2 的显式 retry 使用新的 attempt/remote identity，不覆盖旧 receipt。
6. receipt 状态至少覆盖 `reserved/started/running/completed/failed/cancelled`，并可在 loop status 文件尚未产生时被独立查询。
7. WSL 与远端节点部署相同 contract。AIstock 后端不得在任一节点仍运行旧 OpenAPI schema 时激活 coordinated submit，也不得回退为无 receipt POST；这只约束执行正确性，不限制研究方向。
...
| receipt reserved、loop 尚未 started | reconciling + `reserved_not_started` evidence；不重复 POST |
...
| 服务端 receipt 已 reserved、background 尚未 started | receipt=`reserved_not_started` | 保持 reconciling，不重复注册；P0-2 可显式创建新 attempt |
...
| `source_kind/source_execution_id` | 强类型来源映射；组合唯一，禁止同一 source execution 重复占位 |
| `qe_task_id/qe_loop_id/submission_intent_hash` | POST 前必填；`node_id+qe_task_id+qe_loop_id` 唯一 |
...
必须提供 active node partial index、source identity unique constraint、remote identity unique constraint、comments、preflight 和 guarded rollback。该表不是第二套 scheduler，也不替代 `qe_evolution_loops`、`qe_experiments`、`qe_multi_alpha_groups` 或 durable attempt；source 业务状态仍由原表权威持有。
...
- `rdagent/app/api_endpoints/qe_evolution_api.py`
  - `LoopRunRequest.submission_intent_hash`、原子 receipt、same-hash replay、different-hash 409、receipt inspect。
- 对应 QE API 测试
  - 并发重复 POST 只注册一次 background execution；reserved-not-started、same-hash replay、hash conflict 和服务重启后 receipt 恢复。
...
3. 在 WSL/远端两个 QE Workspace 节点验证 OpenAPI 和 same-hash replay contract；不启动 Alpha 训练。
...
- same-hash duplicate replay、different-hash 409、reserved-not-started、running、completed、failed、cancelled、network unknown 状态矩阵。
...
- 服务重启后 receipt 可恢复；same-hash 返回同一 loop，different-hash 返回结构化 409。
...
| F-309 | 提交响应丢失、backend restart 和 lease takeover 通过服务端 submission receipt 保证不产生第二个远端执行。 |
| F-310 | remote unknown/network error 进入 reconciling 并保留证据，不伪造 terminal failure。 |
| F-311 | `infra.qe_execution_reservation` 覆盖所有生产 QE submit 来源，旧 active source 只做受控导入与核对。 |
| F-312 | capacity check、reservation INSERT 与 source claim 在同一 PostgreSQL advisory transaction 中原子完成。 |
| F-313 | WSL 上限 2、远端上限 4；满载保持 queued/waiting_capacity，不标记失败。 |
...
| F-316 | schema/worker 不可用仅影响 multi-alpha write path；禁止静默回退旧 daemon，非 QE 模块零影响。 |
...
| F-319 | QE Workspace owning service 持久化 submission receipt；same-hash replay 不重复执行，different-hash 返回 409。 |
| F-320 | task immutable identity 与 run 场景参数分离；历史 task 可承载不同 OOS/TopK/资金/baseline/timeout run。 |
| F-321 | additive reservation migration、preflight、comments/indexes、active-source import 和 guarded rollback 合同完整。 |
...
|---|---|---|---|---|
| F-201 | §2、§3、§18 | `git diff --check`; `backend/tests/multi_alpha/test_durable_contract.py` | design_ready | none |
| F-204 | §6.3、§9、§13 | `backend/tests/multi_alpha/test_durable_orchestrator_restart.py` | design_ready | none |
| F-205 | §6.3、§10 | `backend/tests/multi_alpha/test_durable_execution_adapter.py`; `backend/tests/test_multi_alpha_remote_dispatch.py` | design_ready | none |
| F-206 | §6.4、§11、§17.1 | `backend/tests/multi_alpha/test_durable_capacity.py`; reservation migration preflight | design_ready | none |
| F-209 | §8.3、§12.4 | `backend/tests/multi_alpha/test_durable_parent_finalization.py` | design_ready | none |
| F-210 | §12、§14 | `backend/tests/multi_alpha/test_durable_repository.py::test_event_failure_rolls_back_the_state_transition` | design_ready | none |
| F-215 | §12.5、§16、§20.7 | `backend/tests/multi_alpha/test_durable_parity.py`; `backend/tests/multi_alpha/test_archive_health.py` | design_ready | none |
| F-216 | §3、§6.6、§18.4 | `backend/tests/multi_alpha/test_durable_contract.py`; `backend/tests/multi_alpha/test_durable_submission.py` | design_ready | none |
| F-218 | §20 | `python -m pytest backend/tests/multi_alpha -q`; `python scripts/aistock_feature_workflow.py validate --design docs/architecture/multi_alpha_qe_evolution_foundation_f2_design_20260718.md --tier F2` | design_ready | none |
| F-301 | §2、§5、§18 | `git diff --check`; `backend/tests/multi_alpha/test_durable_contract.py` | design_ready | none |
| F-302 | §6.1、§7、§15 | `backend/tests/multi_alpha/test_durable_submission.py::test_legacy_submit_creates_durable_run_without_daemon_owner` | design_ready | none |
| F-303 | §6.1、§10.3、§15 | `backend/tests/multi_alpha/test_durable_submission.py::test_sync_wait_timeout_returns_202_without_cancelling_durable_run` | design_ready | none |
| F-304 | §6.2、§8 | `backend/tests/multi_alpha/test_durable_plan.py` | design_ready | none |
| F-305 | §6.5、§12、§14 | `backend/tests/multi_alpha/test_durable_repository_postgres.py`; `backend/tests/multi_alpha/test_durable_orchestrator_restart.py` | design_ready | none |
| F-306 | §6.3、§10 | `backend/tests/multi_alpha/test_durable_execution_adapter.py::test_local_and_remote_nodes_use_qe_workspace_client` | design_ready | none |
| F-307 | §8.2、§10、§13 | `backend/tests/multi_alpha/test_durable_execution_adapter.py::test_artifact_publish_is_atomic_and_hash_verified` | design_ready | none |
| F-308 | §9 | `backend/tests/multi_alpha/test_durable_repository_postgres.py::test_remote_identity_is_bound_before_submission_event_commit` | design_ready | none |
| F-309 | §9、§13、§20.5 | `backend/tests/multi_alpha/test_durable_orchestrator_restart.py::test_response_lost_restart_does_not_duplicate_remote_loop`; `F:/Dev/RD-Agent-main/test/app/test_qe_evolution_submission_receipt.py` | design_ready | none |
| F-310 | §10.2、§13 | `backend/tests/multi_alpha/test_durable_execution_adapter.py::test_remote_unknown_enters_reconciling_without_terminal_failure` | design_ready | none |
| F-311 | §6.4、§11、§17.1 | `backend/tests/multi_alpha/test_durable_capacity.py::test_all_production_submitters_use_canonical_reservation` | design_ready | none |
| F-312 | §11.2、§14 | `backend/tests/multi_alpha/test_durable_capacity.py::test_capacity_reservation_and_source_claim_are_one_transaction` | design_ready | none |
| F-313 | §6.4、§11.4 | `backend/tests/multi_alpha/test_durable_capacity.py::test_wsl_two_remote_four_and_full_capacity_stays_queued` | design_ready | none |
| F-314 | §8.3、§12.4 | `backend/tests/multi_alpha/test_durable_parent_finalization.py` | design_ready | none |
| F-315 | §12.5、§16、§20.7 | `backend/tests/multi_alpha/test_durable_parity.py`; `backend/tests/multi_alpha/test_archive_health.py` | design_ready | none |
| F-316 | §6.6、§15.3、§18.4 | `backend/tests/multi_alpha/test_durable_submission.py::test_schema_unavailable_is_explicit_and_has_no_legacy_fallback`; `backend/tests/multi_alpha/test_durable_contract.py` | design_ready | none |
| F-317 | §3.2、§20.1 | `backend/tests/multi_alpha/test_durable_contract.py::test_p0_1b_has_no_gpu_telemetry_or_research_gate` | design_ready | none |
| F-318 | §20 | `python -m pytest backend/tests/test_multi_alpha_combine_backtest.py backend/tests/test_multi_alpha_remote_dispatch.py backend/tests/multi_alpha -q`; `python scripts/aistock_feature_workflow.py validate --design docs/architecture/multi_alpha_p0_1b_durable_execution_orchestrator_f2_design_20260719.md --tier F2` | design_ready | none |
| F-319 | §9.4、§20.4、§20.5 | `artifact: F:/Dev/RD-Agent-main/test/app/test_qe_evolution_submission_receipt.py`; planned nodeids `test_concurrent_same_hash_post_registers_execution_once/test_different_hash_returns_409/test_receipt_survives_server_restart` | design_ready | none |
| F-320 | §6.1、§7、§20.2 | `backend/tests/multi_alpha/test_durable_submission.py::test_task_identity_allows_distinct_run_scenarios` | design_ready | none |
| F-321 | §11、§17.1、§20.3 | `backend/migrations/qe_execution_reservation_20260719.preflight.sql`; `backend/tests/multi_alpha/test_durable_capacity.py::test_reservation_schema_import_and_concurrency_contract` | design_ready | none |
| F-322 | §12.5、§16、§20.6 | `backend/tests/multi_alpha/test_archive_health.py::test_post_terminal_archive_states_are_visible_and_retry_is_idempotent` | design_ready | none |
| F-323 | §10.3、§15、§20.2、§20.6 | `backend/tests/multi_alpha/test_durable_submission.py::test_sync_wait_timeout_returns_202_without_cancelling_durable_run`; `backend/tests/multi_alpha/test_durable_orchestrator_restart.py::test_completed_after_deadline_is_ingested_with_evidence` | design_ready | none |
...
- P0-1B reservation DDL、AIstock 代码与 RD-Agent receipt contract：均为设计完成，未实现、未合入、未激活。
- backend/frontend dependency：预计 `noop`，以实际实现为准。
- 本设计任务不重启服务、不创建 QE 实验、不修改生产 DB。
...
4. 先部署 WSL 与远端 QE Workspace receipt contract，验证 OpenAPI、same-hash replay 和 different-hash 409。
...
| 风险 | 对策 |
|---|---|
| HTTP create 成功但响应丢失 | bind-before-submit + QE Workspace 原子 receipt；same-hash replay 不再次注册 execution |
| receipt reserved 但 background 未 started | 显式 `reserved_not_started` + reconciling；不自动重提，P0-2 用新 attempt 恢复 |
| 多 backend 重复 claim | SKIP LOCKED + lease/fencing/row-version CAS |
