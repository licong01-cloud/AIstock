...
1. 本章是后续开发的强制对照表，不是参考清单。开发完成后必须逐条确认 `LDM-MCP-001` 至 `LDM-MCP-028` 的状态。
2. 任何矩阵项未完成时，不得将功能报告为完成，不得以“后续补充”“简化版”“只读版”“临时 POC”替代设计要求。
3. 如实现过程中发现设计项确实需要变更，必须先更新本设计文档和验收矩阵，再继续开发；不能在代码中静默偏离设计。
4. 每个矩阵项的验收结论只能是：`通过`、`不通过`、`阻塞待设计变更`。除本设计明确排除的范围外，不使用 `不适用`。
5. 合入 main 前必须提交或在 PR 中附上逐项验收记录，记录至少包含：矩阵编号、代码位置、测试命令、测试结果、未覆盖风险。
...
| LDM-MCP-026 | 设计一致性验收报告 | 开发完成后有逐项对照记录，能证明工具、API、UI、记忆/图谱与本设计一致 | PR/验收报告检查 |
| LDM-MCP-027 | 测试证据完整性 | 每类测试均记录命令、环境、结果和失败项；失败项不得被忽略或静默降级 | 验收报告检查 |
| LDM-MCP-028 | 安全执行闭环 | 写操作必须经历只读检查、计划生成、用户确认、执行、复查五步；缺少任一步不得通过 | 集成测试 + UI smoke |

### 10.3 测试方案和最低通过标准

后续代码开发必须按以下测试层级执行，并在验收记录中写明结果。实际命令可根据当时测试目录调整，但覆盖范围不得减少。

| 测试层级 | 覆盖范围 | 最低通过标准 | 对应矩阵项 |
| --- | --- | --- | --- |
| 静态一致性检查 | Gateway 工具目录、profile、Capability Registry、排除范围、禁止直接 DB/脚本调用 | 工具清单与本文 5 章一致；`local_data` MCP module 不直接连接数据库、不直接 import 调度器、不调用 subprocess；首批工具不包含因子独立指标计算、Xtquant/miniQMT 同步和实盘路径 | LDM-MCP-001、002、022、023 |
| 后端单测 | facade API 参数校验、状态聚合、gaps 计算、repair plan、错误透传、确认口令校验 | 成功路径和失败路径均覆盖；失败不得返回 success；确认型接口缺少确认时必须拒绝执行 | LDM-MCP-003 至 018、024、028 |
| 后端集成测试 | 使用测试库或受控测试数据创建同步 job、计划任务变更、data_stats 刷新、告警确认、repair apply | 每个写操作都有 trace；执行后能复查状态变化；失败时停止后续步骤并保留真实原因 | LDM-MCP-008 至 018、024、028 |
| MCP 工具测试 | 通过 MCP Gateway 调用只读工具和确认型工具 | MCP 入参、返回结构、风险级别、中文摘要、trace 与后端 facade 保持一致 | LDM-MCP-001 至 018 |
| 助手对话测试 | 用户用自然语言要求“检查本地数据同步并修复” | 助手先解释理解结果，再执行只读检查；写操作前必须请求确认；主视图不展示 raw JSON | LDM-MCP-019、021、028 |
| 前端 UI smoke | 任务状态卡片、阶段卡片、审计详情、错误展示、计划任务 diff | 主视图为人类可读中文卡片；内部 ID 和 JSON 只能在审计详情中展开；失败展示真实错误和下一步建议 | LDM-MCP-019、021、024 |
| 图谱/记忆测试 | 本地数据管理能力 seed、实体关系 seed、重复执行幂等 | seed 可预览、可写入、可重复执行；助手能解释本地数据状态与 QE / Selection Center / Paper v2 的影响关系 | LDM-MCP-020 |
| 合入前总验收 | 全部矩阵项、生产 DDL gate、未完成项 | `LDM-MCP-001` 至 `LDM-MCP-028` 均有结论和证据；若有 DB 变更必须完成 production DDL gate，否则报告 `production_ddl_gate=noop` | 全部 |

### 10.4 逐项验收记录模板

开发完成后必须使用以下格式形成验收记录，可放入 PR 描述、开发总结或单独的 `docs/validation/` 文档。

| 编号 | 结论 | 代码位置 | 测试命令/证据 | 备注 |
| --- | --- | --- | --- | --- |
| LDM-MCP-001 | 通过/不通过/阻塞待设计变更 | 相关文件和函数 | 命令、日志、截图或测试报告 | 未覆盖风险或设计变更链接 |

### 10.5 不允许的验收方式

1. 只证明页面能打开，不能替代 MCP/API/业务闭环验收。
2. 只证明后端接口返回 200，不能替代真实错误、确认口令、trace 和复查状态验收。
3. 只展示 raw JSON，不能替代面向用户的中文卡片和对话解释。
4. 只实现查询工具，不能替代本设计要求的计划生成、确认执行、计划任务管理、告警确认和修复编排。
5. 只在开发环境手工点一次，不能替代可重复执行的测试命令和逐项验收记录。

## 11. 分阶段实施目标

### Phase 0：设计落地和图谱更新

交付：本设计文档、数据同步控制面文档补充、记忆图谱文档补充、图谱 SVG/PNG 预览更新。验收：文档前后一致，工具清单覆盖本地数据管理主要可执行任务，验收矩阵、测试方案和逐项验收模板可作为后续代码开发检查表。

### Phase 1：后端 facade 和 Gateway module

交付：`/api/v1/local-data/*` facade、`backend/mcp/modules/local_data.py`、Gateway profiles、全部只读与确认型工具、后端单测和集成测试证据。验收：`LDM-MCP-001` 至 `LDM-MCP-018`、`LDM-MCP-022` 至 `LDM-MCP-025`、`LDM-MCP-027`、`LDM-MCP-028` 通过，且不触碰因子独立指标计算和 Xtquant 同步。

### Phase 2：助手对话和任务状态集成

交付：Prompt Tree 增加 `local_data_management` 分支；对话可识别“检查数据同步情况并修复”；任务状态卡片展示检查、计划、确认、执行、复查；工具调用详情可审计但不作为主视图。验收：`LDM-MCP-019`、`LDM-MCP-021`、`LDM-MCP-024`、`LDM-MCP-026` 至 `LDM-MCP-028` 通过。

### Phase 3：记忆/图谱 seed 和长期状态沉淀

交付：本地数据管理架构记忆 seed、图谱实体和关系 seed、修复任务结果进入 `task_state` 或 candidate memory。验收：`LDM-MCP-020`、`LDM-MCP-026`、`LDM-MCP-027` 通过，助手能回答哪些数据状态会影响 QE / Selection Center / Paper v2。
