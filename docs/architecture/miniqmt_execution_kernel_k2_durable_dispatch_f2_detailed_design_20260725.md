...
> 已合入前置：K1-A/B/C `implemented_verified + merged`。K2-A PR #2729 七项首轮审核补修之后，第二轮独立复审新增发现 initial-state、CANCEL synchronous ACK 后 callback closure、完整 scalar/carrier projection、非自证 catalog fingerprint 四项阻断；当前为 `review_fix_in_progress`，`source_merge=pending_review_fix`。K2-B/C/D与K3/K4 `not_started`，产品 runtime 未切换。
>
...
- 当前 GREEN（最终 classifier/F2/CI 尚在运行）：contracts=`33 passed`；DEV repository=`12 passed`；DEV migration=`11 passed`；repository+contracts coverage run=`45 passed`，`kernel_repository.py` line=`87.12%`、branch=`72.10%`；catalog projection SHA-256=`6e4fc4ae4c6e403d3316c124da6ae5933eb33184129569fd6bf1cf750e27f762`，helper body SHA-256=`8d9c8b09b5c27a0b0caeeaea3663556b9876b0eea179057d691bbf2fce29c107`，canonical-LF forward SHA-256=`24b4e1894f93f1383d7690ff145c55e100a26cecfc9e60a9070b71a57524d083`。
- 当前状态保持 `K2-A review_fix_in_progress`、`source_merge=pending_review_fix`；只有本轮完整 classifier-selected plans、L0/registry、三份F2 validator、DESIGN-COMPLIANCE-001、required CI 与最终独立复审闭合后才恢复 `implemented_verified / pending_user_authorization`。K2 overall仍需K2-B/C/D，不能由K2-A推断完成。
...
本轮正式审核补修RED基线位于`F:\Dev\AIstock_worktrees\miniqmt-k2a-red-baseline-0fb5d31a-20260725`：targeted matrix=`14 failed, 21 passed`，覆盖arbitrary/wrong fence、incomplete calendar、CANCEL mapping/outbox、repository/schema drift、migration checksum与同名错误CHECK/FK/index predicate/type/null/default。当前同三文件targeted matrix=`35 passed`；repository coverage line/branch=`88.29%/75.17%`。DEV PostgreSQL repository=`3 passed`；migration clean first/second apply、guarded rollback及六类drift均通过。
...
| `F-062` | §4.0–§4.10、§9；`plugin_contracts.py`、`kernel_repository.py`、K2 migration triplet | `backend/tests/miniqmt_execution_runtime/test_kernel_contracts.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_repository_postgres.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_migration_postgres.py` targeted矩阵=`35 passed`；DB内catalog fingerprint=`c9d5f192eb4522f54519c8e0c63540218c2674155471c1455c3150bea7a809c4`，canonical-LF forward SHA-256=`ff16497da04755342f2ac1a605c6ef739a75baa6e56e4fcb0b7f7639cb4534b0`；K2-B/C/D继续消费同一schema authority | design_ready | none |
...
| no silent error/fake success | pass | commit后独立连接readback、commit-return unknown只读确认、strict recovery enum、schema drift fail-loud；无空transition、默认ACK、`broker_called`强转、非法status空结果或event/delivery丢弃 |
