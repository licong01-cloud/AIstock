...
6. K6按[`miniqmt_execution_kernel_k6_product_cutover_f2_detailed_design_20260801.md`](miniqmt_execution_kernel_k6_product_cutover_f2_detailed_design_20260801.md)实施：K6-A提供base contracts/migration/repository；K6-C0已补齐strict command_json、V2 proceeds/ledger/coordination、MATERIALIZE/REJECT/DEFER carriers、K6 product-owned deferred mapping contract、successor migration与versioned schema readback。BUG-953已补齐SUBMIT deterministic mapping/child、CANCEL existing-mapping reuse、lifecycle disposition/status/broker/authority closure及exact 29-key physical mapping JSON/scalar equality，并完成source merge、production DDL/readback、用户重启与post-restart verification；shared transition OMS/RISK/ROUTE ref继续与per-command strict receipts分层。当前第一优先级为K6-C1 generic per-command atomic materializer；之后K6-B只把同一physical mapping row/command id由product RESERVED显式交接给现有K2 outbox/dispatch lifecycle；K6-D完成new-instance唯一KERNEL_V2 cutover与旧route drain/retirement。只增加现有K2缺失的coordination/product-authority事实，不复制K2 event/transition/outbox/OMS/Gateway，不进入算法plugin，不允许dual route/fallback，并以successor migration、独立readback、guarded rollback和正常交易日真实SIM验收。
...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1/K2/K3/K4/K5 overall均已`implemented_verified + merged`。K6 base design=`design_ready + merged`（PR #2993），K6-A=`implemented_verified + merged`（PR #3004），revision=`design_revision_ready + merged`（PR #3024），K6-C0=`implemented_verified + merged`（PR #3032，merge `2a3622a3ba63585e3dfe12ef7ccb3f33b00dcb63`）。BUG-953=`implemented_verified + merged + runtime_verified`（PR #3048，merge `f4da00f6838f6da6344223f6bba55dfe606def3e`），Issue #3045已关闭；K6-C1/K6-B/K6-D仍按C1->B->D推进。产品runtime未切换；production DDL=`applied_and_verified`，其余production config/binding/broker/runtime activation gates均为`noop/not_run`。该架构重构不得被设置为当前模拟盘运行的人工审批或额外stop gate，并与现有阻断BUG、生产rollout和运行观测分别记录。
...
| `F-102` | §9 P0-H K6 detailed §4.1；`kernel_product_contracts.py` | `python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py -q`：V2 proceeds/ledger/coordination initial/successor与strict readback | k6c0_implemented_verified | none |
...
| `F-104` | §9 P0-H K6 detailed §4.2、§8；V3 carriers与BUG-953 lineage/lifecycle closure | C0：`python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py -q`；BUG-953 direct+DEV combined=`91 passed`；C1 target `test_kernel_product_authority.py` | implemented_verified | none |
| `F-105` | §9 P0-H K6 detailed §5；K6-A immutable migration + `miniqmt_execution_kernel_k6c_20260802.*` | `AISTOCK_RUN_MINIQMT_K2_DEV_DB=1 python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_k6_migration_postgres.py -q` | k6c0_implemented_verified | none |
| `F-106` | §9 P0-H K6 detailed §6；versioned repository preflight与BUG-953 exact mapping CHECK | `AISTOCK_RUN_MINIQMT_K2_DEV_DB=1 python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_k6_migration_postgres.py backend/tests/miniqmt_execution_runtime/test_kernel_product_repository_postgres.py -q`；combined direct matrix=`91 passed`；C1 target `test_kernel_dependent_buy_postgres.py` | implemented_verified | none |
...
| `F-111` | §9 P0-H K6 detailed §12–§13 | artifact: ordered `K6-C0 -> K6-C1 -> K6-B -> K6-D` PR receipts + separately reported production/runtime states | design_ready | none |
...
| `SIM-P-069` | `F-101..112` | K6 base design、K6-A与readiness revision已合入。K6-C0已实现dependent-BUY V2 proceeds/ledger/coordination、product authority V3 strict full command/evidence carriers、MATERIALIZE/REJECT/DEFER、K6 product-owned mapping carrier及versioned catalog。BUG-953已补齐SUBMIT deterministic lineage、CANCEL existing-mapping reuse、lifecycle status/broker/authority identity closure及exact 29-key physical mapping JSON/scalar CHECK；shared transition projection set与per-command strict OMS/risk/route receipt继续分层。未实现C1 materializer、B coordinator/cross-carrier dispatch handoff或D cutover，也未改signal/selection/package/target/side/quantity/算法/LocalSIM | PR #3032 / merge `2a3622a3ba63585e3dfe12ef7ccb3f33b00dcb63`；BUG-953 PR #3048 / merge `f4da00f6838f6da6344223f6bba55dfe606def3e`，source fix direct+DEV=`91 passed`；production exact K6-C0 preflight=`8/8 true`；post-restart descendant identity与四项probe=`passed`；migration catalog=`f4fc093c...`，forward canonical-LF=`368fc29048ac...` | BUG_953_IMPLEMENTED_VERIFIED_CLOSED | base/K6-A/revision/K6-C0与BUG-953已merge，Issue #3045 closed；K6-C1/B/D=`not_started`；product runtime未切换；production DDL=`applied_and_verified`，其余config/binding/broker/runtime activation/normal-day=`noop/not_run` |
...
| Control | Review result | Design evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass | K1–K5完整authority保持；K6修订闭合strict command_json、V2 ledger/proceeds/coordination、0/1/N MATERIALIZE/REJECT/DEFER、same-command release、single-writer事务、successor migration、cutover/retirement和正常交易日验收，不把K6-A V1或design revision伪报实现完成 |
...
| `production state separation` | pass | K1–K5及K6 base/K6-A/revision/K6-C0 source merge已闭合；BUG-953已完成source merge、production DDL/readback、用户重启、post-restart receipt与Issue关闭，之后仍按C1->B->D；production DDL=`applied_and_verified`，DML/dependency/config/binding/broker/runtime activation/normal-day observation均为`noop/not_run`，产品runtime未切换 |
