...
| `F-079` | §9 P0-H K3 detailed §11–§12、§15 failure/diagnostics/rollback | `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_restart.py`、`test_kernel_callback_events.py`、`test_kernel_outbox_outcome_ingress.py` | implemented_verified_k3a_scope | none |
| `F-080` | §9 P0-H K3 detailed §10.2、§14–§16 visible transport suppression、validation/routing/gates/K6 prerequisite | `python -m nox -s miniqmt_execution_runtime_l2`=`934/27`；`python -m nox -s paper_v2_backend`=`1050/2/2`；DEV=`1 passed`；coverage过80/70；`unmapped_code_files=[]` | implemented_verified_k3a_scope | none |
...
| `SIM-P-066` | `F-071..080` | K3 current-three runtime migration保持两个shadow-only切片。K3-A已实现Sniper/BestLimit/TWAP Lite真实pure `ExecutionAlgoPluginV2`、3.0.0 exact factory/class/binding、transition-first state/effect、pending/unknown/terminal-trade lifecycle、strict ORDER/TRADE/RECONCILE/COMMAND_OUTCOME、durable mapping/outbox/algo闭包、EOD/restart恢复与TWAP raw/effective timer authority；QMT ledger trade fallback必须有canonical hash、first-ingest、parent、amount和价量闭包。K3-B committed-fact shadow source、zero-write inventory及ALGO_LOCAL parity仍未开始。dependent-BUY继续由K6独立durable coordinator拥有；无永久flag、双route、fallback或人工门禁 | direct=`317 passed`；DEV disposable PostgreSQL=`1 passed`；MiniQMT=`934 passed,27 skipped`；Paper=`1050 passed,2 skipped,2 xfailed`；核心coverage=`86.51/70.97`与`86.26/70.39`；classifier MiniQMT/Paper、`unmapped_code_files=[]`；L0/registry通过；PR #2840 / merge `aa155222a1072d6c1110f4cc8a11b4f501d8dd1b` | K3_A_IMPLEMENTED_VERIFIED_MERGED | `source_merge=merged_pr_2840`；K3-A=`implemented_verified + merged`；K3-B/K4/K6=`not_started`；K3 overall=`implementation_in_progress`；产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
