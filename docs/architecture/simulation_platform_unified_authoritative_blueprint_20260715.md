...
| `F-142` | §5.12、§11；legacy-retirement detailed §4.5.1、§4.6.4、§4.8、§4.8.1、`SIM-LR-C`；PR #4035 / `f2eb8ed03`；BUG-1291 | C baseline direct=`30 passed`、Validation Center=`795 passed,2 skipped`、DEV PostgreSQL LocalSIM=`1 passed`、RA Phase 5=`13 passed`；BUG-1291 target `backend/tests/simulation_runtime/test_localsim_cutover_inventory.py` + `backend/tests/scripts/test_prepare_localsim_successor_cutover.py` 覆盖跨日binding保留、显式authority date、同日歧义/future-only、manifest/package drift、date-bound authorization及精确readback | sim_lr_c_source_complete | explicitly approved longline remediation boundary：production-shape审核确认retained daily binding历史是合法事实，不得清理或任意选最新创建行；必须按显式trade-date cutoff解析最大生效日唯一authority。BUG-1291合入前不得执行production preparation；production B2/C schema、lineage DML/readback、用户重启与runtime evidence仍严格后置 |
...
| `SIM-P-086` | `F-133..F-148` LocalSIM legacy retirement与data/signal/execution隔离 | 设计：`localsim_legacy_retirement_and_layer_isolation_f2_detailed_design_20260830.md`；Stage A PR #3994、PR-B1 #4015 / `6ec349a19`、PR-B2 #4020 / `a5ea403f2`、SIM-LR-C #4035 / `f2eb8ed03`均已合入并完成源清理。C交付neutral profile/account/replay/ledger/query、Selection链接、统一frontend/MCP/scripts/lifecycle及旧产品mutation removal；current-head direct=`30+136 passed`、Validation Center=`795 passed,2 skipped`、CI源代码lane全绿；DEV PostgreSQL LocalSIM=`1 passed`、RA Phase 5=`13 passed` | 当前精确状态：`source_merged_pending_production_cutover_preparation`。待授权目标为B2 `localsim_successor_core_20260831.sql`、C `localsim_product_cutover_bridge_20260831.sql`及task-owned retained lineage DML/readback；随后才由用户重启并执行fresh-process/正常交易日/六个月replay验收 | SIM_LR_C_SOURCE_MERGED_PENDING_PRODUCTION_CUTOVER_PREPARATION | source/merge/DEV/cleanup已闭合；production DDL/DML、lineage economic-hash/owner readback、用户restart、runtime、normal-day和`SIM-LR-D`均未完成。不得以#4035合入宣称legacy retirement完成 |
