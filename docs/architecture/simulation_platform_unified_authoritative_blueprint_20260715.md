...
| `SIM-P-086` | `F-133..F-148` LocalSIM legacy retirement与data/signal/execution隔离 | 设计：`localsim_legacy_retirement_and_layer_isolation_f2_detailed_design_20260830.md`；Stage A source已建立`simulation_data/simulation_signal/simulation_execution`边界，迁移共享contract与Selection owner，删除已迁出的V25 day-feature、legacy previous-close和current-day DB-minute入口；本地直接证据：architecture/data/signal `19 passed`、核心no-drift `337 passed`、Paper v2 `452 passed, 1 skipped, 2 xfailed` | target：`SIM-LR-A..D`四阶段、最多六个源PR；layer import、legacy absence、route uniqueness、query budget、single-writer、atomic create、six-month replay、safe-boundary live transition、DEV/restart/normal-day receipts | STAGE_A_SOURCE_IMPLEMENTED_PR_PENDING | Stage A source尚未合入；PR/merge、DEV/production DDL/DML、用户restart、runtime、normal-day及`SIM-LR-B..D`旧产品物理退役分别pending；本阶段production gates仍为noop |
