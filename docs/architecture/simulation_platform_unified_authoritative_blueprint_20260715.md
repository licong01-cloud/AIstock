...
| `F-142` | §5.12、§11；legacy-retirement detailed §4.5.1、§4.6.4、§4.8、§4.8.1、`SIM-LR-C` | target `backend/tests/simulation_architecture/test_localsim_route_uniqueness.py`、`backend/tests/simulation_runtime/test_localsim_ledger_scope_bridge_postgres.py`、`backend/tests/simulation_runtime/test_localsim_cutover_preparation_postgres.py`和commit-bound lineage economic-hash receipt | design_ready + explicitly approved design-only scope | source merge可先于production preparation，但user restart/activation严格后置；缺B2/C schema和lineage readback时新mutation 503且无旧route fallback |
...
| `SIM-P-086` | `F-133..F-148` LocalSIM legacy retirement与data/signal/execution隔离 | 设计：`localsim_legacy_retirement_and_layer_isolation_f2_detailed_design_20260830.md`；Stage A PR #3994已合入清理；PR-B1 PR #4015 / `6ec349a19`已合入清理；PR-B2本地交付internal account/release/binding、lineage、126日replay、safe-boundary和additive schema，Simulation Runtime=`716 passed,3 skipped`、DEV PostgreSQL=`1 passed` | target：`SIM-LR-A..D`四阶段、最多六个源PR；当前B2生产migration固定`localsim_successor_core_20260831.sql`，DEV 5433已验证，生产未授权未执行 | STAGE_A_PR_B1_MERGED_PR_B2_SOURCE_REVIEW | PR-B2仍须最终L2/current-head CI与合入清理；production successor DDL/DML需用户对精确target/migration另行授权，授权/readback前阻断`SIM-LR-C`但不阻断B2 source merge；临时兼容面、用户restart、runtime、normal-day与旧产品退役保持独立状态 |
