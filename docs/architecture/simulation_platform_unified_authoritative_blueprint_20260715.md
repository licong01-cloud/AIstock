...
| `F-033` | §5.4、§9 P0-G Trading Core board-lot/VWAP authority；execution algos/adapter/minute engine；BUG-718 CI mapping；BUG-795 final-child residual contract | backend/tests/trading_core/test_minute_execution.py small-position multi-slice matrix；backend/tests/paper_trading_v2/test_localsim_backend.py LocalSIM 300-share SELL direct test；V25/V25.1 contract tests | implemented_verified | none |
...
| `SIM-P-050` | `F-033,024` | BUG-795 已统一全部注册分钟算法的 non-final board-lot/final exact residual 语义；Base child 默认拒绝 residual，TWAP/POV/V25 只在可证明 final 时显式启用，V24 等继承统一默认；ledger/StepFill authority 保持不变 | BUG-795 / issue #2530；Trading Core TWAP/VWAP/AC/POV/SBB 300-share SELL matrix、50-share final residual、LocalSIM 300-share TWAP direct test，以及 V25/V25.1 contract regression | IMPLEMENTED_VERIFIED | 完成本地 gate、PR/CI；合入并由用户重启后在正常交易日只读核验，不执行 DDL/DML/config/broker call |
...
| Control | Review result | Implementation evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass for BUG-794/795 scope；platform remains open | 首分钟 durable recovery 与 SELL child sizing 均有产品实现和正反 direct tests；mark/economic coupling 与 runtime policy authority 继续分别保留为 `SIM-P-051/052 REPAIR_REQUIRED`，不把已修 slice 冒充 LocalSIM 全面完成 |
