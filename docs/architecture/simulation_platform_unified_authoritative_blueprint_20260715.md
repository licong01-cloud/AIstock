...
| `F-054` | §9 P0-H K1；K1 detailed design §4-§5 strict contracts/canonical codec | target backend/tests/miniqmt_execution_runtime/test_algo_plugin_contracts.py | design_ready | none |
| `F-055` | §9 P0-H K1；K1 detailed design §7 immutable registry | target backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py | design_ready | none |
| `F-056` | §9 P0-H K1；K1 detailed design §6 deterministic context | target backend/tests/miniqmt_execution_runtime/test_deterministic_execution_context.py | design_ready | none |
| `F-057` | §9 P0-H K1；K1 detailed design §1、§8 current-three/legacy projection；K1 shadow-only、runtime switch 属于 K3 | target backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py + existing parity/restart tests | design_ready | none |
...
| Progress ID | Acceptance IDs | Current state after this PR（base `origin/main@ef1da755`） | Evidence | Status | Next implementation slice |
| --- | --- | --- | --- | --- | --- |
...
| `SIM-P-057` | `F-053..060` | 完成 K1 实施级详细设计：strict/frozen DTO、canonical JSON/hash、code-owned immutable registry、explicit factory binding、deterministic logical time/ID/hash draw、current-three config/state/event/capability/source manifest、legacy mixed-config shadow projection、pinned vn.py source/signature/DTO/enum/return/error lock/receipt、import boundary 与 changed-file test routing；K1 保持 shadow-only，不切换现有 runtime | `miniqmt_execution_kernel_k1_contracts_registry_f2_detailed_design_20260722.md`；parent/umbrella F2 validator；DESIGN-COMPLIANCE-001 | DESIGN_READY | `source_merge=pending_pr`；`close_sync=not_applicable_feature_design`；`production_ddl=noop`；`production_dml=noop`；dependency/config/binding/broker/restart/runtime 均 `noop`。下一步为 K1-A strict contracts/canonical/determinism 代码开发，不得把详细设计合入写成 implementation completed |
...
| `no_business_semantic_drift` | pass | exact owner routing 与 predecessor CAS 固定 callback/slot order；Selection/Target、方向数量、A 股规则、B0 quote authority、OMS/Gateway 和 broker route 保持不变；算法迁移要求 trace parity |
| `no_unrequested_gate_or_approval` | pass | capability 仅按静态 unsupported/当前暂缺/非法 observation 自动处理并在 EOD 终结；未新增 RBAC、人工 acknowledge、审批、confirm-run、全局 stop gate 或永久 enable flag |
