...
mark/NAV 是经济事实提交后的派生估值，不得反向成为已发生 fill/state 的事务提交门禁。首根 causal bar 等待已按上一节的 no-mark generation 实施；任一 active symbol 或 passive holding 的 transient mark 缺失也必须采用同一“经济事实先提交、估值显式 pending、后续 cadence 自动补齐”的原则。当前若仍因单一持仓 mark 缺失而回滚同 cadence 健康 symbol 的已形成事实，即属于 `F-030/F-031` 未闭环，不得以 broker 层的逐 symbol 单测宣称全链完成。
...
- `AUDIT-LS-MARK-001 / REPAIR_REQUIRED`：broker 已能把单 symbol 行情失败投影为 waiting，但 scheduler account-mark 仍可能因该 symbol/被动持仓无 mark 而回滚同 generation 全部健康 economic facts。后续独立 BUG 必须实现 economic-first、valuation-pending、next-cadence mark completion，并覆盖 healthy fill + unavailable held symbol + restart/outbox；不得使用旧 mark、计划价、0 价或通用 price map 静默补值。
- `AUDIT-LS-POLICY-001 / IMPLEMENTED_VERIFIED`：BUG-797 已删除 production context 的 release→portfolio fallback 和 `LocalSimBackend` 的 manifest/flat-policy fallback；shared snapshot validator 要求恰好一个已登记 ID 字段、exact schema、完整 policy JSON、normalized hash 以及 release ID/SHA 一致。binding admission 仍只检查 package lifecycle，不新增 policy/package 内容门禁；LocalSIM run context 只校验实际运行必需的冻结 component，明确不复核 package alpha/model/assets，single alpha、multi-alpha parent、需要/不需要 model code 的 package 走同一路径。历史不完整 release typed fail loud 并退休/忽略，不补数据、不增加审批或人工恢复。Paper v2 daemon demo/E2E 作为精确调用依赖改为显式 snapshot wiring，不在 broker 内恢复 manifest fallback。
...
| `F-013` | §5.5；`PaperTradingV2Repository.local_sim_economic_transaction`；`SimulationRuntimeRepository.stage_local_sim_economic_commit`；BUG-794 no-mark economic generation | backend/tests/simulation_runtime/test_lifecycle_scheduler.py；PostgreSQL single-connection commit/rollback、InMemory cross-repository rollback、first-wait order/state/receipt/outbox transaction and independent readback tests | implemented_verified | none |
| `F-014` | §5.2、§5.5；`LocalSimEconomicReceiptV1`、`LocalSimProjectionOutboxV1`、`LocalSimProjectionReceiptV1`；BUG-794 automatic recovery | backend/tests/simulation_runtime/test_lifecycle_scheduler.py；same-bar restart dedupe、CAS、connection retry max-3、wait projection retry、projected-generation receipt/state/Paper exact readback、mismatch terminal and no-parent-resubmit tests | implemented_verified | none |
...
| `F-030` | §4.3、§5.5、§9 P0-G symbol isolation；`LocalSimExecutionStateV1` | backend/tests/paper_trading_v2/test_localsim_backend.py；artifact: docs/architecture/simulation_platform_unified_authoritative_blueprint_20260715.md §9 `AUDIT-LS-MARK-001` | design_ready | none |
...
| `SIM-P-051` | `F-030,031,013,015,024` | 2026-07-21 定向审计确认 broker-level per-symbol waiting 已实现，但 scheduler valuation 与 economic commit 仍耦合；单一 active/passive holding mark 暂缺可拖住整代健康事实 | `AUDIT-LS-MARK-001` source trace：broker snapshot/state isolation → scheduler `_local_sim_position_marks` before economic commit；缺少 healthy fill + unavailable held symbol + restart/outbox oracle | REPAIR_REQUIRED | 独立 P0 实现 economic-first/valuation-pending/next-cadence completion；禁止旧价/计划价/0 价 fallback，不放宽 mark identity/hash |
| `SIM-P-052` | `F-004,005,024` | BUG-797 删除 scheduler release→portfolio 与 broker manifest/flat-policy fallback；LocalSIM run context 验证 persisted release snapshot exact schema/normalized hash/ID-SHA，binding admission 仍只检查 package lifecycle；Paper daemon 显式 wiring；历史 incomplete release typed fail/retire | BUG-797 / issue #2532；single/multi/model-code required/optional binding、scheduler release-over-portfolio、incomplete release no-fallback、broker missing/empty/hash/alias、daemon E2E direct tests | IMPLEMENTED_VERIFIED | 创建修复 PR 并检查 CI；合入后由用户重启并只读核对实际 release policy reason；DDL/DML/config/broker 均为 noop；与 BUG-795/796 独立 |
...
| Control | Review result | Implementation evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass for BUG-794 scope；platform remains open | 首分钟不是 catch-and-ignore：order/state/economic receipt/outbox/Paper run event、projection retry/readback、restart recovery、action chain 和 diagnostics 均有产品实现与正反 direct tests；同时将 SELL child 与 mark/economic coupling 分别标为 `SIM-P-050/051 REPAIR_REQUIRED`，不把本 slice 冒充 LocalSIM 全面完成 |
...
| Control | Review result | Implementation evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass for BUG-797 scope；platform remains open | 不只在 scheduler 删除一行 fallback：release frozen identity、production context、broker constructor、roll-forward、Paper daemon wiring 与直接/恢复路径统一消费 exact snapshot；binding admission 保持 lifecycle-only；missing/empty/hash/alias/ID-SHA conflict 和 single/multi/model-code variants 均有正反 direct tests |
| `no_silent_error` | pass | snapshot 缺失、字段不精确、多 ID alias、空 policy JSON、normalized hash drift、release model identity drift均以 `LOCALSIM_EXECUTION_POLICY_*` / `SIMULATION_RELEASE_EXECUTION_POLICY_HASH_CONFLICT` typed fail loud；不返回 portfolio/manifest/flat policy，不把缺字段解释为“未冲突” |
