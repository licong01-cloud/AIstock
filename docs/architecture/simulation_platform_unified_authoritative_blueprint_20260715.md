...
4. K4 交付不拥有第二套 runtime/OMS/Gateway、且对每个注册算法生成 exact compatibility receipt 的 `VnpyAlgoEngineFacadeV1`；实施级合同为 `miniqmt_execution_kernel_k4_vnpy_facade_f2_detailed_design_20260729.md`，严格限定transition-scoped façade、existing K1/K2/K3 authority复用、current-three conformance和Iceberg/Stop characterization-only，不注册Iceberg/Stop、不改current-three binding、不进入K6 cutover；
...
| `F-085` | Tick/Order/Trade/Contract DTO、enum与event routing逐字段映射，missing/unsupported无合成/fallback |
...
| `F-083` | §9 P0-H K4 detailed §4–§5 | `backend/tests/miniqmt_execution_runtime/test_vnpy_algo_engine_facade.py` create/freeze/ordinal/retry/restart | design_ready_k4 | none |
| `F-084` | §9 P0-H K4 detailed §6 | `backend/tests/miniqmt_execution_runtime/test_vnpy_algo_engine_facade.py` six methods/template/callback/missing diagnostics | design_ready_k4 | none |
| `F-085` | §9 P0-H K4 detailed §7–§8 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_projection.py` DTO/enum/event/lineage matrix | design_ready_k4 | none |
| `F-086` | §9 P0-H K4 detailed §5.3、§9 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_k2_shadow_integration.py` existing command/mapping identity与zero broker | design_ready_k4 | none |
| `F-087` | §9 P0-H K4 detailed §11 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_conformance_receipts.py` writer/readback/truncation/authority drift/zero partial set | design_ready_k4 | none |
| `F-088` | §9 P0-H K4 detailed §3.2、§12 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_characterization.py` current-three parity + Iceberg/Stop characterization-only | design_ready_k4 | none |
...
| `F-090` | §9 P0-H K4 detailed §18–§20 | `python -m nox -s miniqmt_execution_runtime_l2`；`python -m nox -s paper_v2_backend`；K4 core line>=80%/branch>=70%；三份F2 validators | design_ready_k4 | none |
...
| `SIM-P-067` | `F-081..090` | K4 F2详细设计将父蓝图既有范围细化为transition-scoped `VnpyAlgoEngineFacadeV1`、exact DTO/enum/event/effect mapping、per-registered-plugin conformance receipt、current-three parity复用及Iceberg/Stop characterization-only；K1-C V2 authority、K2 durable owner、K3 factory/binding保持不变，无第二runtime/OMS/Gateway/catalog/route，无K5 plugin注册或K6 cutover；正式审核补齐真实code symbol和pinned `round_to` helper source authority | artifact: `docs/architecture/miniqmt_execution_kernel_k4_vnpy_facade_f2_detailed_design_20260729.md`；PR #2861；F2 validators=`10/10,48/48,90/90`且warnings=0；DESIGN-COMPLIANCE-001逐项通过 | K4_DESIGN_READY_FOR_IMPLEMENTATION | `source_merge=pending_user_authorization`；K4 implementation=`not_started`；K5/K6=`not_started`；产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
