...
| `SIM-P-056` | `F-043..070` | MiniQMT统一kernel/plugin F2蓝图保持唯一权威；K2-A七项审核阻断已完成本地source与直接/DEV验证，含command-aware CANCEL、active-child atomic bundle、exact fence、schema fingerprint、post-commit readback、recovery及真实calendar/session owner closure；未启动任何worker或产品route | K2 design PR #2714 / merge `50dbcbf6c5d943944a7c7a0a4e5f3e69f5fbd12e`；K2-A PR #2729 review baseline `0fb5d31a`；RED=`14/21`、GREEN=`35 passed`、coverage=`88.29/75.17`、MiniQMT/Paper/L0/registry/F2全绿；required CI run `30162089997` green | IMPLEMENTATION_IN_PROGRESS | K2-A=`implemented_verified`；K2-B/C/D、K3/K4=`not_started`；source merge=`pending_user_authorization`；production DDL/DML/dependency/config/broker/restart/runtime均`noop` |
...
| `SIM-P-061` | `F-061..070` | K2-A已完成七项正式审核补修闭环：strict carriers、command-aware mapping/outbox、algo durable recount、exact lease/fence、DB catalog authority、independent readback、strict recovery与真实CalendarSnapshotSet/session composite owner；保持K1唯一DTO authority、shadow-only且K3前不切换产品route | `plugin_contracts.py`、`kernel_repository.py`、K2 migration triplet、四份MiniQMT tests（含import-boundary）；targeted=`35 passed`、DEV repository/migration全绿、coverage=`88.29/75.17`、classifier/L2/Paper/L0/registry/F2通过；required CI run `30162089997` green | IMPLEMENTED_VERIFIED | `source_merge=pending_user_authorization`；K2-A=`implemented_verified`；K2-B/C/D、K3/K4=`not_started`；`close_sync=not_applicable_feature`；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
