...
| `F-102` | §9 P0-H K6 detailed §4.1 | target `backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py` dependent-BUY strict/negative matrix | design_ready | none |
...
| `F-105` | §9 P0-H K6 detailed §5 | target `backend/tests/miniqmt_execution_runtime/test_kernel_k6_migration_postgres.py` clean/reapply/drift/readback/rollback | design_ready | none |
| `F-106` | §9 P0-H K6 detailed §6 | target `backend/tests/miniqmt_execution_runtime/test_kernel_k6_repository_postgres.py` atomicity/CAS/fence/concurrency/commit-unknown | design_ready | none |
...
| `SIM-P-069` | `F-101..112` | K6 F2详细设计已合入：只为dependent-BUY coordination和generic per-command product authority增加缺失durable事实，复用K2 event/delivery/transition/mapping/child/outbox/callback/reconcile；固定qmt strategy ledger settled-cash authority、SELL TRADE/ORDER/ACCOUNT/EOD状态机、single-writer/CAS/fence/no-double-release、0/1/N command V2 aggregate、V1 product拒绝、new-instance唯一KERNEL_V2 cutover、legacy drain/retirement、additive migration/readback/guarded rollback及正常交易日验收。设计不修改signal/selection/package/target/side/quantity/算法/LocalSIM，不增加dual route/fallback/RBAC/审批/人工acknowledge或manual recovery | `docs/architecture/miniqmt_execution_kernel_k6_product_cutover_f2_detailed_design_20260801.md`；`F-101..112` index/matrix；三份F2 validator=`12/12,70/70,112/112`且warnings=0；PR #2993 / merge `f2a7a23d` | K6_DESIGN_READY_MERGED | `source_merge=merged_pr_2993`；K6-A/B/C/D implementation=`not_started`；product runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation/normal-day observation全部`noop/not_run` |
...
| `production state separation` | pass | K1–K5 source merge已闭合；K6 design=`design_ready + merged`（PR #2993 / merge `f2a7a23d`）、K6-A/B/C/D implementation=`not_started`；DDL/DML/dependency/config/binding/restart/broker/runtime activation/normal-day observation继续分开且均为`noop/not_run`，产品runtime未切换 |
