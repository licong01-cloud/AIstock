...
| `F-130` | §5.3；daily-context detailed §1–§3/§7；目标broker-specific resolver与source matrix | 目标 `backend/tests/simulation_runtime/test_broker_limit_authority.py`：MiniQMT exact detail/date/freshness/no-limit、LocalSIM Tushare/TDX trigger taxonomy与cross-broker deny | design_ready | explicitly approved staged delivery：本次先冻结设计，source not implemented |
| `F-131` | §5.3；daily-context detailed §4/§8；目标`DailyTradingContextV2`、decision/plan/recovery | 目标 `backend/tests/simulation_runtime/test_daily_trading_context_v2.py`：V1/V2 discriminated readback、tamper/hash、no-limit、symbol isolation与no reallocation | design_ready | explicitly approved staged delivery：本次先冻结设计，source not implemented |
...
| `SIM-P-085` | `F-130..F-132` broker-specific daily limit authority | 2026-08-26用户明确批准先更新蓝图与详细设计：MiniQMT direct instrument detail；LocalSIM stk_limit优先+TDX reference派生备选；V2与symbol隔离 | 本设计PR、F2 validator、多轮DESIGN-COMPLIANCE与diff/link checks；源码/BUG/运行证据尚未开始 | DESIGN_REVISION_REVIEW_PASSED_READY_FOR_MERGE | source implementation、DEV、merge、用户restart、MiniQMT/LocalSIM normal-day receipts分别pending；DDL/DML/dependency/config/broker/service-control均noop |
