...
| `SIM-P-069` | `F-101..112` | K6-A/C0/C1/B与K6-D source slice保持既有闭合：SIM/runtime-binding-account exact closure、code-owned KERNEL_V2 cutover、full-five统一ALGO_START、真实QMT callback、scheduler-driven SESSION/TIMER/EOD、K2 outbox/reconcile、dependent-BUY及legacy product caller物理删除均已合入。2026-08-10首次正常交易日生产观察证明B0 physical feed与58-symbol callback正常，但历史P1-D named event type/source allowlist仍拒绝KERNEL_V2 uppercase source，K2 composite另缺ALGO_START v2/COMMAND_OUTCOME；因此source完成不等于production schema/runtime完成。BUG-1004只修复该唯一schema authority，不恢复legacy route、不改算法/信号/数量/OMS/Gateway，也不增加审批门禁 | 原product integration/clock、quote activation、DEV writer/materialization、MiniQMT/Paper/Simulation与PR #3146 / merge `33c09049e82c11cdbae7cd9b596b3666cb481349`证据保持；新增生产只读pg_catalog证据=`27 type/7 source/10 composite/0 KERNEL_V2 rows`，BUG-1004 DEV successor migration已覆盖完整12 composite | K6_SOURCE_IMPLEMENTED_VERIFIED_MERGED_RUNTIME_BLOCKED_BY_SCHEMA | `source_merge=merged_pr_3146`；K6 source slices=`5/5`；BUG-1004 `source_merge=pending_pr`、`production_ddl_gate=pending`；当前runtime `FAILED_RETRYABLE`且broker child/order=0；用户restart与正常交易日仍未闭合 |
...
| `SIM-P-075` | `F-021,F-105,F-107,F-109` | BUG-1004修复KERNEL_V2 product runtime与历史event CHECK authority断裂：保留P1-D全部LEGACY/quote literals并与现有运行时`EventTypeV2/EventSourceV2`及唯一内部routing table形成只读schema投影，不修改冻结plugin contract source identity；新增operator-only additive successor migration，把named type/source与composite从27/7/10 predecessor原子升级为36/14/12 target。SQL和Python readback均拒绝constraint shape、literal、migration-state与composite drift；rollback仅在零KERNEL_V2-only durable fact时恢复predecessor。应用startup不执行DDL，既有quote schema gate不再对predecessor假报`applied_and_verified`。 | 生产只读pg_catalog复现`ck_miniqmt_event_source`拒绝`EXCHANGE_SESSION_CLOCK`且现有K2 composite缺ALGO_START v2/COMMAND_OUTCOME；direct schema/migration=`16 passed,1 gated skip`，line/branch=`92.76%/76.47%`；DEV PostgreSQL首次/二次apply、drift拒绝、SESSION/ALGO_START v2/COMMAND_OUTCOME、rollback refusal、cleanup和双rollback=`1 passed`；MiniQMT L2=`1351 passed,70 skipped` | BUG_1004_IMPLEMENTED_VERIFIED_PENDING_PR | GitHub Issue #3237 OPEN；implementation commit=`03632ea83776da526ba9993b21483fe5eb4aaef8`；`source_merge=pending_pr`；`production_ddl_gate=pending`且未授权/未执行；`backend_restart_owner=user`；第二个独立virtual-account BUG不得并入本scope；broker/config/binding/dependency/service-control均`noop` |
