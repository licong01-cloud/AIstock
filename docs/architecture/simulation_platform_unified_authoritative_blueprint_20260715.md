...
| `SIM-P-074` | `F-007,F-009,F-012,F-014,F-021` | BUG-992 first source slice closes LocalSIM lifecycle authority and economic readback with one pure contract over current run/binding/trade-date/plan identity, strict continuous receipt generations/high-watermark, exact receipt/outbox identity and latest committed `state_id -> state_hash` set。2026-08-13 用户重启后只读验证再次确认 merged source identity 已加载，但生产 run `simrun_7bf1e0c1b6b7d055` 仍为 `FAILED_RETRYABLE`。实时 retry evidence 证明其 receipt generation 1..3 属于 side-effect-free rebuild 前的 plan，generation 4 属于 rebuild 后经 LocalSIM cash-fit 得到的 current plan；旧 strict closure 错把全部历史 receipt/state 强制绑定 current plan，并且只允许 `FAILED_TERMINAL` active generation 进入 durable minute-loop recovery。最终 continuation 只接受一条 hash-closed、generation 连续且单向的 predecessor→current plan replacement：replacement metadata、LocalSIM backend、cash-fit authority、每代 receipt identity/economic facts、predecessor/current exact state hashes、同一 intent set及 portfolio/symbol/side/quantity/fill/status/algo/schedule/causality/plan semantics全部闭合；latest generation仍是唯一执行 owner，predecessor states仅审计保留。`FAILED_RETRYABLE` 与 `FAILED_TERMINAL` 均可在禁止 parent resubmit、禁止 predecessor projection replay 的同一 durable minute loop 上自动推进到 terminal closure，并保留各自独立 reason/schema evidence。任意未知 plan、plan sequence 回跳、state hash、intent set或执行语义漂移继续 typed fail loud。 | post-restart production RED：merged runtime 下目标 run generation=`4`、current plan=`plan_4e346c75b6a5f0e5`，恢复连续 15 次失败于 `LOCALSIM_DURABLE_STATE_AUTHORITY_RECEIPT_IDENTITY_CONFLICT`；实现前合法跨 plan authority nodeid=`1 failed,1 passed`。补修 focused authority/post-close/historical matrix=`41 passed`；生产只读 payload 通过新 pure closure后精确选择 current plan 246 个 authoritative active states，不修改生产 row。最终模块计划、coverage、classifier、L0、registry 与 F2 evidence在本 continuation PR final HEAD 后绑定。原 PR #3186 / merge `60261969efcba6fa760fcc29e83b4ac938f2ad2c` 与 PR #3205 / merge `152f1bca62929f0a4ca1433030957cbabeae8ddd` 保留为前两次 source slice。 | BUG_992_POSTRESTART_CONTINUATION_IN_PROGRESS | `prior_source_merge=merged_pr_3186_and_pr_3205`；`current_source_merge=pending_pr`；`close_sync=pending_user_restart_runtime_identity_and_business_readback`；`post_restart_identity_gate=pending_current_continuation_source`；`post_restart_business_gate=pending_target_run_terminal_recovery`；`backend_restart_owner=user`；本次仅执行生产只读 authority/readback，未修改生产 run；production DDL/DML/dependency/config/binding/broker/service-control 全部 `noop` |
...
| Control | Review result | Implementation evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass after 2026-08-13 post-restart continuation review; prior runtime-effective claim withdrawn again | Shared closure仍验证 current durable identity、全部 receipt generation、high-watermark、exact latest state authority、empty-authority carrier absence和同-plan history；跨 plan 只接受由 LocalSIM side-effect-free rebuild/cash-fit metadata、严格单向 generation plan 序列、predecessor/current receipt facts与state hashes、同一intent set和完整执行语义共同证明的replacement。不是忽略旧receipt或直接把 `FAILED_RETRYABLE` 加入成功列表。 |
...
| `no_business_semantic_drift` | pass | Frozen Selection/current plan intents、strategy package/assets、side、quantity、algorithm、causal minute authority、broker route、CAS/single-writer/transaction/outbox/projection和既有post-close decision均未改变。旧plan generation只能作为hash-closed审计历史；current latest generation仍是唯一执行owner。恢复不重建plan、不重提parent、不调用broker、不重放predecessor projection。 |
