...
>
> 2026-08-08 的112项audit是历史快照；2026-08-12扩展为121项，2026-08-16增加 `F-122..F-125` 的 TWAP-only 与 V25 execution-algorithm retirement 契约，2026-08-21增加 `F-126..F-129` 的每日交易事实冻结、LocalSIM TDX-only 热路径与 MiniQMT 有界异常契约，现为129项。`F-113/F-114/F-115/F-118/F-119`已有 BUG-1041 source/DEV evidence，`F-121`已有1M no-action局部容量证据但缺正常交易日 receipt；`F-116/F-117/F-120/F-126..F-128`仍未完成全部源码与运行时证据，`F-129` 已由 BUG-1144 完成 source/L2 验证但 source merge、用户重启和正常交易日 runtime evidence 仍待后续。凡旧条目依赖durable TICK、per-wait event、NO_FILL append、unbounded history、每 cadence 静态行情 loader 或 realtime previous-close/ST limit 推导，其历史`implemented_verified`只保留审计记录，不再代表满足当前目标。凡旧条目把 V25 描述为 LocalSIM/MiniQMT 当前可执行算法，其结论同样只保留为历史审计记录，不再代表当前 runtime authority。
>
...
| `F-126` | §5.3；daily-context detailed §1/§3；BUG-1143 | `DailyTradingContextProvider` 在09:10前零probe，之后对`stk_limit/suspend_d`各一次dataset readiness并对`stk_limit/suspend_d/stock_st`各一次exact set-based SQL；raw limit exact coverage/finite/order/hash、ST/suspend独立事实和alias collision fail loud；DEV rollback-only 8-symbol真实表验证固定5 queries且四类fixture readback为0 | source_merged_direct_and_dev_verified | 用户重启与正常交易日380-symbol/query-fingerprint evidence pending |
| `F-127` | §5.3；daily-context detailed §4；BUG-1143 | strict `DailyTradingContextV1/DailyTradingSymbolFactV1`、row/symbol/context hash、decision reference、ExecutionPlan唯一carrier、LocalSIM admission/readback missing/corrupt/partial coverage fail loud；schema复用既有plan JSON，无DDL | source_merged_direct_and_dev_verified | 用户重启与真实persisted plan readback pending |
| `F-128` | §5.3、§7.5；daily-context detailed §5/§7–§9；BUG-1143 | `LocalSimMarketSnapshotV2`绑定daily context identity；真实Paper provider的TDX live入口只接受frozen `market.stk_limit` symbol fact并只调用TDX，删除previous-close/ST derived live authority，V25 day-feature live入口typed reject；Paper直接109项与Simulation lifecycle 329项通过、2项既有skip | source_merged_direct_verified_dev_query_budget_passed | 用户重启、整日soak及正常交易日forbidden SQL fingerprint evidence pending |
| `F-129` | §5.9、§7.5；daily-context detailed §6/§9；`_ProcessLocalQuoteFailureGovernor` 与 `QuoteIngressSupervisor` single-owner assembly | BUG-1144 / Issue #3653；`backend/tests/miniqmt_execution_runtime/test_quote_ingress.py`：100,000 identical contextual exceptions 仅首次与窗口 aggregate、fingerprint/message/source 变化立即可见、256 fingerprint/3 symbol sample 有界、成功 observation 自动清除 active 且保留累计统计、projection/worker 同一 governor；`backend/tests/miniqmt_execution_runtime/test_hot_market_data_boundary.py`：governor/projection/worker 零 repository/query/write/outbox seam，既有 economic effect failure 继续 fail loud；focused=`120 passed`；`miniqmt_execution_runtime_l2=1510 passed,82 skipped` | implemented_verified_source_ready_for_pr | none |
...
| `SIM-P-084` | daily trading context / DB storm remediation | design PR #3651 / merge `f8b9d16106a7841a153d1504ba73022f34b176b5`；BUG-1143 PR #3657 merged (`daea8091`)；BUG-1144 PR #3662 source/L2 verified，合入前主线同步复审中 | BUG-1143 DEV rollback-only query-budget passed；BUG-1144 focused/L2 passed；DDL/DML/dependency gates noop | source 尚未进入运行进程；用户重启与正常交易日 evidence pending | `F-126..F-128` source merged、runtime pending；`F-129` source verified pending merge；正常交易日 query/log-rate evidence pending |
...
| `production state separation` | pass | 设计PR #3651已合入；BUG-1143与BUG-1144分别承载LocalSIM和MiniQMT源码切片。BUG-1143当前仅source/direct tests与DEV rollback-only query-budget完成，source merge、用户重启、正常交易日runtime evidence仍pending；DDL、持久DEV/生产DML、dependency/config/binding/broker/service-control均`noop`，不得把本行写成runtime activated |
