...
>
> MiniQMT execution-kernel进度（2026-07-30）：K1/K2/K3 overall=`implemented_verified + merged`；K4-A pure façade/adapter、strict contract、pinned source identity与fail-closed observation slice已通过 PR #2883 / merge `527b2f4a58d3fe84f85c0b1f4ba2fe375d181dda` 完成`implemented_verified_contract_slice + merged`，K4 overall=`implementation_in_progress`；K4-B exact source executor/positive conformance/integration、K5/K6=`not_started`，产品runtime未切换。
>
...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1-A/B/C、K2-A、K2-A-M1、K2-B、K2-C和K2-D均已`implemented_verified + merged`。K2-D final source `82c69fbf7e7245e0af76262ddc7b7f59ce7d996b` 经 required CI run `30269640126` 验证，并通过 PR #2804 / merge `fc4170faa10847c0b58aa8088b4a8b6d0ca26b29` 合入，`source_merge=merged_pr_2804`；K2 overall=`implemented_verified + merged`。K3-A 已通过 PR #2840 / merge `aa155222a1072d6c1110f4cc8a11b4f501d8dd1b` 完成`implemented_verified + merged`；K3-B 已通过 PR #2848 / merge `38434e10d530edd883fa75f904de5b025158f918` 完成`implemented_verified + merged`，`source_merge=merged_pr_2848`，K3 overall=`implemented_verified + merged`；K4详细设计已通过 PR #2861 / merge `8250b64ff3c2deb04eb3594f1ae3fba3acd1e6ce` 合入，状态=`design_ready_for_implementation`、`source_merge=merged_pr_2861`、implementation=`not_started`，K5/K6仍为`not_started`。`close_sync=not_applicable_feature`，产品 runtime 未切换，production DDL/DML/dependency/config/binding/broker/restart/runtime activation gates 均为`noop`。该架构重构不得被设置为当前模拟盘运行的人工审批或额外 stop gate；按 K0-K6 独立审核切片实施，并与现有阻断 BUG、生产 rollout 和运行观测分别记录。
...
| `F-081` | §9 P0-H K4 detailed §0–§2 | `backend/tests/miniqmt_execution_runtime/test_vnpy_k4_scope_boundaries.py` | design_ready_k4 | none |
| `F-082` | §9 P0-H K4 detailed §3 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_contracts.py`与`test_vnpy_facade_characterization.py`闭合六source、K1 delegated path、bytes/hash/readback | implementation_in_progress_k4a_verified | none |
| `F-083` | §9 P0-H K4 detailed §4–§5 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade.py`、`test_vnpy_facade_adapter.py`、`test_vnpy_facade_lifecycle.py`闭合strict input、collector ordinal/freeze、constructor-once/retry与direct SPI bypass | implementation_in_progress_k4a_verified | none |
| `F-084` | §9 P0-H K4 detailed §5.4.1、§6 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade.py`与`test_vnpy_facade_adapter.py`闭合六方法、四callback router和typed missing/rounded-zero/error语义 | implementation_in_progress_k4a_verified | none |
| `F-085` | §9 P0-H K4 detailed §6.3–§8 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_projection.py`闭合DTO/enum/round_to/source/readback | implementation_in_progress_k4a_verified | none |
| `F-086` | §9 P0-H K4 detailed §5.4.1、§5.6、§9–§10 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_adapter.py`与`test_vnpy_facade_lifecycle.py`闭合constructor-once、restore、callback fact、active-child CLEAN terminal、retry | implementation_in_progress_k4a_verified | none |
| `F-087` | §9 P0-H K4 detailed §4.1、§11 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_contracts.py`与`test_vnpy_facade_characterization.py`闭合live implementation/method/DTO/state/terminal/isolated contracts、bounded failure及writer/readback | implementation_in_progress_k4a_verified | none |
| `F-088` | §9 P0-H K4 detailed §3.2、§6.3、§10、§12 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_characterization.py` current-three init/transition parity + Iceberg TIMER durable TICK/Stop characterization-only | design_ready_k4 | none |
| `F-089` | §9 P0-H K4 detailed §13–§17 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_failure_restart.py` error/retry/restart/diagnostics | design_ready_k4 | none |
| `F-090` | §9 P0-H K4 detailed §18–§20 | `python -m pytest -q backend/tests/miniqmt_execution_runtime/test_vnpy_facade_*.py`核心coverage=`49 passed`；MiniQMT=`1038/29`、Paper=`1050/2/2`；classifier无unmapped | implementation_in_progress_k4a_verified | none |
...
| `SIM-P-067` | `F-081..090` | K4设计已合入；K4-A现已实现pure `VnpyAlgoEngineFacadeV1`、通用adapter、strict carriers、DTO/enum/state/terminal/isolated bindings、deterministic observation primitives和六source identity authority。正式审核补修了market-data strict projection、delivery/predecessor/mapping closure、callback-before-ACK broker identity与sealed adapter receipt closure；caller expected→actual observation始终产生显式FAILED，K4-B exact pinned-source executor到位前algorithm binding/conformance set zero-publication。K1只委托四个exact K4 artifact路径且继续拒绝unknown extra；active-order由durable mapping+ORDER/TRADE callback facts重建，active-child非CLEAN不得terminal。K4-A不修改K2 creation/delivery/repository、current-three factory/binding或产品route；K4-B继续拥有optional invocation/read-only query、真实current-three parity与Iceberg/Stop完整characterization | design PR #2861 / merge `8250b64ff...`；K4-A PR #2883 / merge `527b2f4a58d3fe84f85c0b1f4ba2fe375d181dda`；review-fix direct=`58 passed`；五核心line/branch=`92.04/79.09,86.67/76.87,83.33/71.43,92.45/71.08,97.45/94.64`；MiniQMT=`1047/29`、Paper=`1050/2/2`；full classifier=`21 files`、ownership=`21/21`、`unmapped_code_files=[]`；F2=`10/10,48/48,90/90`；live contract=`59f9aed0...`、source manifest=`e0284a6d...` | K4_IMPLEMENTATION_IN_PROGRESS_K4A_CONTRACT_VERIFIED_MERGED | `k4a_source_merge=merged_pr_2883`；K4-A=`implemented_verified_contract_slice + merged`、K4-B/K5/K6=`not_started`；产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
...
| Control | Review result | Design evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass | ALGO_START、exact routing、per-algo ordering、strict callback/outcome、state/timer/failure transition、unknown outbox、pinned façade、迁移和旧route退役均闭合；K3 parity必须由committed legacy repository facts和DEV readback驱动；K4多command trace不得截断、复用单command receipt或冒充V1 product materialization，不以fixture/helper-only冒充完成 |
| `no_silent_error` | pass | stale DISPATCHING不false重提；同步ACK不伪装ORDER/TRADE/OMS event，deterministic COMMAND_OUTCOME ingress推进outbox outcome；K4 pure/K2-V1-shadow conformance不得冒充product command authority；malformed payload/state/lifecycle/source、sequence/identity/hash conflict、持久化失败和unknown均typed fail loud |
| `no_business_semantic_drift` | pass | exact owner routing与predecessor CAS固定callback/slot order；TWAP raw/effective due分权保持午休/EOD/restart；legacy重复cancel仅在同target/reason pending transport时可见抑制，negative outcome后的新cancel不被吞；Selection/Target、方向数量、A股规则、B0、OMS/Gateway和broker route不变 |
