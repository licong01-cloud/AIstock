...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1/K2/K3/K4 overall均已`implemented_verified + merged`。K4-A=`implemented_verified_contract_slice + merged`；K4-B已通过PR #2953 / merge `cbb5f12871f41d1fd529a9a98e8811484eac8ba0`合入。K5详细设计[`miniqmt_execution_kernel_k5_iceberg_stop_plugins_f2_detailed_design_20260731.md`](miniqmt_execution_kernel_k5_iceberg_stop_plugins_f2_detailed_design_20260731.md)的design source已合入PR #2968；implementation本地direct/DEV/coverage/MiniQMT/Paper已闭合并开放PR #2978，状态=`implemented_verified_local_pr_2978_pending_required_ci`、source merge=`pending_pr_2978`；K6=`not_started`。`close_sync=not_applicable_feature`，产品 runtime 未切换，production DDL/DML/dependency/config/binding/broker/restart/runtime activation gates 均为`noop`。该架构重构不得被设置为当前模拟盘运行的人工审批或额外 stop gate；按 K0-K6 独立审核切片实施，并与现有阻断 BUG、生产 rollout 和运行观测分别记录。
...
| `F-100` | §9 P0-H K5 detailed §13 | artifact: K5 detailed design、父蓝图、统一蓝图；PR #2978 已开放，required CI/merge/aftercare 与生产/runtime状态分离 | implemented_verified_local | none |
...
| `SIM-P-068` | `F-091..100` | K5 exact Iceberg/Stop manifest/config/source、K4 provenance到K1 full-five shadow bridge、code-owned/fresh V2 binding、config-only factory、shared conformance evaluator、strict state codec、Iceberg TIMER/cancel/late callback/`>=`、Stop TICK exactly-once/`==`、K2 broker-neutral shadow均已本地实现。正式代码审核进一步闭合fractional durable carrier、factory real probe、typed error、terminal CLEAN、public binding readback、SPI ownership/import-boundary、DEV并发/ingress及branch coverage，不增加产品route、algo kernel分支或门禁 | direct aggregate=`38 passed`、lifecycle=`15 passed`、manifest final=`4 passed`、DEV=`1 passed`；coverage binding/factory/manifest/shadow line/branch=`100/100,95.74/75,89.86/70.97,93.83/72.22`；MiniQMT=`1127 passed,31 skipped`、Paper=`1050 passed,2 skipped,2 xfailed`；classifier=`targeted_ci_required`、19 files、MiniQMT+Paper、unmapped=[]；implementation commit=`09e0755b`，PR #2978 open | K5_IMPLEMENTED_VERIFIED_LOCAL_PR_OPEN_PENDING_REQUIRED_CI | `k5_design_source_merge=merged_pr_2968`；implementation source merge=`pending_pr_2978`，required CI/merge/aftercare pending；K6=`not_started`；product runtime未切换；production gates全部`noop` |

`SIM-P-068` 的 PR #2978 initial required CI run `30630489853` 在 Linux/Python 3.12 显式拒绝了 Windows/Python 3.13 生成的 K5 binding。根因不是算法或 route，而是 K4 AST hash 依赖 Python minor-version 的 empty-field 默认值，且 source executor signature 绑定绝对 checkout 路径与 `WindowsPath/PosixPath`。复审补修将 AST 固定为 Python 3.12 full-field canonical shape，将 executor signature 固定为 repo-relative structured payload；Windows/Python 3.13 K5 direct=`12 passed`，Linux/Python 3.12 exact authority=`3 passed`且 fresh binding 相同。该补修不改变 Iceberg/Stop、signal/target/side/quantity、B0、OMS/Gateway、K2 shadow 或产品 route，也不新增 gate/审批；`source_merge=pending_pr_2978`，等待新 required CI。
...
| `production state separation` | pass | K1-B/K1-C与K2/K3/K4 source merge已闭合；K5 design source已合入PR #2968，implementation=`implemented_verified_local_pr_2978_pending_required_ci`且source merge=`pending_pr_2978`；DDL/DML/dependency/config/binding/restart/broker与runtime evidence继续分开且均为`noop`，产品runtime未切换 |
