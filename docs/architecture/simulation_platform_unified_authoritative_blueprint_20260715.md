...
| `SIM-P-065` | `F-066,F-068,F-069,F-070` | K2-D实现shadow-only durable outbox/reconcile/observability：唯一dispatcher/reconciler与gateway adapter执行PENDING/FAILED_RETRYABLE→CLAIMED→DISPATCHING→ACK/reject/unknown；broker边界前冻结callback watermark，unknown只在exact capability、watermark变化和nonacceptance receipt闭合时安全重试；sync ACK不伪造mapping event lineage，后续真实ORDER/TRADE/RECONCILE atomic ingress完成闭合；每次reconcile snapshot先写append-only history再CAS，crash retry复用durable receipt；只读diagnostics区分NOT_APPLIED/NOT_ACTIVATED/NOT_FOUND并输出低基数metrics、auto-clear alerts与bounded chains；additive migration具有独立catalog readback和有数据rollback保护；正式审核补修缺失broker query静默空快照、仅数值broker identity的CANCEL匹配和无用`plugin_id` metric label allowlist | `backend/tests/miniqmt_execution_runtime/test_kernel_outbox.py`与`test_kernel_diagnostics.py` direct=`42 passed`；含DEV repository coverage矩阵=`43 passed`，migration DEV=`15 passed`；`kernel_outbox.py` line/branch=`90.83%/75.41%`、`kernel_diagnostics.py`=`89.38%/81.48%`、`kernel_repository_diagnostics.py`=`97.22%/96.15%`；classifier仅MiniQMT/Simulation且required CI receipt待PR闭合 | IMPLEMENTED_VERIFIED_SOURCE_PENDING_MERGE | `source_merge=pending_pr`；`close_sync=not_applicable_feature`；K3/K4=`not_started`；shadow-only且产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
