...
| `F-062` | K2 durable schema、identity/hash与CHECK/unique/composite FK实施级合同完整 |
...
| `F-066` | K2 outbox、nullable broker-called、callback race和unknown reconcile不重复broker side effect |
| `F-067` | K2 ExchangeSessionClock、durable timer、午休/catch-up/EOD/restart完整 |
...
| `F-061` | §9 P0-H K2 detailed §1–§3 | artifact: `docs/architecture/miniqmt_execution_kernel_k2_durable_dispatch_f2_detailed_design_20260725.md`；exact `backend/services/miniqmt_execution_runtime/runtime.py`、`repository.py` 与 migration symbols | design_ready | none |
| `F-062` | §9 P0-H K2 detailed §4、§9 | target `backend/tests/miniqmt_execution_runtime/test_kernel_repository_postgres.py`、`test_kernel_contracts.py` | design_ready | none |
| `F-063` | §9 P0-H K2 detailed §5–§6.2 | target `backend/tests/miniqmt_execution_runtime/test_kernel_ingress.py` | design_ready | none |
| `F-064` | §9 P0-H K2 detailed §4.2–4.4、§6.3、§7 | target `backend/tests/miniqmt_execution_runtime/test_kernel_delivery.py` | design_ready | none |
| `F-065` | §9 P0-H K2 detailed §4.4、§6.3、§7.2 | target `backend/tests/miniqmt_execution_runtime/test_kernel_failure_recovery.py` | design_ready | none |
| `F-066` | §9 P0-H K2 detailed §4.5、§6.4–6.5 | target `backend/tests/miniqmt_execution_runtime/test_kernel_outbox.py` | design_ready | none |
| `F-067` | §9 P0-H K2 detailed §4.6、§8 | target `backend/tests/miniqmt_execution_runtime/test_exchange_session_clock.py` | design_ready | none |
| `F-068` | §9 P0-H K2 detailed §9 | target `backend/tests/miniqmt_execution_runtime/test_kernel_migration_postgres.py` | design_ready | none |
...
| `F-070` | §9 P0-H K2 detailed §11–§13 | command `python -m nox -s miniqmt_execution_runtime_l2`；K2 F2 validator command | design_ready | none |
...
| `SIM-P-056` | `F-043..070` | 完成 MiniQMT 统一 durable execution kernel、无副作用算法插件 SPI、真实 timer/session clock、command outbox 与受控 vn.py compatibility façade 的 F2 实现蓝图；正式审核补齐 ALGO_START identity、exact event routing、per-algo predecessor ordering、nullable stale-dispatch truth、plugin failure/active-child closure、capability 三分法和 pinned exact façade receipt；K2下位设计进一步固定唯一 durable schema/repository、transaction、clock/outbox/reconcile与K2-A..D切片 | `miniqmt_execution_kernel_vnpy_plugin_architecture_f2_design_20260722.md`、`miniqmt_execution_kernel_k2_durable_dispatch_f2_detailed_design_20260725.md`；F2 validator；DESIGN-COMPLIANCE-001；PR #2620 / merge `14962c7328ebab6cddd579c402d896d4b90e6670`；state-sync PR #2625 / merge `7897384868d2ab2fb224f4e507981dadca0be201` | IMPLEMENTATION_IN_PROGRESS | K1-A/B/C implemented_verified + merged；K1-C PR #2685 / merge `e4faeb53663cb4d19eb4e07d833953725a40fdc1`，`source_merge=merged_pr_2685`；K1 overall `implemented_verified + merged`；K2 detailed design `design_ready`、source `not_started`，K3/K4 `not_started`，产品 runtime 未切换；production DDL/DML/dependency/config/broker/restart/runtime 均 `noop` |
...
| `SIM-P-061` | `F-061..070` | K2 F2实施级详细设计闭合现有同步直接Gateway副作用、非原子event/delivery与metadata state缺口；固定唯一K1 DTO authority、additive DDL/composite FK、ALGO_START/ingress/delivery transaction、failure/skip、ExchangeSessionClock、outbox three-phase、nullable broker-called、OUTCOME_UNKNOWN reconcile、diagnostics/retention/runbook与K2-A..D切片。明确K2 source shadow-only、K3前不切换产品route、legacy inventory不伪造V2历史事实 | `docs/architecture/miniqmt_execution_kernel_k2_durable_dispatch_f2_detailed_design_20260725.md`；parent/umbrella F2 validator；DESIGN-COMPLIANCE-001；changed-files docs routing | DESIGN_READY | `source_merge=pending_pr`；K2 source `not_started`；K3/K4 `not_started`；`close_sync=not_applicable_feature_design`；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
