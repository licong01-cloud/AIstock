...
LocalSIM 新control plane至少原子创建`SimulationAccountV1 + StrategyRuntimeRelease + SimulationReleaseBinding`，并提供successor release、pause/resume/retire、isolated historical replay及read-only account/release/binding/run/ledger查询。`SimulationAccountV1`冻结account/hash、LOCAL_SIM backend、package/admission identity、initial capital和CAS lifecycle，不保存第二份release/policy快照；current effective binding只由binding lifecycle查询。创建和readback任一步失败必须整体rollback且零orphan；UI/MCP不得创建session、主动tick或调用旧readiness/scheduler入口。产品router不得公开scheduler `start/stop/tick` mutation；scheduler进程内生命周期只由backend application lifecycle owner管理，保留read-only status/verification/diagnostics。

产品切流只有一个批准顺序：

1. 在非生产隔离范围验证新data/signal/execution/control-plane/replay能力；
2. 一个cutover release同时启用新LocalSIM产品route并删除旧mutation route/caller，生产不允许dual write或shadow order；
...
3. **`SIM-LR-C` Atomic Product Cutover（LR-6，1 PR）**：只在successor schema/lineage生产readback和legacy session/auto-run/sentinel/scheduler active owner归零后，在non-trading safe boundary且零in-flight economic transaction时，router/frontend/MCP/scripts/application lifecycle一次切换到统一runtime；新UI固定为`/simulation/localsim`，旧`/paper-v2`business route/API/types不保留redirect/410；同PR删除旧mutation caller、session/tick/auto-run UI、公开scheduler start/stop/tick mutation与旧scheduler hook；现有正常LocalSIM复用原经济事实范围且hash不变；禁止把route增加与删除拆开，禁止dual route、shadow、translator、fallback、feature flag和长期410 façade。
...
| `F-138` | LocalSimEconomicCoordinator是state/order/fill/cash/position/mark/receipt/outbox唯一writer，projector/read API无broker/signal/第二写路径 |
...
| `F-142` | 产品切流一次完成，新旧路径无production双写/shadow/translator/fallback；现有正常LocalSIM通过唯一lineage在安全边界延续且经济事实hash不变；cutover后旧mutation route/caller不存在 |
...
| `F-137` | §4.3/§4.6、§5.1–§5.4；legacy-retirement detailed §4.4、`SIM-LR-B/PR-B1` | `pytest backend/tests/simulation_execution`=`66 passed`；execution+lifecycle=`382 passed, 2 skipped`；`python -m nox -s paper_v2_backend`=`396 passed, 1 skipped, 2 xfailed`；MiniQMT runtime直接矩阵=`1469 passed, 82 skipped` | pr_b1_source_complete | explicitly approved phased gap：PR-B1 source scope closed；用户重启、正常交易日与最终旧源删除仍按`SIM-LR-C/D`独立验收 |
| `F-138` | §4.6、§5.5–§5.6；legacy-retirement detailed §4.5、`SIM-LR-B/PR-B1` | `pytest backend/tests/simulation_execution/test_localsim_economic_transaction.py backend/tests/simulation_execution/test_localsim_projection.py`；scheduler zero direct transaction/staging；rollback/CAS/readback/restart matrix included in`382 passed, 2 skipped` | pr_b1_source_complete | explicitly approved phased gap：PR-B1 source scope closed；生产DDL/DML、restart与runtime activation均未执行且不由source完成推定 |
| `F-139` | §5.1、§4.6；legacy-retirement detailed §4.1、LR-4 | target `backend/tests/simulation_runtime/test_localsim_control_plane.py` | design_ready + explicitly approved design-only scope | successor release implementation pending LR-4 |
| `F-140` | §5.12；legacy-retirement detailed §4.6、LR-4/LR-6 | target `backend/tests/simulation_runtime/test_localsim_control_plane.py` and `frontend/tests/paper-v2/localsim-unified-control-plane.spec.ts` | design_ready + explicitly approved design-only scope | API/UI evidence pending LR-4/LR-6 |
| `F-141` | §4.6/§5.12；legacy-retirement detailed §4.7、LR-5 | target `backend/tests/simulation_runtime/test_localsim_replay_live_transition.py` | design_ready + explicitly approved design-only scope | six-month replay/runtime evidence pending LR-5/LR-9 |
| `F-142` | §5.12、§11；legacy-retirement detailed §4.8、§4.8.1、`SIM-LR-C` | target `backend/tests/simulation_architecture/test_localsim_route_uniqueness.py` and lineage economic-hash unchanged receipt | design_ready + explicitly approved design-only scope | cutover and current-account continuation evidence pending `SIM-LR-C` |
...
| `SIM-P-086` | `F-133..F-148` LocalSIM legacy retirement与data/signal/execution隔离 | 设计：`localsim_legacy_retirement_and_layer_isolation_f2_detailed_design_20260830.md`；Stage A PR #3994已合入并清理；PR-B1 source已迁移LocalSIM runtime/planning/economic/persistence/projector/valuation与唯一models owner，本地证据：execution+lifecycle=`382 passed, 2 skipped`、Paper v2=`396 passed, 1 skipped, 2 xfailed`、MiniQMT runtime=`1469 passed, 82 skipped` | target：`SIM-LR-A..D`四阶段、最多六个源PR；layer import、legacy absence、route uniqueness、query budget、single-writer、atomic create、six-month replay、safe-boundary live transition、DEV/restart/normal-day receipts | STAGE_A_MERGED_PR_B1_SOURCE_READY | PR-B1仍须绑定最终PR head/CI后合入；临时import-only兼容面绑定`SIM-LR-D/PR-D1`物理删除；PR-B2、产品切流、DEV/production DDL/DML、用户restart、runtime、normal-day与旧产品退役保持独立状态；当前production gates仍为noop |
