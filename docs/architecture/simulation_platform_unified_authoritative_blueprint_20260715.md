...
| `SIM-P-075` | `F-021,F-105,F-107,F-109` | BUG-1004修复KERNEL_V2 product runtime与历史event CHECK authority断裂：保留P1-D全部LEGACY/quote literals并与公开`EventTypeV2/EventSourceV2/EVENT_ROUTING_COMPOSITES_V1`形成唯一union；新增operator-only additive successor migration，把named type/source与composite从27/7/10 predecessor原子升级为36/14/12 target。SQL和Python readback均拒绝constraint shape、literal、migration-state与composite drift；rollback仅在零KERNEL_V2-only durable fact时恢复predecessor。应用startup不执行DDL，既有quote schema gate不再对predecessor假报`applied_and_verified`。 | 生产只读pg_catalog复现`ck_miniqmt_event_source`拒绝`EXCHANGE_SESSION_CLOCK`且现有K2 composite缺ALGO_START v2/COMMAND_OUTCOME；direct authority/schema/migration tests=`72 passed,1 gated skip`；DEV PostgreSQL首次/二次apply、SESSION/ALGO_START v2/COMMAND_OUTCOME、rollback refusal、cleanup和双rollback=`1 passed` | BUG_1004_IMPLEMENTATION_IN_PROGRESS_DEV_VERIFIED | GitHub Issue #3237 OPEN；`source_merge=pending_pr`；`production_ddl_gate=pending`且未授权/未执行；`backend_restart_owner=user`；第二个独立virtual-account BUG不得并入本scope；broker/config/binding/dependency/service-control均`noop` |
