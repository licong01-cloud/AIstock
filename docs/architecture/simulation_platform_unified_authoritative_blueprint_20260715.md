...
| `F-078` | §9 P0-H K3 detailed §3–§3.4、§10.1.1、§13 committed legacy snapshot→strict event adapter→K2 public seams | `backend/tests/miniqmt_execution_runtime/test_current_three_shadow_source_postgres.py`与`backend/tests/miniqmt_execution_runtime/test_current_three_durable_shadow_postgres.py`验证真实DEV source与K2 durable shadow，zero dispatch attempt | implemented_verified_k3b | none |
...
| `F-080` | §9 P0-H K3 detailed §10.2、§14–§16 visible transport suppression、validation/routing/gates/K6 prerequisite | `python -m nox -s miniqmt_execution_runtime_l2`=`974 passed,29 skipped`；DEV=`2 passed`；六核心模块line≥87%/branch≥70%；ownership无unmapped/ambiguous | implemented_verified_k3 | none |
...
| `SIM-P-066` | `F-071..080` | K3 current-three runtime migration两个shadow-only切片均已实现。K3-A已通过PR #2840合入三个pure `ExecutionAlgoPluginV2`与strict lifecycle；K3-B从单事务committed legacy snapshot构建strict policy/state/dependent-BUY zero-write inventory与ALGO_LOCAL parity，完成visible transport suppression、stable runtime identity及真实K2 creation/ingress/delivery/materializer/mapping/outbox broker-neutral shadow。K3修复generic frozen-config factory边界及transaction lifecycle projection identity，未增加algo分支、第二route、fallback或人工门禁。dependent-BUY durable coordinator和产品route cutover仍由K6拥有 | K3-B focused=`184 passed`；DEV disposable PostgreSQL source+durable shadow=`2 passed`，repeated ingress/restart不重复mapping/outbox且dispatch attempt=0；MiniQMT L2=`974 passed,29 skipped`；六个K3-B核心文件line≥87%/branch≥70%；ownership无unmapped/ambiguous；K3-A PR #2840 / merge `aa155222a1072d6c1110f4cc8a11b4f501d8dd1b` | K3_IMPLEMENTED_VERIFIED_SOURCE_PENDING | `source_merge=pending_pr_k3b`；K3-A=`implemented_verified + merged`；K3-B/K3 overall=`implemented_verified`；K4/K6=`not_started`；产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
