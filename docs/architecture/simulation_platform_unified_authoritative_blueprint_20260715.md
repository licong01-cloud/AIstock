...
当前状态为 `IMPLEMENTATION_VERIFIED_MERGED`：K1-A/B/C、K2-A、K2-A-M1、K2-B、K2-C和K2-D均已`implemented_verified + merged`。K2-D final source `82c69fbf7e7245e0af76262ddc7b7f59ce7d996b` 经 required CI run `30269640126` 验证，并通过 PR #2804 / merge `fc4170faa10847c0b58aa8088b4a8b6d0ca26b29` 合入，`source_merge=merged_pr_2804`；K2 overall=`implemented_verified + merged`。K3详细设计经正式审核并通过 PR #2816 / merge `d4a7fb2c8d4fcb191d75addd3fbc0faef2632b8e` 合入，当前为`design_ready_for_implementation`、`source_merge=merged_pr_2816`，K3/K4 implementation仍为`not_started`。`close_sync=not_applicable_feature`，产品 runtime 未切换，production DDL/DML/dependency/config/binding/broker/restart/runtime activation gates 均为`noop`。该架构重构不得被设置为当前模拟盘运行的人工审批或额外 stop gate；按 K0-K6 独立审核切片实施，并与现有阻断 BUG、生产 rollout 和运行观测分别记录。
...
| `F-072` | §9 P0-H K3 detailed §3–§5.1.3 exact factory/class/binding、transition-first construction、v3 pending command与lifecycle projection | target `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_plugins.py`、`test_current_three_plugin_manifests.py`、`test_algo_plugin_contracts.py`、`test_algo_plugin_registry.py`及临时身份回填/lifecycle drift negative matrix | design_ready | none |
...
| `F-075` | §9 P0-H K3 detailed §5、§8 plugin raw due + K2 clock effective due | target `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_restart.py`覆盖11:29:59→PM、午休零occurrence、无burst、duration/EOD/restart和authority drift | design_ready | none |
| `F-076` | §9 P0-H K3 detailed §5.1.1–§5.4、§11 strict payload/lifecycle/outbox-outcome ingress | target `backend/tests/miniqmt_execution_runtime/test_kernel_callback_events.py`、`test_kernel_outbox_outcome_ingress.py`、`test_current_three_kernel_plugins.py`覆盖terminal ORDER before TRADE、OMS closure、accepted/rejected/unknown/fill/EOD | design_ready | none |
...
| `F-080` | §9 P0-H K3 detailed §10.2、§14–§16 visible transport suppression、validation/routing/gates/K6 prerequisite | target `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_parity.py` cancel pending/rejected/new lifecycle；`python -m nox -s miniqmt_execution_runtime_l2`、`python -m nox -s paper_v2_backend`；line>=80/branch>=70、`unmapped_code_files=[]` | design_ready | none |
...
| `SIM-P-066` | `F-071..080` | K3 current-three runtime migration F2详细设计经正式审核修订并固定两个shadow-only切片：K3-A把Sniper/BestLimit/TWAP Lite实现为真实pure `ExecutionAlgoPluginV2`，以3.0.0 exact factory/class/binding和transition-first construction闭合；v3 state完整表达SUBMIT/CANCEL pending与unknown，materializer消费同snapshot mapping/outbox lifecycle projection；ORDER/TRADE/COMMAND_OUTCOME使用strict payload，同步ACK不伪装ORDER/TRADE/OMS event，terminal/unknown outbox经deterministic COMMAND_OUTCOME ingress推进mapping/plugin state且restart幂等；TWAP由plugin产生raw due、K2唯一session authority解析effective due。K3-B由单事务committed legacy repository snapshot构造production-shape input，K3 command/mapping与legacy child按step/effect/价量reason一对一关联后走真实callback-before-ACK ingress，不伪造shadow ACK或broker call；同时生成zero-write inventory和ALGO_LOCAL immutable parity receipt。legacy重复cancel仅在同target/reason pending lifecycle下记录hash-covered transport suppression，negative outcome后的新cancel仍是业务effect。dependent-BUY跨parent ledger/cash语义继续由K6独立durable coordinator拥有；无永久flag、双route、fallback或人工门禁 | artifact: `docs/architecture/miniqmt_execution_kernel_k3_current_three_runtime_migration_f2_detailed_design_20260727.md`；target strict callback/outcome、plugin/parity/restart/inventory/shadow-source DEV repository tests；F2 validator与DESIGN-COMPLIANCE已在PR #2816修订HEAD通过 | DESIGN_READY | `design_status=design_ready_for_implementation`；PR #2816 / merge `d4a7fb2c8d4fcb191d75addd3fbc0faef2632b8e`；`source_merge=merged_pr_2816`；K3 implementation=`not_started`；K4/K6=`not_started`；K6 dependent-BUY coordinator=`not_designed_not_implemented`；产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
...
| Control | Review result | Design evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass | ALGO_START、exact routing、per-algo ordering、strict callback/outcome、state/timer/failure transition、unknown outbox、pinned façade、迁移和旧route退役均闭合；K3 parity必须由committed legacy repository facts和DEV readback驱动，不以fixture/helper-only冒充完成 |
| `no_silent_error` | pass | stale DISPATCHING不false重提；同步ACK不伪装ORDER/TRADE/OMS event，deterministic COMMAND_OUTCOME ingress推进outbox outcome；malformed payload/state/lifecycle/source、sequence/identity/hash conflict、持久化失败和unknown均typed fail loud |
