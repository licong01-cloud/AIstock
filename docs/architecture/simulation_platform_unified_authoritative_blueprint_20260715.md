...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1/K2/K3 overall均已`implemented_verified + merged`。K4详细设计PR #2861已合入，K4-A已通过PR #2883 / merge `527b2f4a58d3fe84f85c0b1f4ba2fe375d181dda`完成`implemented_verified_contract_slice + merged`；K4-B实施级补充设计已通过PR #2914 / merge `15c38cb8d3a2c3a1710a29526ac6ac07ef580238`合入并保持`design_ready_pending_implementation`，代码尚未开始，K4 overall仍为`implementation_in_progress`，K5/K6仍为`not_started`。`close_sync=not_applicable_feature`，产品 runtime 未切换，production DDL/DML/dependency/config/binding/broker/restart/runtime activation gates 均为`noop`。该架构重构不得被设置为当前模拟盘运行的人工审批或额外 stop gate；按 K0-K6 独立审核切片实施，并与现有阻断 BUG、生产 rollout 和运行观测分别记录。
...
| `F-081` | §9 P0-H K4 detailed §0–§2 | `backend/tests/miniqmt_execution_runtime/test_vnpy_k4_scope_boundaries.py` | design_ready | none |
| `F-082` | §9 P0-H K4 detailed §3、§12.1 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_source_execution_v2.py` planned source/fresh readback matrix | design_ready | none |
| `F-083` | §9 P0-H K4 detailed §4–§5 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_kernel_invocation.py` planned CLAIMED/transition/ordinary path matrix | design_ready | none |
| `F-084` | §9 P0-H K4 detailed §5.4.1、§6 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_source_execution_v2.py` planned actual method/callback trace | design_ready | none |
| `F-085` | §9 P0-H K4 detailed §6.3–§8、§15 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_repository_postgres.py` planned same-transaction/zero-write matrix | design_ready | none |
| `F-086` | §9 P0-H K4 detailed §5.4.1、§5.6、§9–§10、§15 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_kernel_invocation.py` planned single/multi-command/restart matrix | design_ready | none |
| `F-087` | §9 P0-H K4 detailed §4.1、§5.1、§11–§12 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_conformance_authority_v2.py` planned sealed V2 authority matrix | design_ready | none |
| `F-088` | §9 P0-H K4 detailed §3.2、§6.3、§10、§12 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_source_execution_v2.py` planned current-three/Iceberg/Stop matrix | design_ready | none |
| `F-089` | §9 P0-H K4 detailed §13–§17 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_failure_restart.py` planned failure/restart/diagnostics matrix | design_ready | none |
| `F-090` | §9 P0-H K4 detailed §18–§20 | artifact: `docs/architecture/miniqmt_execution_kernel_k4_vnpy_facade_f2_detailed_design_20260729.md` | design_ready | none |
...
| `SIM-P-067` | `F-081..090` | K4设计与K4-A已合入；K4-A实现pure façade/adapter、strict carriers、DTO/enum/state/terminal/isolated bindings、deterministic observation和six-source authority，并永久保持V1 observation-only/zero-positive-publication。K4-B实施级补充设计现已闭合：V2 full executable vector/full actual trace/source execution set/characterization/conformance authority；fresh spawned writer/readback和sealed process authority；真实CLAIMED delivery pre-call transition identity；existing K2 locked transaction中的ALGO_START/latest prior native B0 TICK read set；current-three live K3 replay与Iceberg/Stop characterization-only矩阵。K4-B不修改current-three binding、不注册Iceberg/Stop、不新增DDL/产品command authority或route | design PR #2861 / merge `8250b64ff...`；K4-A PR #2883 / merge `527b2f4a58d3fe84f85c0b1f4ba2fe375d181dda`及既有direct/coverage/L2/Paper证据；K4-B三份权威设计已通过PR #2914 / merge `15c38cb8d3a2c3a1710a29526ac6ac07ef580238`合入并通过三份F2 validator、docs routing与DESIGN-COMPLIANCE，代码/DEV/L2/coverage证据留待K4-B实现PR | K4_IMPLEMENTATION_IN_PROGRESS_K4B_DESIGN_READY | `k4a_source_merge=merged_pr_2883`；`k4b_design_source_merge=merged_pr_2914`；K4-A=`implemented_verified_contract_slice + merged`、K4-B=`design_ready_pending_implementation`、K5/K6=`not_started`；产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
