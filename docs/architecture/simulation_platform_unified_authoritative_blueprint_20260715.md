...
当前状态仅为 `DESIGN_READY`：未实现代码、未执行 DDL/DML、未修改配置、未调用 broker、未重启服务。该架构重构不得被设置为当前模拟盘运行的人工审批或额外 stop gate；按 K0-K6 可独立审核切片实施，并与现有阻断 BUG、生产 rollout 和运行观测分别记录。
...
| `F-053` | §9 P0-H K1；K1 detailed design §3 target modules/dependency/import boundary | target backend/tests/miniqmt_execution_runtime/test_plugin_import_boundaries.py | design_ready | none |
| `F-054` | §9 P0-H K1；K1 detailed design §4-§5 strict contracts/recursive FrozenJson/raw-digest+hex codec | target backend/tests/miniqmt_execution_runtime/test_algo_plugin_contracts.py nested-mutation/canonical/error matrix | design_ready | none |
| `F-055` | §9 P0-H K1；K1 detailed design §7 route-independent catalog、descriptor/process binding、creation binding、aggregate/route receipts | target backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py route-isolation/zero-partial matrix | design_ready | none |
| `F-056` | §9 P0-H K1；K1 detailed design §6 exact keyed deterministic context | target backend/tests/miniqmt_execution_runtime/test_deterministic_execution_context.py raw-digest u53/retry/restart matrix | design_ready | none |
| `F-057` | §9 P0-H K1；K1 detailed design §1、§8 current-three exact state/TWAP session/legacy projection；K1 shadow-only、runtime switch 属于 K3 | target backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py + existing parity/restart tests | design_ready | none |
| `F-058` | §9 P0-H K1；K1 detailed design §1.2、§9 pinned compatibility lock/receipt；façade runtime 属于 K4 | target backend/tests/miniqmt_execution_runtime/test_vnpy_compatibility_receipts.py | design_ready | none |
| `F-059` | §9 P0-H K1；K1 detailed design §11 ownership/test routing/coverage | command `python -m nox -s l0`；implementation changed files route to target plan `miniqmt_execution_runtime_l2` | design_ready | none |
| `F-060` | §9 P0-H K1；K1 detailed design §2、§10、§12-§13 rollout/rollback/state separation | artifact: docs/architecture/miniqmt_execution_kernel_k1_contracts_registry_f2_detailed_design_20260722.md；command: `python scripts/aistock_feature_workflow.py validate --design docs/architecture/miniqmt_execution_kernel_k1_contracts_registry_f2_detailed_design_20260722.md --tier F2`；DESIGN-COMPLIANCE-001 | design_ready | none |
...
| Progress ID | Acceptance IDs | Current state after this PR（base `origin/main@e111e479`） | Evidence | Status | Next implementation slice |
| --- | --- | --- | --- | --- | --- |
...
| `SIM-P-058` | `F-054,F-056` | 实现 K1-A strict/frozen contracts、recursive FrozenJson、canonical JSON/raw digest/hex、decimal/time/error evidence、完整 runtime event composite DTO、state/start/initialization/effect closure，以及 logical-time keyed deterministic ID/raw-digest u53/ordinal helpers；保持 K1-A shadow-only 和产品 runtime 零接线 | `plugin_canonical.py`、`plugin_contracts.py`、`deterministic_context.py`；direct 56 passed；K1-A coverage 95%/86%/97%；classifier only `miniqmt_execution_runtime_l2`；module L2 462 passed/1 skipped | K1_A_IMPLEMENTED_VERIFIED | `source_merge=pending_pr`；`close_sync=not_applicable_feature`；K1-B catalog/current-three 与 K1-C compatibility/import boundary 均未开始；production DDL/DML/dependency/config/binding/broker/restart/runtime 全部 `noop` |
