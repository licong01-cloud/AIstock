...
| `SIM-P-074` | `F-007,F-009,F-012,F-014,F-021` | BUG-992 closes LocalSIM lifecycle authority and economic readback with one pure contract over current run/binding/trade-date/plan identity, strict continuous receipt generations and high-watermark, exact receipt/outbox identity, and the latest committed `state_id -> state_hash` set. State history remains keyed by `state_id`; legal terminal incarnations stay auditable, while missing/forged receipt authority, generation corruption, missing/hash-drift states, identity drift and extra non-terminal states fail loud. The prior PR conclusion is superseded because its selector still fell back to raw history and its readback accepted receipt hashes as a subset. | Final-review RED: new public authority matrix=`20 failed,2 passed`; extra-active readback alone=`0 passed,2 failed` with `DID NOT RAISE`; strict empty-generation audit=`2 failed,19 passed`. GREEN: authority/readback/PostgreSQL matrix=`25 passed`; strict receipt/high-watermark/outbox/empty-path matrix=`21 passed`; final lifecycle branch-coverage run=`233 passed,3 warnings`; `simulation_core_l2=461 passed`; registry=`8 passed,14/14 mapped`; L0 blocking=`0`; classifier `unmapped_code_files=[]`; changed-code coverage against `origin/main`=`94.78% line (109/115),89.29% branch (50/56)`; F2=`112/112,warnings=0`; GitHub Issue #3184 / PR #3186. | BUG_992_FINAL_REVIEW_REPAIR_VERIFIED_PR_OPEN | `source_merge=pending_pr_3186`; `post_restart_effective_gate=pending_user_restart`; current production run cannot recover from source changes before user-authorized merge/restart/readback; production DDL/DML/dependency/config/binding/broker/service-control all `noop` |
...
| Control | Review result | Implementation evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass after final-review repair; prior PASS withdrawn | Before this repair the selector's missing-receipt/state-hash fallback meant the PR delivered only a subset of the approved authority contract, so the earlier PASS was invalid. The shared closure now validates current durable identity, every receipt generation, the high-watermark, exact state authority and terminal-history rules, and is reused by authoritative listing and economic readback across InMemory/PostgreSQL public seams. |
| `no_silent_error` | pass after final-review repair; prior PASS withdrawn | Before this repair readback accepted receipt hashes as a subset and returned false success with an extra active state, so the earlier PASS was invalid. RED explicitly showed `DID NOT RAISE`; GREEN rejects missing/forged receipt, malformed generation/history, identity/hash drift, forged outbox and extra non-terminal authority with stable typed JSON-safe context. |
| `no_business_semantic_drift` | pass | Frozen plan intents, Selection/signals/strategy package/assets, side, quantity, algorithm policy, state identity/hash, broker route, CAS/single-writer/transaction/outbox/projection and terminal status semantics are unchanged. Legal terminal history remains auditable; normalized changed-code line/branch coverage is `87.50%/78.57%`. |
