...
| `F-062` | §9 P0-H K2 detailed §4.0–§4.10、§9；strict carriers、repository与migration triplet | `backend/tests/miniqmt_execution_runtime/test_kernel_contracts.py`、`test_kernel_repository_postgres.py`、`test_kernel_migration_postgres.py`当前总计25 passed；后续slice复用该authority | design_ready | none |
...
| `F-064` | §9 P0-H K2 detailed §4.2–4.4、§6.3、§7 | `backend/tests/miniqmt_execution_runtime/test_kernel_repository_postgres.py` lock/CAS/fence/rollback primitives；K2-B target delivery | design_ready | none |
...
| `F-066` | §9 P0-H K2 detailed §4.5–§4.7、§6.4–6.5 | `backend/tests/miniqmt_execution_runtime/test_kernel_repository_postgres.py` mapping/child/outbox/attempt/incarnation/CAS；K2-D target dispatcher/reconcile | design_ready | none |
| `F-067` | §9 P0-H K2 detailed §4.8–§4.9、§8 | `backend/tests/miniqmt_execution_runtime/test_kernel_contracts.py`与`test_kernel_repository_postgres.py` timer/session/FK/CAS/readback/concurrency；K2-C target clock | design_ready | none |
| `F-068` | §9 P0-H K2 detailed §9 | `backend/tests/miniqmt_execution_runtime/test_kernel_migration_postgres.py` DEV preflight、first/second apply、partial catalog、readback、rollback | design_ready | none |
...
| `F-070` | §9 P0-H K2 detailed §11–§13 | command `python -m nox -s miniqmt_execution_runtime_l2`；K2-A direct/DEV PostgreSQL=25 passed、line/branch=88.22/72.96、classifier MiniQMT only/unmapped=0；PR #2729 required CI run `30154314170` green且OPEN/CLEAN/MERGEABLE | design_ready | none |
...
| `SIM-P-056` | `F-043..070` | MiniQMT统一kernel/plugin F2蓝图保持唯一权威；K2-A已从首个carrier checkpoint推进到全部strict durable carriers、additive migration与production PostgreSQL repository，本地验证闭合event/delivery/transition/mapping/child/outbox/timer/session/incarnation schema/readback，但未启动任何worker或产品route | K2 design PR #2714 / merge `50dbcbf6c5d943944a7c7a0a4e5f3e69f5fbd12e`；K2-A implementation PR #2729 / HEAD `d437c74c`；direct/DEV PostgreSQL `25 passed`；repository line/branch=`88.22%/72.96%`；classifier MiniQMT L2 only、unmapped=0；required CI run `30154314170` green | IMPLEMENTATION_IN_PROGRESS | K2-A=`implemented_verified`；K2-B/C/D、K3/K4=`not_started`；source merge=`pending_user_authorization`；production DDL/DML/dependency/config/broker/restart/runtime均`noop` |
...
| `SIM-P-061` | `F-061..070` | K2详细设计进入实施：K2-A已实现本slice要求的§4 strict carriers、additive DDL/composite FK、DB-sequenced worker incarnation records、event/delivery与transition/mapping/child/outbox repository seam、timer/session authority、bounded recovery和controlled rollback；保持K1唯一DTO authority、shadow-only且K3前不切换产品route | `backend/services/miniqmt_execution_runtime/plugin_contracts.py`、`kernel_repository.py`、K2 migration triplet、三份K2 tests；RED missing seams与正式审计冲突→GREEN 25 passed；coverage 88.22/72.96；PR #2729 required CI run `30154314170` green且OPEN/CLEAN/MERGEABLE | IMPLEMENTATION_IN_PROGRESS | `source_merge=pending_user_authorization`；K2-A=`implemented_verified`；K2-B/C/D、K3/K4=`not_started`；`close_sync=not_applicable_feature`；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
