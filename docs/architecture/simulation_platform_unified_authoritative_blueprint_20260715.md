...
- `BUG-779`：LocalSIM economic snapshot 必须支持 Pydantic、dataclass、immutable `Mapping`/`MappingProxyType` 和严格 JSON value；只有 durable economic transaction 已 staged/committed 才可声明 broker/economic side effect。pre-commit 失败必须持久化 `economic_commit_staged=false`、`broker_called=false` 和完整失败数量；历史 run 若曾声称提交但无 receipt/state/outbox，可读 diagnostics 必须标记 `LOCAL_SIM_DURABLE_FACTS_UNRECONSTRUCTIBLE`，不得补写或伪造经济事实；
...
- `BUG-781`：BUY 与普通 SELL child 继续遵守 symbol-aware board-lot；SELL 仅在 child quantity 等于 authoritative available whole position 时允许低于最小手数或带 odd-lot residual，任意 partial odd-lot child 必须在 gateway 前拒绝。runtime、vn.py core、managed-order preflight 共用同一语义，不用 `min_volume=1` 放宽所有 SELL；
- `BUG-782`：恢复必须先把 runtime-owned broker trade snapshot 与 archived durable trade event 按 exact trade identity 合并、去重和冲突校验，再通过同一 `record_trade_event` 路径重放。BUY 原冻结按 order price 释放并写 cash/T+1 lot，SELL 写 cash/lot/realized facts；event 已 append 但 settlement/child update 失败时，后续同 identity retry 必须续做未完成阶段且不重复经济事实。多 fill 必须用规范化 broker time 排序；缺失/非法/冲突 identity、owner、quantity、price、time 均 fail loud；
- `BUG-783`：daily run、qmt batch、order intent 与 runtime journal 必须形成精确可重建投影。parent outcome count 与实际 child count 分离；同 parent 后续 rejected slice 不得覆盖既有 accepted broker fact；restart recovery 必须把全部 recovered child/trade 投影回原 batch/intent。read-only diagnostics 比较 runtime child/event/trade/count/hash 与 run/batch carrier，陈旧或冲突显示 `MINIQMT_RUNTIME_PROJECTION_STALE`、低基数 metric 和自动解除 alert，不自动 repair、不形成 execution gate；
- `BUG-784`：runtime trade date 由 frozen runtime authority 决定；broker date/time 只作为原始证据及 mismatch 字段，不得反向改写 runtime identity。epoch seconds/milliseconds、14 位 compact calendar、compact fractional、HHMMSS、ISO 各自精确判型；compact/time-only 按 Asia/Shanghai exchange local 解释后转 UTC，歧义长度或非法日期必须拒绝，不得由通用 timestamp 猜测落入错误年份。
...
| `F-005` | §5.1 `FrozenSimulationIdentityV1` | backend/tests/simulation_runtime/test_strategy_runtime_release.py；unchanged/changed manifest and immutable binding tests | design_ready | none |
...
| `F-007` | §4.3 LocalSIM event model | backend/tests/paper_trading_v2/test_localsim_backend.py；240 causal minute-bar schedule test | design_ready | none |
...
| `F-009` | §5.2、§10.3 | backend/tests/paper_trading_v2/test_localsim_backend.py；crash/restart/no-duplicate fill tests | design_ready | none |
...
| `F-013` | §5.5；`PaperTradingV2Repository.local_sim_economic_transaction`；`SimulationRuntimeRepository.stage_local_sim_economic_commit` | backend/tests/simulation_runtime/test_lifecycle_scheduler.py；PostgreSQL single-connection commit/rollback、InMemory cross-repository rollback、economic receipt/state/outbox readback tests | implemented_verified | none |
| `F-014` | §5.2、§5.5；`LocalSimEconomicReceiptV1`、`LocalSimProjectionOutboxV1`、`LocalSimProjectionReceiptV1` | backend/tests/simulation_runtime/test_lifecycle_scheduler.py；same-bar restart dedupe、CAS、connection retry max-3、business-conflict terminal、projection readback recovery、tamper/schema fail-loud tests | implemented_verified | none |
| `F-015` | §5.6；`LocalSimMarketMarkV1`；`LocalSimBackend.load_authoritative_position_marks`；`_local_sim_position_marks` | backend/tests/paper_trading_v2/test_localsim_backend.py；generic price/plan fallback rejection、realtime/historical/suspended as-of/source/provenance/hash、transient reuse reason/lineage/new-hash、malformed previous outbox fail-loud tests | implemented_verified | none |
...
| `F-021` | §5.10；`SimulationLifecycleBackgroundScheduler._record_loop_exception/_record_loop_success`；`SimulationLifecycleScheduler._with_unattended_roll_forward_bindings/_validated_unattended_roll_forward_quote_control`；`SimulationRuntimeOpsService._scheduler_loop_health/_current_trade_date_blockers/platform_diagnostics`；`SimulationPlatformObservability` | backend/tests/simulation_runtime/test_ops_api.py；BUG-681/687/697 process/lifecycle/binding/backend/durability/business health direct tests | implemented_verified | none |
...
| `F-030` | §4.3 symbol isolation；`LocalSimExecutionStateV1` | backend/tests/paper_trading_v2/test_localsim_backend.py；unavailable symbol waits while healthy symbol fills and payload conflict terminalizes only affected state | implemented_verified | none |
| `F-031` | §5.3 `LocalSimMarketSnapshotV1`；LocalSim broker snapshot builder；§5.6 mark/outbox | backend/tests/simulation_runtime/test_lifecycle_scheduler.py；cadence union/no-refetch、canonical hash、mark reuse lineage and malformed outbox direct tests | implemented_verified | none |
...
| `F-033` | §5.4 Trading Core board-lot/VWAP authority；execution algos/adapter/minute engine；BUG-718 CI mapping | backend/tests/trading_core/test_vnpy_style_execution_assets.py；main-board/STAR algorithms、participation board-lot、VWAP profile and classifier direct tests | implemented_verified | none |
| `F-034` | §5.5、§7、§9 P0-F；`SimulationLifecycleScheduler._persist_local_sim_execution_result`；`SimulationPlatformObservability` | backend/tests/simulation_runtime/test_lifecycle_scheduler.py；immutable mapping canonical snapshot and pre-commit error truth；backend/tests/simulation_runtime/test_ops_api.py unreconstructible projection | implemented_verified | explicitly approved production-state separation：normal trading-day runtime observation remains separate |
...
| `F-036` | §5.9、§9 P0-F；vn.py base、runtime child validator、`QmtManagedOrderService._sell_board_lot_error` | backend/tests/qmt_strategy_ledger/test_order_service_preflight.py；backend/tests/trading_core/test_vnpy_style_execution_assets.py；whole-position allow and partial residual reject | implemented_verified | none |
...
| Progress ID | Acceptance IDs | Current state after this PR（base `origin/main@f2550365`） | Evidence | Status | Next implementation slice |
| --- | --- | --- | --- | --- | --- |
...
| `SIM-P-043` | `F-034,024` | BUG-779 规范化 immutable LocalSIM snapshot，并让 pre-commit failure、broker_called、durable receipt/state/outbox truth 一致；不可重建历史 run 只读 BLOCKED，不补写经济事实 | BUG-779 / issue #2488；mappingproxy canonical、scheduler persistence failure、ops/platform unreconstructible direct tests | IMPLEMENTED_VERIFIED | PR/CI/merge 后由用户重启并只读核对；DDL/DML/config/broker call 均为 noop |
...
| `SIM-P-045` | `F-036,024` | BUG-781 统一 vn.py/runtime/managed-order SELL board-lot：仅 exact whole available position 可携带 odd-lot residual，partial odd-lot child 在 gateway 前拒绝 | BUG-781 / issue #2490；order preflight、vn.py asset、runtime child validator 正反 direct tests | IMPLEMENTED_VERIFIED | PR/CI/merge 后正常交易日观察 child quantity；DDL/DML/config/restart/broker activation 均分离 |
| `SIM-P-046` | `F-037,039,024` | BUG-782 以 exact broker trade identity/time 合并 snapshot 与 archived event，重放 BUY/SELL cash/lot/child/algo；event-before-settlement retry 可续做且 exactly-once | BUG-782 / issue #2491；BUY/SELL double recovery、partial snapshot union、settlement fault injection、identity/time negative direct tests | IMPLEMENTED_VERIFIED | PR/CI/merge 后由真实 broker snapshot readback 验证；本 slice 未调用生产 broker |
| `SIM-P-047` | `F-038,024` | BUG-783 分离 parent outcome 与 child/trade count，restart 回投影 batch/intent，accepted child 不被后续 reject 覆盖；只读 diagnostics 对 stale/conflict 生成 metric/auto-clear alert | BUG-783 / issue #2492；scheduler one-parent/multi-child、client recovery、ops/platform stale/clear direct tests | IMPLEMENTED_VERIFIED | PR/CI/merge 和用户重启后读取 platform diagnostics；不自动 repair、不新增 execution gate |
| `SIM-P-048` | `F-039,024` | BUG-784 固定 runtime trade-date authority，保留 broker raw date/time/mismatch；TCA numeric/compact/time-only/ISO 精确解析，exchange-local 转 UTC，歧义输入 fail loud | BUG-784 / issue #2493；TCA parser full matrix 与 runtime broker-time recovery direct tests | IMPLEMENTED_VERIFIED | PR/CI/merge 后只读核对实际 broker/TCA 时间；DDL/DML/config/restart 均为 noop |
...
| `no_silent_error` | pass | unsupported/non-finite snapshot、unapplied no-id trade、source/hash/depth conflict、partial odd-lot、trade owner/identity/time/cumulative conflict、settlement failure、projection schema/count/hash conflict和歧义 timestamp 均 typed fail loud；仅 transient per-symbol quote/depth 形成有 durable reason 的自动 wait，不伪报 broker call 或成功 |
