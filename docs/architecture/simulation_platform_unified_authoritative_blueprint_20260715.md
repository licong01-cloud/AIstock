...
| Progress ID | Acceptance IDs | Current state after this PR（base `origin/main@5cae5861`） | Evidence | Status | Next implementation slice |
| --- | --- | --- | --- | --- | --- |
...
| `SIM-P-009` | `F-020,021` | BUG-680 已移除 MiniQMT tick-driver/recovery/pending/submitted count 的 pass/归零并收紧 callback/OMS canonical fact；BUG-681 已消除 scheduler thread-alive false green；BUG-682 已消除 TCA quote/preflight 静默归一；BUG-683 已移除 durable request/result 过滤、错配、padding 以及 raw batch/status/total/default carrier。已识别的 F-020 raw parse 与本行 scheduler false-green 子切片闭合；全平台聚合 health 不并入本行，继续由 `SIM-P-011` 独立跟踪 | BUG-680 / issue #2232 direct 31 passed；BUG-681 / issue #2236 related 20+1 passed；BUG-682 / issue #2239 related 37 + lifecycle 2 passed；BUG-683 / issue #2243 direct 34 passed、related 18 + 24 passed，另有 1 个 exact-main no-child 旧语义测试失败 | IMPLEMENTED_VERIFIED | BUG-683 source PR/CI/merge 后由用户重启并只读核对 tick-driver/batch readback；平台 metrics/alerts/operator runbook 继续 `SIM-P-011` |
...
| `SIM-P-028` | `F-020,022,024` | BUG-682 为 TCA quote/preflight observation 建立 exact payload/alias/time/price/boolean/count/deadline status 校验和 `TcaCaptureDataError`；lifecycle decision 与 event-loop client 均将非法证据写既有 durable sidecar capture error，错误载荷有界，原 B0 execution/broker fact 不变 | BUG-682 / issue #2239；source PR #2241 merge `bef8512d`；close-sync PR #2242 merge `03d7bdd1`；direct parser、hash、deadline、durable sidecar、existing valid/missing/policy-error/no-broker-drift related matrix 37 passed + lifecycle exact-reason 2 passed；Ruff changed-files pass | IMPLEMENTED_VERIFIED | 用户重启后只读核对 TCA sidecar；DDL/DML/config/broker call 均为 noop，本项不替代 `SIM-P-011` 剩余工作 |
