...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1-A/B 已实现验证并合入；K1-C C1/C2 已闭合 repo-owned pinned source、structured surface/receipt、source-isolated/standard import 与 K1-B real catalog integration，并通过 direct/coverage/classifier/F2/正式审核及 PR #2685 implementation HEAD `a0ba749e` required CI，当前为 `implemented_verified`、`source_merge=pending_user_authorization`；K1 overall 仍为 in progress，K2/K3/K4 均 `not_started`，`close_sync=not_applicable_feature`，产品 runtime 仍未切换，production DDL/DML/dependency/config/binding/broker/restart/runtime activation gates 均为 `noop`。该架构重构不得被设置为当前模拟盘运行的人工审批或额外 stop gate；按 K0-K6 可独立审核切片实施，并与现有阻断 BUG、生产 rollout 和运行观测分别记录。
...
| `F-058` | §9 P0-H K1；repo-owned exact pinned requirement/source/surface/receipt 与 K1-B component seam | `backend/tests/miniqmt_execution_runtime/test_vnpy_compatibility_receipts.py` = 24 passed；真实六文件 exact path set/license/manifest/characterization、drift aggregate、immutable receipt、fresh-process、current-three catalog closure | implemented_verified | none |
| `F-059` | §9 P0-H K1；ownership/test routing/coverage | `backend/tests/miniqmt_execution_runtime/test_vnpy_compatibility_receipts.py` / `test_plugin_import_boundaries.py` / `test_algo_plugin_registry.py` / `test_current_three_plugin_manifests.py` / `test_algo_plugin_contracts.py` = 24/32/55/39/60 passed；K1-C module line/branch 均达到 80/70；25 files mapped，unmapped/ambiguous=0/0；MiniQMT L2 624 passed/1 skipped，paper_v2 1050 passed/2 skipped/2 xfailed；PR #2685 implementation HEAD `a0ba749e` required CI 11/11 green | implemented_verified | none |
| `F-060` | §9 P0-H K1；state-separated rollout/rollback | validation-receipt: 三份 F2 结果 8/8、18/18、60/60，Feature Workflow pre-PR 与 DESIGN-COMPLIANCE-001 PASS；K1-C `source_merge=pending_user_authorization`、K1 overall `in_progress`、K2/K3/K4 `not_started`、`close_sync=not_applicable_feature`、production/runtime gates `noop` | implemented_verified | none |
