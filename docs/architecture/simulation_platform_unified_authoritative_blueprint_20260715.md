...
| `SIM-P-069` | `F-101..112` | K6 base/K6-A/C0/BUG-953保持既有merged/runtime schema状态。K6-C1已实现strict `ProductCommandAuthorityEnvelopeV3`、唯一pure evaluator、0/1/N与MIXED aggregate、MATERIALIZE/REJECT/DEFER atomic K2 lineage、typed pre-call no-broker reject、same-authority advisory-lock幂等、rollback/commit-unknown和fresh independent readback；正式复审闭合malformed public carrier typed failure、terminal MATERIALIZE physical mapping projection、DEFER waiting/block/EOD/released-outbox lifecycle、immutable initial lineage、strict CAS/SHA及coordination successor UTC hash。这里只验证K6-B未来合法状态的C1 readback兼容性，未实现K6-B decision/release writer或K6-D cutover，也未改signal/selection/package/target/side/quantity/算法/LocalSIM | C1 contracts/authority/DEV PostgreSQL=`105 passed`，其中DEV=`20 passed`；coverage authority/contracts/repository=`84.21/76.47`、`90.52/73.11`、`86.63/70.37`；production只读K6-C0 preflight=`8/8 true`、authority总行/V3行=`0/0`；required CI run `30804132306` green；PR #3080 / merge `c0ca2e7e356a891ee5b2aad270d391071c942ae4` | K6_C1_IMPLEMENTED_VERIFIED_MERGED | K6-C1 `source_merge=merged_pr_3080`；K6-B/D=`not_started`；product runtime未切换；production DDL保持既有`applied_and_verified`，本轮production DML/config/binding/broker/restart/runtime activation/normal-day=`noop/not_run` |
