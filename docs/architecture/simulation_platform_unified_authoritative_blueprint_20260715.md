...
>
> MiniQMT execution-kernel进度（2026-08-02）：K1/K2/K3/K4/K5 overall=`implemented_verified + merged`。K6 base design、K6-A与implementation-readiness revision已分别通过PR #2993/#3004/#3024合入。K6-C0 strict contracts、successor migration与versioned repository preflight已通过PR #3032 / merge `2a3622a3ba63585e3dfe12ef7ccb3f33b00dcb63`完成`implemented_verified + merged`；BUG-953 deterministic lineage、lifecycle authority与exact physical mapping closure已`implemented_verified`、`source_merge=pending_pr`。剩余优先级固定为`BUG-953 -> K6-C1 -> K6-B -> K6-D`。K6 overall=`implementation_in_progress`，产品runtime未切换。
>
...
6. K6按[`miniqmt_execution_kernel_k6_product_cutover_f2_detailed_design_20260801.md`](miniqmt_execution_kernel_k6_product_cutover_f2_detailed_design_20260801.md)实施：K6-A提供base contracts/migration/repository；K6-C0已补齐strict command_json、V2 proceeds/ledger/coordination、MATERIALIZE/REJECT/DEFER carriers、K6 product-owned deferred mapping contract、successor migration与versioned schema readback。BUG-953补齐SUBMIT deterministic mapping/child、CANCEL existing-mapping reuse、lifecycle disposition/status/broker/authority closure及exact 29-key physical mapping JSON/scalar equality；shared transition OMS/RISK/ROUTE ref继续与per-command strict receipts分层。BUG-953合入后第一优先级才恢复为K6-C1 generic per-command atomic materializer；之后K6-B只把同一physical mapping row/command id由product RESERVED显式交接给现有K2 outbox/dispatch lifecycle；K6-D完成new-instance唯一KERNEL_V2 cutover与旧route drain/retirement。只增加现有K2缺失的coordination/product-authority事实，不复制K2 event/transition/outbox/OMS/Gateway，不进入算法plugin，不允许dual route/fallback，并以successor migration、独立readback、guarded rollback和正常交易日真实SIM验收。
...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1/K2/K3/K4/K5 overall均已`implemented_verified + merged`。K6 base design=`design_ready + merged`（PR #2993），K6-A=`implemented_verified + merged`（PR #3004），revision=`design_revision_ready + merged`（PR #3024），K6-C0=`implemented_verified + merged`（PR #3032，merge `2a3622a3ba63585e3dfe12ef7ccb3f33b00dcb63`）。BUG-953=`implemented_verified`且`source_merge=pending_pr`；其合入前不得开始K6-C1。之后K6-C1/K6-B/K6-D仍按C1->B->D推进。`close_sync`尚不适用，产品runtime未切换；production DDL未执行并等待独立授权，其余production gates均为`noop`。该架构重构不得被设置为当前模拟盘运行的人工审批或额外stop gate，并与现有阻断BUG、生产rollout和运行观测分别记录。
...
| `F-104` | §9 P0-H K6 detailed §4.2、§8；V3 carriers与BUG-953 lineage/lifecycle closure | C0：`python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py -q`；BUG-953 direct+DEV combined=`88 passed`；C1 target `test_kernel_product_authority.py` | implemented_verified | none |
...
| `SIM-P-069` | `F-101..112` | K6 base design、K6-A与readiness revision已合入。K6-C0已实现dependent-BUY V2 proceeds/ledger/coordination、product authority V3 strict full command/evidence carriers、MATERIALIZE/REJECT/DEFER、K6 product-owned mapping carrier及versioned catalog。BUG-953现补齐SUBMIT deterministic lineage、CANCEL existing-mapping reuse、lifecycle status/broker/authority identity closure及exact 29-key physical mapping JSON/scalar CHECK；shared transition projection set与per-command strict OMS/risk/route receipt继续分层。未实现C1 materializer、B coordinator/cross-carrier dispatch handoff或D cutover，也未改signal/selection/package/target/side/quantity/算法/LocalSIM | PR #3032 / merge `2a3622a3ba63585e3dfe12ef7ccb3f33b00dcb63`；BUG-953 Issue #3045，source fix direct+DEV=`88 passed`，`source_merge=pending_pr`；migration catalog=`f4fc093c...`，forward canonical-LF=`368fc29048ac...` | BUG_953_IMPLEMENTED_VERIFIED_PENDING_PR | base/K6-A/revision/K6-C0已merge；BUG-953待PR；K6-C1/B/D=`not_started`；product runtime未切换；production DDL未执行并等待独立授权，其余gates/normal-day=`noop/not_run` |
...
| `production state separation` | pass | K1–K5及K6 base/K6-A/revision/K6-C0 source merge已闭合；BUG-953=`implemented_verified`、`source_merge=pending_pr`且在K6-C1之前，之后仍按C1->B->D；production DDL未执行并等待独立授权，DML/dependency/config/binding/restart/broker/runtime activation/normal-day observation均为`noop/not_run`，产品runtime未切换 |
