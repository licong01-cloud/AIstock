...
| `F-102` | §9 P0-H K6 detailed §4.1；`kernel_product_contracts.py` | `backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py`；K6 direct/DEV aggregate=`34 passed`；contracts line/branch=`90.86%/72.50%` | implemented_verified_local | none |
...
| `F-105` | §9 P0-H K6 detailed §5；K6 migration triplet | `backend/tests/miniqmt_execution_runtime/test_kernel_k6_migration_postgres.py`；DEV PostgreSQL clean/reapply/catalog+function drift/readback/append-only/guarded rollback通过 | implemented_verified_local | none |
| `F-106` | §9 P0-H K6 detailed §6；`kernel_product_repository.py` | `backend/tests/miniqmt_execution_runtime/test_kernel_product_repository_postgres.py`；transaction/CAS/fence/idempotency/mixed lineage/commit-unknown/readback drift；repository line/branch=`87.17%/70.42%` | implemented_verified_local | none |
...
| `SIM-P-069` | `F-101..112` | K6设计已合入；K6-A已本地实现strict dependent-BUY/product-authority/lifecycle/materialization/route carriers、七张additive表、append-only/CAS triggers、独立catalog+function-body authority、guarded rollback及唯一repository writer/readback。0/1/N mixed materialized/rejected command closure、idempotent retry、stale fence、commit-unknown和route不可逆均fail-loud；未实现K6-B coordinator、K6-C product materializer或K6-D runtime cutover，不修改signal/selection/package/target/side/quantity/算法/LocalSIM，也不增加fallback/RBAC/审批/人工acknowledge | K6 direct/DEV=`34 passed`；contracts=`90.86/72.50`、repository=`87.17/70.42` line/branch；MiniQMT L2最终=`1159 passed,35 skipped`；L0/registry/Ruff/format/compile/diff通过；首次模块回归暴露的intentional repository API/MRO characterization已补齐 | K6_A_IMPLEMENTED_VERIFIED_LOCAL | design `source_merge=merged_pr_2993`；K6-A source=`pending_pr`；K6-B/C/D=`not_started`；product runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime/normal-day全部`noop/not_run` |
