...
| Control | Review result | Implementation evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass | 修复位于真实 historical-recovery seam：完整 hash-closed predecessor/rebuild/current lineage closure 保持在前，retired-plan 识别只在其后、context/行情/minute-loop 之前；evidence carrier 闭合 plan identity、retired/required algo、双 plan 的 authoritative/active state 计数与 canonical set hash；`FAILED_RETRYABLE` 与 `FAILED_TERMINAL` 双形态、carrier 幂等 skip、malformed carrier、非 retired policy error 各有独立测试；不是把 retired reject 加入白名单、调低重试或改写历史 state |
| `no_silent_error` | pass | retired plan 产生稳定 `LOCALSIM_HISTORICAL_FAILED_RUN_LEGACY_PLAN_RETIRED` typed reason 与完整 evidence carrier；malformed carrier 以 `LOCALSIM_HISTORICAL_RECOVERY_FAILURE_CARRIER_INVALID` fail loud 且 run 停留 `FAILED_RETRYABLE`；非 retired 的 policy error 原样 re-raise 进入既有 retryable 路径；无 catch-and-pass、静默 quarantine、人工 acknowledge 或伪造 terminal success |
