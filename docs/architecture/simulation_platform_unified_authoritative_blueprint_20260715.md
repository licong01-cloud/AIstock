...
| `F-130` | §5.3；daily-context detailed §1–§3/§7；BUG-1197 P1-A broker matrix、既有规则复用与live-reference纯函数 | `backend/tests/simulation_runtime/test_broker_limit_authority.py`、`backend/tests/trading_core/test_a_share_live_limit_rule.py`、`backend/tests/dataset_release/test_a_share_limit_rule.py`：cross-broker deny、规则版本/比例/舍入/no-limit/非法输入与历史parity | p1a_source_review_passed_ready_for_pr + explicitly approved staged scope | P1-B MiniQMT exact detail与P1-C LocalSIM trigger/resolver尚未接线 |
| `F-131` | §5.3；daily-context detailed §4/§8；BUG-1197 P1-A `DailyTradingContextV2`/source root/parser | `backend/tests/simulation_runtime/test_daily_trading_context_v2.py`：V1/V2 readback、mixed-source/root/context hash tamper、no-limit、symbol-failed与broker负例 | p1a_source_review_passed_ready_for_pr + explicitly approved staged scope | decision/plan/recovery wiring pending P1-B/P1-C；P1-A仅contract |
| `F-132` | §5.3/§7.5；daily-context detailed §5/§9/§14–§16 | 目标 `backend/tests/paper_trading_v2/test_localsim_limit_authority_fallback.py`、`backend/tests/miniqmt_execution_runtime/test_instrument_limit_authority.py`：zero-stk-limit SQL、TDX故障备选、整日零market-SQL/recompute | design_ready | explicitly approved staged delivery：source/runtime/正常交易日 evidence pending |
...
| `SIM-P-085` | `F-130..F-132` broker-specific daily limit authority | 设计PR #3812 / merge `7905c0ab`；BUG-1197 / Issue #3813 实施P1-A contract：V2、V1 parser、broker matrix、symbol states、source/root hash与既有规则复用/live-reference纯函数 | direct/V1/dataset parity=`77 passed`；`simulation_core_l2=678 passed,2 skipped`；`platform_api_backend=13 passed`；registry=`8 passed,14/14 mapped`；L0 blocking=0；F2/Ruff/fresh-process通过 | P1A_SOURCE_REVIEW_PASSED_READY_FOR_PR | P1-A PR/CI/merge、用户restart，以及P1-B/P1-C/P1-D与正常交易日receipts分别pending；DDL/DML/dependency/config/broker/service-control均noop |
