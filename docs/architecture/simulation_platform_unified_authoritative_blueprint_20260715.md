...
>
> MiniQMT execution-kernel进度（2026-07-31）：K1/K2/K3 overall=`implemented_verified + merged`；K4-A=`implemented_verified_contract_slice + merged`；K4-B正式审核补修=`implemented_verified_source_pending_pr_ci`，K4 overall=`implemented_verified_pending_source_merge`；K5/K6=`not_started`，产品runtime未切换。
>
...
| `F-090` | §9 P0-H K4 detailed §18–§20 | 非DB direct=`202 passed,1 skipped`；DEV=`2 passed`；coverage八核心line/branch均`>=80/>=70`；`python -m nox -s l0`、`python -m nox -s validation_module_registry_l0`、`python -m nox -s miniqmt_execution_runtime_l2`、`python -m nox -s paper_v2_backend`及三份F2 validator | implemented_verified | none |
