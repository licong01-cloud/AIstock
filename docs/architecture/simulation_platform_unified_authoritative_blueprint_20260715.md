...
| `SIM-P-070` | `F-021,F-022` | BUG-981 修复 platform diagnostics 与 LocalSIM repository durable carrier 的 schema 漂移：只接受唯一权威的 `state_id -> LocalSimExecutionStateV1` map，复用 repository reader 完成 schema/hash/key identity closure；legacy list、malformed state 与 identity conflict 均 typed fail loud，不做转换或 fallback | `backend/tests/simulation_runtime/test_ops_api.py -k platform_diagnostics`=`30 passed,22 deselected`；新增 authoritative map 正例及 list/schema/identity 负例；public `SimulationRuntimeOpsService.platform_diagnostics` fresh-process path | BUG_981_SOURCE_VERIFIED_PENDING_MERGE | `source_merge=pending_user_authorization`；用户重启前 `post_restart_effective_gate=pending_user_restart`；DDL/DML/dependency/config/binding/broker/service-control 全部 `noop` |
