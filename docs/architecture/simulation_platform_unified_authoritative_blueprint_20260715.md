...
| `F-062` | §9 P0-H K2 detailed §4.0–§4.10、§9 exact receipt domains、parent status、projection/mapping/incarnation/calendar carriers | target `backend/tests/miniqmt_execution_runtime/test_kernel_repository_postgres.py`、`test_kernel_contracts.py` | design_ready | none |
...
| `F-065` | §9 P0-H K2 detailed §4.2–§4.4、§6.3、§7.2 parent status/failure/active-child closure | target `backend/tests/miniqmt_execution_runtime/test_kernel_failure_recovery.py` | design_ready | none |
| `F-066` | §9 P0-H K2 detailed §4.5–§4.7、§6.4–6.5 command mapping/outbox/incarnation | target `backend/tests/miniqmt_execution_runtime/test_kernel_outbox.py` | design_ready | none |
| `F-067` | §9 P0-H K2 detailed §4.8–§4.9、§8 calendar/session authority与exchange clock | target `backend/tests/miniqmt_execution_runtime/test_exchange_session_clock.py` | design_ready | none |
...
| `SIM-P-056` | `F-043..070` | 完成 MiniQMT 统一 durable execution kernel、无副作用算法插件 SPI、真实 timer/session clock、command outbox 与受控 vn.py compatibility façade 的 F2 实现蓝图；K2正式审核补齐exact receipt hash domains、父蓝图status一致性、command-child-broker完整映射、现有OMS/risk/route一次决定冻结、DB-epoch worker incarnation、exact calendar/session authority和对应DDL/negative tests；K2下位设计固定唯一 durable schema/repository、transaction、clock/outbox/reconcile与K2-A..D切片，不新增业务门禁 | `miniqmt_execution_kernel_vnpy_plugin_architecture_f2_design_20260722.md`、`miniqmt_execution_kernel_k2_durable_dispatch_f2_detailed_design_20260725.md`；F2 validator；DESIGN-COMPLIANCE-001；PR #2620 / merge `14962c7328ebab6cddd579c402d896d4b90e6670`；state-sync PR #2625 / merge `7897384868d2ab2fb224f4e507981dadca0be201`；K2 design PR #2714 | IMPLEMENTATION_IN_PROGRESS | K1-A/B/C implemented_verified + merged；K1-C PR #2685 / merge `e4faeb53663cb4d19eb4e07d833953725a40fdc1`，`source_merge=merged_pr_2685`；K1 overall `implemented_verified + merged`；K2 detailed design `design_ready`、source `not_started`，K3/K4 `not_started`，产品 runtime 未切换；production DDL/DML/dependency/config/broker/restart/runtime 均 `noop` |
