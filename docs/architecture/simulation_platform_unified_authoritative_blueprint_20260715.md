...
K2-B 对callback与quantity进一步只有一套authority：ORDER/TRADE/RECONCILE以strict `KernelCallbackMappingUpdateV1`把mapping、existing child projection、FAILED algo active-child closure、event/receipt/delivery-set和runtime sequence放入同一transaction，禁止ACK后第二writer或按payload猜归属；state的`parent_quantity`必须等于durable target，`traded_quantity`必须严格、非负、不回退且不超过target，remaining只由kernel派生，`FILLED`只有在traded等于target时成立。stale CLAIMED recovery保持同一attempt，只推进exact lease successor；attempt 5后不存在第6次plugin调用。当前三算法pure façade属于K4，K2-B对不兼容旧binding保留durable fail-loud evidence，不fallback legacy route。
...
3. K3 把 Sniper、BestLimit、TWAP Lite 迁移到同一 SPI，并以旧/新 trace parity 固定既有业务行为；
4. K4 交付不拥有第二套 runtime/OMS/Gateway、且对每个注册算法生成 exact compatibility receipt 的 `VnpyAlgoEngineFacadeV1`；
5. K5 以 Iceberg、Stop 两个不同事件需求的插件证明新增算法不修改 kernel；
6. K6 退役 algorithm-specific dispatcher/helper route，执行 additive migration、readback、rollback 和正常交易日真实 SIM 验收。
...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1-A/B/C 均已实现验证并合入；K1-C PR #2685 的 dual-upstream V2 authority、间接/动态/transitive helper import side-effect boundary、bounded repo-relative evidence 与 malformed isolated carrier 已完成 RED/GREEN、direct/coverage/classifier/L2/F2，并由 implementation `52e1c5a2`、final CI run `30119969033` 及 merge commit `e4faeb53663cb4d19eb4e07d833953725a40fdc1` 闭合为 `implemented_verified + merged`。`source_merge=merged_pr_2685`，K1 overall 为 `implemented_verified + merged`。K2 下位详细设计 `miniqmt_execution_kernel_k2_durable_dispatch_f2_detailed_design_20260725.md` 保持 `implementation_in_progress`；K2-A最终 source HEAD `fc261aaf47a6fade01b1037efd5c8cb8ccda5235` 已通过 PR #2729 / merge commit `0b46f7819f4147c97a36908e25ca948ce5450661` 合入，因此K2-A为`implemented_verified + merged`、`source_merge=merged_pr_2729`。K2-A-M1 repository maintainability也已通过PR #2753 / merge commit `024bcf70537c2f1b267417c72f8539937dd21a3f`闭合。K2-B shadow-only ingress/creation/delivery/materializer/repository transaction已完成本地direct+disposable DEV=`109 passed`、MiniQMT L2=`770 passed,23 skipped`、classifier与逐模块80/70 coverage；正式审核移除nonterminal predecessor event-ingress过度门禁，并按last-applied authority修正FAILED last-good state readback。K2-B implementation `ab9f9f63`已推送至PR #2773，当前为`implemented_verified`、`source_merge=pending_user_authorization`且required CI pending；K2-C/D与K3/K4仍为 `not_started`，因此整体架构保持 `IMPLEMENTATION_IN_PROGRESS`。`close_sync=not_applicable_feature`，产品 runtime 仍未切换，production DDL/DML/dependency/config/binding/broker/restart/runtime activation gates 均为 `noop`。该架构重构不得被设置为当前模拟盘运行的人工审批或额外 stop gate；按 K0-K6 可独立审核切片实施，并与现有阻断 BUG、生产 rollout 和运行观测分别记录。
...
| `F-049` | §9 P0-H K3；current-three plugin migration | target backend/tests/miniqmt_execution_runtime/test_vnpy_plugin_parity.py；existing backend/tests/miniqmt_execution_runtime/test_miniqmt_vnpy_algo_parity_sniper.py | design_ready | none |
| `F-050` | §9 P0-H K5；Iceberg/Stop manifests/plugins | target backend/tests/miniqmt_execution_runtime/test_vnpy_plugin_extensibility.py | design_ready | none |
...
| `F-063` | §9 P0-H K2 detailed §4.1、§4.6、§5–§6.2、§12 K2-B | `backend/tests/miniqmt_execution_runtime/test_kernel_ingress.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_creation.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_repository_postgres.py`验证exact routing、ALGO_START、nonterminal predecessor queue、callback single transaction/rollback/readback；direct+DEV=`109 passed` | implemented_verified_local | none |
| `F-064` | §9 P0-H K2 detailed §4.2–4.4、§6.3、§7、§12 K2-B | `backend/tests/miniqmt_execution_runtime/test_kernel_delivery.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_repository_k2b_validation.py`及DEV applied/failure/skip/reclaim；new modules line/branch均≥80/70；`python -m nox -s miniqmt_execution_runtime_l2`=`770 passed,23 skipped` | implemented_verified_local | none |
| `F-065` | §9 P0-H K2 detailed §4.2–§4.4、§6.3、§7.2、§12 K2-B | `backend/tests/miniqmt_execution_runtime/test_kernel_delivery.py`与`backend/tests/miniqmt_execution_runtime/test_kernel_repository_postgres.py`覆盖provider-only bounded retry、attempt 5 terminal/no sixth call、last-good state、queued successor SKIPPED、stale same-attempt reclaim；classifier仅MiniQMT、`unmapped_code_files=[]` | implemented_verified_local | none |
| `F-066` | §9 P0-H K2 detailed §4.5–§4.7、§6.4–6.5、§12 K2-A-M1 | `backend/tests/miniqmt_execution_runtime/test_kernel_contracts.py`与`test_kernel_repository_postgres.py`验证SUBMIT/CANCEL、terminal outbox immutable、FAILED+CLEAN、rollback与post-commit readback；K2-D target dispatcher/reconcile | design_ready | none |
| `F-067` | §9 P0-H K2 detailed §4.8–§4.9、§8、§12 K2-A-M1 | `backend/tests/miniqmt_execution_runtime/test_kernel_contracts.py`、`test_kernel_repository_postgres.py`与`test_plugin_import_boundaries.py`=`66 passed`验证CalendarSnapshotSet、session/timer、bounded recovery、stable ordering和fresh-process import；K2-C target clock | design_ready | none |
| `F-068` | §9 P0-H K2 detailed §9 | `backend/tests/miniqmt_execution_runtime/test_kernel_migration_postgres.py`验证DEV clean first/second apply、guarded rollback及CHECK/FK/partial predicate/type/null/default六类catalog drift | design_ready | none |
| `F-069` | §9 P0-H K2 detailed §10 | target `backend/tests/miniqmt_execution_runtime/test_kernel_diagnostics.py`；artifact `docs/operations/simulation_platform_operator_runbook_20260717.md` | design_ready | none |
| `F-070` | §9 P0-H K2 detailed §11–§13 | `python -m nox -s miniqmt_execution_runtime_l2`本地/CI=`723 passed,21 skipped`；structure/contracts/DEV repo/DEV migration/import=`4/33/12/11/66 passed`、combined=`49 passed`、aggregate coverage=`87.83/72.04`；classifier仅MiniQMT、ownership=`11/0/0`，L0/registry和三份F2通过；PR #2753 initial run `30190525169`失败根因为跨平台换行断言，implementation HEAD `92c3910`纠正run `30190835098`与pre-review docs-closeout checkpoint `75d9398a`的required CI run `30191268890`均全绿，后续source HEAD与required CI以PR实时readback为准 | design_ready | none |
...
| `SIM-P-056` | `F-043..070` | MiniQMT统一kernel/plugin F2蓝图保持唯一权威；K2-A strict durable contracts已合入，K2-A-M1只拆分repository内部物理职责且不修改public/transaction/schema/migration/business contracts；未启动任何worker或产品route | K2 design PR #2714 / merge `50dbcbf6c5d943944a7c7a0a4e5f3e69f5fbd12e`；K2-A final source HEAD `fc261aaf47a6fade01b1037efd5c8cb8ccda5235`、PR #2729 / merge `0b46f7819f4147c97a36908e25ca948ce5450661`；M1 evidence由`SIM-P-062`单独记录 | IMPLEMENTATION_IN_PROGRESS | K2-A与K2-A-M1均=`implemented_verified + merged`，M1=`source_merge=merged_pr_2753`；K2-B/C/D、K3/K4=`not_started`；production DDL/DML/dependency/config/broker/restart/runtime均`noop` |
...
| Control | Review result | Design evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass | ALGO_START、exact routing、per-algo ordering、state/timer/failure transition、unknown outbox、pinned exact façade、迁移、旧 route 退役和真实 SIM 均闭合；不以方法子集、单算法或 helper-only 交付冒充平台完成 |
| `no_silent_error` | pass | stale DISPATCHING 不以 false 重提；plugin exception 原子 failure/active-child cancel/SKIPPED；malformed event/state/capability/command、sequence conflict、持久化失败和 unknown outcome 均 typed fail loud |
| `no_business_semantic_drift` | pass | exact owner routing 与 predecessor CAS 固定 callback/slot order；K1 固定 current-three exact state 与 TWAP exchange-active seconds/午休/EOD/restart，legacy alias drift 不自动重解释；Selection/Target、方向数量、A 股规则、B0 quote authority、OMS/Gateway 和 broker route 保持不变；算法迁移要求 trace parity |
| `no_unrequested_gate_or_approval` | pass | route-independent plugin catalog 与 per-plugin/per-route compatibility receipt 分离，单个 route/plugin unsupported 不阻止其它 plugin；当前暂缺/非法 observation 自动处理并在 EOD 终结；未新增 RBAC、人工 acknowledge、审批、confirm-run、全局 stop gate 或永久 enable flag |
| `production state separation` | pass | K1-B code/CI/source merge 已通过 PR #2655 / merge `ae1035a1` 闭合，K1-C 已通过 PR #2685 / merge `e4faeb53` 闭合；DDL/DML/dependency/config/binding/restart/broker 与正常交易日 runtime evidence 继续分开且均为 `noop`，产品 runtime 未切换 |
...
MiniQMT 插件化架构只有在 `F-043..F-060` 全部由 `design_ready` 更新为 `implemented_verified`，现有三算法迁移与 Iceberg/Stop 扩展性、restart/outbox exactly-once、旧 algorithm-specific route 退役及正常交易日 MiniQMT SIM 证据均闭合后，才可宣称实现完成；蓝图或 K1 详细设计合入本身不代表代码或生产 runtime 已具备该能力。
