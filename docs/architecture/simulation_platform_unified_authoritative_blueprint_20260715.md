...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1-A 已通过 PR #2637 / `e69e72db` 实现验证并合入，K1-B registry/current-three manifests 正在实现，K1-C 仍未开始；产品 runtime 仍未切换，未执行 DDL/DML、未修改配置、未调用 broker、未重启服务。该架构重构不得被设置为当前模拟盘运行的人工审批或额外 stop gate；按 K0-K6 可独立审核切片实施，并与现有阻断 BUG、生产 rollout 和运行观测分别记录。
...
| `F-055` | §9 P0-H K1；`backend/services/miniqmt_execution_runtime/plugin_registry.py` immutable catalog、binding、aggregate、supplied receipt 与 route isolation | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py`；K1-B direct 34 passed | design_ready | none |
...
| `F-057` | §9 P0-H K1；`backend/execution_algos/vnpy_style/plugin_manifests.py` current-three exact schema/source/state/TWAP/legacy projection；shadow-only | `backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py`；K1-B direct 34 passed | design_ready | none |
