...
| `F-137` | §4.3/§4.6、§5.1–§5.4；legacy-retirement detailed §4.4、LR-3 | target `backend/tests/simulation_execution/test_localsim_runtime.py` and `backend/tests/simulation_execution/test_twap_policy_authority.py` | design_ready + explicitly approved design-only scope | source/runtime evidence pending LR-3/LR-9 |
| `F-138` | §4.6、§5.5–§5.6；legacy-retirement detailed §4.5、LR-3 | target `backend/tests/simulation_execution/test_localsim_economic_transaction.py` and `backend/tests/simulation_execution/test_localsim_projection.py` | design_ready + explicitly approved design-only scope | DEV/restart evidence pending LR-3/LR-9 |
...
| `SIM-P-086` | `F-133..F-148` LocalSIM legacy retirement与data/signal/execution隔离 | 设计：`localsim_legacy_retirement_and_layer_isolation_f2_detailed_design_20260830.md`；Stage A source已建立`simulation_data/simulation_signal/simulation_execution`边界，迁移共享contract与Selection owner，删除已迁出的V25 day-feature、legacy previous-close和current-day DB-minute入口；真实Selection实现唯一属于`simulation_signal`，`simulation_runtime.selection`暂为无class/def的import-only兼容面；本地直接证据：`simulation_core_l2`=`718 passed, 2 skipped` + `63 passed, 1 skipped`、核心no-drift=`337 passed`、`paper_v2_backend`=`1129 passed, 3 skipped, 2 xfailed` | target：`SIM-LR-A..D`四阶段、最多六个源PR；layer import、legacy absence、route uniqueness、query budget、single-writer、atomic create、six-month replay、safe-boundary live transition、DEV/restart/normal-day receipts | STAGE_A_SOURCE_IMPLEMENTED_PR_PENDING | Stage A sourcePR #3994尚未合入；临时import-only兼容面绑定`SIM-LR-D/PR-D1`物理删除；merge、DEV/production DDL/DML、用户restart、runtime、normal-day及`SIM-LR-B..D`旧产品物理退役分别pending；本阶段production gates仍为noop |
