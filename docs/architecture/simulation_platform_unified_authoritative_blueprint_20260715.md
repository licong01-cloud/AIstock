...
>
> 2026-08-08 的112项audit是历史快照；2026-08-12扩展为121项，2026-08-16增加 `F-122..F-125`，2026-08-21增加 `F-126..F-129`，2026-08-26增加 `F-130..F-132`，现为132项。`F-126..F-129` 的既有source/DEV/merge/runtime证据作为V1冻结与热路径历史事实保留，但共同 `market.stk_limit` 权威只适用于既有V1 carrier；新计划的broker-specific authority、V2兼容和故障隔离由`F-130..F-132`约束。设计PR #3812已合入；BUG-1197 P1-A已由PR #3816合入merge `8e1aecf5`、runtime待用户重启；BUG-1202 P1-B MiniQMT direct resolver已完成source implementation与最终审核，等待PR/CI/merge。P1-C LocalSIM resolver、P1-D和runtime/正常交易日证据仍未完成。凡旧条目依赖durable TICK、per-wait event、NO_FILL append、unbounded history、每cadence静态行情loader或minute adapter隐式派生limit，其历史`implemented_verified`只保留审计记录，不再代表满足当前目标。凡旧条目把V25描述为LocalSIM/MiniQMT当前可执行算法，其结论同样只保留为历史审计记录，不再代表当前runtime authority。
>
...
| `SIM-P-085` | `F-130..F-132` broker-specific daily limit authority | 设计PR #3812 / `7905c0ab`；BUG-1197 P1-A PR #3816 / merge `8e1aecf5`；BUG-1202 P1-B MiniQMT direct authority source | P1-A direct=`77 passed`、CI全绿；P1-B direct=`355 passed,2 skipped`、`simulation_core_l2=687 passed,2 skipped`、`paper_v2_backend=1130 passed,3 skipped,2 xfailed`、QMT/registry/L0/F2全绿 | P1A_SOURCE_MERGED_RUNTIME_PENDING + P1B_SOURCE_REVIEW_PASSED_READY_FOR_PR | P1-B PR/CI/merge、P1-C/P1-D、用户restart与正常交易日receipts分别pending；DDL/DML/dependency/config/broker/service-control均noop |
...
模拟盘平台整体完成必须是 §15 所有 `REPAIR_REQUIRED` 项、`SIM-P-080/F-113..F-121`、`SIM-P-082/F-122..F-125`、`SIM-P-084/F-126..F-129`以及`SIM-P-085/F-130..F-132`均由真实实现和直接证据更新为`IMPLEMENTED_VERIFIED`，Phase 0B/Adaptive阶段按各自批准范围完成；在此之前，只能汇报具体slice的完成状态，不能宣称整个平台已经完成。
