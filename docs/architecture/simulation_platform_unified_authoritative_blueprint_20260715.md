...
| QE/回测/训练/演进/catalog | 2026-08-16 implementation slice 已接入统一退休 authority，source/DEV 验证完成、PR/merge 待处理 | 新 V25 execution task 统一返回 `V25_EXECUTION_ALGO_RETIRED` | 历史 artifact/metrics/checkpoint 可读，不可激活 |
...
- V25 仓库级禁用不物理删除源码、模型或历史 artifact；2026-08-16 implementation slice 已用 `F-124/F-125` 直接测试和 DEV catalog migration 闭合 source/DEV，PR/merge、生产 DML、用户重启与 runtime readback 继续独立记录；
...
| `SIM-P-082` | BUG-1096 + V25 retirement | PR #3498/#3519 merged；repository retirement implementation branch pending PR | DEV catalog migration passed；production DML pending separate authorization | BUG-1096 runtime passed；F-124/F-125 source/DEV passed | implementation PR/CI/merge、production DML、用户重启/runtime readback 分别 pending |
...
| `production state separation` | pass | 蓝图 PR #3519 当时只同步权威设计，`F-124/F-125` 当时为 design_ready；当前 repository-retirement source/DEV 状态以 §14、`SIM-P-082` 和本文末 P0-I implementation review 为准。BUG-1096 的 source/merge/restart/runtime 已独立闭合；DDL/DML/dependency/config/broker/service-control 全部 `noop` |
...
| `production state separation` | pass | source、DEV disposable-schema migration、PR/CI/merge、production catalog DML、用户重启/runtime readback 与后续物理 retention cleanup 分开记录。本 slice 未执行生产 DDL/DML、依赖安装、broker 调用或进程控制；frontend 依赖未安装，typecheck 留给既有 CI。 |
