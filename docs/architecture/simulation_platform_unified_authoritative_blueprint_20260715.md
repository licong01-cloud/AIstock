...
| `F-062` | §9 P0-H K2 detailed §4.0–§4.10、§9；strict carriers、repository与migration triplet | `backend/tests/miniqmt_execution_runtime/test_kernel_contracts.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_repository_postgres.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_migration_postgres.py` targeted矩阵=`35 passed`；DB catalog fingerprint、forward checksum与后续slice authority闭合 | design_ready | none |
...
| `SIM-P-056` | `F-043..070` | MiniQMT统一kernel/plugin F2蓝图保持唯一权威；K2-A第二轮补修增加 exact initial facts、terminal CANCEL outbox不变的later callback closure、complete carrier/scalar projection/recovery closure与code-owned independent catalog proof；未启动任何worker或产品route | K2 design PR #2714 / merge `50dbcbf6c5d943944a7c7a0a4e5f3e69f5fbd12e`；K2-A PR #2729；第二轮direct/DEV targeted与coverage已转绿，classifier/F2/DESIGN-COMPLIANCE/required CI待最终闭合 | IMPLEMENTATION_IN_PROGRESS | K2-A=`review_fix_in_progress`；K2-B/C/D、K3/K4=`not_started`；source merge=`pending_review_fix`；production DDL/DML/dependency/config/broker/restart/runtime均`noop` |
...
| `SIM-P-061` | `F-061..070` | K2-A第二轮审核补修正在闭合：strict initial carriers + repository first-write validation、CANCEL ACK后later callback mapping/algo atomic closure、complete pure scalar projection/exact recovery readback、independent `pg_catalog` recomputation；保持K1唯一DTO authority、shadow-only且K3前不切换产品route | `plugin_contracts.py`、`kernel_repository.py`、K2 migration triplet、四份MiniQMT tests（含import-boundary）；contracts=`33 passed`、DEV repository=`12 passed`、DEV migration=`11 passed`、repository coverage=`87.12/72.10`；最终 classifier/L2/Paper/L0/registry/F2/CI待刷新 | IMPLEMENTATION_IN_PROGRESS | `source_merge=pending_review_fix`；K2-A=`review_fix_in_progress`；K2-B/C/D、K3/K4=`not_started`；`close_sync=not_applicable_feature`；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
