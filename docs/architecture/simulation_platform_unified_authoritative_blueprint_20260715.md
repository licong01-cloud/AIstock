...
| `F-053` | §9 P0-H K1；K1-B shadow manifest import boundary | `backend/tests/miniqmt_execution_runtime/test_miniqmt_vnpy_algo_import_boundary.py` = 2 passed；authoritative-review delta ownership 7/7 mapped、unmapped/ambiguous=0，PR classifier 8/8 mapped；K1-C compatibility generator/import closure 仍 not_started | design_ready | none |
...
| `F-055` | §9 P0-H K1；`backend/services/miniqmt_execution_runtime/plugin_registry.py` immutable catalog、descriptor/process binding、canonical persisted readback、exact creation/restore、bounded aggregate、supplied receipt 与 route isolation | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py` = 15 passed；PR #2655 required CI green | implemented_verified | none |
...
| `F-057` | §9 P0-H K1；`backend/execution_algos/vnpy_style/plugin_manifests.py` current-three exact AISTOCK_DERIVED schema/source/executable behavior trace/active-order decimal-quantity-lineage state/TWAP/legacy projection；shadow-only | `backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py` = 33 passed；PR #2655 required CI green | implemented_verified | none |
...
| `F-059` | §9 P0-H K1；ownership/test routing/coverage | `python -m pytest -q backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py` = 48 passed，import-boundary = 2 passed；coverage line/branch 88.04%/73.10%；authoritative-review delta ownership 7/7 mapped，PR classifier 8/8 mapped；`python -m nox -s miniqmt_execution_runtime_l2` = 522 passed/1 skipped；`python -m nox -s paper_v2_backend` = 1048 passed/2 skipped/2 xfailed；L0/registry PASS | implemented_verified | none |
