...
>
> MiniQMT execution-kernel进度（2026-08-02）：K1/K2/K3/K4/K5 overall=`implemented_verified + merged`。K6 base design已通过PR #2993合入，K6-A已通过PR #3004完成`implemented_verified + merged`。2026-08-02 implementation-readiness revision已通过PR #3024 / merge `1586c15d88f11ad176a6763a15fbc584409f72c7`完成`design_revision_ready + merged`，剩余优先级固定为`K6-C0/C1 -> K6-B -> K6-D`，三者仍为`not_started`，K6 overall=`implementation_in_progress`，产品runtime未切换。
>
...
6. K6按[`miniqmt_execution_kernel_k6_product_cutover_f2_detailed_design_20260801.md`](miniqmt_execution_kernel_k6_product_cutover_f2_detailed_design_20260801.md)实施：K6-A提供base contracts/migration/repository；第一优先级K6-C0/C1补齐strict command_json、V2 proceeds/ledger/coordination、MATERIALIZE/REJECT/DEFER与generic per-command materializer；第二优先级K6-B只把deferred mapping的同一command id原子推进到现有K2 outbox；K6-D完成new-instance唯一KERNEL_V2 cutover与旧route drain/retirement。只增加现有K2缺失的coordination/product-authority事实，不复制K2 event/transition/outbox/OMS/Gateway，不进入算法plugin，不允许dual route/fallback，并以successor migration、独立readback、guarded rollback和正常交易日真实SIM验收。
...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1/K2/K3/K4/K5 overall均已`implemented_verified + merged`。K6 base design=`design_ready + merged`（PR #2993）；K6-A=`implemented_verified + merged`（PR #3004），其dependent-BUY V1与hash-only product authority V2不等于本修订新增的dependent-BUY V2/product authority V3。2026-08-02 revision已通过PR #3024 / merge `1586c15d88f11ad176a6763a15fbc584409f72c7`完成source merge；K6-C/B/D=`not_started`，下一步只允许先做K6-C0/C1，再做K6-B，最后K6-D。`close_sync=not_applicable_feature`，产品runtime未切换，production DDL/DML/dependency/config/binding/broker/restart/runtime activation gates均为`noop`。该架构重构不得被设置为当前模拟盘运行的人工审批或额外stop gate，并与现有阻断BUG、生产rollout和运行观测分别记录。
...
| `F-102` | §9 P0-H K6 detailed §4.1；target `backend/services/miniqmt_execution_runtime/kernel_product_contracts.py` | target `python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py -q` | design_ready | none |
...
| `F-104` | §9 P0-H K6 detailed §4.2、§8 | target `python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_authority.py -q` | design_ready | none |
| `F-105` | §9 P0-H K6 detailed §5；K6-A immutable migration + K6-C0 successor | target `AISTOCK_RUN_MINIQMT_K2_DEV_DB=1 python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_k6c_migration_postgres.py -q` | design_ready | none |
| `F-106` | §9 P0-H K6 detailed §6；target `backend/services/miniqmt_execution_runtime/kernel_product_repository.py` | target `AISTOCK_RUN_MINIQMT_K2_DEV_DB=1 python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_repository_postgres.py backend/tests/miniqmt_execution_runtime/test_kernel_dependent_buy_postgres.py -q` | design_ready | none |
...
| `F-111` | §9 P0-H K6 detailed §12–§13 | artifact: ordered `K6-C0/C1 -> K6-B -> K6-D` PR receipts + separately reported production/runtime states | design_ready | none |
...
| `SIM-P-069` | `F-101..112` | K6 base design与K6-A已合入；K6-A提供dependent-BUY V1、hash-only product authority V2、七张additive表与repository base。2026-08-02 implementation-readiness review确认这些版本不能支撑fresh-process product route；权威修订固定strict command_json/evaluation evidence、dependent-BUY V2 proceeds/ledger/coordination、product authority V3 MATERIALIZE/REJECT/DEFER、terminal no-broker reject、deferred mapping/no-outbox及same-command release，并将优先级改为K6-C0/C1后K6-B再K6-D。修订不改signal/selection/package/target/side/quantity/算法/LocalSIM，也不增加fallback/RBAC/审批/人工acknowledge | review=`F2 12/12,70/70,112/112 warnings=0`；classifier=`docs_fast_update`、unmapped=[]；L0=0；registry=`8 passed,14/14 mapped`。K6-A历史代码证据不冒充V2/V3实现；revision source merge=PR #3024 / `1586c15d...` | K6_IMPLEMENTATION_READINESS_REVISION_MERGED | base design `source_merge=merged_pr_2993`；K6-A `source_merge=merged_pr_3004`；revision `source_merge=merged_pr_3024`；K6-C/B/D=`not_started`；product runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime/normal-day全部`noop/not_run` |
...
| `production state separation` | pass | K1–K5 source merge已闭合；K6 base design与K6-A已合入，2026-08-02 revision尚未source merge；K6-C/B/D=`not_started`且优先级C0/C1->B->D；DDL/DML/dependency/config/binding/restart/broker/runtime activation/normal-day observation继续分开且均为`noop/not_run`，产品runtime未切换 |
