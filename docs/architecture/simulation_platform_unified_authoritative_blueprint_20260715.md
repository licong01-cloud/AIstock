...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1-A 已通过 PR #2637 / `e69e72db` 实现验证并合入，K1-B registry/current-three manifests 已通过 PR #2655 实现验证且 source merge 待用户授权；K1-C committed/source 尚未进入 main/PR、`source_merge=not_committed`，本地隔离 worktree 观察为 `C1=in_progress`、source-isolated import-boundary `implemented_verified_local`、compatibility requirement/locked surface/receipt `blocked_by_design_and_source_authority`，不构成已提交实现；K1 overall 仍为 in progress，K2/K3/K4 均 `not_started`，产品 runtime 仍未切换，未执行 DDL/DML、未修改配置、未调用 broker、未重启服务。该架构重构不得被设置为当前模拟盘运行的人工审批或额外 stop gate；按 K0-K6 可独立审核切片实施，并与现有阻断 BUG、生产 rollout 和运行观测分别记录。
...
| `F-053` | §9 P0-H K1；K1-B shadow manifest import boundary | `backend/tests/miniqmt_execution_runtime/test_miniqmt_vnpy_algo_import_boundary.py` = 2 passed；authoritative-review delta ownership 7/7 mapped、unmapped/ambiguous=0，PR classifier 8/8 mapped；K1-C committed/source 尚未进入 main/PR，本地 source-isolated import-boundary `implemented_verified_local`，compatibility generator/locked surface/receipt `blocked_by_design_and_source_authority` | design_ready | none |
...
| `F-055` | §9 P0-H K1；`backend/services/miniqmt_execution_runtime/plugin_registry.py` immutable catalog、descriptor/process binding、canonical persisted readback、exact creation/restore、bounded aggregate、builder/readback 共用 supplied receipt component authority 与 route isolation | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py` = 16 passed；K1-B-FINAL-REVIEW incomplete component readback RED 1 failed 后 GREEN；PR #2655 required CI 以最终 HEAD 为准 | implemented_verified | none |
...
| `F-057` | §9 P0-H K1；`backend/execution_algos/vnpy_style/plugin_manifests.py` current-three exact AISTOCK_DERIVED schema/source/executable behavior trace/active-order parent/local/broker/command/decimal/quantity/native-market-data/order-trade lineage state/TWAP/legacy projection；shadow-only | `backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py` = 36 passed；K1-B-FINAL-REVIEW child/parent fill 与 parent/market lineage RED 2 failed 后 GREEN；PR #2655 required CI 以最终 HEAD 为准 | implemented_verified | none |
| `F-058` | §9 P0-H K1；pinned requirement lock 与 K1-B expected component hash seam | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py` supplied receipt missing/FAILED/component mismatch | design_ready | none |
| `F-059` | §9 P0-H K1；ownership/test routing/coverage | `python -m pytest -q backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py` = 52 passed，import-boundary = 2 passed；coverage line/branch 88.17%/73.68%；final-review delta ownership 7/7 mapped，PR classifier 8/8 mapped；`python -m nox -s miniqmt_execution_runtime_l2` = 526 passed/1 skipped；`python -m nox -s paper_v2_backend` = 1048 passed/2 skipped/2 xfailed；L0/registry PASS | implemented_verified | none |
| `F-060` | §9 P0-H K1；state-separated rollout/rollback | validation-receipt: 三份 F2 validator 8/8、18/18、60/60 与 DESIGN-COMPLIANCE-001 local PASS；production gates noop | design_ready | none |
...
| Progress ID | Acceptance IDs | Current state after this PR（base `origin/main@3f77aa16`） | Evidence | Status | Next implementation slice |
| --- | --- | --- | --- | --- | --- |
...
| `SIM-P-056` | `F-043..052` | 完成 MiniQMT 统一 durable execution kernel、无副作用算法插件 SPI、真实 timer/session clock、command outbox 与受控 vn.py compatibility façade 的 F2 实现蓝图；正式审核补齐 ALGO_START identity、exact event routing、per-algo predecessor ordering、nullable stale-dispatch truth、plugin failure/active-child closure、capability 三分法和 pinned exact façade receipt；固定唯一 runtime/OMS/Gateway owner、K0-K6 切片和旧 route 退役边界 | `miniqmt_execution_kernel_vnpy_plugin_architecture_f2_design_20260722.md`；新旧蓝图 F2 validator；DESIGN-COMPLIANCE-001；PR #2620 / merge `14962c7328ebab6cddd579c402d896d4b90e6670`；state-sync PR #2625 / merge `7897384868d2ab2fb224f4e507981dadca0be201` | IMPLEMENTATION_IN_PROGRESS | `source_merge=merged_pr_2620`；`close_sync=not_applicable_feature_design`；K1-A source 已实现验证并合入，K1-B implemented_verified/source merge 待用户授权；K1-C committed/source `source_merge=not_committed`，本地 C1/import-boundary/blocked compatibility 状态独立记录且不构成 implemented；产品 runtime 未切换；`production_ddl=noop`；`production_dml=noop`；dependency/config/broker/restart/runtime 均 `noop` |
...
| `SIM-P-058` | `F-054,F-056` | 实现 K1-A strict/frozen contracts、recursive FrozenJson、canonical JSON/raw digest/hex、decimal/time/error evidence、完整 runtime event composite DTO、state/start/initialization/effect closure，以及 logical-time keyed deterministic ID/raw-digest u53/ordinal helpers。正式审核补修公开 FrozenJson marker 深冻结、JSON Schema definition/config/state instance authority、exact source key set 与 ALGO_START tuple、delivery/local-order/command/timer/diagnostic readback identity、state/diagnostic logical-time authority及 error-renderer 二次异常；same command ID/different quantity 不再共享 effect hash；schema violation evidence 有界且显式标记截断；generic helper 收窄为内部 ACTION，消除第二 identity authority。保持 K1-A shadow-only 和产品 runtime 零接线 | `plugin_canonical.py`、`plugin_contracts.py`、`deterministic_context.py`；RED audit matrix 5 failed；GREEN direct 67 passed；coverage 94/85/97%；classifier `targeted_ci_required`、unmapped empty、only `miniqmt_execution_runtime_l2`；module L2 473 passed/1 skipped；PR #2637 / merge `e69e72dbdd6e5fd8d414721adaa86ccec7fafc2f` | K1_A_IMPLEMENTED_VERIFIED | `source_merge=merged_pr_2637`；`close_sync=not_applicable_feature`；K1-B catalog/current-three implemented_verified/source merge 待用户授权；K1-C committed/source 未进入 main/PR，本地隔离状态不构成 K1-C implemented；production DDL/DML/dependency/config/binding/broker/restart/runtime 全部 `noop` |
| `SIM-P-059` | `F-055,F-057,F-059,F-060` | K1-B 已实现并经两轮独立审核补修 route-independent immutable catalog、serializable descriptor/process binding、canonical persisted snapshot readback、exact creation/restore PluginKey、deterministic bounded aggregate failure、builder/readback 共用 supplied pinned receipt component authority、per-route static isolation、current-three exact AISTOCK_DERIVED manifests/strict config-state/source-executable-behavior closure、active-order exact parent/local/broker/command/decimal/quantity/native-market-data/order-trade lineage、parent/child cumulative fill、TWAP terminal/latest-view/zero-slice 与 observation-only legacy effective projection；保持 shadow-only，未修改产品 runtime wiring | `plugin_registry.py`、`plugin_manifests.py`；首轮独立 RED 12 failed/1 passed；K1-B-FINAL-REVIEW RED 3 failed；GREEN registry/current-three direct 16/36 passed，import-boundary 2 passed；coverage line/branch 88.17%/73.68%；final-review delta ownership 7/7 mapped，PR classifier 8/8 mapped；L0/registry PASS；module L2 526 passed/1 skipped，paper_v2 L2 1048 passed/2 skipped/2 xfailed；F2 validators 8/8、18/18、60/60；PR #2655 required CI 以最终 HEAD 为准 | K1_B_IMPLEMENTED_VERIFIED | `source_merge=pending_user_authorization`；`close_sync=not_applicable_feature`；K1-C committed/source `source_merge=not_committed`，本地为 `C1=in_progress`、source-isolated import-boundary `implemented_verified_local`、compatibility requirement/locked surface/receipt `blocked_by_design_and_source_authority`，不构成 K1-C implemented；K2/K3/K4 `not_started`；production DDL/DML/dependency/config/binding/broker/restart/runtime 全部 `noop` |
...
| `production state separation` | pass | 当前仅设计文档；代码、CI、merge、DDL/DML、config、restart、broker 与正常交易日 runtime evidence 分开追踪 |
