...
相同失败也不得按 scheduler cadence 无限重放。每个 `run_id + retry_key` 使用唯一 `simulation_scheduler_retry_control_v1`，entry 固定包含 run-source fingerprint、failure fingerprint/stage、连续失败数、attempt count、首次/最近失败时间、`next_retry_at`、attempt lease、完整 JSON-safe error evidence 以及 entry/root SHA-256。首次真实失败后按 `60s,120s,240s...` 指数退避，最大 3600 秒；到期 claim 在 repository row lock 内原子建立 600 秒 single-writer lease，重启后从同一 durable carrier 继续。相同 source/failure 不得提前重试；source fingerprint 变化自动清除旧 entry 并立即恢复；成功自动清除。schema/hash/identity/timeline drift 必须 fail loud。`RETRY_BACKOFF/RECOVERY_BACKOFF` 是只读、自动解除的运行事实，不计为新失败或 terminal success，不要求人工 acknowledge、审批、重启或业务开关。
...
| scheduler 相同 failed/recovery fingerprint 重复出现 | 首次 fail loud 后持久化 retry control；未到 `next_retry_at` 只返回 backoff；到期由 row-lock lease 单 writer 自动重试；source 变化或成功自动清除 | 每 30 秒重建 Selection/plan/context、扫描每个 symbol 的数据库行情、只写日志、静默跳过、永久 quarantine 或要求人工 ack |
...
| `SIM-P-077` | `F-007,F-009,F-014,F-021,F-022,F-031` | BUG-1010 修复 Simulation Runtime 失败恢复读取风暴：同一 run/recovery fingerprint 首次 typed fail loud 后写入 hash-closed durable retry control，按 60 秒起步、最大 3600 秒自动指数退避；到期 attempt 由 row-lock lease 保证 single writer，source 变化或成功自动清除。existing-plan continuation/recovery 不再重读 `kline_daily_raw`、`suspend_d`、`stock_st` 等计划期行情表：LocalSIM 使用 frozen tradability 与 causal TDX minute，MiniQMT 使用 frozen tradability 与 B0 broker quote；账户、持仓、ledger、outbox 和 durable state 等运行必要事实仍按各自 authority 读取。generic `FAILED_RETRYABLE` 与 product-root/schema failure 不再冒充 plan-rebuild 条件；仅 exact manifest conflict、明确 insufficient-cash 或 LocalSIM pre-trade blocked replan 保留自动重规划，并同样受 durable backoff 约束。未到期项直接 strict-readback 候选查询已载入的 carrier，不再逐 run 建立 claim/readback 事务；只有到期、source 变化或首次执行才进入 row-lock CAS。retry carrier schema/hash/identity/timeline 损坏按 run 隔离且 fail loud，不静默清空、不永久 quarantine、不要求人工 acknowledge、审批、重启或业务开关 | RED/GREEN direct 覆盖 MiniQMT product-root、LocalSIM historical recovery、LocalSIM pre-trade blocked replan、non-due zero-claim、single-writer due claim、source change、14 类 malformed carrier、unsupported context normalization、claim corruption per-run isolation、existing-plan MiniQMT/LocalSIM 零数据库行情 loader；lifecycle=`272 passed,1 skipped`；DEV PostgreSQL independent writer/readback=`1 passed`；`simulation_core_l2=500 passed,1 skipped`；changed-code line/branch=`83.30%/72.28%`；L0 blocking=`0`；registry=`8 passed,14/14 mapped`；classifier=`simulation_core_l2`、`unmapped_code_files=[]`；F2=`112/112,warnings=0`；PR #3251 required CI=`16 passed,0 pending,0 failed` | BUG_1010_SOURCE_VERIFIED_PR_READY | GitHub Issue #3247；`source_merge=pending_user_authorization`；`close_sync=pending_user_restart_and_runtime_readback`；production DDL/DML/dependency/config/binding/broker/service-control 全部 `noop`；Multi-Alpha idle polling 不属于本 BUG，另行评估 |
...
| --- | --- | --- |
| `no_simplified_delivery` | pass | durable retry control 同时覆盖 binding failure、LocalSIM historical recovery 与 pre-trade blocked replan；existing-plan LocalSIM/MiniQMT 分别接入真实 TDX/B0 execution source，并保留账户、持仓、ledger、outbox、callback 等运行必要 authority；direct、DEV PostgreSQL、完整 simulation module 与 changed-code coverage 均已闭合 |
| `no_silent_error` | pass | 首次失败保留 typed evidence；retry carrier schema/hash/identity/timeline drift 继续 fail loud；未到期只返回显式 `RETRY_BACKOFF/RECOVERY_BACKOFF`，不计新失败或假 terminal success；malformed carrier、per-run claim isolation、独立 transaction readback 和 recovery negative matrix 已通过 |
