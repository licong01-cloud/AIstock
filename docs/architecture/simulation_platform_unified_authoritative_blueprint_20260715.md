...
| `SIM-P-011` | `F-021,022,024` | BUG-687 已实现平台级 LocalSIM/MiniQMT 六层聚合 health、五类 identity 定向只读查询、exact payload/identity/cardinality validation、bounded metrics、当前 facts 自动解除 alerts 与固定 operator runbook；不存在 feed/DB/broker/order replay side effect | BUG-687 / issue #2254；platform direct 17 passed、run-detail exact 1 passed、ops related matrix 33 passed、live-admission related 3 passed；new observability line 85.80%、branch 71.85%；Ruff/format/pycompile/diff-check、CodeGraph/UA 4/4、F2 25/25、l0/registry pass；CI pending | IMPLEMENTED_VERIFIED | source PR/CI/merge 后由用户重启；按 runbook 分别补生产 LocalSIM/MiniQMT readback，不新增人工门禁 |
...
| `SIM-P-030` | `F-021,022,024` | BUG-687 建立 `/platform-diagnostics` 和 `simulation_platform_diagnostics_v1`：五类 identity、六层 facts、LocalSIM/MiniQMT exact durable/business projection、低基数 metrics、自动解除 alerts、read-only side-effect contract 与固定 operator runbook；异常扫描/cardinality/payload/identity 及 live-admission bool/count 均 typed fail loud | BUG-687 / issue #2254；platform direct 17 passed、run-detail exact 1 passed、ops related matrix 33 passed、live-admission related 3 passed；new observability statements 85.80%、branches 71.85%；Ruff/format/pycompile/diff-check、CodeGraph/UA 4/4、F2 25/25、l0/registry pass；CI pending | IMPLEMENTED_VERIFIED | source PR/CI/merge 后由用户重启；分别补正常交易日 LocalSIM/MiniQMT platform readback，production DDL/dependency/config/binding/broker 均为 noop |
