...
>
> MiniQMT execution-kernel进度（2026-07-31）：K1/K2/K3/K4 overall=`implemented_verified + merged`；K4-A=`implemented_verified_contract_slice + merged`；K4-B已通过PR #2953 / merge `cbb5f12871f41d1fd529a9a98e8811484eac8ba0`完成`implemented_verified + merged`；K5详细设计=`design_ready`、implementation=`not_started`，K6=`not_started`，产品runtime未切换。
>
...
5. K5 以 Iceberg、Stop 两个不同事件需求的插件证明新增算法不修改 kernel；K5只允许对K4 conformance builder抽取一个由原current-three writer和exact full-five writer共享的pure evaluator，既有receipt/hash与K4输出不变，不形成平行authority或任意plugin-set入口；
...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1/K2/K3/K4 overall均已`implemented_verified + merged`。K4-A=`implemented_verified_contract_slice + merged`；K4-B正式审核补修已完成本地direct/DEV/MiniQMT/Paper/coverage/L0/registry/F2及required CI run `30573914476`闭环，并通过PR #2953 / merge `cbb5f12871f41d1fd529a9a98e8811484eac8ba0`合入。K5详细设计[`miniqmt_execution_kernel_k5_iceberg_stop_plugins_f2_detailed_design_20260731.md`](miniqmt_execution_kernel_k5_iceberg_stop_plugins_f2_detailed_design_20260731.md)为`design_ready`，implementation仍为`not_started`；K6=`not_started`。`close_sync=not_applicable_feature`，产品 runtime 未切换，production DDL/DML/dependency/config/binding/broker/restart/runtime activation gates 均为`noop`。该架构重构不得被设置为当前模拟盘运行的人工审批或额外 stop gate；按 K0-K6 独立审核切片实施，并与现有阻断 BUG、生产 rollout 和运行观测分别记录。
...
| `F-050` | §9 P0-H K5；K5 detailed design §0–§13 exact Iceberg/Stop manifests/plugins/full-five shadow catalog | target `backend/tests/miniqmt_execution_runtime/test_vnpy_plugin_extensibility.py`、`test_vnpy_k5_adapter_lifecycle.py`、`test_vnpy_k5_shadow_postgres.py` | design_ready | none |
...
| `F-091` | §9 P0-H K5 detailed §0–§3 | `backend/tests/miniqmt_execution_runtime/test_vnpy_k5_scope_boundaries.py` import/no-diff/algo-branch checks | design_ready | none |
| `F-092` | §9 P0-H K5 detailed §4 | `backend/tests/miniqmt_execution_runtime/test_vnpy_k5_plugin_manifests.py` exact identity/config/source/event/capability + K4 disposition/K1 registration bridge matrix | design_ready | none |
| `F-093` | §9 P0-H K5 detailed §2.3、§4.6、§5.3 | `backend/tests/miniqmt_execution_runtime/test_vnpy_k5_facade_conformance.py` K4 byte/disposition parity + full-five bridge + forged/partial negatives | design_ready | none |
| `F-094` | §9 P0-H K5 detailed §5 | `backend/tests/miniqmt_execution_runtime/test_vnpy_plugin_extensibility.py`、`test_vnpy_k5_facade_conformance.py` code-owned/fresh V2 binding equality、catalog/creation/fresh-process closure | design_ready | none |
| `F-095` | §9 P0-H K5 detailed §5.2、§5.4、§6 | `backend/tests/miniqmt_execution_runtime/test_vnpy_k5_plugin_manifests.py`、`test_vnpy_k5_adapter_lifecycle.py` config-only factory/no-hidden-authority/config/state/pointer/mapping/restore corruption matrix | design_ready | none |
| `F-096` | §9 P0-H K5 detailed §7 | `backend/tests/miniqmt_execution_runtime/test_vnpy_k5_adapter_lifecycle.py` Iceberg interval/tick cutoff/cancel-pending resubmit/late-old-terminal/`< == >` traded/restart/multi-command vectors | design_ready | none |
| `F-097` | §9 P0-H K5 detailed §8 | `backend/tests/miniqmt_execution_runtime/test_vnpy_k5_adapter_lifecycle.py` Stop BUY/SELL/limit/exactly-once/`< == >` traded/order/restart vectors | design_ready | none |
| `F-098` | §9 P0-H K5 detailed §9–§11 | `backend/tests/miniqmt_execution_runtime/test_vnpy_k5_shadow_postgres.py` real K2 seam/retry/concurrency/no-broker；diagnostics direct tests | design_ready | none |
| `F-099` | §9 P0-H K5 detailed §12 | `python -m pytest backend/tests/miniqmt_execution_runtime/test_vnpy_k5_plugin_manifests.py backend/tests/miniqmt_execution_runtime/test_vnpy_k5_facade_conformance.py backend/tests/miniqmt_execution_runtime/test_vnpy_k5_adapter_lifecycle.py backend/tests/miniqmt_execution_runtime/test_vnpy_k5_scope_boundaries.py -q`；coverage `>=80/70`；classifier-selected plans；L0/registry/F2 | design_ready | none |
| `F-100` | §9 P0-H K5 detailed §13 | artifact: `docs/architecture/miniqmt_execution_kernel_k5_iceberg_stop_plugins_f2_detailed_design_20260731.md`；PR/CI/readback/rollback与source/production/runtime gate-separated report | design_ready | none |
...
| `SIM-P-067` | `F-081..090` | K4-A保持V1 observation-only/zero-positive-publication；K4-B已实现81项V2 full-input/full-actual-trace vector、六项K3 committed parity material、fresh spawned writer/readback、sealed conformance authority、真实CLAIMED pre-call transition identity、existing K2 locked transaction中的ALGO_START/latest prior native B0 TICK read、current-three live K3重放与Iceberg/Stop characterization-only。2026-07-31正式审核补修canonical-LF跨平台authority、primary failure保留、bounded omitted hash/path evidence及characterization active-failure build-cycle；不修改current-three算法语义、不注册Iceberg/Stop、不新增DDL/产品command authority或route | artifact semantic/vector/file=`37dc70e5.../4a3117fa.../ec7bc3c7...`、live K3 binding=`123b3349...`；非DB direct=`202 passed,1 skipped`、DEV=`2 passed`、MiniQMT=`1099 passed,30 skipped`、Paper=`1050 passed,2 skipped,2 xfailed`，八核心coverage line/branch均`>=80/>=70`；final source HEAD `3b740721...`与required CI run `30573914476`已通过PR #2953 / merge `cbb5f128...`闭合 | K4_IMPLEMENTED_VERIFIED_MERGED | `k4a_source_merge=merged_pr_2883`；`k4b_design_source_merge=merged_pr_2914`；`k4b_code_source_merge=merged_pr_2953`；K4 overall=`implemented_verified + merged`；K5 detailed design=`design_ready`且implementation=`not_started`；K6=`not_started`；产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
| `SIM-P-068` | `F-091..100` | K5 F2详细设计已闭合exact Iceberg/Stop manifest/config/source、K4 `CHARACTERIZATION_ONLY_K5`到K1 shadow registration桥接、code-owned immutable V2 binding literal与fresh sealed K4 binding equality、config-only factory、K1 full-five shadow catalog/creation binding、K4 shared pure conformance evaluator、strict façade state codec、Iceberg exchange-active TIMER/sequence-cutoff native B0 quote/cancel-pending resubmit/late callback/`traded >= target`、Stop native TICK exactly-once/`traded == target`、K2 broker-neutral shadow、多command/K6边界、failure/diagnostics/测试/rollback合同。正式审核已修订K4 builder hardcode、registered与product activation混淆、宽松state、TIMER quote漂移、Stop re-arm、multi-command静默压缩、partial catalog fallback、scope creep、package import surface、Iceberg pointer/mapping误等价、factory V2 authority缺口、terminal运算符模糊和K4 disposition桥接缺失十三项风险 | artifact: `docs/architecture/miniqmt_execution_kernel_k5_iceberg_stop_plugins_f2_detailed_design_20260731.md`；F2 IDs=`F-091..100`；PR #2968 / merge `1e739dce8a5a18d9e9e4c16027801a7a81e34384`；implementation test paths与coverage/routing/DEV/no-broker验收已固定 | K5_DESIGN_READY_MERGED | `k5_design_source_merge=merged_pr_2968`；K5 implementation=`not_started`；K6=`not_started`；product runtime未切换；close-sync/root-sync/cleanup与production DDL/DML/dependency/config/binding/broker/restart/runtime activation分开记录，当前生产与运行gates全部`noop` |
...
| Control | Review result | Design evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass | ALGO_START、exact routing、per-algo ordering、strict callback/outcome、state/timer/failure transition、unknown outbox、pinned façade、迁移和旧route退役均闭合；K3 parity必须由committed legacy repository facts和DEV readback驱动；K4/K5多command trace不得截断、复用单command receipt或冒充V1 product materialization；K5要求code-owned/fresh-equal V2 binding、config-only factory、真实full-five catalog/creation binding、两算法生命周期与DEV K2 shadow，不以fixture/helper-only或静态PASSED冒充完成 |
| `no_silent_error` | pass | stale DISPATCHING不false重提；同步ACK不伪装ORDER/TRADE/OMS event，deterministic COMMAND_OUTCOME ingress推进outbox outcome；K4 pure/K2-V1-shadow conformance不得冒充product command authority；K5 binding literal与fresh authority任意漂移使full-five candidate zero publication，multi-command V1 materialization显式拒绝；malformed payload/state/lifecycle/source、sequence/identity/hash conflict、持久化失败和unknown均typed fail loud |
| `no_business_semantic_drift` | pass | exact owner routing与predecessor CAS固定callback/slot order；TWAP raw/effective due分权保持午休/EOD/restart；K5 Iceberg保持exchange-active TIMER + prior native B0 quote、cancel-pending/pointer/late callback及`traded >= target`，Stop保持native TICK exactly-once与`traded == target`且不re-arm；Selection/Target、方向数量、A股规则、B0、OMS/Gateway和broker route不变 |
| `no_unrequested_gate_or_approval` | pass | route-independent plugin catalog 与 per-plugin/per-route compatibility receipt 分离，单个 route/plugin unsupported 不阻止其它 plugin；当前暂缺/非法 observation 自动处理并在 EOD 终结；K5 shadow catalog不成为产品enable gate；未新增 RBAC、人工 acknowledge、审批、confirm-run、全局 stop gate 或永久 enable flag |
| `production state separation` | pass | K1-B code/CI/source merge 已通过 PR #2655 / merge `ae1035a1` 闭合，K1-C 已通过 PR #2685 / merge `e4faeb53` 闭合；K5仅为`design_ready`且implementation=`not_started`；DDL/DML/dependency/config/binding/restart/broker 与正常交易日 runtime evidence 继续分开且均为 `noop`，产品 runtime 未切换 |
