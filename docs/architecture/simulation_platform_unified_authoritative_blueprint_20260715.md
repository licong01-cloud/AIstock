...
>
> MiniQMT execution-kernel进度（2026-07-29）：K1/K2/K3 overall=`implemented_verified + merged`；K4-A pure façade/adapter、contracts与pinned source authority=`implemented_verified`且source merge待本PR，K4 overall=`implementation_in_progress`；K4-B/K5/K6=`not_started`，产品runtime未切换。
>
...
| `SIM-P-067` | `F-081..090` | K4设计已合入；K4-A现已实现pure `VnpyAlgoEngineFacadeV1`、通用adapter、strict carriers、DTO/enum/state/terminal/isolated bindings、deterministic characterization primitives和六source authority。K1只委托四个exact K4 artifact路径且继续拒绝unknown extra；active-order由durable mapping+ORDER/TRADE callback facts重建，active-child非CLEAN不得terminal。K4-A不修改K2 creation/delivery/repository、current-three factory/binding或产品route；K4-B继续拥有optional invocation/read-only query、真实current-three parity与Iceberg/Stop完整characterization | design PR #2861 / merge `8250b64ff...`；K4-A direct+import=`118 passed`，coverage=`49 passed`，五核心line/branch=`93/70,97/95,90/76,87/78,85/71`；MiniQMT=`1038 passed,29 skipped`，Paper=`1050 passed,2 skipped,2 xfailed`；classifier=`unmapped_code_files=[]`；live contract=`ec763c54...`、source manifest=`e0284a6d...` | K4_IMPLEMENTATION_IN_PROGRESS_K4A_VERIFIED | `k4a_source_merge=pending_pr`；K4-A=`implemented_verified`、K4-B/K5/K6=`not_started`；产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
