...
| `F-140` | §5.12；legacy-retirement detailed §4.6、`SIM-LR-B/PR-B2`/LR-6 | `pytest backend/tests/simulation_runtime/test_localsim_successor_schema.py backend/tests/simulation_runtime/test_schema_comments.py`；`python -m nox -s localsim_successor_core_dev_db`=`1 passed` | pr_b2_source_complete | explicitly approved phased gap：内部control plane与DEV schema闭合；统一API/UI/MCP仍由`SIM-LR-C`一次切流，不提前暴露 |
