...
| `F-063` | §9 P0-H K2 detailed §5–§6.2 | `backend/tests/miniqmt_execution_runtime/test_kernel_repository_postgres.py` event+receipt+deliveries transaction/readback；K2-B target ingress | design_ready | none |
| `F-064` | §9 P0-H K2 detailed §4.2–4.4、§6.3、§7、§12 K2-A-M1 | `backend/tests/miniqmt_execution_runtime/test_kernel_contracts.py`、`test_kernel_repository_postgres.py`与`test_kernel_repository_structure.py`验证exact fence/epoch/stale worker、fault rollback、无同名override与唯一connection owner；K2-B target delivery | design_ready | none |
| `F-065` | §9 P0-H K2 detailed §4.2–§4.4、§6.3、§7.2 | `backend/tests/miniqmt_execution_runtime/test_kernel_contracts.py`与`test_kernel_migration_postgres.py` init/failure/skip/closure负例；后续slice实现恢复编排 | design_ready | none |
...
| `SIM-P-062` | `F-061..070` | K2-A-M1将repository从3102行单文件拆为38行唯一public façade、shared DB owner、唯一projection/schema authority和event/delivery、transition/outbox/callback、timer/session/recovery私有模块；27个public方法、30个private helper、public signature、identity/hash、SQL/lock/CAS/fencing、transaction/readback、migration与业务语义不变，无第二repository或fallback | `test_kernel_repository_structure.py` RED=`3 passed,1 failed`后GREEN=`4 passed`；contracts/DEV repository/DEV migration/import=`33/12/11/66 passed`、combined=`49 passed`、aggregate line/branch=`87.83%/72.04%`、MiniQMT L2=`723 passed,21 skipped`；public signature SHA-256=`7045f3c2...`，migration canonical-LF triplet=`e2a244d0.../24b4e189.../cb408aaf...`；classifier仅MiniQMT且ownership=`11 mapped,0 unmapped,0 ambiguous`；PR #2753 initial CI `30190525169`失败后由implementation HEAD `92c3910`闭合跨平台checksum，run `30190835098`全绿；pre-review docs-closeout checkpoint `75d9398a`的required CI run `30191268890`同样全绿 | IMPLEMENTED_VERIFIED | `source_merge=pending_user_authorization`；K2-A仍=`implemented_verified + merged`；K2 overall=`implementation_in_progress`；K2-B/C/D、K3/K4=`not_started`；shadow-only且产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
...
| production state separation | pass | §10.5、§11、§12、`F-024`分离merge/DDL/config/restart/binding/runtime evidence；K2-A已合入与K2-A-M1待独立source授权分别记录，M1全部production/runtime gates为`noop` |
