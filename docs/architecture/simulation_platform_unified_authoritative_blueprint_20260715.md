...
>
> MiniQMT execution-kernel进度（2026-07-30）：K1/K2/K3 overall=`implemented_verified + merged`；K4-A=`implemented_verified_contract_slice + merged`；K4-B exact source executor/positive conformance/K2 optional integration=`implemented_verified_source_pending_pr`，K4 overall=`implemented_verified_pending_source_merge`；K5/K6=`not_started`，产品runtime未切换。
>
...
| `F-088` | §9 P0-H K4 detailed §3.2、§6.3、§10、§12 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_source_execution_v2.py` live K3/Iceberg/Stop matrix | implemented_verified | none |
| `F-089` | §9 P0-H K4 detailed §13–§17 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_diagnostics.py`与timeout/malformed matrix | implemented_verified | none |
| `F-090` | §9 P0-H K4 detailed §18–§20 | `python -m nox -s miniqmt_execution_runtime_l2`=`1088/30`；`python -m nox -s paper_v2_backend`=`1050/2/2`；F2/L0/registry final gate | implemented_verified | none |
...
| `SIM-P-067` | `F-081..090` | K4-A保持V1 observation-only/zero-positive-publication；K4-B已实现81项V2 full-input/full-actual-trace vector、六项K3 committed parity material、fresh spawned writer/readback、sealed conformance authority、真实CLAIMED pre-call transition identity、existing K2 locked transaction中的ALGO_START/latest prior native B0 TICK read、current-three live K3重放与Iceberg/Stop characterization-only。terminal ORDER/late TRADE、callback/outcome次序、native market-data lineage、single/multi-command shadow边界均显式闭合；不修改current-three binding、不注册Iceberg/Stop、不新增DDL/产品command authority或route | artifact semantic/vector/file=`f43313e0.../71065024.../5c912f80...`；direct=`159 passed,15 skipped`、coverage matrix=`98 passed,1 skipped`且八核心逐文件line/branch达标；DEV=`2 passed`；MiniQMT=`1088 passed,30 skipped`；Paper=`1050 passed,2 skipped,2 xfailed`；F2/L0/registry/required CI由实现PR final gate闭合 | K4_IMPLEMENTED_VERIFIED_SOURCE_PENDING_PR | `k4a_source_merge=merged_pr_2883`；`k4b_design_source_merge=merged_pr_2914`；`k4b_code_source_merge=pending_pr`；K4 overall=`implemented_verified_pending_source_merge`；K5/K6=`not_started`；产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
