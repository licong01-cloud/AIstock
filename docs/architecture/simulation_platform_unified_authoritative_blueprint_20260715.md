...
| `F-062` | §9 P0-H K2 detailed §4.0–§4.10、§9 exact receipt domains、parent status、projection/mapping/incarnation/calendar carriers | `backend/services/miniqmt_execution_runtime/plugin_contracts.py`首组strict carriers；`backend/tests/miniqmt_execution_runtime/test_kernel_contracts.py` RED ImportError→GREEN 7 passed | design_ready | none |
...
| `SIM-P-056` | `F-043..070` | 完成 MiniQMT 统一 durable execution kernel、无副作用算法插件 SPI、真实 timer/session clock、command outbox 与受控 vn.py compatibility façade 的 F2 实现蓝图；K2正式审核补齐exact receipt hash domains、父蓝图status一致性、command-child-broker完整映射、现有OMS/risk/route一次决定冻结、DB-epoch worker incarnation、exact calendar/session authority和对应DDL/negative tests；K2-A首个代码checkpoint已开始strict ingress/lineage/projection/risk/error carrier writer-readback，不新增业务门禁 | K2 design PR #2714 / merge `50dbcbf6c5d943944a7c7a0a4e5f3e69f5fbd12e`；`plugin_contracts.py`、`test_kernel_contracts.py` RED ImportError→GREEN 7 passed | IMPLEMENTATION_IN_PROGRESS | K1 overall `implemented_verified + merged`；K2-A `implementation_in_progress`，migration/repository/mapping/outbox/timer/incarnation未完成且未被虚报；K2-B/C/D、K3/K4 `not_started`；产品runtime未切换；production DDL/DML/dependency/config/broker/restart/runtime均`noop` |
