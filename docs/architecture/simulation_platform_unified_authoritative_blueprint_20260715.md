...
当前状态为 `IMPLEMENTATION_VERIFIED_MERGED`：K1-A/B/C、K2-A、K2-A-M1、K2-B、K2-C和K2-D均已`implemented_verified + merged`。K2-D final source `82c69fbf7e7245e0af76262ddc7b7f59ce7d996b` 经 required CI run `30269640126` 验证，并通过 PR #2804 / merge `fc4170faa10847c0b58aa8088b4a8b6d0ca26b29` 合入，`source_merge=merged_pr_2804`；K2 overall=`implemented_verified + merged`。K3详细设计=`design_ready_for_review`、PR #2816、`source_merge=pending_user_review`，K3/K4 implementation仍为`not_started`。`close_sync=not_applicable_feature`，产品 runtime 未切换，production DDL/DML/dependency/config/binding/broker/restart/runtime activation gates 均为`noop`。该架构重构不得被设置为当前模拟盘运行的人工审批或额外 stop gate；按 K0-K6 独立审核切片实施，并与现有阻断 BUG、生产 rollout 和运行观测分别记录。
...
| `F-071` | §9 P0-H K3 detailed §1–§3.4 current facts/boundaries/dependent-BUY carrier gap | artifact: `docs/architecture/miniqmt_execution_kernel_k3_current_three_runtime_migration_f2_detailed_design_20260727.md`；target route/import/ledger-cross-parent static tests | design_ready | none |
| `F-072` | §9 P0-H K3 detailed §3–§5.1.1 plugin identity/source/factory/catalog、v3 state与public transition ID | target `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_plugins.py`、`backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py`、`backend/tests/miniqmt_execution_runtime/test_algo_plugin_contracts.py`、`backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py` | design_ready | none |
| `F-073` | §9 P0-H K3 detailed §5–§6 Sniper exact behavior | target `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_parity.py` Sniper parity/negative/restart vectors | design_ready | none |
| `F-074` | §9 P0-H K3 detailed §5、§7 BestLimit deterministic behavior | target `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_parity.py` deterministic ordinal/retry/restart/price-change vectors | design_ready | none |
| `F-075` | §9 P0-H K3 detailed §5、§8 TWAP timer/session/restart | target `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_restart.py` AM/lunch/PM/duration/EOD/restart vectors | design_ready | none |
| `F-076` | §9 P0-H K3 detailed §5.1.1、§5.3–§5.4、§11 pre-ACK/callback/terminal closure | target `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_plugins.py` COMMAND_PENDING、callback-before-ACK、ORDER-ahead-of-TRADE、fill closure、EOD cancel/unknown/no-duplicate tests | design_ready | none |
| `F-077` | §9 P0-H K3 detailed §9–§10 policy/state/dependent-BUY inventory与ALGO_LOCAL parity contracts | target `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_parity.py`、`backend/tests/miniqmt_execution_runtime/test_current_three_legacy_inventory.py` | design_ready | none |
| `F-078` | §9 P0-H K3 detailed §3–§3.4、§13 K2 public shadow seams | target `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_plugins.py`、`backend/tests/miniqmt_execution_runtime/test_plugin_import_boundaries.py` no-branch/no-broker/no-ledger/no-cross-parent tests | design_ready | none |
| `F-079` | §9 P0-H K3 detailed §11–§12、§15 failure/diagnostics/rollback | target `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_restart.py` malformed input、plugin-local isolation、DB failure、source rollback tests | design_ready | none |
| `F-080` | §9 P0-H K3 detailed §14–§16 validation/routing/gates/K6 prerequisite | target `python -m nox -s miniqmt_execution_runtime_l2`、`python -m nox -s paper_v2_backend`及真实依赖plans；line>=80/branch>=70、`unmapped_code_files=[]` | design_ready | none |
...
| `SIM-P-066` | `F-071..080` | K3 current-three runtime migration F2详细设计固定两个shadow-only切片：K3-A把Sniper/BestLimit/TWAP Lite实现为真实pure `ExecutionAlgoPluginV2`并以3.0.0 exact manifest/source/factory/catalog闭合；三个state schema升v3，以`COMMAND_PENDING`和K2 RESERVED mapping/PENDING outbox同事务表达pre-ACK事实，禁止重复submit或伪造broker ID，public transition-ID helper成为唯一hash authority；K3-B生成legacy policy/state及dependent-BUY coordinator zero-write inventory和ALGO_LOCAL immutable parity receipt，复用K2 creation/ingress/delivery/materializer/outbox public seams且两侧`broker_called=false`。设计明确Sniper/BestLimit/TWAP price/quantity/timer/restart/EOD、ORDER/TRADE因果和active-order closure、typed failure、coverage/routing和K3/K4/K6边界；同时确认K2尚无dependent-BUY deferred-command durable carrier，禁止把cross-parent ledger/cash协调塞入plugin或普通diagnostic，K6必须先以独立F2设计/实现闭合该coordinator，再退役legacy product helper和切换唯一route；不引入永久feature flag、双broker route、fallback或人工门禁 | artifact: `docs/architecture/miniqmt_execution_kernel_k3_current_three_runtime_migration_f2_detailed_design_20260727.md`；target direct/parity/restart/inventory tests；F2 validator与DESIGN-COMPLIANCE已在本设计工作树通过 | DESIGN_ONLY | `design_status=design_ready_for_review`；PR #2816；`source_merge=pending_user_review`；K3 implementation=`not_started`；K4/K6=`not_started`；K6 dependent-BUY coordinator=`not_designed_not_implemented`；产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
