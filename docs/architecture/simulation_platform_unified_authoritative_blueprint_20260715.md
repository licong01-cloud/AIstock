...
| `SIM-P-061` | `F-061..070` | K2-A已完成七项正式审核补修的本地闭环：strict carriers、command-aware mapping/outbox、algo durable recount、exact lease/fence、DB catalog authority、independent readback、strict recovery与真实CalendarSnapshotSet/session composite owner；保持K1唯一DTO authority、shadow-only且K3前不切换产品route | `plugin_contracts.py`、`kernel_repository.py`、K2 migration triplet、四份MiniQMT tests（含import-boundary）；targeted=`35 passed`、DEV repository/migration全绿、coverage=`88.29/75.17`、classifier/L2/Paper/L0/registry/F2通过；required CI pending | IMPLEMENTATION_IN_PROGRESS | `source_merge=pending_user_authorization`；K2-A=`review_fix_in_progress`；K2-B/C/D、K3/K4=`not_started`；`close_sync=not_applicable_feature`；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
