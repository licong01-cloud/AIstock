...
| `F-104` | §9 P0-H K6 detailed §4.2、§8；V3 envelope/evaluator/materializer与BUG-953 lineage/lifecycle closure | `python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_authority.py backend/tests/miniqmt_execution_runtime/test_kernel_repository_structure.py -q`=`21 passed`；DEV PostgreSQL=`18 passed` | k6c1_implemented_verified_local | none |
...
| `F-110` | §9 P0-H K6 detailed §11 | `python -m nox -s miniqmt_execution_runtime_l2`=`1226 passed,60 skipped`；C1 direct+DEV=`39 passed`；authority line/branch=`87.76%/81.48%`、repository=`87.50%/70.95%` | k6c1_implemented_verified_local | none |
...
| `SIM-P-069` | `F-101..112` | K6 base/K6-A/C0/BUG-953保持既有merged/runtime schema状态。K6-C1已实现strict `ProductCommandAuthorityEnvelopeV3`、唯一pure evaluator、0/1/N与MIXED aggregate、MATERIALIZE/REJECT/DEFER atomic K2 lineage、typed pre-call no-broker reject、same-authority advisory-lock幂等、rollback/commit-unknown和fresh independent readback；正式复审闭合malformed public carrier typed failure、terminal MATERIALIZE physical mapping projection、DEFER waiting/block/EOD/released-outbox lifecycle、immutable initial lineage、strict CAS/SHA及coordination successor UTC hash。这里只验证K6-B未来合法状态的C1 readback兼容性，未实现K6-B decision/release writer或K6-D cutover，也未改signal/selection/package/target/side/quantity/算法/LocalSIM | C1 contracts/authority/DEV PostgreSQL=`105 passed`，其中DEV=`20 passed`；coverage authority/contracts/repository=`84.21/76.47`、`90.52/73.11`、`86.63/70.37`；production只读K6-C0 preflight=`8/8 true`、authority总行/V3行=`0/0`；最终MiniQMT L2/CI pending final HEAD | K6_C1_IMPLEMENTED_VERIFIED_LOCAL | K6-C1 `source_merge=pending_pr`；K6-B/D=`not_started`；product runtime未切换；production DDL保持既有`applied_and_verified`，本轮production DML/config/binding/broker/restart/runtime activation/normal-day=`noop/not_run` |
...
| `production state separation` | pass | K1–K5及K6 base/K6-A/revision/K6-C0 source merge已闭合；BUG-953已完成source merge、production DDL/readback、用户重启、post-restart receipt与Issue关闭；K6-C1仅为local verified、source pending PR，之后仍按B->D；production DDL=`applied_and_verified`，本切片DML/dependency/config/binding/broker/restart/runtime activation/normal-day observation均为`noop/not_run`，产品runtime未切换 |
