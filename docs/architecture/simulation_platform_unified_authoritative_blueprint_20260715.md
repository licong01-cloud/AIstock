...
>
> MiniQMT execution-kernel进度（2026-08-02）：K1/K2/K3/K4/K5 overall=`implemented_verified + merged`。K6 base design、K6-A与implementation-readiness revision已分别通过PR #2993/#3004/#3024合入。K6-C0 strict contracts、successor migration与versioned repository preflight现为`implemented_verified`、`source_merge=pending_pr`；剩余优先级固定为`K6-C1 -> K6-B -> K6-D`且均为`not_started`。K6 overall=`implementation_in_progress`，产品runtime未切换。
>
...
| `F-104` | §9 P0-H K6 detailed §4.2、§8；V3 carriers已实现 | C0：`python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py -q`；C1 target `test_kernel_product_authority.py` | design_ready | none |
...
| `F-106` | §9 P0-H K6 detailed §6；versioned repository preflight | C0：`AISTOCK_RUN_MINIQMT_K2_DEV_DB=1 python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_repository_postgres.py -q`；C1 target `test_kernel_dependent_buy_postgres.py` | design_ready | none |
...
| `SIM-P-069` | `F-101..112` | K6 base design、K6-A与readiness revision已合入。K6-C0现已实现dependent-BUY V2 proceeds/ledger/coordination、product authority V3 strict full command/evidence carriers、MATERIALIZE/REJECT/DEFER presence matrix、K6 product-owned `DEFERRED_DEPENDENT_BUY -> RESERVED|TERMINAL` mapping carrier、immutable successor migration及versioned K2/K6/K6-C0 catalog authority；同一physical mapping identity被复用且K1/K3共享mapping authority保持不变，DB status/initial/product-state三层CHECK闭合真实version=1→2 transition，shared transition projection set与per-command strict OMS/risk/route receipt分层闭合。未实现C1 materializer、B coordinator/cross-carrier dispatch handoff或D cutover，也未改signal/selection/package/target/side/quantity/算法/LocalSIM | C0 direct/DEV PostgreSQL、coverage、classifier、F2和CI evidence在当前PR最终HEAD闭合；migration catalog=`841717e7...`，forward canonical-LF=`782f3020...` | K6_C0_IMPLEMENTED_VERIFIED_PENDING_SOURCE_MERGE | base/K6-A/revision已merge；K6-C0 `source_merge=pending_pr`；K6-C1/B/D=`not_started`；product runtime未切换；production DDL=`pending_after_source_merge`且未执行，其余gates/normal-day=`noop/not_run` |
...
| `production state separation` | pass | K1–K5及K6 base/K6-A/revision source merge已闭合；K6-C0=`implemented_verified`且`source_merge=pending_pr`，K6-C1/B/D=`not_started`且优先级C1->B->D；production DDL=`pending_after_source_merge`且未执行，DML/dependency/config/binding/restart/broker/runtime activation/normal-day observation均为`noop/not_run`，产品runtime未切换 |
