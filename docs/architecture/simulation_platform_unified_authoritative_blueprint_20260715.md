...
| `F-082` | §9 P0-H K4 detailed §3 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_characterization.py` exact five-source matrix | design_ready_k4 | none |
| `F-083` | §9 P0-H K4 detailed §4–§5 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_kernel_invocation.py` initialization/transition authority、pure-plugin unchanged、adapter/create/freeze/retry/restart、single-command exact与multi-command V1 refusal | design_ready_k4 | none |
| `F-084` | §9 P0-H K4 detailed §5.4.1、§6 | `backend/tests/miniqmt_execution_runtime/test_vnpy_algo_engine_facade.py` six methods/template/callback、not-running/missing/rounded-zero diagnostics及K4/K5 V1 product-use refusal | design_ready_k4 | none |
| `F-085` | §9 P0-H K4 detailed §6.3–§8 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_projection.py` DTO/enum/event/lineage + DEV TIMER immutable cutoff matrix | design_ready_k4 | none |
| `F-086` | §9 P0-H K4 detailed §5.4.1、§5.6、§9–§10 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_state_adapter.py` lifecycle/state/two-phase terminal closure + `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_k2_shadow_integration.py` single-command materialization、multi-command complete trace/refusal与zero broker | design_ready_k4 | none |
| `F-087` | §9 P0-H K4 detailed §4.1、§11 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_conformance_receipts.py` live binding/source/isolated-module/runtime+command disposition writer-readback、V1禁止product authority值且product-use fail loud、truncation/authority drift/zero partial set | design_ready_k4 | none |
...
| `F-090` | §9 P0-H K4 detailed §18–§20 | direct + DEV PostgreSQL read-only matrix；`python -m nox -s miniqmt_execution_runtime_l2`；`python -m nox -s paper_v2_backend`；K4 core line>=80%/branch>=70%；三份F2 validators | design_ready_k4 | none |
...
| `SIM-P-067` | `F-081..090` | K4 F2详细设计将父蓝图既有范围细化为initialize/transition-scoped `VnpyAlgoEngineFacadeV1`、通用façade-backed adapter、existing K2 optional invocation/read-only authority seam、exact DTO/enum/state/effect mapping、per-registered-plugin conformance receipt、current-three parity复用及Iceberg/Stop characterization-only。正式审核补修闭合catalog+gateway route authority现场重建、actual projection payload、constructor-once/restore/extract/freeze、live implementation/isolated-module contract hash、deterministic uniform/round_to binding、not-running/missing/rounded-zero diagnostic及Iceberg TIMER通过indexed algo-delivery双sequence cutoff读取durable TICK lineage；同时准确披露K2 V1单command shadow与multi-command product-authority缺口：多command trace完整但不伪装materialization，K4/K5 V1不能作为product activation proof，generic aggregate与独立versioned product receipt由K6 F2拥有。K1-C V2 authority、K2 durable owner、K3 factory/binding保持不变，无第二runtime/OMS/Gateway/catalog/route，无K5 plugin注册或K6 cutover | artifact: `docs/architecture/miniqmt_execution_kernel_k4_vnpy_facade_f2_detailed_design_20260729.md`；PR #2861 / merge `8250b64ff3c2deb04eb3594f1ae3fba3acd1e6ce`；K4/parent/unified F2=`10/10,48/48,90/90`、warnings=0；DESIGN-COMPLIANCE-001逐项pass；Iceberg/Stop/utility固定upstream bytes size/hash只读核验一致 | K4_DESIGN_READY_FOR_IMPLEMENTATION | `source_merge=merged_pr_2861`；K4 design=`merged`、implementation=`not_started`；K5/K6=`not_started`；产品runtime未切换；production DDL/DML/dependency/config/binding/broker/restart/runtime activation全部`noop` |
