...
| `F-007` | §4.3、§5.4 LocalSIM event model；BUG-794 first causal wait generation；BUG-984 durable continuation authority | backend/tests/paper_trading_v2/test_localsim_backend.py；backend/tests/simulation_runtime/test_lifecycle_scheduler.py first-wait→same-cursor replay→first-bar resume、`broker_called=false` 但 exact state/outbox/receipt closure 存在时 fresh-process continuation direct tests | implemented_verified | none |
...
| `F-009` | §5.2、§10.3；BUG-794 exact failed-run recovery；BUG-984 active-run restart continuation | backend/tests/paper_trading_v2/test_localsim_backend.py；backend/tests/simulation_runtime/test_lifecycle_scheduler.py projection retry、exact receipt/state/Paper readback、no-parent-resubmit、`broker_called` observation drift、missing outbox 和 mismatch terminal tests | implemented_verified | none |
...
| `F-012` | §5.4 terminal contract；BUG-984 post-close state-first closure | backend/tests/simulation_runtime/test_lifecycle_scheduler.py；post-close active durable state 必须先由既有 minute loop 消费 EOD，state/plan/persistence 冲突 fail loud，run terminal 与 per-intent terminal closure direct tests | implemented_verified | none |
...
| `SIM-P-074` | `F-007,F-009,F-012,F-014,F-021` | BUG-992 fixes LocalSIM durable state authority selection across lifecycle incarnations: state history remains keyed by `state_id`, while restore, continuation, post-close and state-hash readback consume only the exact `state_id -> state_hash` set named by the latest committed economic receipt. An extra non-terminal state outside that generation remains typed fail-loud; terminal history no longer masks the current plan authority. | RED=`LOCALSIM_POST_CLOSE_STATE_PLAN_MISMATCH` with duplicated intent rows; GREEN=`test_localsim_post_close_uses_committed_generation_authority_over_terminal_history` and `test_localsim_state_authority_rejects_duplicate_missing_extra_and_hash_conflict`, plus existing restart/continuation/post-close matrix; lifecycle file 190 passed, simulation L2 418 passed, registry 8 passed with 14/14 ownership mapping, changed-code line/branch coverage 83.33%/70.00%, classifier `unmapped_code_files=[]`, F2 112/112 with zero warnings; GitHub Issue #3184 / PR #3186 | BUG_992_SOURCE_VERIFIED_PR_OPEN | `source_merge=pending_pr_3186`; `post_restart_effective_gate=pending_user_restart`; current production run remains pending user-authorized restart/readback; production DDL/DML/dependency/config/binding/broker/service-control all `noop` |
...
| Control | Review result | Implementation evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass | The fix keeps the repository's state-id history and uses the committed economic receipt as the exact authority; it does not delete, deduplicate, slice, or choose a row by insertion order/timestamp. Restore, continuation, post-close, CAS/readback and duplicate-active rejection are covered through real scheduler/repository seams; lifecycle file 190 passed and simulation L2 418 passed. |
| `no_silent_error` | pass | Missing authority state, authority hash conflict, malformed authority, and extra active history remain typed failures; terminal history is accepted only when the committed generation names the current state set. Direct fail-loud tests, L2, and F2 112/112 with zero warnings passed. |
| `no_business_semantic_drift` | pass | Frozen plan intents, state identity, algorithm policy, quantities, broker route, economic generation, and terminal status semantics are unchanged; only the already committed generation's exact state set is consumed. Changed-code line/branch coverage is 83.33%/70.00%, and ownership/classifier routing is complete with `unmapped_code_files=[]`. |
| `no_unrequested_gate_or_approval` | pass | No RBAC, acknowledgement, manual recovery, new runtime gate, broker call, config/binding change, service control, DDL or DML was added or executed. |
| `production state separation` | pass | Source, tests, BUG metadata and this authoritative blueprint are the only tracked changes; merge, user restart, post-restart runtime readback and any current-run data repair remain separate pending states. |
