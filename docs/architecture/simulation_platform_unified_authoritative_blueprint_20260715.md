...
>
> 2026-08-11 runtime blocking audit：生产 `ck_miniqmt_event_type/source` 仍停留在 P1-D allowlist，而 `ck_miniqmt_k2_event_composite` 已允许 KERNEL_V2，三者互相矛盾并使合法 `SESSION + EXCHANGE_SESSION_CLOCK` 每个 quote callback 都重复进入数据库读取、锁、失败 INSERT 和 rollback。BUG-1019 / Issue #3274 已登记为 P0；source repair、DEV successor migration 和自动退避验证进行中。后端由用户主动停止，`production_ddl_gate=pending`，source merge、生产 DDL、用户 restart、runtime identity 和正常交易日验收继续分别记录。
>
...
| `F-012` | §5.4 terminal contract；BUG-984 post-close state-first closure；BUG-992 lifecycle-incarnation authority与跨日终结 continuation | backend/tests/simulation_runtime/test_lifecycle_scheduler.py；post-close active durable state 必须先由既有 minute loop 消费 EOD；receipt-authoritative generation 之外仅同 run/plan/intent 的 terminal history 可审计保留；历史 `FAILED_RETRYABLE` 仅在 terminal states、terminal persistence、PROJECTED outbox及双 receipt/Paper readback全部闭合时自动终结，extra/non-terminal/corrupt authority typed fail loud | implemented_verified | none |
...
| `SIM-P-074` | `F-007,F-009,F-012,F-014,F-021` | BUG-992 first source slice closes LocalSIM lifecycle authority and economic readback with one pure contract over current run/binding/trade-date/plan identity, strict continuous receipt generations/high-watermark, exact receipt/outbox identity and latest committed `state_id -> state_hash` set。2026-08-08 用户重启后只读验证确认新 source identity 已加载，但生产 run `simrun_7bf1e0c1b6b7d055` 已被旧进程写成历史 `FAILED_RETRYABLE`；既有 stale/post-close 扫描只覆盖 active status 或 current trade date，使该合法、全量 PROJECTED、terminal persistence generation 永久掉出自动恢复集合。continuation 增加独立跨日 failed-run recovery：查询在 `LIMIT` 前按 `trade_date < scheduler_trade_date` 过滤；仅在 frozen run/release/binding/plan/account-slot identity、连续 economic authority、PROJECTED outbox、唯一 projection receipt、terminal states/persistence 与 Paper economic facts全部 strict readback 后复用既有 post-close terminal decision，并在同一 run update 写入 no-resubmit/no-broker-replay/no-projection-replay evidence。pending/retryable projection 不误终结；malformed outbox/failure carrier、identity/hash/generation/state/Paper drift typed fail loud且 per-run 隔离；重复 cadence 幂等。 | 初始 continuation RED：production-equivalent fresh scheduler 跨日后仍缺 `localsim_post_close_terminalization`；GREEN direct=`14 passed`，覆盖 active/failed terminal history、restart、idempotency、zero broker/order/fill/cash/projection replay、pending/missing/malformed outbox、active generation、authority corruption、run/release/binding/plan identity、failure-carrier drift及 query-before-limit；lifecycle file=`253 passed`（DEV PostgreSQL enabled）/`252 passed,1 gated skip`；changed-code coverage=`98.63% line (72/73),93.48% branch (43/46)`；final-main-sync `simulation_core_l2=480 passed,1 skipped`；classifier=`simulation_core_l2`、`unmapped_code_files=[]`；L0 blocking=`0`；registry=`8 passed,14/14 mapped`；F2=`112/112,warnings=0`。原 PR #3186 / merge `60261969efcba6fa760fcc29e83b4ac938f2ad2c` 保留为 first source slice；continuation PR #3205 / merge `152f1bca62929f0a4ca1433030957cbabeae8ddd` 已合入，source-state PR #3207 / merge `3e3153c8dac7074572abaaa56721a9ea88f16c7a` 已持久化 BUG metadata。 | BUG_992_CONTINUATION_SOURCE_MERGED_RUNTIME_PENDING_USER_RESTART | `first_source_merge=merged_pr_3186`；`continuation_source_merge=merged_pr_3205@152f1bca62929f0a4ca1433030957cbabeae8ddd`；`source_state_sync=merged_pr_3207@3e3153c8dac7074572abaaa56721a9ea88f16c7a`；`close_sync=pending_user_restart_runtime_identity_and_business_readback`；`post_restart_identity_gate=pending_for_continuation_source`；`post_restart_business_gate=pending_historical_failed_retryable_recovery`；`backend_restart_owner=user`；生产 run 未被本 continuation 修改；production DDL/DML/dependency/config/binding/broker/service-control 全部 `noop`；DEV-only disposable row测试沿用既有 guarded contract并精确清理 |
...
| `SIM-P-079` | `F-021,F-043,F-047,F-052,F-107,F-110,F-112` | BUG-1019 修复 P1-D allowlist 与 KERNEL_V2 composite 的生产合同冲突：单一 successor 同时闭合36 event types、14 sources、LEGACY=P1-D exact namespace及12个KERNEL_V2 exact composites，NULL/跨命名空间组合拒绝；六项完整CHECK同锁事务替换、frozen artifact/definition identity、fixed-search-path/C-collation helper + independent K2/K2-D catalog readback、full same-snapshot startup gate及拒绝全部K2/K2-D/K6 facts的guarded rollback/post-commit readback前后一致，合法LEGACY shared rows不误阻断rollback。运行期外部schema漂移由per-runtime/generation/per-operation single-flight自动退避保护，首次typed fail loud，60秒起步、3600秒封顶；每runtime持久callback actor、exact symbol/sink ownership、`ASYNC_IN_FLIGHT`与late-failure`RETRY_READY`避免per-tick线程、跨runtime广播及false-green；callback/watchdog/shared-supervisor/release reconciliation exact worker peer隔离，suppression显式、有界latest-per-symbol pending，异路径成功不清错，release/shutdown消费exact generation/token outcome并fence stale callback，阻塞owner不拖住健康runtime且同路径成功自动恢复；batch/pending/lifecycle drop均有有界reason/count/identity；不新增legacy fallback、人工gate或过期tick replay | 初始RED=`4 failed`暴露缺successor authority/artifacts及CHECK failure callback重打；正式审核追加symbol cross-runtime广播、late async false-green、batch尾帧静默丢失、K2-D proconfig漏检、LEGACY rollback post-COMMIT误报、NULL三值漏口、forged helper、TOCTOU、异路径假恢复、release race/orphan lease、阻塞sink/supervisor/release peer starvation、跨generation worker result和silent drop/false STOPPED RED；当前direct activation=`63 passed`、quote ingress=`52 passed`、DEV repository=`19 passed`、event-contract exact DEV node=`1 passed`，完整路由验证和最终fresh-process/CI证据仍待冻结；Issue #3274 | BUG_1019_SOURCE_IMPLEMENTATION_IN_PROGRESS | `source_merge=not_committed`；`production_ddl_gate=pending`；DEV DDL仅disposable schema；生产DDL/DML未执行；`backend_restart_owner=user`且后端当前停止；runtime identity/activation/正常交易日=`pending_source_merge_ddl_user_restart` |
...
| Control | Review result | Implementation evidence |
| --- | --- | --- |
| `no_simplified_delivery` | in progress | 修复同时覆盖六项生产 CHECK 的 exact successor、read-only preflight、atomic forward、guarded rollback、application/repository readback、K2-D helper configuration、真实 projection→KERNEL_V2 runtime seam、exact symbol ownership与callback/watchdog single-flight；不得以只扩大一项 allowlist、mock-only exception、全symbol广播或停订阅冒充完成；最终 changed-file路由与CI证据未冻结前不升级为pass |
| `no_silent_error` | in progress | 首次真实 SQLSTATE/constraint failure 保留 typed cause/context；未到期 suppression 进入 activation `DEGRADED` health 与计数而不访问 DB；到期重试失败再次响亮，成功自动清除 active 并保留 last failure；非 schema 晚失败保持`RETRY_READY`而非假`HEALTHY`，batch/pending尾帧逐一记录drop identity且不生成假ACK；最终fresh-process与CI证据未冻结前不升级为pass |
...
| production state separation | pass | 当前后端由用户停止；source尚未提交/合入，production DDL=`pending`且未执行，production DML/dependency/config/binding/broker=`noop`；后续生产 DDL、用户 restart、runtime identity 与交易日验证分别取证 |
...
| Control | Review result | Implementation evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass after continuation review; prior first-slice runtime-effective claim withdrawn | Shared closure仍验证 current durable identity、全部 receipt generation、high-watermark、exact state authority、empty-authority carrier absence和terminal-history规则；continuation不是直接把 `FAILED_RETRYABLE` 加入成功列表，而是从 public scheduler cadence 重新执行 run/release/binding/plan/account-slot、economic receipt、PROJECTED outbox、projection receipt、state hash及Paper facts完整 readback。PostgreSQL证据仍只使用 committed DEV rows和独立真实 repository connections。 |
| `no_silent_error` | pass after continuation review | Malformed outbox、failure carrier、run/plan/release/binding identity、receipt/state hash及Paper readback均返回稳定 typed reason/context；pending projection明确保持 `FAILED_RETRYABLE`，corrupt run返回 `RECOVERY_FAILED`且不被终结。没有 catch-and-pass、默认成功、空 authority fallback、failure清除后假成功或格式转换掩盖。 |
| `no_business_semantic_drift` | pass | Frozen Selection/plan intents、strategy package/assets、side、quantity、algorithm、causal minute authority、broker route、CAS/single-writer/transaction/outbox/projection和既有 post-close terminal decision 均未改变。continuation不重建plan、不重提parent、不调用broker、不重放order/fill/cash/projection；合法terminal history继续审计保留，pending/active generation不被提前终结。 |
| `no_unrequested_gate_or_approval` | pass | 新逻辑是既有scheduler自动cadence的技术恢复，不要求RBAC、审批、acknowledge、confirm、人工恢复或新增业务开关；exact durable readback是原restart contract，不是策略包/选股/交易 admission gate。 |
| `production state separation` | pass | 首次source merge、用户restart、runtime identity和business run recovery分开记录：首个source已加载，但目标历史run未恢复，因此close-sync未执行。continuation仅修改source/test/BUG metadata/唯一蓝图；DEV PostgreSQL测试使用disposable rows，production DDL/DML/dependency/config/binding/broker/service-control保持`noop`，生产run未人工修改。 |
