...
| `F-142` | §5.12、§11；legacy-retirement detailed §4.5.1、§4.6.4、§4.8、§4.8.1、`SIM-LR-C`；PR #4035 / `f2eb8ed03`；BUG-1291/1293 | C baseline direct=`30 passed`、Validation Center=`795 passed,2 skipped`；BUG-1291 PR #4099 / `5214bd03a` 已闭合daily authority；BUG-1293 target `backend/tests/simulation_runtime/test_localsim_cutover_inventory.py` 覆盖`fills.run_id -> run.portfolio_id`归属、原始fill稳定哈希及零silent fallback | sim_lr_c_source_complete | explicitly approved longline remediation boundary：production B2/C schema已应用且强readback通过；lineage DML因BUG-1293在写入前fail closed，修复合入与重新冻结source/date-bound authority前保持pending；用户重启与runtime evidence仍严格后置 |
