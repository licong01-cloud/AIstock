...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1-A 已通过 PR #2637 / `e69e72db` 实现验证并合入，K1-B registry/current-three manifests 已通过 PR #2655 实现验证且 source merge 待用户授权，K1-C 仍未开始；K1 overall 仍为 in progress，产品 runtime 仍未切换，未执行 DDL/DML、未修改配置、未调用 broker、未重启服务。该架构重构不得被设置为当前模拟盘运行的人工审批或额外 stop gate；按 K0-K6 可独立审核切片实施，并与现有阻断 BUG、生产 rollout 和运行观测分别记录。
...
| `F-053` | §9 P0-H K1；K1-B shadow manifest import boundary | `backend/tests/miniqmt_execution_runtime/test_miniqmt_vnpy_algo_import_boundary.py`；classifier mapped 8/8、unmapped/ambiguous=0 | design_ready | none |
...
| `F-055` | §9 P0-H K1；`backend/services/miniqmt_execution_runtime/plugin_registry.py` immutable catalog、binding、aggregate、supplied receipt 与 route isolation | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py`；K1-B direct 34 passed；PR #2655 required CI green | implemented_verified | none |
...
| `F-057` | §9 P0-H K1；`backend/execution_algos/vnpy_style/plugin_manifests.py` current-three exact schema/source/state/TWAP/legacy projection；shadow-only | `backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py`；K1-B direct 34 passed；PR #2655 required CI green | implemented_verified | none |
...
| `F-059` | §9 P0-H K1；ownership/test routing/coverage | `python -m nox -s l0`；`python -m nox -s miniqmt_execution_runtime_l2` = 508 passed/1 skipped；coverage 88.90%/71.79% | design_ready | none |
...
| `SIM-P-059` | `F-055,F-057,F-059,F-060` | K1-B 已实现 route-independent immutable catalog、serializable descriptor/process binding、exact creation/restore PluginKey、bounded aggregate failure、supplied pinned receipt component closure、per-route static isolation、current-three exact manifests/strict config-state/source-behavior closure与 observation-only legacy projection；保持 shadow-only，未修改产品 runtime wiring | `plugin_registry.py`、`plugin_manifests.py`；RED collection 2 errors；GREEN direct 34 passed；coverage statement/branch 88.90%/71.79%；classifier 8/8 mapped、unmapped/ambiguous=0；L0/registry PASS；module L2 508 passed/1 skipped；F2 validators 8/8、18/18、60/60；PR #2655 required CI green | K1_B_IMPLEMENTED_VERIFIED | `source_merge=pending_user_authorization`；`close_sync=not_applicable_feature`；K1-C/K2/K3 not started；production DDL/DML/dependency/config/binding/broker/restart/runtime 全部 `noop` |
