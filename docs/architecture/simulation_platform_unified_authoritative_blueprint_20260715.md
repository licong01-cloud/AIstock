...
| Control | Review result | Implementation evidence |
| --- | --- | --- |
| `no_simplified_delivery` | pass for BUG-794/795/796 scope；platform remains open | 首分钟 durable recovery、SELL child sizing 与 mark/economic 解耦均有产品实现和正反 direct tests；BUG-795 的 legacy、V24、V25 与 V25.1 non-final/final residual 分支均有直接 oracle，BUG-796 的 pending economic generation/readback/recovery 均有故障注入；仅 runtime policy authority 继续保留为 `SIM-P-052 REPAIR_REQUIRED`，不把已修 slice 冒充 LocalSIM 全面完成 |
