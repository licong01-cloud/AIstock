...
| `F-124` | §0.4、§9 P0-I；`backend/services/trading_core/execution_algo_retirement.py`、registry/capability、QE、StrategyPackage、Paper v2、Simulation、catalog/API/MCP、frontend projection 与启动型 CLI | `python -m pytest backend/tests/trading_core/test_v25_execution_retirement.py backend/tests/quantevolver/test_v25_execution_retirement.py backend/tests/strategy_package/test_v25_strategy_package_retirement.py backend/tests/paper_trading_v2/test_v25_paper_retirement.py backend/tests/mcp/test_v25_mcp_retirement.py -q`=`47 passed`；扩展受影响契约=`313 passed,38 skipped`；`backend/tests/strategy_package/test_v25_catalog_retirement_postgres.py` DEV catalog=`2 passed` | implemented_verified_dev_ready_for_pr | none |
...
| `SIM-P-082` | `F-011,F-122..F-125` | LocalSIM current/legacy TWAP-only 与 MiniQMT pre-broker reject 保持；repository slice 新增唯一 immutable retired set/reason/pure validator，移除 active registry/capability V25 construction，并在 QE config/runtime、StrategyPackage create/promotion/activation、Paper replay/live/readiness、Simulation bridge、catalog/API/MCP、frontend 和启动型 CLI 前置拒绝。历史类/模板/配置/run/model/report 保持只读，semantic matcher 不扫描股票池、universe、label 或普通文本。 | BUG-1096 runtime evidence；repository direct=`47 passed`；扩展受影响契约=`313 passed,38 skipped`；DEV catalog migration=`2 passed`；core changed-file Ruff 与 diff-check=`passed`；retention inventory=`docs/analysis/v25_execution_retirement_inventory_20260816.md` | V25_REPOSITORY_RETIREMENT_IMPLEMENTED_VERIFIED_DEV_READY_FOR_PR | source PR/CI/merge pending；production catalog DML 需独立授权；合入后用户重启与 runtime readback pending；物理 cleanup 未授权；frontend typecheck 由 CI 执行 |
