...
>
> 2026-08-03 final-architecture-only revision=`design_revision_ready_local`、`source_merge=pending_pr`：后续所有MiniQMT execution-kernel研发预算只投入最终KERNEL_V2隔离架构。legacy产品代码冻结，禁止C2、旧路线修复/backport、legacy parity、compatibility bridge、translator、双写和额外shadow product route；旧实例只使用当前版本自然drain，K6-D必须删除旧产品caller并完成唯一route与正常交易日验收。
>
...
当前状态为 `IMPLEMENTATION_IN_PROGRESS`：K1/K2/K3/K4/K5 overall均已`implemented_verified + merged`。K6 base design=`design_ready + merged`（PR #2993），K6-A=`implemented_verified + merged`（PR #3004），revision=`design_revision_ready + merged`（PR #3024），K6-C0=`implemented_verified + merged`（PR #3032，merge `2a3622a3ba63585e3dfe12ef7ccb3f33b00dcb63`）。BUG-953=`implemented_verified + merged + runtime_verified`（PR #3048，merge `f4da00f6838f6da6344223f6bba55dfe606def3e`），Issue #3045已关闭；K6-C1=`implemented_verified + merged`、`source_merge=merged_pr_3080`（merge `c0ca2e7e356a891ee5b2aad270d391071c942ae4`）。final-architecture-only revision=`design_revision_ready_local`、`source_merge=pending_pr`；K6源码切片A/C0/C1/B/D=`3/5`（60%），严格完成验收项F-102/F-104/F-105/F-106=`4/12`，F-110=`design_ready`且只保留C1部分证据，runtime cutover=`0%`。后续研发预算100%归属最终KERNEL_V2的K6-B/K6-D，legacy/C2/parity/bridge/backport/额外shadow预算=0。产品runtime未切换；production DDL=`applied_and_verified`，其余production config/binding/broker/runtime activation gates均为`noop/not_run`。
...
| `F-111` | §9 P0-H K6 detailed §12–§13 | artifact: 三份权威设计的final-architecture-only/no-legacy-R&D约束；后续K6-B/K6-D PR receipts与分离的production/runtime states | design_revision_ready_local | none |
...
| `SIM-P-069` | `F-101..112` | K6 base/K6-A/C0/BUG-953保持既有merged/runtime schema状态。K6-C1已实现strict `ProductCommandAuthorityEnvelopeV3`、唯一pure evaluator、0/1/N与MIXED aggregate、MATERIALIZE/REJECT/DEFER atomic K2 lineage、typed pre-call no-broker reject、same-authority advisory-lock幂等、rollback/commit-unknown和fresh independent readback；正式复审闭合malformed public carrier typed failure、terminal MATERIALIZE physical mapping projection、DEFER waiting/block/EOD/released-outbox lifecycle、immutable initial lineage、strict CAS/SHA及coordination successor UTC hash。2026-08-03 revision冻结legacy并将后续研发预算全部限定到最终KERNEL_V2：禁止C2、legacy修复/backport、parity、bridge、translator、双写和额外shadow；K6-B直接实现最终coordinator/invocation，K6-D删除旧caller并完成唯一route/正常交易日验收 | C1 contracts/authority/DEV PostgreSQL=`105 passed`，其中DEV=`20 passed`；coverage authority/contracts/repository=`84.21/76.47`、`90.52/73.11`、`86.63/70.37`；production只读K6-C0 preflight=`8/8 true`、authority总行/V3行=`0/0`；required CI run `30804132306` green；PR #3080 / merge `c0ca2e7e356a891ee5b2aad270d391071c942ae4` | K6_FINAL_ARCHITECTURE_REVISION_READY_LOCAL | K6-C1 `source_merge=merged_pr_3080`；revision `source_merge=pending_pr`；K6-B/D=`not_started`；source slices=`3/5`、strict acceptance=`4/12`、runtime cutover=`0%`；product runtime未切换；production DDL保持既有`applied_and_verified`，本轮production DML/config/binding/broker/restart/runtime activation/normal-day=`noop/not_run` |
