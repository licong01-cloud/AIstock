...
| Phase 4 UI/历史操作面 | `LOOP_ARCHIVE_UI_IMPLEMENTED_VERIFIED` | `LongTrendEvaluationPanel.tsx`、`LongTrendComparisonPanel.tsx`、专属 API client；DB 恢复、唯一 POST、五期限/三 barrier、episode/execution/sector/artifact 展示；同 snapshot 跨 run/model/seed/factor-set 对比；后端定向 `47 passed`、tsc PASS、mock Playwright `2 passed`、当前 8001 live Archive 只读 smoke `1 passed` | 任务创建页不可变 profile 开关、独立历史输入可用性预览仍待后续源码；历史评价执行和其余 R8B 物化仍需单独授权 |
...
审计结论更新为 `SINGLE_CANARY_DATA_PATH_VERIFIED_PHASE4_TO_5_PENDING`。这表示 Phase 3 的一个真实 existing-CAS 已闭合 DB/API/MCP 路径和 exact replay；它不表示其余 R8B、UI、历史批量补算或中断恢复已经完成，也不是科研审批、方向门禁或未来生产 DDL 授权。
...
| API/MCP/UI 可控输出 | `READY` | keyset pagination、limit <= 100、默认 compact、不内联 Parquet；UI 单一幂等操作并从 DB 恢复进度 |
...
| 验证归属 | `READY_AFTER_DESIGN_CLARIFICATION` | Phase 3 同步扩展 file ownership、module registry、test plan 和 route/schema regression；不得让新增文件游离于验证中心 |
...
7. `[COMPLETED_UI_SLICE]` Phase 4 Loop/Archive UI、DB 恢复、唯一幂等操作、同 vintage 对比与专属 mock/live 只读验证已实现；
8. `[NEXT_SOURCE_DELIVERY]` 任务创建页不可变 profile 开关、历史输入可用性预览，以及 Phase 5 重启、重复 callback、CAS-DB 中断恢复、非 QE 零影响 E2E；
...
| QE result schema/API/MCP/UI | Phase 3 source/schema/runtime、单 canary 数据链与 Phase 4 Loop/Archive UI slice 已验证；Phase 4 余项和 Phase 5 扩展 E2E 待实施 | R8B Loop4 已有 2,004 metric + 6 artifact；detail/quality API、bounded MCP 与 Archive live UI 非空；其余 5 个 R8B、创建页 profile、历史输入预览和恢复 E2E 继续作为独立 `platform_delivery_status`，不控制科研分析 |
...
本矩阵同时记录设计完整性与分阶段实施证据。`core_implemented_verified` 表示 Phase 1 纯计算能力，`runtime_canary_verified` 表示 Phase 2 已有真实运行证据，`single_canary_materialization_verified` 表示 Phase 3 已对一个真实 existing-CAS 闭合 DB/API/MCP 与 exact replay，`phase4_ui_contract_verified` 表示 Loop/Archive UI slice 已闭合源码、mock flow 与 live Archive 只读回读。不得把单 canary 或 UI slice 包装成全量 R8B、Phase 4 全部历史操作面或 F-014 平台整体完成。
...
| F-017 | `LongTrendEvaluationPanel.tsx`、`LongTrendComparisonPanel.tsx`、`LoopDetailPanel.tsx`、`qe-archive/api.ts` | `frontend/tests/quantevolver/qe_long_trend_evaluation.spec.ts`：single action body、DB progress recovery、同 vintage run/model/seed/factor 对比；mock `2 passed` + 当前 8001 live Archive 只读 `1 passed`；tsc PASS | phase4_ui_contract_verified | none |
...
| F-020 | full delivery controls | `nox -s qe_long_trend_phase3_platform` 为 `102 passed`；`nox -s qe_long_trend_phase4_ui` 为 mock `2 passed, 1 skipped`；当前 8001 Archive live 只读 smoke `1 passed`；TypeScript `tsc --noEmit` PASS；`python scripts/aistock_feature_workflow.py validate --design docs/architecture/qe_long_trend_evaluation_f2_design_20260714.md --tier F2`；单 canary 数据链已验证 | phase4_ui_contract_verified | none |
| F-021 | QE-only ownership/import/runtime isolation | `backend/tests/test_validation_catalog_integrity.py`、`backend/tests/test_validation_module_ownership.py`；本次只写 `qe_archive.run_evaluation_metric/artifact`，未触碰非 QE schema/runtime | single_canary_materialization_verified | none |
...
- [x] `no_simplified_delivery`：Phase 1/2 已完整交付；Phase 3 仅报告一个真实 canary；Phase 4 仅报告 Loop/Archive UI slice，未把任务创建 profile、历史输入预览、其余 5 个 R8B 或 Phase 5 E2E 冒充完成。
...
- [x] Phase 4 Loop/Archive UI 已实现唯一幂等入口、DB 状态恢复、family/platform/metric/artifact 展示和同 outcome-vintage 比较；live 验证仅只读 Archive 回读，没有执行评价 POST。
