...
- 版本：v1.17
- 日期：2026-07-15；Phase 2 运行态核验：2026-07-23；Phase 3 源码实现与本地验证：2026-07-28；Phase 3 单 canary 物化核验：2026-07-29；Phase 4 Loop/Archive UI 源码与只读 live smoke：2026-07-29；PR #2875 第三次正式审核、合入及用户重启后运行态回读：2026-07-29；Phase 4 follow-up 正式审核、PR #2906 合入与 root sync、PR #2935 post-merge 状态同步、生产 DDL 执行及回读：2026-07-30
...
### 12.2.1 v1.17 任务 profile、历史输入预检与生产 DDL 进度（2026-07-30）
...
当前 `source_merge=merged_bba48911342a`，`post_merge_docs_sync=merged_005e0e8a3ef3`，`root_sync=verified_contains_merges`，`dev_migration_gate=verified_zero_residue`，`production_ddl_gate=applied_and_verified`，`production_dml_gate=noop`，`runtime_activation=pending_user_authorized_restart`，`historical_evaluation_execution=not_run`。这些是平台交付状态，不是科研许可或方向门禁。
...
| QE result schema/API/MCP/UI | Phase 3 source/schema/runtime 与单 canary 数据链已验证；PR #2875 Loop/Archive UI slice 已合入并完成后端运行态 API 回读；PR #2906 task profile/input preview source+CI 已合入，task-profile 生产 DDL 已应用并验证，运行态/live 验证待补 | R8B Loop4 已有 2,004 metric + 6 artifact；生产 `aistock` 的 `long_trend_profile_id` 已通过重连 readback，但服务重启、real Recorder preview 与 live browser 未执行。其余 5 个 R8B 和恢复 E2E 继续作为独立 `platform_delivery_status` |
...
本矩阵同时记录设计完整性与分阶段实施证据。`core_implemented_verified`、`runtime_canary_verified`、`single_canary_materialization_verified` 分别表示 Phase 1、Phase 2 与 Phase 3 单 canary 证据；`phase4_ui_slice_source_ci_verified` 只表示 PR #2875 的 Loop/Archive 合同、受控 runner 与源码 CI 闭合，不表示运行态激活、Phase 4 全部历史操作面或 F-014 平台整体完成。`phase4_task_profile_source_ci_merged_prod_ddl_verified` 与 `phase4_task_profile_preview_source_ci_merged_prod_ddl_verified` 只增加 task-profile 生产 schema apply/readback 事实，不表示 backend 已重启、profile 已在新 Loop 生效、真实 Recorder preview 或 live UI 已验证。
...
- [ ] 运行时激活、real Recorder preview 与 live browser 尚未完成；这些状态不得由 source merge、DEV/生产 DDL、本地单测或 mock Playwright 冒充。
