...
- v1.15 增量状态覆盖：任务创建不可变 profile 与独立历史输入预检已完成源码和本地 mock/单元验证；新增 migration 已在现有 `127.0.0.1:5433/aistock_dev` 完成 forward/preflight/comment/readback/二次幂等 apply/guarded rollback/零残留验证。上一行末尾“仍待”的表述保留为 v1.14/PR #2875 历史快照。当前 PR、生产 DDL、运行时激活、live browser、其余 5 个 R8B 与 Phase 5 均仍待独立完成。
...
| local focused gates | `PASS` | 后端定向 `91 passed`；相关 Phase 2/config/snapshot 回归 `157 passed / 38 skipped` 后仅命中一个与本次 diff 无关、主线已存在的 payload-summary SQL 断言漂移；TypeScript 0 error；定向 ESLint 0 warning/error；mock Playwright `3 passed / 1 live skipped`；F2 `24/24`、catalog `0 findings`、module registry `14/14 mapped`、changed-only L0 `blocking=0` | PR CI 与 live browser 待后续 |
...
| QE result schema/API/MCP/UI | Phase 3 source/schema/runtime 与单 canary 数据链已验证；PR #2875 Loop/Archive UI slice source/CI 已通过并合入，用户重启后后端运行态 API 已验证，前端可视验收待补 | R8B Loop4 已有 2,004 metric + 6 artifact；PR source 补齐 as-of、失败证据、coverage/censoring 与 server filters；当前 8001 OpenAPI 和固定 evaluation readback 已通过，旧 receipt evidence-level 查询为空符合 fail-closed 合同。其余 5 个 R8B、创建页 profile、历史输入预览、前端逐控件验收和恢复 E2E 继续作为独立 `platform_delivery_status` |
...
| F-020 | full delivery controls | `pytest -q backend/tests/unified_engine/test_qe_long_trend_phase4_task_profile.py backend/tests/unified_engine/test_qe_long_trend_phase3_api.py backend/tests/unified_engine/test_experiment_config.py backend/tests/unified_engine/test_backtest_executor.py`：91 passed；`nox -s qe_long_trend_phase4_ui`；`nox -s validation_catalog_integrity`；`nox -s validation_module_registry_l0` | phase4_task_profile_preview_local_verified | none |
...
| F-023 | staged delivery truth | 既有 family/platform 分列；`backend/tests/unified_engine/test_qe_long_trend_phase3_api.py` 验证 `preview_inputs` 独立列出 dataset/pred/label/params/position/report/indicator/order/trade 可用性、typed reason 与 data action，缺项不禁用创建入口 | phase4_input_preview_source_local_verified | none |
...
- [x] `no_silent_error / v1.15 source slice`：非法 profile 在任何 task/stock-pool 写前返回 `QELT_PROFILE_INVALID`；task profile 变化明确拒绝；snapshot/Recorder/catalog/artifact 缺口逐项返回 reason/data action，既不猜路径/频率也不返回伪 ready。
...
- [ ] 本轮 PR、生产 DDL 授权/执行/readback、运行时激活与 live browser 尚未完成；这些状态不得由 DEV 门禁、本地单测或 mock Playwright 冒充。
