...
- v1.20 human-search correction：真实截图与 8001 只读聚合确认 Top-K 可用覆盖和人类检索缺口。Run、Task、outcome snapshot、L2 sector 的 canonical ID 继续作为后端身份，但只允许由业务选择器产生，操作员不能输入或复制 ID；查询选项按名称、日期、类型、模型、标签期限和 as-of 做 bounded 搜索。QE Archive/F-014 操作员视图不得显示 raw JSON、JSON 代码、原始 payload 或未翻译的技术枚举。Top-K 缺失展示覆盖范围、业务原因和恢复行动，不把 8 条历史缺失冒充全库 missing，也不自动回填或写 DB。只读 options API、业务选择器、Top-K 投影、Playwright 断言和 PR artifact 上传已完成源码实现；controlled Chromium 与当前 3000 runtime visual 均保持待执行状态。
...
| F-025 | bounded operator options + business search selectors；ID 仅作隐藏 value | `python -m pytest backend/tests/qe_archive/test_qe_archive_operator_options.py backend/tests/unified_engine/test_qe_long_trend_phase3_api.py -q -p no:cacheprovider` -> 21 passed；`frontend/tests/quantevolver/qe_long_trend_evaluation.spec.ts` 包含无自由文本 ID 断言 | source_and_api_tests_verified | controlled Chromium execution pending，用户明确批准由 PR pipeline 验收 |
| F-026 | QE Archive/F-014 business projection；禁止 raw JSON/技术枚举 | 定向 TypeScript/ESLint -> 0 error；`frontend/tests/quantevolver/qe_long_trend_evaluation.spec.ts` 包含无 `<pre>`/JSON payload 与中文业务组件断言 | source_static_gates_verified | controlled Chromium execution pending，用户明确批准由 PR pipeline 验收 |
| F-027 | Top-K coverage/status/error/action projection | `frontend/tests/quantevolver/qe_long_trend_evaluation.spec.ts` 断言不补造 source、不把首条 missing 冒充整体状态；8001 只读基线为前 100 条 92 ok / 8 missing / 92 joined nonzero | source_and_readonly_data_verified | success screenshot pending，用户明确批准由 PR pipeline 验收 |
| F-028 | controlled Chromium success screenshots + HTML artifact | `python -m nox -s qe_long_trend_phase4_ui`；`.github/workflows/test.yml` 上传 `frontend-ui-evidence-*`；测试显式写入 `tmp/playwright-results-f014-phase4/qe-archive-human-search.png` | pipeline_wired_execution_pending_approved_by_user | PR pipeline execution 与 artifact URL 待回填；live 3000 保持独立 |
...
- [x] `no_simplified_delivery / v1.20 human-search design`：同时覆盖 Run/Task/snapshot/sector 搜索、raw JSON 清除、Top-K 真实覆盖说明与流水线截图；不以只删除三个文本框的子集冒充完整可用性修订。
- [x] `no_silent_error / v1.20 human-search design`：保留 92 ok / 8 missing 的真实局部分布；缺失必须有业务原因和恢复行动，未知状态不直接显示技术值，也不填默认 source/0 值伪造结果。
- [x] `no_business_semantic_drift / v1.20 human-search design`：canonical ID、同-vintage、bounded query、Top-K 前向计算和历史回填策略保持不变；新增 options 全部只读且不新增 DDL/写入。
- [x] `no_unrequested_gate_or_approval / v1.20 human-search design`：选择器与缺失说明只改善操作面，不增加审批、RBAC、确认文本、研究准入或方向阻断。
