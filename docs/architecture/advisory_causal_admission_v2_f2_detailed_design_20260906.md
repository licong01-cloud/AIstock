...
| F-240 | §§3、7～12；`scripts/advisory_causal_admission_v2_mve.py` false gates | `python -m pytest backend/tests/advisory_model_first/test_causal_admission_v2_delivery.py -q`；`python scripts/aistock_feature_workflow.py validate --design docs/architecture/advisory_causal_admission_v2_f2_detailed_design_20260906.md --tier F2` | SOURCE_VERIFIED | approved_by_user: backend/DB/API/UI/runtime/DDL/restart均不变更 |
