...
> 状态修订日期：2026-07-20
> 代码状态：PR `#2481` 已合入，merge commit `6d400b40dec3be1d9a97c4bf361fc88d00b55af7`
> 发布状态：R1 DEV/production DDL 均未执行，runtime 未激活；R1 code acceptance 与数据库发布状态独立记录
...
| migration contract 与开发期 apply/exact reapply | `add_advisory_historical_range_phase1r_20260719.sql` | 历史 disposable PostgreSQL 16 apply/exact reapply；按最新 v1.5 仅作为开发证据，不替代现有 DEV plan/apply/verify/exact-reapply | implemented_r1_code_dev_release_pending |
...
| F-954 | release_pending_dev_validation_r1 | migration 未连接 DEV/production；数据库连接只允许显式 `.env`/AIstock pool。按最新 v1.5，首次 schema-backed DEV DML/E2E 前仍须在现有 DEV 完成 plan/apply/verify/exact-reapply，不得新建测试数据库替代。 |
...
- 历史 disposable PostgreSQL 16 开发证据：首次 migration PASS；携带真实数据 exact reapply PASS；13 张独立 Phase 1R 表可见。该证据不再作为 DEV-first release gate 的替代物。
...
```text
code_merge = merged_pr_2481_commit_6d400b40
dev_ddl = pending_existing_dev_plan_apply_verify_exact_reapply
production_ddl = pending_until_dev_verified_and_explicit_production_authorization
service_restart = not_required
runtime_activation = none
selection_paper_simulation_qe_side_effect = none
```

R1 的 code acceptance 已闭合，但 Phase 1R 尚不具备数据库发布或运行激活状态。下一实现批次为 R2；R2 不新增 migration、表或独立 DML 脚本，只完成中立共享计算、现有 Selection wrapper parity、admitted projection 和单/原生多 Alpha 历史日 candidate adapter，并通过 R1 repository 承接后续 DEV candidate 持久化。
