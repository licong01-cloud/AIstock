...
- source/DDL/config/binding/restart/runtime/normal-day evidence分开记录。剩余顺序固定为`K6-C1 source merge -> K6-B -> K6-D`；K6-C1未合入前不得宣称K6-B完整或并行修改共享蓝图；总工期按详细设计修订后的范围记录，不能用观察时间或工期压缩验收范围。
...
| `F-110` | K6 detailed design §11 | `backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py`、`test_kernel_product_authority.py`、`test_kernel_product_materialization_postgres.py` direct=`105 passed`；authority/contracts/repository line/branch=`84.21/76.47`、`90.52/73.11`、`86.63/70.37`；最终MiniQMT L2由changed-files classifier及PR CI在最终HEAD闭合 | k6c1_implemented_verified_local | none |
