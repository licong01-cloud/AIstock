...
>
> K1 下位详细设计：[`miniqmt_execution_kernel_k1_contracts_registry_f2_detailed_design_20260722.md`](miniqmt_execution_kernel_k1_contracts_registry_f2_detailed_design_20260722.md) 当前为 `implementation_verified`；K1-A/B/C均为`implemented_verified + merged`。K2-A、K2-A-M1、K2-B和K2-C均已`implemented_verified + merged`。K2-C final source `c87748cd...` 已通过 PR #2794 / merge `801dc3c9...`及required CI run `30235878200`闭合，`source_merge=merged_pr_2794`；K2-D与K3/K4 `not_started`。现有产品runtime未切换，production/runtime gates均为`noop`。
>
> K2 下位详细设计：[`miniqmt_execution_kernel_k2_durable_dispatch_f2_detailed_design_20260725.md`](miniqmt_execution_kernel_k2_durable_dispatch_f2_detailed_design_20260725.md) 当前为 `implementation_in_progress`；K2-A、K2-A-M1、K2-B与K2-C均为`implemented_verified + merged`。K2-C direct/import/structure=`122 passed`、DEV migration=`13 passed`、clock line/branch=`90.10%/80.85%`、MiniQMT L2=`789 passed,25 skipped`，final source `c87748cd...` 已通过 PR #2794 / merge `801dc3c9...`及required CI run `30235878200`闭合，`source_merge=merged_pr_2794`；K2-D=`not_started`。未启动常驻worker、未调用Gateway/broker、未执行生产DDL/DML，也未切换产品runtime。
>
> 日期：2026-07-22。

## 0. Executive Decision / 核心决策

K1-C remains `implemented_verified + merged` through PR #2685 / merge `e4faeb53663cb4d19eb4e07d833953725a40fdc1`; K1 overall is unchanged. K2 detailed design remains `implementation_in_progress`: K2-A, K2-A-M1, K2-B and K2-C are `implemented_verified + merged`. K2-C final source `c87748cd...` implements the strict ExchangeSessionClock, deterministic SESSION/TIMER/EOD ingress, durable atomic claim/finalize/reclaim, lunch/EOD/catch-up/restart closure and an additive guarded migration; PR #2794 / merge `801dc3c9...` and required CI run `30235878200` close `source_merge=merged_pr_2794`. K2-D and K3/K4 remain `not_started`, product runtime is not switched, and production gates remain `noop`.
