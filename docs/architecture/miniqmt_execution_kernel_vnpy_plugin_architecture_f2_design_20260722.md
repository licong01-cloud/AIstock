...
>
> K1 下位详细设计：[`miniqmt_execution_kernel_k1_contracts_registry_f2_detailed_design_20260722.md`](miniqmt_execution_kernel_k1_contracts_registry_f2_detailed_design_20260722.md) 当前为 `implementation_verified`；K1-A/B/C均为`implemented_verified + merged`。K2-A、K2-A-M1、K2-B、K2-C和K2-D均已`implemented_verified + merged`；K2-D final source `82c69fbf7e7245e0af76262ddc7b7f59ce7d996b` 通过 PR #2804 / merge `fc4170faa10847c0b58aa8088b4a8b6d0ca26b29` 合入，`source_merge=merged_pr_2804`。K3 overall=`implemented_verified + merged`。K4详细设计已通过 PR #2861 / merge `8250b64ff3c2deb04eb3594f1ae3fba3acd1e6ce` 合入；K4-A pure façade/adapter、strict contract与pinned source identity slice已通过 PR #2883 / merge `527b2f4a58d3fe84f85c0b1f4ba2fe375d181dda` 完成`implemented_verified_contract_slice + merged`，raw observation不能生成PASSED/binding/conformance；K4 overall=`implementation_in_progress`，K4-B实施级补充设计已通过 PR #2914 / merge `15c38cb8d3a2c3a1710a29526ac6ac07ef580238` 合入并保持`design_ready_pending_implementation`，K5/K6=`not_started`；现有产品runtime未切换，production/runtime gates均为`noop`。
>
...
K1、K2、K3 overall均保持`implemented_verified + merged`。K4详细设计已通过PR #2861合入；K4-A pure façade/adapter、strict contracts、source identity与fail-closed observation boundary现为`implemented_verified_contract_slice + merged`（PR #2883 / merge `527b2f4a...`），K4 overall=`implementation_in_progress`；K4-B已闭合V2 source executor/positive conformance/K2 integration实施级设计并通过PR #2914 / merge `15c38cb8d3a2c3a1710a29526ac6ac07ef580238`合入，但代码尚未开始，K5/K6=`not_started`。产品runtime未切换，production gates保持`noop`。
...
K4 current catalog没有façade-backed descriptor：current-three保持K3 pure binding，Iceberg/Stop归K5。K4可用production constructors构建strict test-only closed candidate覆盖adapter seam，但不得写入catalog/creation binding/DB或把它当成registered runtime evidence；K4完成还必须由真实current-three conformance、pinned Iceberg/Stop characterization及DEV read-only证据共同闭合。K5交付首个真实registered shadow adapter，仍不修改kernel。
...
- K1 overall 当前为 `implemented_verified + merged`；K2 detailed design 为 `implementation_verified`，K2-A/K2-A-M1/K2-B/K2-C/K2-D 均为 `implemented_verified + merged`。K2-D final source `82c69fbf...`已通过 PR #2804 / merge `fc4170fa...`闭合为`source_merge=merged_pr_2804`；K3 overall已通过PR #2848完成`implemented_verified + merged`；K4-A pure façade/adapter、strict contracts、source identity与fail-closed observation boundary已`implemented_verified_contract_slice + merged`且`source_merge=merged_pr_2883`，K4-B exact source executor/integration=`design_ready_pending_implementation`且`design_source_merge=merged_pr_2914`、K4 overall=`implementation_in_progress`；`close_sync=not_applicable_feature`，产品 runtime switch、DDL/DML、配置和 broker 行为均未发生；
- 预计 1–2 PR，7–10 人日。
...
- 不引入第二 runtime；
- 实施级合同：[`miniqmt_execution_kernel_k4_vnpy_facade_f2_detailed_design_20260729.md`](miniqmt_execution_kernel_k4_vnpy_facade_f2_detailed_design_20260729.md)，design PR #2861 / merge `8250b64ff3c2deb04eb3594f1ae3fba3acd1e6ce`；K4-A=`implemented_verified_contract_slice + merged`（PR #2883 / merge `527b2f4a...`），K4-B exact pinned-source executor/positive conformance/integration=`design_ready_pending_implementation`且设计已通过PR #2914 / merge `15c38cb8...`合入，K4 overall=`implementation_in_progress`；
...
- K4-B补充设计固定V1永久observation-only、V2 full executable input/full actual trace/source-execution set/characterization/conformance authority；以sealed process authority接入existing K2 optional invocation，以precomputed existing transition identity修复CLAIMED pre-call事实，并在existing locked transaction按delivery/event双cutoff读取ALGO_START与latest eligible B0 TICK。current-three expected trace必须由live K3 factory重放，Iceberg/Stop仍只characterization；无新表/迁移、无第二factory、无产品route切换；
- 预计 1–2 PR，9–14 人日。

### K5：Iceberg/Stop 扩展性验收

- 只新增Iceberg/Stop plugin/manifest/algorithm binding/state codec/tests，复用K4通用adapter和K2 shadow invocation seam，不修改kernel；产品activation与generic per-command authority仍由K6拥有；
- 验证不同 timer/tick/order lifecycle；
- 预计 1–2 PR，6–9 人日。

### K6：旧 helper 退役、生产迁移与真实 SIM

- 先以独立F2详细设计和代码实现durable dependent-BUY coordinator：qmt_strategy ledger cash authority、SELL TRADE/ORDER terminal/ACCOUNT/EOD lineage、defer/release/block/residual、single-writer/CAS/fence、restart/no-double-release和release后正常K2 outbox；不得下沉进算法plugin；
- 在任何façade-backed产品binding激活前，以同一K6 F2切片或其明确子切片关闭generic per-command authority aggregate、writer/readback、execution projection hash、materializer和restart/reconcile；不得按algo code分支，也不得复用单command receipt冒充多command closure；
...
- dependent-BUY coordinator设计/实现预计1–2 PR、6–10人日；route retirement/生产迁移和正常交易日观察另计，不能把观察时间折算成代码完成。
...
| `F-081` | K4 detailed design §0–§2 | `backend/tests/miniqmt_execution_runtime/test_vnpy_k4_scope_boundaries.py` | design_ready | none |
| `F-082` | K4 detailed design §3、§12.1 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_source_execution_v2.py` planned source/fresh readback matrix | design_ready | none |
| `F-083` | K4 detailed design §4–§5 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_kernel_invocation.py` planned CLAIMED/transition/ordinary path matrix | design_ready | none |
| `F-084` | K4 detailed design §5.4.1、§6 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_source_execution_v2.py` planned actual method/callback trace | design_ready | none |
| `F-085` | K4 detailed design §6.3–§8、§15 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_repository_postgres.py` planned same-transaction/zero-write matrix | design_ready | none |
| `F-086` | K4 detailed design §5.4.1、§5.6、§9–§10、§15 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_kernel_invocation.py` planned single/multi-command/restart matrix | design_ready | none |
| `F-087` | K4 detailed design §4.1、§5.1、§11–§12 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_conformance_authority_v2.py` planned sealed V2 authority matrix | design_ready | none |
| `F-088` | K4 detailed design §3.2、§6.3、§10、§12 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_source_execution_v2.py` planned current-three/Iceberg/Stop matrix | design_ready | none |
| `F-089` | K4 detailed design §13–§17 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_failure_restart.py` planned failure/restart/diagnostics matrix | design_ready | none |
| `F-090` | K4 detailed design §18–§20 | artifact: `docs/architecture/miniqmt_execution_kernel_k4_vnpy_facade_f2_detailed_design_20260729.md` | design_ready | none |

## 19. DESIGN-COMPLIANCE-001 / 设计复核

| control | result | evidence |
| --- | --- | --- |
| no simplified delivery | pass | K3-B证据继续来自committed facts/DEV；K4-B设计要求V2 full executable input/full actual trace/fresh source readback、sealed authority、same-transaction repository read与DEV zero-write；多command trace不得截断、V1 observation不能冒充PASSED，mock-only只计辅助测试，无placeholder、partial或第二route |
| no silent error | pass | K3 strict callback/outcome保持；K4 V1永不PASSED，spawn/trace/receipt/claimed-delivery/repository drift fail loud；not-running/missing/rounded-zero/TICK unavailable分别产生typed diagnostic，pure-shadow或K2-V1-shadow receipt不能冒充product command authority PASSED |
| no business semantic drift | pass | K3 predecessor/TWAP/cancel语义保持；K4 Iceberg TIMER消费durable sequence-cutoff B0 TICK、active-child clean前不伪造terminal、config不擅自收紧source semantics；signal/target/方向数量/B0/OMS/Gateway不变 |
...
| production state separation | pass | K2/K3 overall=`implemented_verified + merged`；K4 design PR #2861已合入，K4-A PR #2883=`implemented_verified_contract_slice + merged`，K4-B=`design_ready_pending_implementation`且`design_source_merge=merged_pr_2914`（merge `15c38cb8d3a2c3a1710a29526ac6ac07ef580238`）；K5/K6未开始，产品runtime未切换，DDL/DML/dependency/config/binding/broker/restart/runtime activation全部为`noop` |
