...
- **K6-C0已实现、K6-C1为当前第一优先级**：C0已以successor migration/contract补齐strict command_json、dependent-BUY V2 proceeds/ledger/coordination、K6 product-owned `ProductCommandChildMappingV1` 的 `DEFERRED_DEPENDENT_BUY -> RESERVED|TERMINAL` lifecycle，以及migration前后两个exact K2/K6 catalog与独立K6-C0 readback；该carrier复用同一physical mapping row/identity但不扩大K1/K3共享mapping enum或initial-state authority。C1再实现generic per-command V3 authority aggregate的atomic materializer、0/1/N MATERIALIZE/REJECT/DEFER、terminal no-broker reject outcome和restart/reconcile；K6-B才实现product RESERVED到既有K2 dispatch lifecycle的显式cross-carrier handoff。product root明确拒绝K4/K5 V1 shadow receipt、K6-A dependent-BUY V1和hash-only product authority V2，不得按algo code分支或复用单command receipt冒充multi-command closure；
...
| `F-104` | K6 detailed §4.2、§8；V3 carriers已实现 | C0：`python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py -q`；C1 target `test_kernel_product_authority.py` | design_ready | none |
...
| `F-106` | K6 detailed §6；versioned repository preflight | C0：`AISTOCK_RUN_MINIQMT_K2_DEV_DB=1 python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_repository_postgres.py -q`；C1 target `test_kernel_dependent_buy_postgres.py` | design_ready | none |
...
| production state separation | pass | K2/K3/K4/K5 overall=`implemented_verified + merged`；K6 base design、K6-A与revision已合入；K6-C0=`implemented_verified`且`source_merge=pending_pr`，K6-C1/B/D=`not_started`且顺序为C1->B->D；产品runtime未切换，production DDL=`pending_after_source_merge`且未执行，其余gates/normal-day observation为`noop/not_run` |
