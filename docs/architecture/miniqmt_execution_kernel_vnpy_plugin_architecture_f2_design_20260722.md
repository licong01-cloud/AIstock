...
- code-owned plugin catalog 只根据 manifest descriptor、调用方显式 supplied 且无默认值的 pinned compatibility receipt 和显式 factory/config-validator/state-codec binding 构造，不允许 kernel 出现具体 algo 分支；process-local callable 不进入 canonical snapshot/hash；所有参与 manifest/catalog 构建的 code-owned characterization/source/algo/default/field table 在模块加载时递归冻结或 sealed，外部 mutation 不得改变 hash；K1-B 按 K1 下位详细设计 §9.2 的 source/method/object/surface component hash 公式只验证 receipt closure，K1-C 才读取 pinned surface 并生成 receipt，禁止固定 PASSED、默认 receipt 或 no-op validator；
...
- K1 overall 当前为 `in_progress`：K1-A 已通过 PR #2637 / `e69e72db` 实现验证并合入，K1-B 已通过 PR #2655 代码/测试/coverage/required CI 实现验证但 source merge 待用户授权；K1-C committed/source 尚未进入 main/PR、`source_merge=not_committed`，本地隔离 worktree 仅观察到 `C1=in_progress`、source-isolated import-boundary `implemented_verified_local`，compatibility requirement/locked surface/receipt 仍 `blocked_by_design_and_source_authority`，不得压缩成整体 not_started 或 implemented；K2/K3/K4 均 `not_started`，产品 runtime switch、DDL/DML、配置和 broker 行为均未发生；
- 预计 1–2 PR，7–10 人日。
...
| `F-055` | `backend/services/miniqmt_execution_runtime/plugin_registry.py` route-independent catalog、descriptor/process binding、exact creation/restore binding、canonical persisted readback、non-empty bounded aggregate、builder/readback 共用 supplied receipt component closure；gateway strict readback/B0/exact-cancel evidence 与 route receipt | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py` = 25 passed，覆盖 permutation/fresh-process/noncanonical/hash-correct component readback/malformed aggregate/truncation/empty receipt/stale gateway/B0/exact-cancel/route-isolation；K1-B-REVIEW-FIX RED 后 GREEN；PR #2655 required CI 以最终 HEAD 为准 | implemented_verified | none |
...
| `F-057` | `backend/execution_algos/vnpy_style/plugin_manifests.py` current-three exact AISTOCK_DERIVED schema/source/executable behavior trace/active-order parent/local/broker/command/decimal/quantity/native-market-data/order-trade lineage state/TWAP/legacy projection；code-owned build authority recursive immutable；schema evidence bounded；shadow-only、runtime switch 属于 K3 | `backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py` = 39 passed，覆盖 strict/state/source/active-order/TWAP/current-core/legacy matrix与 top-level/nested/table mutation、fresh-process hash、31/32/>32 schema evidence；K1-B-REVIEW-FIX RED 后 GREEN；PR #2655 required CI 以最终 HEAD 为准 | implemented_verified | none |
...
| `F-059` | K1 detailed design §11 ownership/test routing/coverage | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py` / `test_current_three_plugin_manifests.py` / `test_miniqmt_vnpy_algo_import_boundary.py` / `test_algo_plugin_contracts.py` = 25/39/2/60 passed；coverage line/branch 88.23%/74.26%；review-fix ownership 8/8 mapped；`python -m nox -s miniqmt_execution_runtime_l2` = 538 passed/1 skipped，`python -m nox -s paper_v2_backend` = 1048 passed/2 skipped/2 xfailed | implemented_verified | none |
| `F-060` | K1 detailed design §2、§10、§12-§13 rollout/rollback/state separation | `python -m nox -s l0`、`python -m nox -s validation_module_registry_l0`、三份 `python scripts/aistock_feature_workflow.py validate --design <design> --tier F2`、Feature Workflow pre-PR 与 DESIGN-COMPLIANCE-001 local PASS；production gates noop | design_ready | none |
