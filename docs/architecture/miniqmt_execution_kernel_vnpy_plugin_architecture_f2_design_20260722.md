...
- 当前 `design_ready`、K2-A source `implementation_in_progress`、K2-B/C/D `not_started`、shadow-only；首个strict carrier checkpoint已实现ingress/lineage/projection/risk/error writer-readback并有7个direct GREEN，migration/repository/mapping/outbox/timer/incarnation仍未完成；预计 4 PR，21–29 人日。
...
| `F-062` | K2 detailed design §4.0–§4.10、§9 exact domains、parent status、projection/mapping/incarnation/calendar carriers与additive schema | `backend/services/miniqmt_execution_runtime/plugin_contracts.py`首组strict carriers；`backend/tests/miniqmt_execution_runtime/test_kernel_contracts.py` RED ImportError→GREEN 7 passed | design_ready | none |
...
| no silent error | pass | transitive helper 的 DB、wall-clock/global-random、dynamic module 与 forbidden owner 在 root execution 前明确失败，不再产生假 PASSED；stale DISPATCHING、plugin exception、schema/hash/identity/capability 同样 typed fail loud，无默认成功或算法 fallback |
