...
| `F-079` | K3 detailed design §11–§12、§15 failure/diagnostics/rollback | `backend/tests/miniqmt_execution_runtime/test_current_three_kernel_restart.py`、`test_kernel_callback_events.py`、`test_kernel_outbox_outcome_ingress.py` | implemented_verified_k3a_scope | none |
| `F-080` | K3 detailed design §10.2、§14–§16 visible transport suppression、validation/routing/gates/K6 prerequisite | `python -m nox -s miniqmt_execution_runtime_l2`=`934/27`；`python -m nox -s paper_v2_backend`=`1050/2/2`；DEV=`1 passed`；核心coverage过80/70；`unmapped_code_files=[]` | implemented_verified_k3a_scope | none |
