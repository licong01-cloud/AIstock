...
| `F-102` | K6 detailed design §4.1；`kernel_product_contracts.py` | `backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py`；K6 direct/DEV aggregate=`34 passed`；contracts line/branch=`90.86%/72.50%` | implemented_verified_local | none |
...
| `F-105` | K6 detailed design §5；K6 migration triplet | `backend/tests/miniqmt_execution_runtime/test_kernel_k6_migration_postgres.py`；DEV PostgreSQL clean/reapply/catalog+function drift/readback/append-only/guarded rollback通过 | implemented_verified_local | none |
| `F-106` | K6 detailed design §6；`kernel_product_repository.py` | `backend/tests/miniqmt_execution_runtime/test_kernel_product_repository_postgres.py`；transaction/CAS/fence/idempotency/mixed lineage/commit-unknown/readback drift；repository line/branch=`87.17%/70.42%` | implemented_verified_local | none |
