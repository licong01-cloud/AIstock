...
>
> 2026-08-03 final-architecture-only revision=`design_revision_ready_local`、`source_merge=pending_pr`：legacy产品代码冻结，后续研发预算只允许投入最终KERNEL_V2的K6-B/K6-D；禁止K6-C2、legacy修复/backport、parity、compatibility bridge、translator、双写和额外shadow product route。旧实例只使用当前版本自然drain，K6-D必须删除旧产品caller并完成唯一route与正常交易日验收。
>
...
| `F-111` | K6 detailed design §12–§13 | artifact: 三份权威设计的final-architecture-only/no-legacy-R&D约束；后续K6-B/K6-D PR receipts和分离的production/runtime states | design_revision_ready_local | none |
...
| production state separation | pass | K2/K3/K4/K5 overall=`implemented_verified + merged`；K6-A/C0/C1均已合入，当前只剩B->D；final-architecture-only revision=`design_revision_ready_local`、source pending PR；产品runtime未切换，production DDL=`applied_and_verified`，其余config/binding/broker/runtime activation/normal-day observation为`noop/not_run` |
