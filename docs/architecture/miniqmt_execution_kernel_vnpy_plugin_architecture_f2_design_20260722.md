...
- K6-B与K6-D所有研发只面向最终隔离架构：K6-B直接实现最终KERNEL_V2 coordinator/invocation seam，不调用或修改legacy，不做parity、bridge、translator、shadow writer或双写；K6-D以durable route cutover receipt冻结new-instance唯一KERNEL_V2 owner，旧实例仅用冻结代码自然drain，退役同步timer for-loop、legacy adapter产品调用与dependent-BUY direct retry，完成static/runtime unique-route proof；禁止dual route、legacy fallback或K6 state回交legacy；
- K6-C0 predecessor production DDL/readback已在独立授权后完成；K6-B successor migration尚未执行生产DDL，source merge后必须单独授权并readback。K6-D后续config/binding与runtime activation仍须分别授权。服务重启由用户执行；正常交易日必须覆盖single/multi、上午/午休/下午/EOD和dependent-BUY/outbox/callback/reconcile闭合；
...
| `F-110` | K6 detailed design §11 | C1与K6-B partial evidence：`backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py`、`test_kernel_product_authority.py`、`test_kernel_product_materialization_postgres.py`、`test_kernel_dependent_buy.py`；K6-B required CI run `30844554277` green；最终route integration/uniqueness、旧调用删除、frozen-input invariants与正常交易日证据由K6-D闭合 | design_ready | none |
| `F-111` | K6 detailed design §12–§13 | artifact: 三份权威设计的final-architecture-only/no-legacy-R&D约束；PR #3098 / merge `b986e8f655137e9aea88f17ea91d3eb153f1c017`；K6-B PR #3120 / merge `48bcf1153dea3dca5c598793d6556d2757ed87de`；后续仅K6-D PR receipt和分离的production/runtime states | design_revision_ready + merged | none |
...
| production state separation | pass | K2/K3/K4/K5 overall=`implemented_verified + merged`；K6-A/C0/C1/B均已合入，K6-B=`source_merge=merged_pr_3120`，之后只剩K6-D；final-architecture-only revision=`design_revision_ready + merged`、`source_merge=merged_pr_3098`；产品runtime未切换，predecessor production DDL=`applied_and_verified`，K6-B successor DDL=`pending_separate_user_authorization_after_source_merge`，其余config/binding/broker/restart/runtime activation/normal-day observation为`noop/not_run` |
