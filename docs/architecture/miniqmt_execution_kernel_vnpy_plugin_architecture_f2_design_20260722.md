...
| production state separation | pass | K2/K3/K4/K5 overall=`implemented_verified + merged`；K6-A/C0/C1/B均已合入，K6-D code=`implemented_verified_local_pending_source_pr`而source PR/CI/merge尚未完成；design PR #3129已合入。产品runtime未切换，predecessor与K6-B successor production DDL=`applied_and_verified`，K6-D new DDL/DML/config/binding=`noop`，dependency gate=`noop`，broker/restart/runtime activation/normal-day observation=`not_run` |
