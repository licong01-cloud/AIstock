...
- **第二优先级 K6-B source closure**：当前=`implemented_verified_local`、`source_merge=pending_pr`。实现未另造event/transition/command，不下沉进算法plugin、不估算cash或直提broker；K3 inventory未作为candidate source、legacy parity、迁移oracle或验收输入；line/branch均达到`>=80/>=70`，classifier只选择MiniQMT。合入仍不自动执行K6-B production DDL或激活产品route；
