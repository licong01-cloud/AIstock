...
| production state separation | pass | K2/K3/K4/K5 overall=`implemented_verified + merged`；K6 base design、K6-A、revision与K6-C0均已合入；K6-C0=`implemented_verified + merged`且`source_merge=merged_pr_3032`，K6-C1/B/D=`not_started`且顺序为C1->B->D；产品runtime未切换，production DDL=`pending_after_source_merge`且未执行，其余gates/normal-day observation为`noop/not_run` |
