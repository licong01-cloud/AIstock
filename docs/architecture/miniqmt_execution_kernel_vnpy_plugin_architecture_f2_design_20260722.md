...
当前 K1-B 三个 binding 仍指向 K4 前的 vn.py-style core class，并不满足上述 pure plugin contract；K2-B 必须对此 fail-loud，不能自动包成 façade、不能调用 legacy product route，也不能把 fixture plugin 声称为 current-three parity。K2-B 交付 generic durable kernel 与真实 PostgreSQL transaction；K4 才用 pinned compatibility authority实现 current-three `ExecutionAlgoPluginV2` façade并更新 exact binding。该阶段边界不降低 K2-B 对 initialization/routing/delivery/failure/retry 原子性的验收范围。
...
- K1 overall 当前为 `implemented_verified + merged`：K1-A/B/C 均已实现验证并合入；K1-C PR #2685 的 dual-upstream authority、indirect/dynamic/transitive helper import boundary 与 malformed isolated carrier 已由 implementation `52e1c5a2`、final CI run `30119969033` 及 merge `e4faeb53663cb4d19eb4e07d833953725a40fdc1` 闭合，`source_merge=merged_pr_2685`；K2 detailed design `implementation_in_progress`，K2-A=`implemented_verified + merged`且`source_merge=merged_pr_2729`，最终 source HEAD `fc261aaf47a6fade01b1037efd5c8cb8ccda5235`、PR #2729、merge `0b46f7819f4147c97a36908e25ca948ce5450661`闭合；K2-A-M1=`implemented_verified + merged`、`source_merge=merged_pr_2753`，最终 source HEAD `df10123bf39cd7f03ead2dce62ba6a2fae268e92`、PR #2753、merge `024bcf70537c2f1b267417c72f8539937dd21a3f`闭合；K2-B/C/D与K3/K4 `not_started`，`close_sync=not_applicable_feature`，产品 runtime switch、DDL/DML、配置和 broker 行为均未发生；
- 预计 1–2 PR，7–10 人日。
...
- 当前 `implementation_in_progress`、K2-A source `implemented_verified + merged`、K2-B=`implemented_verified`、K2-C/D `not_started`、shadow-only；K2-A-M1 repository maintainability已将3102行单文件拆为38行public façade和六个private responsibility modules，唯一`KernelRepositoryBase._connection`保持connection/transaction ownership，projection与schema proof各自只有一套authority。K2-B交付exact ingress/ALGO_START/callback、pure delivery/materialization、failure/skip/retry/reclaim transaction；审核修复了nonterminal predecessor误阻断事件入库，以及FAILED last-good state被错误对齐总transition sequence两项过度门禁。direct/DEV=`109 passed`、MiniQMT L2=`770 passed,23 skipped`，classifier仅MiniQMT且`unmapped_code_files=[]`，五个新模块line/branch均满足80/70；PR #2773 OPEN、`source_merge=pending_user_authorization`、required CI pending，预计K2整体仍为4 PR，K2-C/D不得由本slice推断完成。
...
| `F-063` | K2 detailed design §4.1、§4.6、§5、§6.1–6.2、§12 K2-B ingress/creation/callback | `backend/tests/miniqmt_execution_runtime/test_kernel_ingress.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_creation.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_repository_postgres.py`验证exact routing、ALGO_START route refs、nonterminal predecessor queue、callback single-transaction/rollback/readback；direct+DEV=`109 passed` | implemented_verified_local | none |
| `F-064` | K2 detailed design §4.2–4.4、§6.3、§7、§12 K2-B delivery/materializer/repository | `backend/tests/miniqmt_execution_runtime/test_kernel_delivery.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_repository_k2b_validation.py`及DEV applied/failure/skip/reclaim；new modules line/branch均≥80/70；`python -m nox -s miniqmt_execution_runtime_l2`=`770 passed,23 skipped` | implemented_verified_local | none |
| `F-065` | K2 detailed design §4.2–§4.4、§6.3、§7.2 failure/skip/active-child closure/retry | `backend/tests/miniqmt_execution_runtime/test_kernel_delivery.py`与`backend/tests/miniqmt_execution_runtime/test_kernel_repository_postgres.py`覆盖provider-only attempts 1–5、no sixth call、deterministic terminal failure、last-good state、queued successor SKIPPED、stale same-attempt reclaim；classifier仅MiniQMT、`unmapped_code_files=[]` | implemented_verified_local | none |
...
| production state separation | pass | K2-A source 已通过 PR #2729 / merge `0b46f7819f4147c97a36908e25ca948ce5450661` 合入，K2-A-M1 source已通过PR #2753 / merge `024bcf70537c2f1b267417c72f8539937dd21a3f`闭合为`source_merge=merged_pr_2753`；M1不改schema/migration/dependency/config/binding/broker/runtime，DDL/DML/restart/runtime activation全部为`noop` |
