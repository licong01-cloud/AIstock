...
| production state separation | pass | K2/K3/K4/K5 overall=`implemented_verified + merged`；K6-A/C0/C1/B均已合入，K6-B=`source_merge=merged_pr_3120`，K6-D detailed=`design_ready_local`/code=`not_started`；final-architecture-only revision=`design_revision_ready + merged`、`source_merge=merged_pr_3098`；产品runtime未切换，predecessor与K6-B successor production DDL=`applied_and_verified`，K6-D new DDL/DML/config/binding=`noop`，dependency gate=`noop`，broker/restart/runtime activation/normal-day observation=`not_run` |
