...
>
> K1 下位详细设计：[`miniqmt_execution_kernel_k1_contracts_registry_f2_detailed_design_20260722.md`](miniqmt_execution_kernel_k1_contracts_registry_f2_detailed_design_20260722.md) 当前为 `implementation_verified`；K1-A/B/C均为`implemented_verified + merged`。K2-A、K2-A-M1和K2-B均已`implemented_verified + merged`。K2-C implementation commit `6cadc7f8` 已通过 direct/DEV/coverage/classifier/L2/F2/DESIGN-COMPLIANCE-001 闭合为`implemented_verified`、`source_merge=pending_pr`；K2-D与K3/K4 `not_started`。现有产品runtime未切换，production/runtime gates均为`noop`。
>
> K2 下位详细设计：[`miniqmt_execution_kernel_k2_durable_dispatch_f2_detailed_design_20260725.md`](miniqmt_execution_kernel_k2_durable_dispatch_f2_detailed_design_20260725.md) 当前为 `implementation_in_progress`；K2-A、K2-A-M1与K2-B均为`implemented_verified + merged`。K2-C implementation commit `6cadc7f8` 已闭合 clock/session/timer/restart 与 additive migration，direct/import/structure=`121 passed`、DEV migration=`13 passed`、clock line/branch=`88.04%/80.43%`、MiniQMT L2=`788 passed,25 skipped`，当前`implemented_verified`且`source_merge=pending_pr`；K2-D=`not_started`。未启动常驻worker、未调用Gateway/broker、未执行生产DDL/DML，也未切换产品runtime。
>
> 日期：2026-07-22。

## 0. Executive Decision / 核心决策

K1-C remains `implemented_verified + merged` through PR #2685 / merge `e4faeb53663cb4d19eb4e07d833953725a40fdc1`; K1 overall is unchanged. K2 detailed design remains `implementation_in_progress`: K2-A, K2-A-M1 and K2-B remain `implemented_verified + merged`. K2-C implementation commit `6cadc7f8` implements the strict ExchangeSessionClock, deterministic SESSION/TIMER/EOD ingress, durable atomic claim/finalize/reclaim, lunch/EOD/catch-up/restart closure and an additive guarded migration; it is `implemented_verified` with `source_merge=pending_pr`. K2-D and K3/K4 remain `not_started`, product runtime is not switched, and production gates remain `noop`.
...
- K1 overall 当前为 `implemented_verified + merged`；K2 detailed design `implementation_in_progress`，K2-A/K2-A-M1/K2-B均为`implemented_verified + merged`。K2-C implementation commit `6cadc7f8`为`implemented_verified`、`source_merge=pending_pr`；K2-D与K3/K4 `not_started`，`close_sync=not_applicable_feature`，产品 runtime switch、DDL/DML、配置和 broker 行为均未发生；
- 预计 1–2 PR，7–10 人日。
...
- 当前 `implementation_in_progress`、K2-A/K2-A-M1/K2-B均`implemented_verified + merged`，K2-C=`implemented_verified`且`source_merge=pending_pr`，K2-D `not_started`，整体保持shadow-only。K2-C通过唯一clock/repository/ingress路径闭合session、timer、午休、catch-up、EOD、restart和additive migration；direct/import/structure=`121 passed`、DEV migration=`13 passed`、clock line/branch=`88.04%/80.43%`、MiniQMT L2=`788 passed,25 skipped`。产品route仍未切换，K2-D不得由本slice推断完成。
...
| `F-067` | K2 detailed design §4.8–§4.9、§8、§12 K2-C；`kernel_clock.py`与timer/session repository的唯一authority、atomic claim/finalize/reclaim及additive migration | `backend/tests/miniqmt_execution_runtime/test_kernel_clock.py`=`15 passed`、line/branch=`88.04%/80.43%`；`test_plugin_import_boundaries.py`/`test_kernel_repository_structure.py` direct合计=`121 passed`；`test_kernel_migration_postgres.py` DEV=`13 passed`；`python -m nox -s miniqmt_execution_runtime_l2`=`788 passed,25 skipped`；L0/registry/classifier通过 | implemented_verified | none |
