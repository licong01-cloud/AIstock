...
K1 compatibility及K2/K3经济执行能力的历史source状态保持merged；execution-kernel overall因hot-market-data persistence correction改为`implemented_baseline_remediation_required`。K4-A为`implemented_verified_contract_slice + merged`；K4-B/K5/K6既有插件、product authority、broker exactly-once与route source仍保持各自历史合入状态，但凡依赖durable TICK/cutoff的验收子项均待`F-113..F-121` remediation，不能以旧overall状态宣称最终架构完成。
...
Immutable ExecutionPlan
  ...
  -> MiniQMTExecutionKernel
       -> DurableEventIngress (business/session events only)
       -> RuntimeEventEnvelopeV2 + AlgoEventDeliveryV1
...
Scheduler-owned quote path
  ...
  -> B0_QUOTE_V2 normalize/eligibility
  -> process-local HotMarketDataIngress
  -> shared immutable MarketDataViewV2
  -> no effect: zero SQL
...
| --- | --- | --- | --- |
| event sequence/fan-out | `MiniQMTExecutionKernel`；TICK使用同一root的in-memory single writer，经济event使用durable sequence | 声明订阅类型 | 自建线程、队列或 timer |
...
| `event_type` | durable envelope只允许 `ALGO_START/TIMER/ORDER/TRADE/ACCOUNT/SESSION/RECONCILE/EOD/OPERATOR`；TICK属于`HotMarketDataEventV1`，不是本DTO |
...
数据库 CHECK 必须按这张删除TICK后的组合表实施，不得只分别校验两个独立 enum。历史TICK rows可只读保留，但新writer和successor CHECK必须拒绝。每行 `source_identity` 的 key set 必须 exact，missing/extra 都拒绝；`ALGO_START.algo_instance_id` 按 §5.5 完整 tuple 重算。`event_key_sha256 = sha256(schema_version, runtime_id, event_type, source, canonical(source_identity))`，`event_id = "mqrtevt_" + event_key_sha256`；sequence、arrival time 和 retry attempt 不进入 identity。correlation、payload 和 payload hash 是同 key 的 immutable closure，任一变化都是 terminal conflict。
...
TICK 使用独立 `HotMarketDataDeliveryV1`：identity=`runtime_id + symbol + source_generation + market_sequence + market_data_sha256`，只存在于进程内；target set来自runtime creation时冻结的manifest subscription，同symbol多个algo共享同一view。它不创建`AlgoEventDeliveryV1`或durable sequence。插件无effect返回`NO_EFFECT`；有effect才由kernel分配下一durable transition identity并写既有economic transition/command字段，不新增行情carrier/hash。
...
一个 durable external event ingress transaction 必须：
...
durable event 与 deliveries 未同时提交时不得 ACK consumer。TICK不进入本transaction；ordinary TICK callback的ACK只表示process-local validation/delivery完成，不表示DB成功或业务成功。
...
每条 durable delivery 在一个 transaction 内完成。hot TICK transition先在进程内pure evaluate；只有有effect时进入同等严格的economic transaction，并只持久化既有transition/state/timer/command/outbox字段：
...
- quote subscriber single writer负责normalize/generation/order；同一kernel hot-ingress actor负责插件投递，两者都不访问repository；
...
| vn.py 角色 | AIstock 实现 |
| --- | --- |
| EventEngine | hot TICK 使用进程内single-writer dispatch；business/session events使用durable ingress + delivery worker |
...
| `AlgoEngine.get_tick(algo) -> TickData | None` | 从当前 immutable process-local `MarketDataViewV2` 投影manifest声明字段；没有当前合法view时返回upstream-compatible `None`并更新bounded process diagnostic，不写wait event、不读DB/旧行情fallback |
...
Iceberg TIMER `get_tick()` 只允许读取同一runtime/symbol/generation的当前 process-local eligible B0 view，并在真正形成slice/reprice command时只把既有command/transition经济字段原子提交，不附加行情carrier/hash。latest view若cross-session/cross-symbol/stale/non-B0则unavailable，不回退更旧view、不读DB、不合成。restart后没有current view即返回`None`并等待下一真实callback；不得以确定性为由恢复durable TICK查询。已提交command的重放输入由其command/order经济字段冻结，不再依赖process cache。
...
1. `execution_runtime_event` 增加 business/session event 所需字段与 `(runtime_id,event_key)` unique；successor composite CHECK明确拒绝新KERNEL_V2 TICK/ordinary quote events，历史TICK只读保留；
...
7. durable event type/source/payload schema 使用 §5.2 删除TICK后的composite CHECK；delivery status、command type/status、timer mutation/catch-up policy 使用 explicit CHECK；
...
10. migration test 必须证明 ALGO_START sequence 1、delivery predecessor chain、stale DISPATCHING nullable truth、FAILED/SKIPPED receipts、compatibility receipt hash以及KERNEL_V2 TICK拒绝均由数据库拒绝非法组合，而不是只靠 Python 校验；
11. 不创建`ExecutionDecisionEvidenceV1`或任何行情evidence carrier/hash。transition/command只允许既有command/order/trade原生price与actual quantity、code-owned/frozen配置阈值、basis、reason与action time；禁止trigger/reference observed market price、source-view hash、L1-L5 arrays、bar、raw/normalized quote。reconciliation fingerprint current projection与run summary `<=64 KiB`需由CHECK/UNIQUE/readback闭合。
...
- 已产生 V2 event/delivery/transition/outbox 后，应用回滚只能部署最后一个兼容读取且保持zero-tick-persistence的build并drain/reconcile，不能删除事实、恢复TICK writer或旧 broker route；无安全build时保持模拟盘停止；
...
- business event/delivery/transition/outbox、normalized state history以及 command→child/order/trade关联在任何algo、command、reconcile、TCA/markout未终结时禁止archive；
...
- timer/diagnostic cadence 中不参与 broker/business chain 的聚合事实 active 14 个自然日后可 soft archive；ordinary tick/reject/wait不产生row；不得按总行数截断 mandatory event 或 unknown-outcome anchor；
- 本阶段不新增物理 delete job；历史tick/NO_FILL/duplicate reconciliation/oversized run JSON cleanup为独立授权DML，不是启动门禁；
...
diagnostics还必须显示 hot-path repository violation count、DB transactions/economic transition ratio、forbidden market-data carrier/hash count、run summary size、reconciliation distinct/repeat counts。ordinary quote metrics仅为process counters/log，不写交易数据库窗口事实。
...
- **K6-C1与K6-B均已实现验证并合入**：C0/BUG-953既有strict command/evidence、deterministic SUBMIT/CANCEL lineage和physical mapping closure保持不变。C1唯一pure evaluator与V3 materializer保持；K6-B公共seam只接收durable event identity，在同一事务strict读取V3 deferred authority、route/session/account、qmt strategy settled trade/cash ledger与K2 ORDER/TRADE/RECONCILE/ACCOUNT/EOD，并原子推进decision/dependency/coordination/mapping与same-command K2 outbox；replay要求decision owner exact set closure。WAIT/BLOCK/EOD、commit-unknown、restart与same-event concurrency均有真实DEV PostgreSQL证据。K6-B successor migration与`preflight_k6b_schema()`使用K6-C0 predecessor、K6-B base/K6C/delta三层独立catalog和fingerprint function metadata/body共同闭合，不通过禁用或放宽preflight接入；不按algo code分支、不复制broker route。product root继续拒绝K4/K5 V1 shadow receipt、K6-A dependent-BUY V1和hash-only product authority V2；
- **K6-B source closure已完成**：当前=`implemented_verified + merged`、PR #3120、merge `48bcf1153dea3dca5c598793d6556d2757ed87de`、`source_merge=merged_pr_3120`，required CI run `30844554277` green。实现未另造event/transition/command，不下沉进算法plugin、不估算cash或直提broker；K3 inventory未作为candidate source、legacy parity、迁移oracle或验收输入；line/branch均达到`>=80/>=70`，classifier只选择MiniQMT。source merge仍不自动执行K6-B production DDL或激活产品route；
- K6-B与K6-D所有研发只面向最终隔离架构：K6-B直接实现最终KERNEL_V2 coordinator/invocation seam，不调用或修改legacy，不做parity、bridge、translator、shadow writer或双写；K6-D以SIM/runtime-binding-account exact closure、durable route cutover receipt、`KernelAlgoCreationRequestV2`双route lineage和覆盖ALGO_START/TICK/TIMER/SESSION/ORDER/TRADE/COMMAND_OUTCOME/ACCOUNT/RECONCILE/EOD及既有typed OPERATOR owner routing的唯一product coordinator冻结new-instance KERNEL_V2 owner，旧实例仅用冻结代码自然drain，退役同步timer for-loop、legacy adapter产品调用与dependent-BUY direct retry，完成source capability及static/runtime unique-route proof；不增加审批型operator command，禁止dual route、legacy fallback或K6 state回交legacy；
- K6-C0 predecessor与K6-B successor production DDL/readback均已在独立授权后完成，K6-B repository checks=`9/9 true`、coordination rows=`0`；K6-D new DDL/DML/config/binding=`noop`。服务重启由用户执行；runtime activation与正常交易日证据独立回读，正常交易日必须覆盖single/multi、上午/午休/下午/EOD和dependent-BUY/outbox/callback/reconcile闭合；
- source/DDL/config/binding/restart/runtime/normal-day evidence分开记录。K6-C1与K6-B source merge已分别通过PR #3080/#3120闭合，剩余唯一切片为`K6-D`；禁止插入C2或任何legacy维护/compatibility/parity/bridge/backport/shadow切片，不得把source/runtime分离实现为长期过渡route；总工期按最终架构范围记录，不能用观察时间或工期压缩验收范围。
...
- TICK process-local owner routing及TIMER/ORDER/TRADE/ACCOUNT/SESSION/EOD/RECONCILE/OPERATOR durable owner routing、缺owner/多owner/cross-runtime negative matrix；
...
| `F-085` | Tick/Order/Trade/Contract DTO、selected Exchange/Status逐字段映射；TIMER读取process-local current B0 view，missing/unsupported无DB读取、合成或fallback |
...
| `F-096` | Iceberg exchange-active TIMER、process-local current native B0 view/economic action、cancel-pending resubmit/late callback、exact `traded >= target`、restart等待下一live quote/multi-command语义精确；无行情evidence/hash |
...
| `F-113` | ordinary TICK process-local single-writer、zero SQL与restart等待下一live callback完整 |
| `F-114` | action只写既有economic transaction字段且禁止行情carrier/hash完整 |
| `F-115` | durable business facts允许集与raw/normalized market-data禁止集完整 |
| `F-118` | outbox/reconcile event-driven wake与禁止per-tick empty scan完整 |
| `F-119` | successor migration、retention、独立cleanup授权与no-tick rollback完整 |
| `F-121` | soak/DEV/normal-day DB traffic/storage/cardinality evidence完整 |
...
| `F-104` | K6 detailed §4.2、§8；V3 envelope/evaluator/materializer与BUG-953 lineage/lifecycle closure | `python -m pytest -q backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py backend/tests/miniqmt_execution_runtime/test_kernel_product_authority.py backend/tests/miniqmt_execution_runtime/test_kernel_product_materialization_postgres.py`=`105 passed`，DEV PostgreSQL=`20 passed`；覆盖terminal mapping及DEFER waiting/block/EOD/released-outbox readback；PR #3080 / merge `c0ca2e7e356a891ee5b2aad270d391071c942ae4` | implemented_verified + merged | none |
| `F-105` | K6 detailed §5；K6-A immutable migration + `miniqmt_execution_kernel_k6c_20260802.*` | `AISTOCK_RUN_MINIQMT_K2_DEV_DB=1 python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_k6_migration_postgres.py -q`；PR #3032 / merge `2a3622a3ba63585e3dfe12ef7ccb3f33b00dcb63` | implemented_verified + merged | none |
| `F-106` | K6 detailed §6；versioned preflight、BUG-953 exact mapping CHECK与C1 atomic writer/readback | `AISTOCK_RUN_MINIQMT_K2_DEV_DB=1 python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_materialization_postgres.py -q`=`20 passed`，覆盖rollback、commit-unknown、same-authority concurrency、drift、CLAIMED/FAILED_TERMINAL及released DEFER lifecycle；production只读preflight=`8/8 true`、authority总行/V3行=`0/0` | implemented_verified + merged | none |
| `F-107` | K6 detailed §4.3/§9；K6-D detailed §3–§5 | `backend/tests/miniqmt_execution_runtime/test_kernel_product_cutover.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_product_runtime_integration.py`及DEV route/ALGO_START matrix | implemented_verified | none |
| `F-108` | K6 detailed §9.2；K6-D detailed §2.2/§6 | `backend/tests/miniqmt_execution_runtime/test_kernel_legacy_route_retirement.py` exact symbol absence + five-root AST/import/call-graph/fresh-process uniqueness | implemented_verified | none |
| `F-109` | K6 detailed §10；K6-D detailed §7 | `backend/tests/miniqmt_execution_runtime/test_kernel_diagnostics.py`、`backend/tests/simulation_runtime/test_ops_api.py`、DEV product repository readback；artifact: `docs/operations/simulation_platform_operator_runbook_20260717.md` | implemented_verified | none |
| `F-110` | K6 detailed §11；K6-D detailed §5/§8 | `python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_runtime_integration.py backend/tests/miniqmt_execution_runtime/test_kernel_clock.py -q`；完整四段native calendar/auction projection、strict frozen policy、commit-unknown/non-sequence conflict保真、callback/clock/EOD/isolation/DEV/coverage | implemented_verified | none |
| `F-111` | K6 detailed §12–§13；K6-D detailed §9–§10 | artifact: `docs/architecture/miniqmt_execution_kernel_k6d_final_route_cutover_f2_detailed_design_20260804.md`；source/production/runtime分离receipts | implemented_verified | none |
| `F-112` | K6 detailed §16–§17；K6-D detailed §13–§14 | artifact: `docs/architecture/miniqmt_execution_kernel_k6d_final_route_cutover_f2_detailed_design_20260804.md`；formal DESIGN-COMPLIANCE-001 | implemented_verified | none |
| `F-113` | §4.1–§4.4、§5.2–§6.4；process-local ingress + V4 targets | `backend/tests/miniqmt_execution_runtime/test_hot_market_data_boundary.py`：1M no-action、same-symbol five-target、restart-next-live-tick、zero economic committer；MiniQMT L2=`1495 passed,82 skipped` | implemented_verified_source | explicitly approved production-state separation：merge/restart/runtime observation pending |
| `F-114` | §5.6、§6.2；unique economic transition committer | `backend/tests/miniqmt_execution_runtime/test_hot_market_data_boundary.py`：economic allowlist、行情carrier/hash拒绝、exact successor/ACK、row-version isolation | implemented_verified_source | explicitly approved production-state separation：merge/restart pending |
| `F-115` | §5.2、§10；durable TICK denylist + DB successor CHECK | `backend/tests/miniqmt_execution_runtime/test_hot_market_data_migration_postgres.py`：repository pre-connection拒绝与DEV exact allow/deny/readback=`5 passed` | implemented_verified_dev | explicitly approved production-state separation：production DDL pending |
| `F-118` | §6.3、§10–§11；typed retry allowlist | `backend/tests/miniqmt_execution_runtime/test_hot_market_data_boundary.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_product_runtime_integration.py`：slow commit、pending refresh零repository、unknown/schema/identity非重试 | implemented_verified_source | explicitly approved production-state separation：normal-day traffic observation pending |
| `F-119` | §10、§14；migration triplet | `backend/tests/miniqmt_execution_runtime/test_hot_market_data_migration_postgres.py`：DEV preflight/forward/second apply/readback/rollback=`5 passed` | implemented_verified_dev | explicitly approved production-state separation：production DDL/cleanup DML pending separately |
| `F-121` | §13–§15 | `backend/tests/miniqmt_execution_runtime/test_hot_market_data_boundary.py`：1M no-action soak；combined line/branch=`85%/76.57%`；三个nox plan通过 | source_capacity_verified | explicitly approved production-state separation：normal-day DB delta/storage/fresh runtime receipt pending |
...
| --- | --- | --- |
| no simplified delivery | pass | K4/K5 exact authority保持；K6修订闭合command_json、V2 ledger/proceeds、MATERIALIZE/REJECT/DEFER、same-command release、DB事务、最终route、旧调用删除和正常交易日验收；legacy parity/bridge/shadow过渡物不得冒充实现 |
...
| production state separation | pass | K2/K3/K4/K5 overall=`implemented_verified + merged`；K6-A/C0/C1/B/D均已合入，K6-D design PR #3129及source PR #3146 / merge `33c09049e82c11cdbae7cd9b596b3666cb481349`已闭合，`source_merge=merged_pr_3146`。产品runtime未切换，predecessor与K6-B successor production DDL=`applied_and_verified`，K6-D new DDL/DML/config/binding=`noop`，dependency gate=`noop`，broker/restart/runtime activation/normal-day observation=`not_run` |
| no hot market-data persistence | pass for source | TICK走同一kernel root的in-memory dispatch，不创建durable event/delivery；action只写既有经济字段且无行情carrier/hash；Iceberg/TIMER/restart均不得从DB反查tick |
| implementation truth | pass | BUG-1041 source/DEV/1M soak已验证但未合入/激活；production DDL、用户restart、runtime identity和正常交易日证据继续 pending，plugin architecture overall=`implemented_baseline_remediation_in_progress` |

## 20. Definition of Done / 完成定义

整项架构优化只有同时满足以下条件才能标为实现完成：

1. `F-043..F-121` 全部更新为 `implemented_verified`，且每项有真实 implementation/test/runtime receipt；
