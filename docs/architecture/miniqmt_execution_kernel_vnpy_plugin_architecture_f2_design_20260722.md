...
| `F-104` | K6 detailed §4.2、§8；V3 carriers与BUG-953 lineage/lifecycle closure | C0：`python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py -q`；BUG-953 direct+DEV combined=`88 passed`；C1 target `test_kernel_product_authority.py` | implemented_verified | none |
