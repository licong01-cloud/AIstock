...
- source/DDL/config/binding/restart/runtime/normal-day evidence分开记录。K6-C1 source merge已通过PR #3080闭合，剩余顺序固定为`K6-B -> K6-D`；不得把K6-C1合入误报为K6-B完整，也不得并行修改共享蓝图；总工期按详细设计修订后的范围记录，不能用观察时间或工期压缩验收范围。
...
| `F-110` | K6 detailed design §11 | `backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py`、`test_kernel_product_authority.py`、`test_kernel_product_materialization_postgres.py` direct=`105 passed`；authority/contracts/repository line/branch=`84.21/76.47`、`90.52/73.11`、`86.63/70.37`；required CI run `30804132306` green | implemented_verified + merged | none |
...
| production state separation | pass | K2/K3/K4/K5 overall=`implemented_verified + merged`；K6 base design、K6-A、revision与K6-C0均已合入；BUG-953已通过PR #3048完成source merge、production DDL/readback、用户重启、post-restart receipt和Issue关闭，当前顺序为C1->B->D；产品runtime未切换，production DDL=`applied_and_verified`，其余config/binding/broker/runtime activation/normal-day observation为`noop/not_run` |
