...
K1、K2、K3 overall均保持`implemented_verified + merged`。K4详细设计已通过PR #2861合入；K4-A pure façade/adapter、contracts、source authority现为`implemented_verified`且source merge待本PR，K4 overall=`implementation_in_progress`；K4-B/K5/K6=`not_started`。产品runtime未切换，production gates保持`noop`。
...
- K1 overall 当前为 `implemented_verified + merged`；K2 detailed design 为 `implementation_verified`，K2-A/K2-A-M1/K2-B/K2-C/K2-D 均为 `implemented_verified + merged`。K2-D final source `82c69fbf...`已通过 PR #2804 / merge `fc4170fa...`闭合为`source_merge=merged_pr_2804`；K3 overall已通过PR #2848完成`implemented_verified + merged`；K4-A pure façade/adapter、contracts与source authority已`implemented_verified`且`source_merge=pending_pr`，K4-B=`not_started`、K4 overall=`implementation_in_progress`；`close_sync=not_applicable_feature`，产品 runtime switch、DDL/DML、配置和 broker 行为均未发生；
- 预计 1–2 PR，7–10 人日。
...
- K4-A已交付五个pure核心文件、五算法+`round_to` exact source manifest、K1 delegated-path closure、implementation/method/DTO/state/terminal/isolated bindings及receipt writer/readback；direct+import=`118 passed`，coverage五核心line/branch=`93/70,97/95,90/76,87/78,85/71`，MiniQMT=`1038 passed,29 skipped`、Paper=`1050 passed,2 skipped,2 xfailed`，classifier无unmapped；K2 optional invocation/read-only query、真实current-three parity和Iceberg/Stop完整characterization明确留给K4-B；
- 预计 1–2 PR，9–14 人日。
...
| `F-081` | K4 detailed design §0–§2 | `backend/tests/miniqmt_execution_runtime/test_vnpy_k4_scope_boundaries.py` | design_ready_k4 | none |
| `F-082` | K4 detailed design §3 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_contracts.py`与`test_vnpy_facade_characterization.py`闭合六source、K1 delegated paths、bytes/hash/readback | implementation_in_progress_k4a_verified | none |
| `F-083` | K4 detailed design §4–§5 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade.py`、`test_vnpy_facade_adapter.py`、`test_vnpy_facade_lifecycle.py`闭合strict input、collector ordinal/freeze、constructor-once/retry与direct SPI bypass | implementation_in_progress_k4a_verified | none |
| `F-084` | K4 detailed design §5.4.1、§6 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade.py`与`test_vnpy_facade_adapter.py`闭合六方法、四callback router、missing/rounded-zero diagnostics和typed failures | implementation_in_progress_k4a_verified | none |
| `F-085` | K4 detailed design §6.3–§8 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_projection.py`闭合DTO/enum/round_to/source/readback | implementation_in_progress_k4a_verified | none |
| `F-086` | K4 detailed design §5.4.1、§5.6、§9–§10 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_adapter.py`与`test_vnpy_facade_lifecycle.py`闭合constructor-once、restore、callback fact、active-child CLEAN terminal和retry | implementation_in_progress_k4a_verified | none |
| `F-087` | K4 detailed design §4.1、§11 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_contracts.py`与`test_vnpy_facade_characterization.py`闭合live binding、DTO/state/terminal/isolated set、failure truncation及writer/readback | implementation_in_progress_k4a_verified | none |
| `F-088` | K4 detailed design §3.2、§6.3、§10、§12 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_characterization.py` current-three init/transition parity + Iceberg TIMER durable TICK/Stop characterization-only | design_ready_k4 | none |
| `F-089` | K4 detailed design §13–§17 | `backend/tests/miniqmt_execution_runtime/test_vnpy_facade_failure_restart.py` error/retry/restart/diagnostics | design_ready_k4 | none |
| `F-090` | K4 detailed design §18–§20 | `python -m pytest -q backend/tests/miniqmt_execution_runtime/test_vnpy_facade_*.py`核心coverage=`49 passed`；MiniQMT=`1038/29`、Paper=`1050/2/2`；classifier=`unmapped_code_files=[]` | implementation_in_progress_k4a_verified | none |

## 19. DESIGN-COMPLIANCE-001 / 设计复核

| control | result | evidence |
| --- | --- | --- |
| no simplified delivery | pass | K3-B证据继续来自committed facts/DEV；K4新增exact initialization/transition invocation、actual authority/projection payload、constructor-once state adapter、terminal mapping、live implementation binding和DEV read-only evidence；多command trace不得截断、复用单command receipt或冒充V1 product materialization，mock-only只计单元测试，无placeholder、partial或第二route |
| no silent error | pass | K3 strict callback/outcome保持；K4 not-running/missing/rounded-zero空返回分别产生typed diagnostic，state/binding/authority/receipt drift fail loud，pure-shadow或K2-V1-shadow receipt不能冒充product command authority PASSED |
...
| production state separation | pass | K2 overall=`implemented_verified + merged`；K3 overall通过PR #2848 / merge `38434e10d530edd883fa75f904de5b025158f918`闭合为`implemented_verified + merged`；K4详细设计PR #2861 / merge `8250b64ff3c2deb04eb3594f1ae3fba3acd1e6ce`=`design_ready_for_implementation`、`source_merge=merged_pr_2861`且implementation=`not_started`；K5/K6未开始，DDL/DML/config/binding/broker/restart/runtime activation全部为`noop` |
