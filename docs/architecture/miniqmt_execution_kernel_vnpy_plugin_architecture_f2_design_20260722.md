...
- source/DDL/config/binding/restart/runtime/normal-day evidence分开记录。K6-C1 source merge已通过PR #3080闭合，剩余顺序固定为`K6-B -> K6-D`；禁止插入C2或任何legacy维护/compatibility/parity/bridge/backport/shadow切片，不得把source/runtime分离实现为长期过渡route；总工期按最终架构范围记录，不能用观察时间或工期压缩验收范围。
...
| `F-110` | K6 detailed design §11 | C1 partial evidence：`backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py`、`test_kernel_product_authority.py`、`test_kernel_product_materialization_postgres.py` direct=`105 passed`；最终route integration/uniqueness、旧调用删除、frozen-input invariants与正常交易日证据由K6-B/K6-D闭合 | design_ready | none |
| `F-111` | K6 detailed design §12–§13 | artifact: 三份权威设计的final-architecture-only/no-legacy-R&D约束；PR #3098 / merge `b986e8f655137e9aea88f17ea91d3eb153f1c017`；后续K6-B/K6-D PR receipts和分离的production/runtime states | design_revision_ready + merged | none |
