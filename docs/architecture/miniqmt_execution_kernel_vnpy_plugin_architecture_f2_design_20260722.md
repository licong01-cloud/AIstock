...
| `F-055` | `backend/services/miniqmt_execution_runtime/plugin_registry.py` route-independent catalog、durable descriptor/process binding、exact creation/restore、canonical snapshot、non-empty bounded aggregate；route writer/structural readback/authority-aware readback 共用 exact catalog + strict gateway pure evaluator | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py` = 51 passed，覆盖 hash-correct authority/failure drift、permutation/fresh process、aggregate、binding drift、B0/exact cancel/isolation；line/branch=91.57%/78.57%；second-final-review RED 后 GREEN；PR #2655 required CI 以最终 HEAD 为准 | implemented_verified | none |
...
| `F-059` | K1 detailed design §11 ownership/test routing/coverage | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py` / `backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py` / `backend/tests/miniqmt_execution_runtime/test_miniqmt_vnpy_algo_import_boundary.py` / `backend/tests/miniqmt_execution_runtime/test_algo_plugin_contracts.py` = 51/39/2/60 passed；完整 PR=9 files、本轮 delta=8 files，unmapped/ambiguous=0/0；`python -m nox -s miniqmt_execution_runtime_l2` = 564 passed/1 skipped；`python -m nox -s paper_v2_backend` = 1048 passed/2 skipped/2 xfailed；L0/module registry/F2 通过 | implemented_verified | none |
