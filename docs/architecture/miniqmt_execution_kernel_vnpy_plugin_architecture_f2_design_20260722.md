...
1. **静态支持能力**：`GatewayCapabilityCatalogV1` 在 algo 创建前证明 route/source/order type 是否可能提供 manifest 声明的 capability；静态不支持时以 `MINIQMT_MARKET_DATA_CAPABILITY_UNSUPPORTED` 拒绝该 algo 创建、`broker_called=false`。这是执行插件技术兼容检查，不读取或二次校验 StrategyPackage，也不新增人工审批。
...
| `F-055` | `backend/services/miniqmt_execution_runtime/plugin_registry.py` route-independent catalog、descriptor/process binding、exact creation/restore binding、canonical persisted readback、bounded aggregate、builder/readback 共用 supplied receipt component closure 与 route receipt | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py` = 16 passed，覆盖 permutation/fresh-process/noncanonical/hash-correct incomplete component readback/malformed aggregate continuation/truncation/route-isolation；K1-B-FINAL-REVIEW RED 1 failed 后 GREEN；PR #2655 required CI 以最终 HEAD 为准 | implemented_verified | none |
...
| `F-057` | `backend/execution_algos/vnpy_style/plugin_manifests.py` current-three exact AISTOCK_DERIVED schema/source/executable behavior trace/active-order parent/local/broker/command/decimal/quantity/native-market-data/order-trade lineage state/TWAP/legacy projection；shadow-only、runtime switch 属于 K3 | `backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py` = 36 passed，覆盖 strict positive/negative/state/source/active-order decimal-quantity-parent-market lineage/same-hash-different-payload/TWAP/current-core trace/legacy effective matrix；K1-B-FINAL-REVIEW RED 2 failed 后 GREEN；PR #2655 required CI 以最终 HEAD 为准 | implemented_verified | none |
