...
- **K6-C1已实现验证并合入，K6-B为下一优先级**：C0/BUG-953既有strict command/evidence、deterministic SUBMIT/CANCEL lineage和physical mapping closure保持不变。C1新增唯一pure evaluator、full `VnpyFacadeAuthorityInputV2`/timer envelope、0/1/N与MIXED atomic materializer、typed pre-call reject、deferred coordination、same-authority advisory-lock幂等、commit-unknown和fresh independent readback；所有 disposition均复用现有K2 transition/mapping/outbox/child/algo/delivery，不按algo code分支、不复制broker route。K6-B才实现product RESERVED到既有K2 dispatch lifecycle的显式cross-carrier handoff。product root继续拒绝K4/K5 V1 shadow receipt、K6-A dependent-BUY V1和hash-only product authority V2；
- **第二优先级 K6-B**：实现独立durable dependent-BUY coordinator，严格读取K6-C deferred authority item、qmt strategy ledger settled cash、SELL TRADE/ORDER terminal/ACCOUNT/EOD lineage；release只把同一 original command id 从deferred mapping原子推进到现有K2 outbox，不另造event/transition/command，不下沉进算法plugin、不估算cash或直提broker；K3 inventory不得作为candidate source、legacy parity、迁移oracle或验收输入；
...
- K6-C0 production DDL/readback已在独立授权后完成；K6-D后续新增或调整的production DDL、config/binding与runtime activation仍须分别授权。服务重启由用户执行；正常交易日必须覆盖single/multi、上午/午休/下午/EOD和dependent-BUY/outbox/callback/reconcile闭合；
...
| production state separation | pass | K2/K3/K4/K5 overall=`implemented_verified + merged`；K6-A/C0/C1均已合入，当前只剩B->D；final-architecture-only revision=`design_revision_ready + merged`、`source_merge=merged_pr_3098`；产品runtime未切换，production DDL=`applied_and_verified`，其余config/binding/broker/runtime activation/normal-day observation为`noop/not_run` |
