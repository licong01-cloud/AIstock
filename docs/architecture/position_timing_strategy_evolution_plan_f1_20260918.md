...
上述文献仅给机制与反证，不据此注册庞大参数搜索。本批制定时的顺序为执行/退出诊断→10路径→历史特征可见性与交易资格解耦→固定基础条件分层→模型动作价值增强；本批已经完成，§7.3链接的PT-NEXT-022随后完成了50～500亿元P1，模型仍未进入该任务。过滤后继续与同人口BH比较，结果未显示择时alpha；业绩增长加速P2/P3仍须先取得合格历史PIT、真实可见时间与单位。HMM/新闻/Agent/QE融合不在当前后续任务，规则优劣不能靠文献背书或提高仓位来保证。
...
2026-09-19后续队列更新（不改本批规格与结果）：[PT-NEXT-022 R8基本面筛选详细设计与结果](position_timing_fundamental_screen_research_f1_20260919.md)已经闭合历史特征可见性与当日交易资格分离，并完成P1的T1/T2对同股BH全量回放；两项均未超过BH、selected=0。用户明确不重复旧策略矩阵，因此不把C2或P1阈值扫描作为下一步；不接QE/HMM/荐股。严格财报PIT仍待数据窗口交付，P2/P3四格保持typed unavailable，只有输入合格后才按同一冻结family补齐。
