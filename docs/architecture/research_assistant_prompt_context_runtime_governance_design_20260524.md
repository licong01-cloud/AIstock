...
- **版本**: v1.0
- **日期**: 2026-05-24
- **状态**: 统一实施设计；整合 Prompt Pack 运行时治理、上下文压缩、Claude Code 可借鉴机制与配置化要求
...
- **硬约束**: 所有可调参数必须来自配置版本与运行 activation；业务代码不得硬编码 token 限制、消息条数、fresh tail 长度、压缩阈值、模型温度、输出 token、模型名或工具权限。
...
| UI 展示 | 是否显示“已自动整理上下文”、trace 可见性、摘要展开权限 | runtime config |
...
1. 修复 `BUG-117`，删除未开发能力负向禁用项。
2. 将 Prompt Pack 和 Context Compression 文档统一到本方案。
3. 明确所有可调参数配置化。
...
只要坚持这些边界，Research Assistant 就可以在超出单次上下文限制时自动整理上下文，让用户连续使用而不被迫重新描述背景，同时避免把关键参数、确认和审批状态丢失在普通摘要中。
