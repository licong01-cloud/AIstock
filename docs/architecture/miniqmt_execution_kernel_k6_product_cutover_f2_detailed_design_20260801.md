...
> K6-D 唯一下位实施合同：[`miniqmt_execution_kernel_k6d_final_route_cutover_f2_detailed_design_20260804.md`](miniqmt_execution_kernel_k6d_final_route_cutover_f2_detailed_design_20260804.md)，design=`design_ready + merged`、PR #3129、merge `5b1e1477c98fedbc1361b5551133fcb3075bb2b3`；code=`implemented_verified + merged`、PR #3146、merge `33c09049e82c11cdbae7cd9b596b3666cb481349`、`source_merge=merged_pr_3146`。产品runtime未切换，用户restart/runtime activation/正常交易日验收未执行。
>
> 设计基线：`main@c499276da91dcae40f0e452de11141c7b0585a1c`；K6-B 最终分支在 push 前必须安全包含当时最新 `origin/main`，具体 readback 在 PR/完成报告中记录，避免把易变流程 SHA 写成业务设计 authority。K1–K5 均为 `implemented_verified + merged`；K6-A/C0/C1/B/D source 已实现并合入，产品 runtime 保持 inactive。K6/BUG-953 predecessor与K6-B successor production DDL均已在独立授权后应用并由exact repository preflight回读通过；K6-B DEV migration=`1 passed`、production `preflight_k6b_schema()`=`9/9 true`、coordination rows=`0`。产品 runtime 仍未切换，production DML、配置、binding、broker 调用和 runtime activation均未执行，服务重启归用户所有并与本次 source 状态分开记录。
...
7. rollback 不得把已有 K6 durable state 交给 legacy。KERNEL_V2 route receipt写入后，源代码或部署回滚只能使用通过`miniqmt_k6d_route_contract_v1` fresh-process capability readback、仍理解ALGO_START V2与route owner/receipt且保持zero legacy caller的版本；当前不存在更早兼容版本时只能使用当前K6-D source或forward-fix。不能删除含 durable rows 的表，也不能绕过 fencing。
...
proceeds reader 使用唯一的 source-fact authority，不接受 caller 给出的 row、payload 或 hash：

- `qmt_trade_fact_sha256=hash_hex_v1("miniqmt_k6_qmt_trade_ledger_fact_v1", {trade_id,intent_id,strategy_id,qmt_order_id,qmt_order_sysid,symbol,side,price,quantity,amount,commission,trade_date,account_id,trade_time,order_remark,raw_json})`；字段逐一来自一条 `qmt_strategy.trade_ledger` 行。其真实 source key 必须是 `(account_id,trade_date,trade_id)`：`broker_trade_id=trade_id` 只保留 broker 语义，`qmt_trade_ledger_id=account_id + ":" + trade_date + ":" + trade_id` 且 carrier 同时持久化 `qmt_trade_account_id=account_id`；不得把数据库复合唯一键降级为单独 `trade_id`。
- `cash_ledger_fact_sha256=hash_hex_v1("miniqmt_k6_cash_ledger_fact_v1", {cash_id,cash_sequence,strategy_id,account_id,trade_date,entry_type,cash_delta,cash_after,frozen_delta,frozen_after,intent_id,trade_id,symbol,reason,metadata,created_at})`；字段逐一来自一条 append-only `qmt_strategy.cash_ledger` 行。
- 两个 JSON 字段使用 K1 canonical JSON；NUMERIC 按现有 canonical Decimal string，timestamp 按 UTC canonical string。reader 必须重新计算两个 hash，验证 `trade_ledger.side=SELL`、trade/cash 的 `(strategy_id,trade_date,trade_id)`、依赖的 `sell_parent_intent_id` 与 coordination owner 完整闭合。缺行、重复、hash drift、trade/cash 跨 strategy/date、cash entry 非 `SELL_FILL` 或 source owner 不同均为 typed authority failure；不得以 amount/price 估算或从 callback payload 补写。

此 source-fact formula 同时是 candidate writer、K6-B trigger reader、decision writer 与 fresh readback 的唯一 authority；K3 inventory、legacy metadata、raw broker object、进程缓存和 caller supplied hash 均不能替代它。
...
2. `qmt_strategy.execution_dependent_buy_dependency`
   ...
   - K6-B successor migration `miniqmt_execution_kernel_k6b_20260803.*` 仅将该表的 `UPDATE/DELETE` trigger 从“完全不可变”收口为“冻结 SELL owner、proceeds 证据只增不减、状态单调、终态不重开、terminal closure 才能首次写 latest-order fact”；不会新建表、不会回填、不会转写旧路线。forward 必须可 second-apply；guarded rollback 仅在 coordination 零行时恢复 K6-C0 immutable trigger。
...
Trigger source contract 不允许自由文本：caller 只提交已持久化 K2 `event_id`，K6-B reader 在同一锁事务中 strict-readback `RuntimeEventEnvelopeV2`、其既有 strict payload（适用时）与 source identity，再生成 `DependentBuyTriggerEventRefV1`。其 exact mapping 为：

- `SELL_TRADE_SETTLED` 只接受 `event_type=TRADE`、`source=QMT_GATEWAY_CALLBACK`、strict `KernelTradeEventPayloadV1.side=SELL`，并要求 `payload.trade_id` 对应 §4.1 重算后的一个 settled proceeds ref；`source_fact_type=qmt_strategy.trade_ledger`、`source_fact_id=trade_id`、`source_fact_sha256=qmt_trade_fact_sha256`。
- `SELL_ORDER_TERMINAL` 只接受 `event_type in ORDER|RECONCILE` 的 strict K2 payload、`side=SELL`、`terminal=true`、exact sell parent/algo owner；`source_fact_type` 分别为 `miniqmt_kernel_order_event_payload_v1` 或 `miniqmt_kernel_order_reconcile_payload_v1`，`source_fact_id` 分别为 `order_event_id` 或 `receipt_id`，`source_fact_sha256` 为 strict payload `fact_sha256`。所有 dependency 都没有可提供充分 proceeds 时才可 BLOCK，其他情况只能 WAIT。
- `ACCOUNT_REFRESHED` 只接受 `event_type=ACCOUNT`、`source=QMT_OMS_PROJECTION`、payload schema `miniqmt_account_projection_v1`，且 `source_identity` 的 `projection_version/projection_sha256` 与同 strategy/trade-date 的 frozen V3 account projection闭合；`source_fact_type=miniqmt_account_projection_v1`、`source_fact_id=projection_version`、`source_fact_sha256=projection_sha256`。reader 仍必须锁定并重读 `virtual_account`，不得信任 payload 中的 cash。
- `SESSION_EOD` 只接受 `event_type=EOD`、`source=EXCHANGE_SESSION_CLOCK`、payload schema `miniqmt_eod_event_v1`。`session_epoch` 是严格 K2 EOD event 的 identity 成分，不能伪称为 `ExchangeSessionAuthorityV1` 的字段；reader 必须同时 strict-readback EOD event 的 `(runtime_id,trade_date,session_epoch)` 与该 runtime/trade-date 唯一 `ExchangeSessionAuthorityV1.authority_sha256`。因此 `source_fact_type=miniqmt_eod_event_v1`、`source_fact_id=event_id`、`source_fact_sha256=event.payload_sha256`，并把 exchange-session authority hash 作为独立 frozen ledger freshness closure；不得以只含 authority hash 的自报 session_epoch 代替 EOD event。EOD evaluator 必须证明该 runtime 所有更小 committed event sequence 已消费或有明确 terminal disposition。

任何 event type/source/schema/owner/fact closure不满足时必须 typed fail，不能把事件重分类、跳过、默认 WAIT 或使用 caller supplied payload/hash。`source_fact_type/id/sha256` 必须由各自 reader重建；同一 trigger identity 的 retry 只能回读同一 decision。
...
1. **Source merged, runtime inactive**：K6-D source合入只改变未来重新加载的进程；当前旧进程继续其冻结代码自然drain。合入后的source不得再包含创建新legacy实例的product caller。
2. **Existing production schema proof**：K6-B successor migration已经应用并独立回读；K6-D不新增DDL，也不改变binding。
...
4. **Legacy drain before activation**：旧进程中的旧实例只消费其既有 callback/timer直至 terminal；不得生成新的 algo instance 或跨 EOD 续跑。用户重启前active legacy inventory必须只读证明为零。
5. **New-instance cutover**：K6-D source由用户重启加载后，新MiniQMT实例从既有immutable binding/release identity重建并写入route receipt；从 `effective_new_instance_sequence` 起新实例只走 KERNEL_V2。K6-D不新增生产config/binding DML或业务审批状态；用户授权边界仅保留服务重启与其后的运行时验收。
6. **Route retirement proof**：source中的legacy product call/import/synchronous loop已经删除；以静态route-uniqueness和正常交易日运行时证据证明唯一，不在激活后追加legacy删除切片。
...
source inventory与process drain inventory必须分开。source item记录`product_callers_before`非空、`product_callers_after`为空、before/after source hash、replacement/test refs和`REMOVED|NON_PRODUCT_TEST_ADAPTER`；`DRAIN_ONLY_VERSION_PINNED`只属于部署前旧进程实例receipt并证明new algo/child/retry counts全为0。两类item与aggregate的exact schema/hash/truncation domain由K6-D detailed §2.2固定；`UNKNOWN`、after caller非空或无caller evidence阻断route retirement。不能用deny gate永久保留未理解代码，也不能把inventory演变为legacy维护项目。

### 9.3 Rollback semantics

- activation 前：正常 source rollback，K6 schema空表可 guarded rollback。
- KERNEL_V2 route receipt写入后，无论是否已有broker call，都不得部署pre-K6-D或任何缺少`miniqmt_k6d_route_contract_v1` capability的source；不存在“撤销route candidate配置”或回到legacy。
- 任一 K6 command 到 PRE_CALL 后：只能部署通过上述capability的K6-D-compatible版本或让实例 drain/reconcile。
...
- `test_kernel_dependent_buy.py`：candidate、deferred mapping/no-outbox、partial sell、multiple trade、§4.1 trade/cash source-fact hash/readback、四类 trigger 的 event/source/schema/owner拒绝矩阵、fresh/stale ledger revision、cancel/no proceeds、same-command release、EOD、late callback、restart、exactly once。
...
- 本文、K6-D下位设计、execution-kernel父蓝图与统一蓝图四份 F2 validator 各 design item 与 matrix 一一对应、warnings=0。
...
- K6-D product clock必须持久化B0日历完整的开盘竞价/AM连续/PM连续/收盘竞价四段authority，只以versioned phase authority抽取恰好两个连续段计算active seconds/TIMER；不得截断为测试用两段、把收盘竞价并入PM或从quote合成竞价。冻结policy只接受compiler-owned exact envelope/payload并重算hash；native ingress只重试exact sequence CAS，commit-unknown与其他authority conflict保留原typed failure。
...
- **K6-C1=`implemented_verified + merged`, `source_merge=merged_pr_3080`**，merge=`c0ca2e7e356a891ee5b2aad270d391071c942ae4`。`kernel_product_authority.py`与`kernel_product_materialization_repository.py`已实现§8唯一pure evaluator、`ProductCommandAuthorityEnvelopeV3`、0/1/N与MIXED aggregate、MATERIALIZE/REJECT/DEFER、typed terminal no-broker reject、atomic K2 transition/mapping/outbox/coordination/timer/diagnostic/algo/delivery writer、same-authority advisory-lock幂等、commit-unknown与独立fresh readback。正式复审补齐了公共入口逐item strict readback、strict positive CAS version、lowercase SHA-256、MATERIALIZE terminal物理mapping投影、DEFER waiting/block/EOD/released-outbox生命周期闭包、immutable initial lineage以及coordination successor UTC writer/readback hash一致性；没有提前实现K6-B decision/release writer。C1 contracts/authority/DEV PostgreSQL direct=`105 passed`，其中 disposable DEV PostgreSQL=`20 passed`；authority、contracts、repository line/branch分别=`84.21%/76.47%`、`90.52%/73.11%`、`86.63%/70.37%`。required CI run `30804132306` green。production仅执行只读验证：K6-C0 preflight=`8/8 true`，product authority总行/V3行=`0/0`，因此没有伪造生产业务样本。未调用broker、未激活产品route；source merge、production只读证据和runtime状态继续分开。
- 五个 plugin 全部通过同一 public seam；仍不激活产品binding。
- 原预计 8–12 个开发日；依赖 K6-A/C0。K6-C1 source 已合入，下一切片为K6-B；K6-B release实现与K6-D cutover仍须按顺序独立验收，不得并行修改三份共享权威设计。
...
- **当前状态：`implemented_verified + merged`，PR #3120，merge `48bcf1153dea3dca5c598793d6556d2757ed87de`，`source_merge=merged_pr_3120`。** 公共入口只接受 durable `event_id` 与 worker/process identity；在同一数据库事务中 strict-readback K2 ORDER/TRADE/RECONCILE/ACCOUNT/EOD、V3 authority、route owner、session authority、virtual account、trade/cash ledger、coordination/dependencies/mapping，并执行 decision + monotonic dependency + coordination CAS + mapping + same-command K2 outbox 的原子写入。WAIT、RELEASE、terminal BLOCK、EOD residual、late/restart、commit-unknown与同事件并发均保持幂等且 fresh independent readback；replay还要求既有decision owner set与事件候选exact set-equal，任何extra/missing/partial identity均fail loud。K6-B migration triplet与`preflight_k6b_schema()`已以disposable DEV PostgreSQL验证K6-C0 predecessor exact hash、K6-B base/K6C/delta三层catalog、fingerprint function metadata/body、forward second-apply、comment/function drift、nonzero state rollback refusal与零行guarded rollback；production DDL已应用并独立回读`9/9 true`，三层catalog=`6eeff2d2.../ef09f8ab.../10ae5be0...`，coordination rows=`0`；未执行broker、配置、binding、服务控制或runtime activation。
- 合入证据：direct contracts/state/repository=`111 passed`；K6-B direct + 六个公共 DEV PostgreSQL modes=`50 passed`；migration exact DEV=`2 passed`；状态机 line/branch=`86.08%/74.53%`，coordinator repository=`85.44%/71.10%`，versioned product repository=`87.37%/70.41%`；K6-B catalog hashes为base=`6eeff2d2...`、K6C=`ef09f8ab...`、delta=`10ae5be0...`，fingerprint body=`7a57ccdf...`，forward canonical-LF=`e09fdd48...`。完整`python -m nox -s miniqmt_execution_runtime_l2`=`1276 passed,68 skipped`；changed-files=`18`、classifier=`targeted_ci_required`且只选择`miniqmt_execution_runtime_l2`、`unmapped_code_files=[]`；required CI run `30844554277` 的 MiniQMT、static 与 verdict 均为green。
- 预计 5–7 个开发日；依赖 K6-C0/C1 source merge，不再声明可与 K6-C 并行完成。

### K6-D — cutover, retirement and runtime acceptance

- 唯一下位实施合同为 [`miniqmt_execution_kernel_k6d_final_route_cutover_f2_detailed_design_20260804.md`](miniqmt_execution_kernel_k6d_final_route_cutover_f2_detailed_design_20260804.md)，当前code=`implemented_verified + merged`，PR #3146 / merge `33c09049e82c11cdbae7cd9b596b3666cb481349`，`source_merge=merged_pr_3146`；final coordinator、真实QMT snapshot callback、scheduler no-quote clock/EOD、binding isolation、legacy caller删除、source capability与只读diagnostics均已实现并合入，用户restart/runtime activation/正常交易日验收仍待独立完成。

- 直接实现 §9 最终route owner、new-instance唯一cutover、冻结旧实例自然drain、完整retirement inventory和旧产品调用删除；不开发新的drain adapter或legacy兼容层。K6-D exact contract增加SIM/runtime-binding-account闭包、`KernelAlgoCreationRequestV2` product route lineage、覆盖ALGO_START/TICK/TIMER/SESSION/ORDER/TRADE/COMMAND_OUTCOME/ACCOUNT/RECONCILE/EOD及既有typed OPERATOR owner routing的唯一product coordinator、first/retry/allowed-successor矩阵、source capability rollback identity，以及source/drain双inventory authority；不新增表或binding/config DML，也不增加审批型operator command。
- 更新 runbook/diagnostics/metrics；K6-D new DDL/DML/config/binding=`noop`；服务重启由用户执行并与source merge、运行时激活、正常交易日验收分开记录。
- 完成正常交易日 SIM 观察与aftercare。
- 预计 5–8 个开发日加至少 1 个正常交易日观察；K6-C与K6-B source merge及联合复审依赖均已满足。
...
K6-A/B/C source 合入仍保持 runtime inactive。K6-D new DDL/DML/config/binding=`noop`；服务restart由用户明确执行，runtime activation与正常交易日验收独立回读。这些是部署状态边界，不是新增产品审批或人工 acknowledge。
...
| `F-108` | legacy helper/direct dependent-BUY/synchronous timer/adapter product route退役的source-before/after与process-drain双inventory、source capability及唯一route证据可执行 |
...
| `F-103` | §4.1、§7；`kernel_dependent_buy.py`、`kernel_dependent_buy_repository.py` | `python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_dependent_buy.py -q`=`44 passed`；`backend/tests/miniqmt_execution_runtime/test_kernel_product_materialization_postgres.py`六个 disposable DEV PostgreSQL modes覆盖ACCOUNT WAIT、SELL terminal BLOCK、settled TRADE RELEASE、EOD residual、commit-unknown/restart和same-event concurrency；line/branch=`86.08/74.53`与`85.44/71.10`；PR #3120 / merge `48bcf1153dea3dca5c598793d6556d2757ed87de`、`source_merge=merged_pr_3120` | implemented_verified + merged | none |
...
| `F-105` | §5；K6-A immutable migration + `miniqmt_execution_kernel_k6c_20260802.*` + `miniqmt_execution_kernel_k6b_20260803.*`；`preflight_k6b_schema()` | `backend/tests/miniqmt_execution_runtime/test_kernel_k6_migration_postgres.py` exact DEV migration=`1 passed`；production preflight/forward/independent readback已闭合，repository checks=`9/9 true`、catalog=`6eeff2d2.../ef09f8ab.../10ae5be0...`、authority columns=`NOT NULL`、successor trigger=`1`、coordination rows=`0`；PR #3120 / merge `48bcf1153dea3dca5c598793d6556d2757ed87de`、`source_merge=merged_pr_3120`、`production_ddl=applied_and_verified` | implemented_verified + merged | none |
| `F-106` | §6–§7；`kernel_product_materialization_repository.py` + `kernel_dependent_buy_repository.py` atomic writer/readback；BUG-953 exact mapping JSON/scalar CHECK | `AISTOCK_RUN_MINIQMT_K2_DEV_DB=1 python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_materialization_postgres.py -q`：C1 DEV PostgreSQL=`20 passed`保持；K6-B六模式公共 event-id seam覆盖single-writer、lock/CAS/fence、same-command release、WAIT/BLOCK/EOD、commit-unknown、restart、same-event concurrency、no-double-release与fresh readback；PR #3120 / merge `48bcf1153dea3dca5c598793d6556d2757ed87de`、`source_merge=merged_pr_3120` | implemented_verified + merged | none |
| `F-107` | §4.3、§9.1、§9.3；K6-D §3–§5 | `backend/tests/miniqmt_execution_runtime/test_kernel_product_cutover.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_product_runtime_integration.py` + DEV route/ALGO_START；PR #3146 / merge `33c09049e82c11cdbae7cd9b596b3666cb481349` | implemented_verified + merged | none |
| `F-108` | §9.2；K6-D §2.2、§6 | `backend/tests/miniqmt_execution_runtime/test_kernel_legacy_route_retirement.py` exact symbol absence + five-root AST/fresh-process capability；PR #3146 / merge `33c09049e82c11cdbae7cd9b596b3666cb481349` | implemented_verified + merged | none |
| `F-109` | §10；K6-D §7 | `backend/tests/miniqmt_execution_runtime/test_kernel_diagnostics.py`、`backend/tests/simulation_runtime/test_ops_api.py` + DEV route readback；artifact: `docs/operations/simulation_platform_operator_runbook_20260717.md`；PR #3146 / merge `33c09049e82c11cdbae7cd9b596b3666cb481349` | implemented_verified + merged | none |
| `F-110` | §11；K6-D §5/§8 | `python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_runtime_integration.py backend/tests/miniqmt_execution_runtime/test_kernel_clock.py -q`；callback/clock/EOD/binding isolation/direct/DEV/coverage；PR #3146 / merge `33c09049e82c11cdbae7cd9b596b3666cb481349` | implemented_verified + merged | none |
| `F-111` | §12–§13；K6-D §9–§10 | artifact: `docs/architecture/miniqmt_execution_kernel_k6d_final_route_cutover_f2_detailed_design_20260804.md`；four design artifacts + state separation；PR #3146 / merge `33c09049e82c11cdbae7cd9b596b3666cb481349` | implemented_verified + merged | none |
| `F-112` | §16–§17；K6-D §13–§14 | artifact: `docs/architecture/miniqmt_execution_kernel_k6d_final_route_cutover_f2_detailed_design_20260804.md`；formal DESIGN-COMPLIANCE-001；PR #3146 / merge `33c09049e82c11cdbae7cd9b596b3666cb481349` | implemented_verified + merged | none |
...
| --- | --- | --- |
| no simplified/subset/POC/placeholder/mock-only completion | pass | K6-B已通过真实event-id public seam和disposable PostgreSQL完成command_json、V2 ledger/proceeds、REJECT/DEFER lifecycle、same-command release及DB事务；最终route、旧调用删除和正常交易日验收仍明确归K6-D，未用legacy parity/bridge/shadow过渡物冒充overall完成 |
...
- K6-A/C0/C1/B/D implementation均为`implemented_verified + merged`；K6-D detailed design PR #3129与source PR #3146均已合入，K6-D `source_merge=merged_pr_3146`，K6 overall仍=`implementation_in_progress`。用户restart、runtime activation与normal-day observation必须分别闭合，不能由source merge代替。
- 2026-08-03 final-architecture-only revision：`design_revision_ready + merged`、`source_merge=merged_pr_3098`，merge `b986e8f655137e9aea88f17ea91d3eb153f1c017`。截至2026-08-05，K6源码切片按A/C0/C1/B/D口径已合入`5/5`（100%）；严格source验收项F-102..F-112=`11/12 implemented_verified + merged`，F-101保持`design_ready`；产品runtime cutover=`0%/not_run`。后续研发预算仅归属最终KERNEL_V2的runtime activation与正常交易日验收，legacy维护、parity、bridge、backport、C2和额外shadow预算=`0`。
...
- predecessor与K6-B successor production DDL=`applied_and_verified`，K6-D new DDL/DML/config/binding=`noop`，dependency gate=`noop`。用户此前完成的backend-main重启与BUG-953 post-restart receipt不代表K6-D生效；broker/service restart/runtime activation/normal trading day observation仍为`not_run`。
