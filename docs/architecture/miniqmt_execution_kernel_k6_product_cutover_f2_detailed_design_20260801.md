...
> Feature tier：`F2`。文档状态：`design_ready_local_pending_pr`；K6 implementation=`not_started`。
>
...
| `F-102` | §4.1 | target `backend/tests/miniqmt_execution_runtime/test_kernel_product_contracts.py` dependent-BUY strict/negative matrix | design_ready | none |
...
| `F-105` | §5 | target `backend/tests/miniqmt_execution_runtime/test_kernel_k6_migration_postgres.py` clean/reapply/drift/readback/rollback | design_ready | none |
| `F-106` | §6 | target `backend/tests/miniqmt_execution_runtime/test_kernel_k6_repository_postgres.py` atomicity/CAS/fence/concurrency/commit-unknown | design_ready | none |
...
- K6 detailed design：`design_ready_local_pending_pr`。
- K6-A/B/C/D implementation：`not_started`。
- product runtime：`not_switched`。
- `source_merge=pending_pr`；`close_sync=not_applicable_feature`；root sync/cleanup未执行。
