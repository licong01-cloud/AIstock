...
- 唯一下位实施合同为 [`miniqmt_execution_kernel_k6d_final_route_cutover_f2_detailed_design_20260804.md`](miniqmt_execution_kernel_k6d_final_route_cutover_f2_detailed_design_20260804.md)，当前code=`implemented_verified_local_pending_source_pr`；final coordinator、真实QMT snapshot callback、scheduler no-quote clock/EOD、binding isolation、legacy caller删除、source capability与只读diagnostics均已实现，source PR/CI/merge和runtime验收仍待完成。
...
| `F-107` | §4.3、§9.1、§9.3；K6-D §3–§5 | `backend/tests/miniqmt_execution_runtime/test_kernel_product_cutover.py`、`backend/tests/miniqmt_execution_runtime/test_kernel_product_runtime_integration.py` + DEV route/ALGO_START | implemented_verified | none |
| `F-108` | §9.2；K6-D §2.2、§6 | `backend/tests/miniqmt_execution_runtime/test_kernel_legacy_route_retirement.py` exact symbol absence + five-root AST/fresh-process capability | implemented_verified | none |
| `F-109` | §10；K6-D §7 | `backend/tests/miniqmt_execution_runtime/test_kernel_diagnostics.py`、`backend/tests/simulation_runtime/test_ops_api.py` + DEV route readback；artifact: `docs/operations/simulation_platform_operator_runbook_20260717.md` | implemented_verified | none |
| `F-110` | §11；K6-D §5/§8 | `python -m pytest backend/tests/miniqmt_execution_runtime/test_kernel_product_runtime_integration.py backend/tests/miniqmt_execution_runtime/test_kernel_clock.py -q`；callback/clock/EOD/binding isolation/direct/DEV/coverage | implemented_verified | none |
| `F-111` | §12–§13；K6-D §9–§10 | artifact: `docs/architecture/miniqmt_execution_kernel_k6d_final_route_cutover_f2_detailed_design_20260804.md`；four design artifacts + state separation | implemented_verified | none |
| `F-112` | §16–§17；K6-D §13–§14 | artifact: `docs/architecture/miniqmt_execution_kernel_k6d_final_route_cutover_f2_detailed_design_20260804.md`；formal DESIGN-COMPLIANCE-001 | implemented_verified | none |
...
- K6-A/C0/C1/B implementation均为`implemented_verified + merged`；K6-D detailed design PR #3129已合入，code=`implemented_verified_local_pending_source_pr`，K6 overall仍=`implementation_in_progress`。source PR/CI/merge、用户restart、runtime activation与normal-day observation必须分别闭合。
- 2026-08-03 final-architecture-only revision：`design_revision_ready + merged`、`source_merge=merged_pr_3098`，merge `b986e8f655137e9aea88f17ea91d3eb153f1c017`。K6源码切片按A/C0/C1/B/D口径已合入`4/5`（80%）；严格source验收项F-102/F-103/F-104/F-105/F-106=`5/12`，F-110=`implementation_in_progress`；产品runtime cutover=`0%`。后续研发预算仅归属最终KERNEL_V2的K6-D，legacy维护、parity、bridge、backport、C2和额外shadow预算=`0`。
