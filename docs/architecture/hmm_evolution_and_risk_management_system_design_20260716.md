...
> **日期**: 2026-07-16  
> **修订日期**: 2026-08-17
> **状态**: Phase 0 已完成；Phase 1 全部外部验收完成（F-006～F-010A verified）且 production v3 已激活。Phase 2 历史 B3/P6/TRAIN-STABILITY/TRANSITION-DWELL-B 继续保持 blocked 事实。P2-3A market-regime + pooled sector-relative jump v2 已以 `296/456` fits 和 `hmm_risk_jump_selection_unavailable` 正式判定 `NOT_AVAILABLE_FOR_PROMOTION`。P2-3B market regime + direct cross-sectional Ridge 亦已完成批准合同内的正式开发执行：producer `24e4ae79780e5bacdf34a3affb63d1db46f6d8a4`，market `152/152` fits 完成并选择 `lambda=4.0/seed=42`；L1 五个 alpha 的 15 个 fold fits 全部完成且指标覆盖均为 `116/116`，按批准顺序选择 `alpha=100.0`，但三折 median Rank IC=`-0.007807285873192439`、median spread=`0.0009441908663057883`，因 Rank IC 非正而在 `167/184` fits 以 `hmm_risk_rotation_development_effect_non_positive` fail closed，L1 final refit 与 L2 均未执行。正式 failure receipt canonical=`d3298654ed9f2080f4623c2c50721ebf9951d2034d42cfdfe225f36e4ee0fc45`；491个嵌套receipt hash、request、fit attempts与metric重聚合全部闭合，target/state unavailable均为0，因此这是时间稳定性不足导致的真实模型失败，不是数据、方向或执行器BUG。P2-3B正式状态为`NOT_AVAILABLE_FOR_PROMOTION`，不得进入P2-4、只交付market、跳过L1或降低阈值。基于fold-2/fold-3系数方向高度一致而产品效果反转的证据，v2.28只保留一个P2-3C候选：固定既有market K=2参数权威，以causal `risk_on|risk_off`对同一五项sector特征构造对称交互的条件Ridge；用户已批准精确D1～D6模型合同，但本次批准不包含源码实施、36-fit实验、P2-4或model/READY。F-011继续blocked，F-012设计已批准，F-013尚未交付；严格进度仍为`11/17=64.71%`，canonical模型、FULL_READY、COVERAGE_AVAILABLE和真实预测/API/UI仍为0。本次DDL/DML/dependency/runtime均为`noop`（§13）
...
3. **P2-3 产品验收精确合同与单一spike（P2-3A/P2-3B均已正式失败；P2-3C设计已获批待实施授权）**：P2-3A以真实横截面产品指标判定 `NOT_AVAILABLE_FOR_PROMOTION`。P2-3B按已批准D1～D6执行至`167/184` fits：market完整，L1全部五个alpha×三fold完整且覆盖116/116，选定alpha100后因median Rank IC为负、median spread为正而按双正向合同停止；L1 final refit与L2未运行，holdout/model/READY/DB/runtime均未触碰。两个候选都不得重跑相同grid、降低门禁、反转语义、删除level或把market局部成功当完整候选。用户已批准P2-3C market-conditioned Ridge精确合同：只用预注册的causal market state与原五特征交互解释跨时期斜率变化；源码和36-fit开发spike仍须另行授权。
4. **P2-4 canonical产品模型与状态闭合（未进入）**：P2-3B未形成冻结candidate，因此禁止访问untouched holdout或执行P2-4。未来只有新的用户批准模型合同先形成完整development candidate，才可沿用样本外横截面Rank IC、trending-fading spread、风险事件precision/recall、分阶段稳定性及coverage代表性裁决canonical family/model；不得用likelihood/occupancy或market局部结果宣称产品有效。
...
- **最小成本：USER_APPROVED_EXACT_CONTRACT**。三fold各执行一个固定参数market fit，L1/L2各执行`5 alpha × 3 folds + 1 final`，通过development后再执行一个market final fit，完整候选最多`36` fits；若L1双正向停止条件失败，则在`18/36`后fail closed，不执行L1 final、L2或market final。固定参数减少的是已由两次正式实验闭合的重复market搜索，不是复制旧artifact或省略产品level。
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v2.28 | 2026-08-17 | 用户批准P2-3C D1～D6唯一后续实验合同，源码与36-fit执行仍未授权。证据锚点为P2-3B alpha100 fold-2/fold-3系数cosine=0.985456但Rank IC与spread同时反转；唯一假设为market regime条件斜率。候选固定market lambda4/seed42并逐fold train-only重拟合，sector只增加五个market-sign交互，完整最多36 fits；D3/D4产品验收、untouched holdout与三状态边界不变。不得并行模型、复制旧path、越权实现或用设计提升11/17进度。 |
