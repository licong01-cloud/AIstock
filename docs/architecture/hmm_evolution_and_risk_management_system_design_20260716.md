...
> **日期**: 2026-07-16  
> **修订日期**: 2026-07-31
> **状态**: Phase 0 已完成；Phase 1 全部外部验收完成（F-006～F-010A verified）且 production v3 已激活。Phase 2 的数据/PIT/observation政策、D3-D7精确合同和Slice 0 model-preparation源码均已完成；固定单线程Conda `AIstock`环境已完成两次fresh-process正式制备，共`5184/5184` fits。D5仅选出`legacy_covfix:L1/seed=43`，该level在D6因`801980.SI` temporal evidence失败，其余三个family/level无eligible candidate；两family均blocked、READY=0。随后`C-008-B3-FORMAL-BLOCKER-DIAG-01`完成348/348 targeted fits与3-entry D6 no-refit replay；`C-008-B3-REMEDIATION-DIAG-02`又完成324/324 train profiles、163/163 completed entries与11/11 initialization sources的no-fit闭合，canonical=`48157a42…bb58`。用户已选择`C-008-B3-REMEDIATION-D1-B`；P1最小必要源码现已实现exact-zero authority、full20→19 projection、identity20 control、共享train-only HMM核心、16-attempt process与双process report状态机，未运行32 fits。D1-B controlled-refit执行与D5 19/20维comparability仍未获执行或语义批准。后续继续evidence-first：32-fit真实受控训练→D5兼容决策→证据支持时才实现mixed-dimension artifact/parser→受影响level正式训练与D5/D6；不先建设通用动态维度框架、调度器、API/UI或占位READY。F-011为`APPROVED_BY_USER_SOURCE_IMPLEMENTED_FORMAL_EXECUTED_DIAGNOSED_BLOCKED_MODEL_ACCEPTANCE`，F-012为`DESIGN_READY_USER_APPROVED`，F-013为`PENDING_UPSTREAM_MODEL_SET`；generator/job/API/UI/runtime未完成。Phase 3只有跨阶段方向，仍待独立实现级F2设计；Phase 4+仍待独立设计。按Design Acceptance Matrix的17个独立验收行（F-001～F-016，另含F-010A）严格计数，当前完成`11/17=64.71%`，不把设计选择、局部源码、受控训练或diagnostic completion算作完整交付。Gate truth：既有`hmm_evolution_v1/v2/v3` production `applied_and_verified`；本次P1源码不改变DDL/dependency/runtime gates，均为`noop`（§13）
...
controlled-refit结果分类；P1现已实现exact-zero authority、固定20→19 projection、identity20 control、共享train-only HMM核心与
controlled-refit process/report，但未运行32 fits，也未实现正式mixed-dimension READY writer/runtime/API/UI。F-012保持设计批准，F-013继续等待完整READY
...
| F-011 | Phase 2 F2详细设计 §4～§9；`backend/services/hmm_risk/{state_model_set,b3_acceptance,b3_training,b3_remediation_diagnostic,b3_d1_inactive_dimension,observation_eligibility,stock_fact_observation,stock_fact_repository}.py`；`scripts/hmm_risk/prepare_state_model_set.py` | formal artifact canonical `e7992f87…39f`；blocker diagnostic canonical `10287e84…cffe8`；remediation DIAG-02 canonical `48157a42…bb58`；`backend/tests/hmm_risk/{test_b3_training,test_b3_acceptance,test_b3_remediation_diagnostic,test_b3_d1_inactive_dimension}.py` | APPROVED_BY_USER_SOURCE_IMPLEMENTED_FORMAL_EXECUTED_DIAGNOSED_BLOCKED_MODEL_ACCEPTANCE | 两family blocked、READY=0；D1-B P1最小必要源码已实现并完成直接审核，32-fit controlled-refit与D5 19/20 score comparability仍pending。执行采用evidence-first路径，禁止把源码或受控训练冒充family/READY完成，也禁止自动改阈值、扩大seed、排除family/sector或无依赖依据重跑完整grid |
