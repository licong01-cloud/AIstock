...
| F-011 | 父蓝图v2.38；Phase 2详细设计§4.3.4.2～§4.3.4.10、§23.35～§23.42；`industry_pit_adapter.py`、`market_relative_ridge_{candidate,holdout}.py`、`rotation_l1_input_bundle.py`及直接测试 | bundle direct tests与RW1 direct tests；601日preflight canonical `e5f204d4…6059`；HR1 parent report `d302afe3…3e44`、child failure `60b56d6e…590c`；首次RW1 artifact仍为0/24、无request/model/bundle | BLOCKED_INPUT_BUNDLE_SOURCE_IMPLEMENTED_PENDING_PR_AND_FORMAL_BUILD | IB-D1～D6已批准且源码完成三轮审核；下一步只闭合实现PR、一次正式bundle build/readback与原24-fit。canonical product bundle、CAPABILITY_AVAILABLE和真实API/UI仍为0 |
...
- **数据资产方向：USER_APPROVED_BLUEPRINT_SCOPE_SOURCE_IMPLEMENTED_PENDING_PR**。已有versioned H5/Qlib Bin继续是优先底层历史数据资产，C-013/PIT与typed missing authority继续是语义sidecar；从属F2详细设计IB-D1～D6已获批准并实现repo-external、不可变、一次构建的最小HMM input bundle。若正式build发现既有资产或批准sidecar不足，必须报告精确缺口并停止，不能回退数据库、当前行业映射、默认值、neutral或近似字段；任何补充authority仍须先修订设计并获得用户确认。
