...
observation/stage/candidate PostgreSQL writer 和外部 receipt store；但所有写入仍限定在 Advisory
Phase 1 自有表和仓库外制品目录。
...
Phase 1G代码必须基于Phase 1F.1 v2 schema实现。Phase 1F.1的详细设计为
`advisory_phase1f1_observation_partition_schema_forward_migration_f2_design_20260714.md`；它修复当前v1
全局content-hash唯一约束与lineage/candidate未分区问题。本设计任务不执行该DDL。v2 schema就绪后，
Phase 1G可完成代码、disposable PostgreSQL和事务型DEV rollback验证；真实persistent DEV L4仍必须
等待single Alpha与原生multi Alpha的immutable DSE/receipt。禁止用fixture、复制、手写DSE或replay
假装完成真实L4。
...
1. 只消费 exact Phase 1F.1 v2 schema receipt 和 `downstream_ready=true` 的 catalog；不执行 DDL。
...
9. 输出 DB `capture_receipt_hash` 与仓库外 content-addressed Phase 1G receipt，供 Phase 1H 填槽。
...
- Phase 1F release receipt 与 runtime-safe schema verifier 的只读消费。
...
- capture membership、selected observation mappings、plan-set hash 和 Phase 1G receipt。
- 单 target 与多 target batch CLI；target 隔离、非零退出和有价值日志。
- pure、disposable PostgreSQL、transactional DEV 和真实 persistent DEV 验证设计。

### 3.2 Non-goals

- 不执行 schema migration、DDL、GRANT/REVOKE、role 或 database backup。
...
6. 当前没有 target execution request/result、selected observation mappings 或 Phase 1G CAS receipt。
7. 现有 capture repository 方法各自持有事务，无法保证 observation rows、membership 和 delivery event
   在同一 plan transaction 中原子提交。
8. outbox natural key 不含 capture attempt，而 envelope 含 batch/fencing；recovery 必须复用前序 attempt
   已写的 exact outbox，不能生成第二份冲突 trace。
...
```text
external Phase 1F receipt + catalog SELECT
...
app.advisory_source_availability_event SELECT as-of
  ...
  -> external Phase 1G immutable receipt
```
...
### 6.1 Phase 1F.1 schema receipt

Phase 1F v1 DEV receipt仍是准确的历史发布证据，但其contract不包含父设计所需的observation
partition layout，因此不能被Phase 1G直接消费。每个run必须显式携带Phase 1F.1 v2 receipt：

```text
release_schema_receipt_path
release_schema_receipt_hash
target_label = DEV | PRODUCTION
```

加载后必须验证 v2 receipt canonical hash、target database identity、
...
lineage/candidate identity+partition payload、compatibility views以及非唯一content-hash indexes。

不接受v1 receipt、旧receipt、partial additive或catalog猜测。这是防止对缺表/漂移schema执行DML的
自动技术事实，不是审批。合法v2 schema自动通过；Phase 1G不拥有release executor reference，也不
自动修复schema。
...
### 6.3 Immutable DSE/artifact/package projection
...
### 7.4 `Phase1GObservationCaptureReceipt`

```text
schema_version = advisory_phase1g_observation_capture_receipt_v1
target_request_hash
...
SUCCESS 必须同时满足 batch `COMPLETE`、DB readback 全部一致、plan set/membership/mapping hashes
闭合且 errors 为空。失败 receipt 不假装 complete，也不能删除已经提交的 immutable rows。

### 7.5 Batch receipt

batch receipt 只汇总有序 target receipt refs：
...
`PostgresObservationCaptureWriter.append()` 每个 plan使用一个短事务：

1. `FOR UPDATE` 锁定 RUNNING capture batch，核对 row version、fencing token、lease和 request hash。
2. `FOR KEY SHARE` 核对 capture plan、source revision set、control binding和 exact outbox row。
...
8. 原子插入 version、lineage、stage evidence、stage candidates。
9. 同事务追加当前 batch 的 TRACE_OUTBOX、SOURCE_REVISION_SET、OBSERVATION_VERSION memberships。
10. 同事务追加/复用 `OBSERVATION_WRITTEN` delivery event并递增 batch row version。
11. commit后新连接完整 readback；不一致返回 post-commit verification failure，保留已提交事实。
...
- COMPLETE：完整 readback后幂等返回同一 receipt。
...
`complete()` 形成 DB `capture_receipt_hash`。随后用新只读连接重算全部 hashes，再写外部 receipt。
...
ADVISORY_PHASE1G_RECEIPT_STORE_FAILED
...
backend/services/advisory_phase1/phase1g_receipt_store.py
backend/services/advisory_phase1/control_binding.py              # additive current/get-or-append primitive
backend/services/advisory_phase1/trace_outbox.py                  # additive exact read/recovery primitive
backend/services/advisory_phase1/capture_foundation.py            # additive caller-owned tx primitives
...
Phase 1G代码批次本身不新增migration；所需Phase 1F.1 forward migration必须按独立详细设计先实现并
进入release contract。不得把该migration藏进Phase 1G startup/worker，或用JSON receipt、不分区表、
修改hash语义等简化方式绕过schema parity。

### 15.1 Import boundary

Phase 1G 可以 import Advisory Phase 0A/1 pure DTO/projection和 `backend.db.pg_pool`，但禁止 import：
...
static transitive import test必须验证上述边界，不能只扫描直接 import。
...
verify-receipt
  --receipt <json>
  --env-file <path optional for DB readback>
  --target-db dev|production
```

`plan`只读，输出 target dispositions和 materialized request hashes；`capture`才执行 Advisory DML。
...
- Phase 1G changed paths无DDL/migration、角色/RBAC/approval/authorization/backup hook；Phase 1F.1为
  独立发布批次和验证证据。
- 无 Selection/inference/Paper/simulation/QMT/QE/Qlib调用。
- schema/version/reason/CLI option/registry hash静态检查。
- Design Acceptance Index和 F2 validator通过。

### 17.2 L1 Pure

- target/batch request和 receipt canonical hash/tamper/order tests。
...
在 pinned PostgreSQL 16一次性数据库中应用完整Phase 1F.1 v2 schema后验证：
...
3. exact retry零新增 observation/version/stage/candidate，receipt一致。
4. 合法 source/artifact revision创建单一 observation successor。
5. 中途失败当前 plan事务零残留，已提交前序 plan保留，batch FAILED。
6. recovery复用 predecessor outbox/observation并完成新 batch。
7. lease/fencing/two-writer/commit-response-loss/post-commit-readback failure。
8. target A失败、target B成功，summary PARTIAL_FAILURE且CLI非零。
9. DB query spy证明无 shared业务表写入、无 market DML、无 runtime DDL。
10. 每个 database/container销毁，无 DEV/production连接或残留。
...
### 17.6 Production Gates (All Noop) / 生产状态

代码合入不自动执行生产 DML或启动任务。未来生产执行单独报告：

```text
phase1f1_production_ddl = not_executed
phase1g_production_dml = not_executed
runtime_activation = none
role_or_approval_gate = none
```

以上是执行状态，不是程序门禁或审批。生产schema未达到exact v2时，Phase 1G生产capture返回schema
不兼容且不自动执行DDL；合法v2 schema和完整业务输入无需角色或人工审批即可通过程序逻辑。
...
| 模块 | 读 | 写 | 行为影响 |
|---|---|---|---|
| Advisory Phase 1F | receipt/catalog | none | 只读 schema guard |
...
| filesystem | external plan/receipt | CAS no-replace | Phase 1G evidence only |
...
- target/batch request、target/batch receipt、capture policy registry；
- Phase 1F receipt/catalog guard；
- external CAS no-replace plan/receipt store；
- L0/L1。
...
- caller-owned capture transaction primitives；
- observation/version/lineage/stage/candidate/membership/delivery atomic writer。

### G4：Service, CLI And Recovery

- per-target service、multi-target batch、state transitions、normal rerun recovery；
...
- F-702：只消费exact Phase 1F.1 v2 ready receipt/catalog，runtime无DDL executor或自动修复。
...
- F-718：DB与external receipt content-addressed、atomic no-replace、完整readback。
...
- F-722：Phase 1G批次无migration、dependency、API、UI、scheduler或startup hook；Phase 1F.1独立实现。
...
- F-727：Phase 1G receipt完整填充Phase 1H所需observation output slots。
...
|---|---|---|---|---|
| F-701 | §3-5、§19 | import/write-scope oracle | design_ready | none |
| F-702 | §6.1、§15 | schema guard/import denylist | design_ready | none |
| F-703 | §6.2、§7 | plan/operation hash tests | design_ready | none |
| F-704 | §6.4、§8 | same-cutoff replay parity | design_ready | none |
| F-705 | §3.2、§6.4 | zero source-event INSERT scan | design_ready | none |
| F-706 | §5、§6.3、§9 | call/import spy | design_ready | none |
| F-707 | §6.3、§9、§17 | dual-track pure/PostgreSQL tests | design_ready | none |
...
| F-718 | §7.4-7.5、§12 | CAS store/collision/readback | design_ready | none |
...
| F-724 | §17.6 | production gate report | design_ready | none |
...
| F-727 | §7.4、§12 | Phase 1H slot closure | design_ready | none |
...
Phase 1G代码开始条件：Phase 1F.1 schema代码已完整实现并在disposable PostgreSQL通过；persistent DEV
DML验证还要求Phase 1F.1 DEV plan/apply/new-verify/new-exact-reapply完成。production DDL不是Phase 1G
DEV开发条件。

Phase 1G代码可请求合入的条件：

1. G1-G4完整实现，L0-L2与transactional DEV zero-residue验证通过。
2. DESIGN-COMPLIANCE-001逐项具有实现/测试证据。
3. 没有用fixture/in-memory-only结果冒充persistent real input完成。
4. production gates准确报告，代码合入不执行生产DDL/DML或runtime activation。
