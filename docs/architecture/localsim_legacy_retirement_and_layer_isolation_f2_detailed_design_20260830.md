...
BUG-1298收紧该合同：`simulation_daily_run`的canonical economic projection保留run identity、状态、计划/提交计数和其余业务字段，但排除`updated_at`及`run_payload_json.pre_run_failure`、`submit_failure`、`simulation_scheduler_retry_control_v1`等仅表示重试观测次数/时间的operational churn；这些字段不得让零broker call、零intent的pre-run retry伪装成经济事实变化。缺失admission receipt时的fallback只由immutable legacy-account/release/binding/package/manifest authority派生，不再依赖可变化的economic hash。`apply`在任何DML前重新读取exact inventory；若candidate漂移则零写入。独立post-write readback失败时，工具只允许对本次调用新建、仍为`PREPARED/version=1`、created-by/account/lineage/economic hash全部精确匹配且无replay/native-scope/binding consumer的bundle执行自动补偿删除；任一身份或依赖不符都fail loud并保留现场。此前已遗留的partial bundle只能通过独立`repair-partial`模式、完整expected identity和单独的`AUTHORIZE_LOCALSIM_LINEAGE_REPAIR:<target>:<database>:<date>:<legacy-account>:<lineage-id>:<account-id>`授权修复，随后必须独立证明account/lineage均不存在，禁止宽泛delete、覆盖immutable lineage或把失败readback记为成功。
...
- `inventory` 只读列出当前统一 Simulation Runtime 仍认领的 LocalSIM account/release/binding/ledger scope、经济事实 hash、in-flight、claim、legacy session/auto-run/sentinel owner；terminal/failed/orphan 不得成为 retained candidate。调用方必须显式冻结 `authority_trade_date`；对每个 retained account 按 `effective_from <= authority_trade_date` 选最大 `effective_from` 的唯一 broker-bound daily binding，跨交易日历史 `SIM_VALIDATING` binding 必须保留且不构成歧义，同一最大生效日存在多条或只有 future binding 时 fail closed；
...
- daily roll-forward 后 portfolio 上的历史 `manifest_sha256` 不是当前 runtime authority；retained account 的 package 必须仍与 portfolio 一致，但 manifest/release/binding identity 必须由上述唯一 daily binding 与其 release 共同给出并彼此精确一致，不得回写 portfolio、删除历史 binding 或任意按创建时间兜底；
- economic-facts digest 必须按真实所有权覆盖全部九类事实：具备 `portfolio_id`/`strategy_id` 的表直接按 ledger scope 过滤；`paper_v2.fills` 不得假设不存在的 `portfolio_id`，必须通过权威 FK `fills.run_id -> run.run_id` 后按 `run.portfolio_id` 归属，并仅哈希原始 fill row 的稳定有序 JSON；不得丢弃 fills、复制 scope 字段或用宽泛异常回退为空；
- `preflight` 验证 B2 production schema与C ledger-scope bridge的comment/FK/index readback、source merge identity、冻结的 `authority_trade_date`、零 legacy writer/claim/in-flight、每个 retained candidate 唯一且经济事实 hash 稳定；
- `apply` 仅在用户对精确 production target、B2 migration、lineage DML、`authority_trade_date` 与 retained account 集合明确授权后执行，并逐 account 调用同一 `LocalSimControlPlaneService.prepare_legacy_lineage` transaction；不复制/更新 economic rows；exact authorization token 必须绑定 target/database/date/account set；
- `readback` 以同一 `authority_trade_date` 重新计算 candidate、lineage、account package/manifest、release/binding、ledger row-count/hash 和 owner inventory，生成不含敏感值的 commit-bound receipt；重复 apply 只能得到 exact same lineage，任何 authority/economic drift 整体 fail loud。
...
| `F-142` | §4.5.1、§4.6.4、§4.8、§4.8.1、`SIM-LR-C`；PR #4035 / `f2eb8ed03`；BUG-1291/1293/1298 | `pytest backend/tests/simulation_runtime/test_localsim_cutover_inventory.py backend/tests/scripts/test_prepare_localsim_successor_cutover.py -q`；`python -m nox -s localsim_successor_core_dev_db`；BUG-1298覆盖canonical daily-run economic projection、pre-write drift、post-write compensation及exact partial repair | sim_lr_c_source_complete | explicitly approved user production-remediation boundary：B2/C schema已应用且强readback通过；首次lineage apply写入1个`PREPARED` bundle后因retry-observation hash漂移被独立readback拒绝，未宣称DML闭合。必须先合入BUG-1298、精确授权修复该partial bundle，再基于新main重新授权apply/readback；用户重启与runtime evidence仍严格后置 |
...
| R37 | BUG-1291 production-shape retained binding复核 | 生产只读证据发现保留账户合法累积64条按日封闭的`SIM_VALIDATING` binding，原inventory把全部非RETIRED行当作单一当前authority且错误要求portfolio历史manifest等于最新release。已改为显式`authority_trade_date`、最大生效日唯一解析、同日歧义/future-only fail closed、release+binding manifest authority、package连续性、date-bound授权及精确lineage readback；不删除历史binding或改写经济事实 | findings fixed; PR #4099 / `5214bd03a` merged; pending user restart |
| R38 | BUG-1293 production economic-scope schema复核 | B2/C production DDL与row/FK/hash readback完成后，真实inventory在任何lineage写入前暴露`fills.portfolio_id`不存在。schema/FK核查确认fills只能通过`fills.run_id -> run.run_id -> run.portfolio_id`归属。修复保留九类完整事实、只哈希原始fill row、稳定排序，拒绝跳过fills或空结果fallback；生产account/lineage仍为0 | findings fixed in source; direct/DEV/CI and merge pending; production DML blocked |
| R39 | BUG-1298 production partial-apply与mutable retry复核 | 首次apply插入1个`PREPARED` account/lineage后，独立readback发现`simulation_daily_run`的`FAILED_RETRYABLE`观测时间/次数持续刷新，原始全row hash将operational diagnostics误作经济事实并留下partial bundle。修订以canonical economic projection排除三类retry observation与row `updated_at`、用immutable authority派生fallback receipt、DML前双读candidate、写后失败只补偿本调用新建且未消费的exact bundle，并为既有partial增加独立精确授权repair；生产旧ledger row不更新 | source fix and repeated review pending; production partial bundle preserved until separately authorized repair |
