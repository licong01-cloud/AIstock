...
| `F-134` | `backend/services/simulation_data/`、`backend/services/simulation_signal/`、`backend/services/simulation_execution/broker.py`、Selection/Paper controller owner migration | `pytest backend/tests/simulation_architecture/test_layer_import_boundaries.py`; direct matrix `337 passed`; `pytest backend/tests/paper_trading_v2` = `452 passed, 1 skipped, 2 xfailed` | stage_a_source_complete | explicitly approved Stage A source boundary; source PR、merge、restart/runtime evidence remain pending |
| `F-135` | data contracts、global calendar adapter、DailyContext owner、TDX causal provider、completed-day historical provider | `pytest backend/tests/simulation_data` included in Stage A direct contract matrix `19 passed`; current-day DB rejection and zero-DB TDX assertions pass | stage_a_source_complete | explicitly approved Stage A source boundary; normal trading-day runtime/query receipt remains pending |
| `F-136` | signal evidence owner、StrategyPackage selection owner、immutable target/rebalance services、Selection/Paper side-effect separation | `pytest backend/tests/simulation_signal/test_target_rebalance_isolation.py`; Selection/StrategyPackage cases included in `337 passed` matrix | stage_a_source_complete | explicitly approved Stage A source boundary; unified successor runtime consumption remains pending `SIM-LR-B` scope |
...
| `F-145` | `backend/tests/simulation_architecture/`、`backend/tests/simulation_data/`、`backend/tests/simulation_signal/` | `pytest backend/tests/simulation_architecture backend/tests/simulation_data backend/tests/simulation_signal` = `19 passed`; `python -m ruff check`、compileall、`git diff --check` and direct no-drift matrices pass locally | stage_a_source_complete | explicitly approved Stage A source boundary; changed-files/ownership/CI receipts bind to final PR head before merge approval |
...
| --- | --- | --- |
| `no_simplified_delivery` | pass：交付完整 Stage A owner 迁移、data/signal/execution contract、Selection 副作用上移、旧 V25/previous-close/current-day DB-minute 能力删除和直接边界测试；未用空目录、facade 或 allowlist 代替迁移 | `pytest backend/tests/simulation_architecture backend/tests/simulation_data backend/tests/simulation_signal` = `19 passed` |
...
| `no_business_semantic_drift` | pass：现有 API route、Paper 创建副作用、MiniQMT KERNEL_V2、StrategyPackage admission payload/schema、订单/成交/资金/持仓 writer 均未变；只迁移 owner 和 import | 核心 no-drift matrix `337 passed`；`pytest backend/tests/paper_trading_v2` = `452 passed, 1 skipped, 2 xfailed` |
