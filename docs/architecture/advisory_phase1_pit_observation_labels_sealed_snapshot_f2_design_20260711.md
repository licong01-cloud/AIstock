...
- Phase 1G开工一致性复核另发现Phase 1F.1 outbox唯一键及capture-gap identity均未包含scope，会阻断或串联同一Selection证据被多个独立Program合法消费。Phase 1F.2唯一实施级设计为`docs/architecture/advisory_phase1f2_scope_aware_trace_identity_forward_migration_f2_design_20260715.md`；代码已通过PR #2144及BUG修复PR #2146/#2150合入，2026-07-15 DEV与production均已plan/apply/new-verify/new-exact-reapply，exact v3为`COMPATIBLE/downstream_ready=true`，catalog fingerprint为`95600e18fbe4a4026f24a374e66289b7e530c874a95a203db2b738855a6a580a`。这是schema技术事实，不是审批、角色或人工门禁。
...
- 唯一父级实施设计为`docs/architecture/advisory_phase1g_source_observation_capture_dml_f2_design_20260714.md`。G1-G4代码已分别由PR #2158、#2167、#2178、#2191合入；G5唯一实施级设计为`docs/architecture/advisory_phase1g_g5_dev_evidence_f2_design_20260716.md`，代码已由PR #2217合入并完成L0-L2。2026-07-16真实DEV inventory准确得到`L3_SOURCE_PENDING`，因此L3、L4均未执行；O2随后仅完成真实single/native-multi package闭包导入，Program、dated binding、DSE v2、historical receipt和Phase 1E计划仍待O3/O4形成。真实persistent DEV L4仍只接受Phase 1E形成的single/native multi Alpha immutable DSE/receipt，不用fixture或L3临时证据冒充。
