...
三个问题沿方法论“方法演进与首轮研究”执行：既有中长期候选复核、近重复候选替换优势、一项两融机制可行性。先构造测试，再少量真实范围比较；扩样依据不确定性来源，不凑成功或候选数量。两融时点未确认交数据 owner，不伪造或开启新数据同步。

每题记录廉价结论、是否及为何扩样、实际成本与有限复核结果、信息/使用价值、下一步。真实结论可以不支持或不确定；没有诊断时会怎样做只能引用事前记录，不凭事后叙事估出方法收益。研究结束、工具可用、方法有效、生产可用是四种不同状态，前三题至多给方法改进线索。
