...
日期：2026-09-11；版本：1.2；F1 单研究模块设计与历史实现回执。依据[蓝图 v1.2](factor_research_evolution_blueprint_20260908.md)和[方法论 v2.1](../analysis/factor_research_methodology.md)。v1.0 完成设计，v1.1 实现比较能力（§12 保留）；v1.2 的原始文档交付状态保留在 §7/§13.4～13.5，后续 BUG-1437/1440/1442 实现状态只在 §13.1/§13.6 更新。真实全量数据评价仍待独立执行。以下既有接口保持兼容；不执行正式因子晋升、QE 实验、生产数据库操作或进程控制。
...
## 13. v1.2 完整评价实施增补（BUG-1430 设计；BUG-1437/1440/1442 实施）

### 13.1 最小改造与 owner

完整评价定义仅引用[方法论完整评价](../analysis/factor_research_methodology.md#full-evaluation)。BUG-1430 当时只改三份文档、四个客户端入口及 BUG 元数据；该历史边界和已有 P1/P2、§12 回执不追溯改写。后续实现仍沿用两表、runner、context_json/records、官方指标引擎及 attach，不加新表/状态机/平台，不启动 QE 实验或数据导出。

| 交付项 | 实现归属与最小方案 | 本轮状态 |
|---|---|---|
| 全目标 PIT 读取与分母 | 共享读取 owner 修复 `qe_eval_v2_qlib_reader._read_instruments` 的同股后段覆盖前段，保留全部历史资格区间；研究层以请求目标网格保留缺失计数，不按已读列缩小分母 | BUG-1440 / PR #4544 已合入；运行时待用户重启，不改 PIT 规则/数据文件 |
| 完整评价编排 | 研究 runner/CLI 复用现有范围参数展开全目标股票与全有效历史；技术预检与完整评价在原 context_json 中说明用途，旧 run 可用于预检，不把旧样本结果重标 full | BUG-1437 实施；显式 `full_evaluation` 不改变无该字段的旧路径 |
| 多窗口/期限独立指标 | 指标 owner 复用现有引擎，提供实际日期明确的全期、2024起、年度、近期/月度结果和 h1/h5/h10/h20 口径；研究层汇总既有字段，缺能力不自己复制第二套引擎 | BUG-1442 / PR #4551 已合入；默认官方 writer 语义不变，运行时待用户重启 |
| 候选对存量相关 | FR-REQ-01 owner 提供指定成对子矩阵及有效配对范围；本批候选×候选/全部存量可评价因子，不重算未受影响的存量×存量 | BUG-1442 指定子矩阵已合入；BUG-1437 按显式 reference artifact 清单分批编排 |
| 结果与解释 | 研究层复用 metrics/records/attach，保存日期/池/口径/完成与缺失项、近期优先报告和两轴结论；既有真实结果复用，错误结论另作关联更正 | BUG-1437 实施；真实全量评价仍须独立运行，不写官方 metrics/correlations/catalog |
...
### 13.6 BUG-1437 实现合同与完成边界

BUG-1437 只在现有 runner 的显式 `full_evaluation` 请求上启用完整评价编排；未声明该字段的既有执行与 attach 路径保持不变。请求必须列出 repo 外、非链接的存量因子值文件及相关计算参数。实现复用官方指标引擎的只读多窗口/全期限接口和相关引擎的指定成对子矩阵，不复制指标公式、不写官方表，也不触发数据导出。

独立指标窗口包括：全部实际有效历史、2024年至截止日、2024年起各自然年、最近6/3/1个月，以及2024年起逐月实际交易日窗口；所有窗口同时计算 h1/h5/h10/h20。相关性只对全期、2024起、年度和最近6/3/1个月计算候选×候选及候选×请求列出的存量参考因子，分批加载参考文件，不计算存量×存量，也不为逐月窗口重复全库相关。输出分别记录指标窗口与相关窗口的实际日期、请求/可用/不可用因子对分母和 typed reason；attach 必须验证这些已声明结果与执行请求一致，不能把缺失因子对静默丢弃。

该实现交付只证明完整评价工具可运行且旧路径兼容。F-307 仍须由真实市场全目标 PIT、全有效历史和全部存量参考目录的执行结果满足；运行失败或参考清单不完整均不能给出因子入库结论。候选输出、评价报告与研究记录继续位于 repo 外研究产物和两张研究表，`official_database_writes=0`。数据集、官方单写指标/相关/目录、QE/数仓、生产 DDL/DML 与进程控制均不在本 BUG 范围内。
