...
| Review-13 | CI 可达性 | 新增 profile/UI-contract/comparison/MCP 测试与两个脚本尚无直接 test-plan 路由，CI classifier fail closed | 增加精确 `qe.active_dataset_universe` ownership；扩展既有 `qe_read_backend` 直接覆盖本次变更的 MCP 合同 nodeid，并保留 `qe_mcp_backend` 独立合同覆盖；不新增流水线或审批 | resolved |
