...
## 9. 待评审确认点
1. **映射粒度**:采用「roster=task,(窗口×topk)run=loop」(���方案推荐),还是「单 run=task,scheme=loop」?
2. **截面语义修正(§3.6)**:轨迹横轴改「配置(窗口×topk)」+ 默认不连线 + SOTA 改「最优配置」——是否采纳(推荐采纳,避免误导);还是坚持照搬时序视觉(承担误读风险)。
3. **裁剪清单(§3.7)**:Agent分析Tab / rerun-retry-append / 编辑配置 / 拓扑连线 在 combine 分支隐藏——是否确认。因子 Tab 改造为「roster 腿+权重」是否确认。
4. **macb 操作集 + 是否页内重跑**:适用操作(删除/导出/无);列表页直接重跑涉及写操作,会突破只读边界并需撞车防护,是否纳入(默认不纳入,属独立 issue)。
