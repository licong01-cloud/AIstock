...
| 设计项 | 缺陷 | 实现文件 | 测试 |
|---|---|---|---|
| 代码智能接入 | DEF-10 hard_pass | `scripts/code_intelligence_adapter.py`(真复用)、`code_intelligence_core.py`、`code_intelligence_adapter_provider.py`、`code_context_refs_repository.py`、`service.py:build_context_pack`、`agent_teams/runtime.py`、`assistant_code_context_refs`; final commit `PENDING_FINAL_HEAD_AFTER_COMMIT`; G1-central `PENDING_G1_CENTRAL` | `test_code_intel_true_reuse.py`, `test_code_intel_context_injection.py`, `test_code_intel_decomposition.py`, `test_code_context_refs_ddl_contract.py`, `test_code_intel_determinism.py`, `test_code_intel_token_safe.py`, `test_code_intel_evidence_contract.py`, `test_code_intel_not_test_replacement.py` |
...
| 编号 | 用户要求 | 设计位置 | 验收标准 |
|---|---|---|---|
| DAI-CODE-001 | code intelligence helps LLM understand task/module impact | L1.6 §16.4 | hard_pass: code query pack has non-empty `code_context_refs` with provenance/as_of, and L3 worker inputs consume them; final commit `PENDING_FINAL_HEAD_AFTER_COMMIT`; G1-central `PENDING_G1_CENTRAL` |
| DAI-CODE-002 | deterministic AST-derived code graph, no embedding | L1.6 §16.4 | hard_pass: core has no embedding/vector/similarity runtime dependency; manifest sort/path/canonical bytes stable; final commit `PENDING_FINAL_HEAD_AFTER_COMMIT`; G1-central `PENDING_G1_CENTRAL` |
