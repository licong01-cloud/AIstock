...
| MCP Gateway Phase 5 manifest/catalog + UI/audit | DAI-GND-001/002 / ANTI-DRIFT-02/11 | `backend/services/research_assistant/mcp_catalog_sync.py`、`backend/services/research_assistant/service.py`、`backend/routers/research_assistant.py`、`frontend/src/app/research-assistant/mcp-tools/page.tsx`、`frontend/src/lib/research-assistant/api.ts`；5a commit `24021029` + 5b commit `43da6fbc`；G1 `research-assistant_20260604_111424_l4_mcp-gateway-phase5-assistant_1096055f_runner-validation__cf8153448f` | `test_ra_manifest_catalog_consumption.py`、`test_phase5_mcp_audit.py`、`test_tool_catalog_gate.py`、`test_worker_tool_isolation.py`、`test_core_no_adapter_import.py`、`frontend/tests/research-assistant/phase5-mcp-gateway-ui.spec.ts`、`python -m nox -s mcp_gateway_phase5_assistant` |
...
| Prompt Lab | DEF-12 | `backend/services/research_assistant/prompt_lab.py`, `backend/services/research_assistant/service.py::run_prompt_lab_offline`, `backend/db/migrations/ra_upgrade/007_prompt_lab_runs.sql` / `assistant_prompt_lab_runs` | `backend/tests/research_assistant/test_prompt_lab_gepa_offline.py` / `backend/tests/research_assistant/test_prompt_lab_judge_gated.py` |
| 技能库 | DEF-12 | `backend/services/research_assistant/skill_library.py`, `backend/services/research_assistant/service.py::deposit_successful_workflow_skill`, `backend/services/research_assistant/qe_autonomy/runtime.py` L4 经验回放, `backend/db/migrations/ra_upgrade/008_skill_library.sql` / `assistant_skill_library` | `backend/tests/research_assistant/test_skill_library.py` |
...
| DAI-LEARN-001 | 自我学习/提示词自改进 | L7 §16.6 | Reflection Card 写入 `personal.episodic.*`；Prompt Lab 离线候选；Skill Library 将成功 workflow 沉淀为可审批 recipe |
| DAI-LEARN-002 | 自改进受门禁 | L7 §16.6 | Prompt/Skill 仅候选或 draft；approval 后才可激活或复用；技能复用仍走 Action Proposal / preflight / approval |
| DAI-DRIFT-002 | 增补项无漂移 | §16.8/§16.9 | 矩阵无空行；消费/门禁断言存在 |

> Phase 7 cross-check note: Section 16.10 DAI-CODE/REPORT/LEARN items are explicitly classified in research_assistant_phase7_expected.yaml; Phase 8 code intelligence, Phase 9 proactive report, Phase 10 Reflection Card, Phase 11 Prompt Lab, and Phase 12 Skill Library have moved to hard_pass; DAI-DRIFT-002 remains hard_pass via the Phase 7 cross-check evidence.
...
> Phase 10 implementation note: DAI-LEARN-001 and the DEF-12 Reflection Card portion are implemented by deterministic Reflection Card generation in `backend/services/research_assistant/reflection_card.py`, the failure/correction/low-confidence trigger path `ResearchAssistantService.generate_reflection_card`, and persisted `assistant_reflection_cards` DDL in `backend/db/migrations/ra_upgrade/006_reflection_cards.sql`. The implementation writes approved low-risk `personal.episodic.*` memories for L1 recall, records source refs and safety flags, hides internal rationale, and does not activate prompt/strategy changes or high-risk action proposals; Prompt Lab and Skill Library complete the remaining DEF-12 submechanisms in Phase 11/12.

> Phase 11 implementation note: DAI-LEARN-002 and the DEF-12 Prompt Lab portion are implemented by offline GEPA/DSPy-style candidate generation in `backend/services/research_assistant/prompt_lab.py`, the gated service entrypoint `ResearchAssistantService.run_prompt_lab_offline`, and persisted `assistant_prompt_lab_runs` DDL in `backend/db/migrations/ra_upgrade/007_prompt_lab_runs.sql`. The implementation uses historical `assistant_trace_events` as the eval set, records `judge_score_json`, `reason_codes`, `warnings`, and `approval_request_id`, leaves status as `candidate`, and refuses prompt activation changes without the existing `assistant_approval_requests` gate; validation is covered by `backend/tests/research_assistant/test_prompt_lab_gepa_offline.py` and `test_prompt_lab_judge_gated.py`.

> Phase 12 implementation note: the DEF-12 Skill Library portion is implemented by deterministic recipe deposition in `backend/services/research_assistant/skill_library.py`, service gates `ResearchAssistantService.deposit_successful_workflow_skill` / `approve_skill_library_entry` / `propose_skill_reuse`, L4 curriculum replay via `backend/services/research_assistant/qe_autonomy/runtime.py`, and persisted `assistant_skill_library` DDL in `backend/db/migrations/ra_upgrade/008_skill_library.sql`. Successful workflows become `draft` skills with source_refs and provenance; approval is recorded through `assistant_approval_requests`; approved skill reuse creates an Action Proposal and must pass confirmation, preflight, and approval instead of direct execution. Validation is covered by `backend/tests/research_assistant/test_skill_library.py`.
