...
> Phase 8 G3 traceability anchor: DEF-10 hard_pass via `backend/services/research_assistant/code_intelligence_core.py`, `code_intelligence_adapter_provider.py`, `code_context_refs_repository.py`, `service.py:build_context_pack`, `agent_teams/runtime.py`, `agent_teams/models.py`, `backend/db/migrations/ra_upgrade/004_code_context_refs.sql`, `tests/aistock_validation/catalog/test_plans.yaml`, `module_registry.yaml`, `file_ownership.yaml`, and `noxfile.py`. Final implementation commit `5fc6ceb5e6b669a0eb44eccf0019389207ad13bd`; G1-central run_id `research-assistant-code-intelligence_20260602_164246_l4_ra-phase8-code-intel_870cf59f_runner-validation__fde5e4a6cb`; production_ddl_gate `required_pending_user_approval`; production `8001`/`3000`/`19080` and production DB not touched.
---
...
| 设计项 | 缺陷 | 实现文件 | 测试 |
|---|---|---|---|
| 代码智能接入 | DEF-10 hard_pass | `scripts/code_intelligence_adapter.py`(真复用)、`code_intelligence_core.py`、`code_intelligence_adapter_provider.py`、`code_context_refs_repository.py`、`service.py:build_context_pack`、`agent_teams/runtime.py`、`assistant_code_context_refs`; final commit `5fc6ceb5e6b669a0eb44eccf0019389207ad13bd`; G1-central `research-assistant-code-intelligence_20260602_164246_l4_ra-phase8-code-intel_870cf59f_runner-validation__fde5e4a6cb` | `test_code_intel_true_reuse.py`, `test_code_intel_context_injection.py`, `test_code_intel_decomposition.py`, `test_code_context_refs_ddl_contract.py`, `test_code_intel_determinism.py`, `test_code_intel_token_safe.py`, `test_code_intel_evidence_contract.py`, `test_code_intel_not_test_replacement.py` |
...
| 编号 | 用户要求 | 设计位置 | 验收标准 |
|---|---|---|---|
| DAI-CODE-001 | code intelligence helps LLM understand task/module impact | L1.6 §16.4 | hard_pass: code query pack has non-empty `code_context_refs` with provenance/as_of, and L3 worker inputs consume them; final commit `5fc6ceb5e6b669a0eb44eccf0019389207ad13bd`; G1-central `research-assistant-code-intelligence_20260602_164246_l4_ra-phase8-code-intel_870cf59f_runner-validation__fde5e4a6cb` |
| DAI-CODE-002 | deterministic AST-derived code graph, no embedding | L1.6 §16.4 | hard_pass: core has no embedding/vector/similarity runtime dependency; manifest sort/path/canonical bytes stable; final commit `5fc6ceb5e6b669a0eb44eccf0019389207ad13bd`; G1-central `research-assistant-code-intelligence_20260602_164246_l4_ra-phase8-code-intel_870cf59f_runner-validation__fde5e4a6cb` |
...
> Phase 8 final evidence note: DEF-10, DAI-CODE-001, and DAI-CODE-002 are promoted to hard_pass by `ra_phase8_code_intel`; validated clean HEAD `5fc6ceb5e6b669a0eb44eccf0019389207ad13bd` passed G1-central `research-assistant-code-intelligence_20260602_164246_l4_ra-phase8-code-intel_870cf59f_runner-validation__fde5e4a6cb`. Later evidence-only commits must be reported and revalidated separately instead of folding old runner evidence into a new HEAD.
