...
下一优先级按正式结果调整为：先以独立预注册对照拆开历史特征热身可见性与当日交易资格，减少完整人口unknown并保持同人口BH；随后才对C2“风险退出保留、无加速止盈”做外部验证。再后研究50～500亿元/PIT业绩条件及模型动作价值：筛选后仍与同一人口BH比较，避免将选股收益错记择时；不得假定财报可见历史已经齐备。不得沿本轮结果搜索止盈比例。QE/HMM/Agent仍不在正确性主线之前，prospective/实时行情不阻塞历史研究。
...
结论：历史首发和模型/形态研究事实保留，仍无可发布的成本后择时alpha支持。PT-NEXT-021已在r7/WSL完成10策略全量横向实验：C2显著提高暴露并降低回撤，但仍落后同股BH，54项family-wise比较无正下界；完整人口继续因unknown不可识别。下一步先解决历史特征热身与交易资格耦合，再独立验证C2方向，不按结果扫描阈值；其后才评估PIT基础筛选与模型增强。无QE/HMM/Agent融合前置，无DB/在线运行/重启/激活。
