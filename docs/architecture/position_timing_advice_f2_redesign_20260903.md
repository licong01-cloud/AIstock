...
> 日期：2026-09-11
> Feature tier：F2
> 状态：`FIRST_RELEASE_RUNTIME_VERIFIED_PT_NEXT_018_SNAPSHOT_FROZEN_FORMAL_PENDING_NOT_SERVING`
...
### 9.21 工程实现及只读快照完成、正式历史回放待运行：PT-NEXT-018 形态择时与后续优化

权威单模块详细设计为[自选股与持仓股形态择时研究设计](position_timing_pattern_strategy_design_20260911.md)，使用独立F1验收索引。工程现已落实同股突破日锚定的等待回踩对照、真实因果持仓上的加速放量退出对照，以及完整规则组合相对buy-and-hold/L1的成本后连续路径；同一离线管线还包含8模板月度选择和core/core+pattern双头模型。实际新增四个职责文件与四个直接测试文件，未修改其他模块，也未新增API、页面、调度器、数据库表或运行服务。

MA5/MA10、清仓等仍为待正式检验的原型假设；等待窗口、容差、减仓比例和衰竭确认只在已冻结R0～R7中由过去12个完整月份选择。一个顶层request同时冻结原型family=4与演进family=5，训练尝试和历史数据重复使用如实记录；当前只有固定训练股前2只的开发烟测，不能据此宣称有效。正式运行所需新评价64股已在source-only阶段冻结，但现有公司行动/停牌snapshot缺少该人口；下一步是获得用户对生产库只读、REPEATABLE READ快照的单独授权，而不是等待最新交易日、HMM、MDE或收益审批。候选场景研究队列及QE只读融合继续后置。

详细设计v1.3 §9.2记录当前执行读回：形态直接测试现为29项、公司行动适配器直接测试9项，完整模块本地回归为286项通过；开发烟测实际跑通34条标签、8模板开发路径、118次模型fit尝试与外层历史预测。多轮审核已经修复现金不足、持仓PIT、净/毛费用、除权停牌日估值、同日独立实施方案、UNKNOWN与联合覆盖、artifact文件集等正确性问题。正式request/bundle、原型证据、优化证据、模型增量证据和selected仍均为`NOT_RUN`；没有发布模型或修改L1/L1a，DB写入、runtime与restart均noop。
...
| F-048 | `pattern_strategy.py`、`pattern_research.py`、`pattern_optimizer.py`、`pattern_model.py`；§9.21与独立F1设计 | 四个`backend/tests/position_timing/test_pattern_*.py`；完整模块pytest；F1/F2 validator；正式bundle仍未运行 | PATTERN_TIMING_ENGINEERING_VERIFIED | none |
...
4. **禁止私增门禁审批**：无样本/MDE、最低金额、HMM/事件、券商核验、sealed holdout 或人工审批阻断。因果/数量/身份错误只阻止对应无效计算或提醒；收益支持控制证据措辞与正式模型政策，不阻断研究、实验分析和 L1 发布。PT-NEXT-012～017 均在单一实施块直接使用历史数据完成；PT-NEXT-018也已在不等最新交易日/HMM的情况下完成工程、开发烟测与授权只读快照，下一步直接运行正式历史request，不依据MDE或点估计放行。held-out coverage 不足只禁止夸大支持态，不阻止工程合入。L4b-2 仍范围外。
...
7. **PT-NEXT-018 专项复核**：新增代码全部位于`backend/services/position_timing/`与直接测试；复用既有纯成交/成本/公司行动/停牌/模型函数但不改其默认值或入口。一个顶层request事前冻结两个family共9项正式比较；UNKNOWN或任一开发、训练、评价、外层覆盖不完整均不能进入SUPPORTED。开发烟测只证明路径可达，正式收益仍为NOT_RUN；旧研究、N0、timing registry/current、card/event/alert/order、数据库与运行态保持不变。

结论：首发 L1/L1a 规则能力已上线，L2 v1 无入选模型，L4b-1 暂无 eligible prospective action-card；它们继续为持仓与显式自选提供正式人工建议和非阻塞旁证。PT-NEXT-004～011 的 action-value core、公司行动/停牌连续路径、分钟覆盖审计及 ATR/行业/资金流/筹码单块历史研究均已完成，但没有形成可发布的联合 alpha 证据；PT-NEXT-012～017 又依次定位并修正了弱 OOF 响应、ENTRY/EXIT 不对称、跨股票评价、空仓 ENTRY 标签外推 ADD、持仓状态监督与入场时点目标问题，相关正式区间仍不可分辨。PT-NEXT-018截至v2.35已完成形态原型、有界模板选择、模型条件化的隔离工程实现和冻结128股的授权只读公司行动/停牌快照，但正式评价尚未运行，不能改变“没有成本后超额收益支持证据”的现状，也不发布正式模型策略。下一动作是以含快照修复的干净提交生成并运行已冻结request，不等待最新交易日、不追加候选。production DDL/DML/dependency/restart/activation 均为 noop；BUG-1403 的源码、用户重启后生产加载、只读 identity/API 验收及 close-sync 已完成，且不改变模型支持态。
