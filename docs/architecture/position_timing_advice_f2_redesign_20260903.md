...
> 日期：2026-09-11
> Feature tier：F2
> 状态：`FIRST_RELEASE_RUNTIME_VERIFIED_PT_NEXT_018_RIGHTS_ISSUE_CLASSIFIED_REPAIR_PENDING_NOT_SERVING`
...
### 9.21 工程与请求前源审计完成、配股与历史因子源待修复：PT-NEXT-018 形态择时与后续优化

权威单模块详细设计为[自选股与持仓股形态择时研究设计](position_timing_pattern_strategy_design_20260911.md)，使用独立F1验收索引。工程现已落实同股突破日锚定的等待回踩对照、真实因果持仓上的加速放量退出对照，以及完整规则组合相对buy-and-hold/L1的成本后连续路径；同一离线管线还包含8模板月度选择和core/core+pattern双头模型。实际新增四个职责文件与四个直接测试文件，并在`pattern_research.py`内增加request前全量因子—公司行动覆盖审计；未修改其他模块，也未新增API、页面、调度器、数据库表或运行服务。

MA5/MA10、清仓等仍为待正式检验的原型假设；等待窗口、容差、减仓比例和衰竭确认只在已冻结R0～R7中由过去12个完整月份选择。一个顶层request同时冻结原型family=4与演进family=5，训练尝试和历史数据重复使用如实记录；训练股开发烟测不能据此宣称有效。前三个正式request及其人口均不可变保留：第三次虽通过artifact inspect和exact retry，但因`300506.SZ`未绑定factor跳变只有63/64评价覆盖，仍不得读取9项比较值。第四批64只评价股已经在source-only阶段确定且公司行动/停牌快照已冻结，但新增预检在128股697个material factor区间中发现4个未绑定区间，因此没有生成第四个request。

详细设计v1.8的只读复核已把四项准确分类：`000970.SZ`、`600008.SH`、`601236.SH`均为正式公告确认的配股，`688109.SH`为与第三批`300506.SZ`相同的全历史factor重述拼接问题。三次配股的记录日/缴款期/除权复牌日分别与异常区间对齐，且现有`market.dividend`快照均无行；配股是自愿现金认购，不能按自动送股或factor数量倍数回放。当前Tushare已将`300506.SZ`与`688109.SH`查询区间各自统一到新factor，本地及2026-09-05/`20260911-r3`候选却仍保留旧/新基准接缝，故r3不能替代修复。数据所属模块须提供PIT配股authority、完整历史修复两只factor并重建新不可变候选；position_timing只在交付后冻结候选/基线共同的配股参与政策并按原人口重放，不在本模块手工补源、不等待最新交易日、HMM、MDE或收益审批。候选场景研究队列及QE只读融合继续后置。

详细设计v1.8 §9.2记录当前执行读回：开发烟测实际跑通34条标签、8模板开发路径、118次模型fit尝试与外层历史预测；多轮审核已修复现金不足、持仓PIT、净/毛费用、除权停牌日估值、公司行动规范化、UNKNOWN/联合覆盖、递归artifact文件集和晚发现因子覆盖等正确性问题。第三次receipt只证明训练输入与artifact路径可达，评价不完整使原型、优化和模型增量证据继续不可用；合入提交`34c18f974a1e4734ff7a83ac95df073da6dab847`上的第四批预检再次以`PATTERN_FACTOR_ACTION_COVERAGE_INCOMPLETE`返回，`outcomes_read=false`、request文件数仍为3且没有新文件。生产库复核使用显式readonly事务并rollback；两个candidate逐值对照、Tushare当前查询与三份官方配股公告均只作源诊断，没有发布模型或修改L1/L1a，生产DB写入、runtime与restart均noop。
...
| F-048 | `pattern_strategy.py`、`pattern_research.py`、`pattern_optimizer.py`、`pattern_model.py`；request v2因子—公司行动预检；§9.21与独立F1设计v1.8 | 四个`backend/tests/position_timing/test_pattern_*.py`；完整模块pytest；F1/F2 validator；前三次bundle均为不可变不完整证据；第四批source-only预检697/693/4；`EVID-PATTERN-RIGHTS-SOURCE-DIAGNOSIS-20260911` | PATTERN_TIMING_ENGINEERING_SOURCE_PREFLIGHT_AND_DIAGNOSIS_VERIFIED | none |
...
2. **禁止静默错误**：保留 v1 typed PIT/source/scope/quote/outcome 语义；新设计追加历史可见版本、晚间 cutoff、forward-label 可用时间、no-fill 与 unknown、模型失败 fallback 的明确区分。PT-NEXT-014 将 candidate 缺失但 DB 明确存在的停牌键作为版本化 source correction，将仍缺公司行动证据的股票保留为 coverage 不足；二者均不静默删股。PT-NEXT-015 的首个 request 在计算前因身份比较和 CLI 错误序列化 typed 失败、无 bundle；PT-NEXT-017 的首个 request 同样因延后入场现金不足 fail closed。两者修复均使用新代码身份和新 request，不改写旧 request 或吞掉错误。PT-NEXT-018进一步把配股与factor历史重述拆开：不将factor跳变冒充账户认购或自动送股，也不将新候选名冒充数据已修复。CPCV 不冒充历史部署收益，未预算的方向不冒充成本后可执行建议。
...
4. **禁止私增门禁审批**：无样本/MDE、最低金额、HMM/事件、券商核验、sealed holdout 或人工审批阻断。因果/数量/身份错误只阻止对应无效计算或提醒；收益支持控制证据措辞与正式模型政策，不阻断研究、实验分析、工程合入和 L1 发布。PT-NEXT-012～017 均在单一实施块直接使用历史数据完成；PT-NEXT-018也未等待最新交易日/HMM，并已连续执行三次不可变历史run。request v2的因子—公司行动预检只防止把来源错误或账户事件未定义的复权跳变算成收益，不能按收益、MDE或人为审批放行，也不得以换股/缩短历史伪造覆盖。配股authority与共同参与政策是正确计算所需输入身份，不是额外审签。coverage 不足不阻止工程合入、L1/L1a或其他独立研究，但修复前不得对该人口产生伪收益或宣称alpha。L4b-2 仍范围外。
...
7. **PT-NEXT-018 专项复核**：新增代码全部位于`backend/services/position_timing/`与直接测试；复用既有纯成交/成本/公司行动/停牌/模型函数但不改其默认值或入口。一个顶层request事前冻结两个family共9项正式比较；UNKNOWN或任一开发、训练、评价、外层覆盖不完整均不能进入SUPPORTED。前三次正式run都保留原人口和失败身份，未按其比较值选下一批；第四批在不读收益时由全量因子—公司行动预检提前发现4个源缺口，不发布request、不做无效长跑。只读复核已以公告把三项定为配股、以DB/Tushare/candidate对照把一项定为factor历史重述拼接；没有在本模块手工造配股行、修改candidate/DB或选择账户政策。旧研究、N0、timing registry/current、card/event/alert/order、数据库与运行态保持不变。

结论：首发 L1/L1a 规则能力已上线，L2 v1 无入选模型，L4b-1 暂无 eligible prospective action-card；它们继续为持仓与显式自选提供正式人工建议和非阻塞旁证。PT-NEXT-004～011 的 action-value core、公司行动/停牌连续路径、分钟覆盖审计及 ATR/行业/资金流/筹码单块历史研究均已完成，但没有形成可发布的联合 alpha 证据；PT-NEXT-012～017 又依次定位并修正了弱 OOF 响应、ENTRY/EXIT 不对称、跨股票评价、空仓 ENTRY 标签外推 ADD、持仓状态监督与入场时点目标问题，相关正式区间仍不可分辨。PT-NEXT-018截至v2.37已完成形态原型、有界模板选择、模型条件化、不可变历史管线、请求前全量源审计及四个源缺口的只读分类；前三次run均因不同完整性问题不是收益证据，第四批未生成request，因此仍不能改变“没有成本后超额收益支持证据”的现状，也不发布正式模型策略。下一动作由数据所属模块提供PIT配股authority、完整历史修复`300506.SZ/688109.SH`因子并重建新不可变candidate；择时模块再冻结共同配股参与政策，以同一第四批人口生成request。不等待最新交易日、不追加候选、不修改形态规格。production DDL/DML/dependency/restart/activation 均为 noop；BUG-1403 的源码、用户重启后生产加载、只读 identity/API 验收及 close-sync 已完成，且不改变模型支持态。
