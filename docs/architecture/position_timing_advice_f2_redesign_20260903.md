...
> 日期：2026-09-10
> Feature tier：F2
> 状态：`FIRST_RELEASE_RUNTIME_VERIFIED_PT_NEXT_015_FROZEN_NOT_RUN`
...
### 9.18 进行中：PT-NEXT-015 标签支持对齐的 OPEN-only 历史确认

本项作为一个实施块完成：先绑定父 v4 训练状态与 PT-NEXT-014 实际 ADD 动作形成零收益读取的结构诊断，再在更早全部 request 股票之外冻结第三批 64 股，复用既有训练与连续 replay 运行唯一 OPEN-only policy，最后回填 immutable receipt。代码面只允许 `action_value_advice.py`、`action_value_research.py`、`action_value_heldout.py` 的最小扩展、新增一个 timing-owned 离线文件和直接测试，以及本蓝图；不新增 router、页面、模型服务、调度器、数据库表、外部模块改动或运行接线。
...
| F-045 | `action_value_advice.py` OPEN-only authority；`action_value_open_only.py` 结构诊断、第三批 held-out 人口与 immutable bundle；复用既有训练/replay | §6.16；§9.18；§10.17；`backend/tests/position_timing/test_action_value_open_only.py`；正式 receipt 待运行 | DESIGN_VERIFIED_NOT_RUN | none |

## 15. DESIGN-COMPLIANCE-001 最终复核

1. **禁止简化交付**：已完成首发、L2 v1 与 L4b-1 的历史事实/receipt 保留；PT-NEXT-004/005 已交付真实代码、正式训练、公司行动连续策略、个股影子推断、API/UI 与覆盖内 256 条分钟核对；PT-NEXT-006/007/008 已分别完成单一冻结 ATR14、行业、资金流块的 matched-core 正式回放。PT-NEXT-009 没有沿用不可识别的空估计，也没有取交集，而是绑定显式停牌证据后按原规格纠错重放；PT-NEXT-011 先冻结单一筹码字段与完整 source-only coverage，再在同一实施块交付代码和真实历史结果，不以 coverage 冒充收益。PT-NEXT-012 已读取四份真实不可变 OOF 与连续路径生成零 trial 正式 receipt，并把描述性诊断与收益证据分开。PT-NEXT-014 真实读取 21 份 prior request、冻结零重叠人口、重训并完成 cross-symbol 历史回放；未把 source-only 清点或测试冒充 held-out 收益。绝不把总体摘要、mock、测试或诊断切片当收益结果。代码交付、研究支持与运行激活分开。
2. **禁止静默错误**：保留 v1 typed PIT/source/scope/quote/outcome 语义；新设计追加历史可见版本、晚间 cutoff、forward-label 可用时间、no-fill 与 unknown、模型失败 fallback 的明确区分。PT-NEXT-014 将 candidate 缺失但 DB 明确存在的停牌键作为版本化 source correction，将仍缺公司行动证据的股票保留为 coverage 不足；二者均不静默删股。CPCV 不冒充历史部署收益，未预算的方向不冒充成本后可执行建议。
3. **禁止改变业务逻辑**：实现日频模型闭环、同股净超额目标、core-first、前向训练、连续回补和本地推断；PT-NEXT-006/007/008/011 只增加显式 optional 研究契约，不修改默认 core hash、共享 defaults、旧 v1 卡/receipt 或既有模块。行业编码只用于 PIT join；资金流只用 canonical CNY、连续全局窗口与 T-1 滞后；筹码块只用同源日 raw close/cost50 距离并整体 T-1；PT-NEXT-009 只更正停牌源身份，PT-NEXT-010 只把连续起点的已有持仓改为外生归一化库存。四个 optional block 的 selected 均为 0、均未发布 serving policy。PT-NEXT-012 证明 optional 字段会改变动作但整体 OOF 排序仍弱，并暴露 ENTRY/EXIT 不对称；PT-NEXT-013 因而只改变离线 policy authority。PT-NEXT-014 只更换评价股票而不改变冻结训练/模型/政策/成本/统计；v2 source correction 只补显式停牌证据。两项均不改共享默认行为或运行卡，正式结果仍不 serving。
4. **禁止私增门禁审批**：无样本/MDE、最低金额、HMM/事件、券商核验、sealed holdout 或人工审批阻断。因果/数量/身份错误只阻止对应无效计算或提醒；收益支持控制证据措辞与正式模型政策，不阻断研究、实验分析和 L1 发布。PT-NEXT-012～014 均在单一实施块直接使用历史数据完成，没有等待最新交易日或外部模块，也没有新增平台、等待窗口或部署审批链；held-out coverage 不足只禁止夸大支持态，不阻止工程合入。L4b-2 仍范围外。

结论：首发规则能力已上线；L2 v1 无入选模型，L4b-1 无 eligible prospective action-card。PT-NEXT-004/005 已完成工程与两次权威离线研究迭代，并能对持仓/显式自选生成逐股实验建议；最终 v4 已闭合公司行动连续回放、停牌连续性、覆盖内分钟审计与重复研究交付身份，但两个主比较的校正区间仍跨零，联合结论为 `INCONCLUSIVE`，selected 为 0。PT-NEXT-006 的 ATR14 增量 CI `[-0.5001,+1.0733] bps/日` 仍跨零、selected 为 0。PT-NEXT-007 的历史 receipt 曾因共同的 `688200.SH/INITIAL_HOLDING_UNAVAILABLE` fail-closed 为 `INCONCLUSIVE`；PT-NEXT-010 纠正已有持仓 estimand 后恢复 118,900 条无 path error 的 paired path，行业相对强弱增量为 `-2.0307 bps/日`、95% CI `[-3.1596,-0.9821]`，正式收敛为 `NEGATIVE/selected=0`。PT-NEXT-008 的历史 receipt 如实保留 `002211.SZ/TARGET_BAR_INVALID`，PT-NEXT-009 以显式 DB 停牌 snapshot 更正源身份；PT-NEXT-010 再纠正共同初始持仓后恢复 137,924 条无 path error 的 paired path，资金流增量为 `-0.0803 bps/日`、95% CI `[-0.8132,+0.6273]`，仍为 `INCONCLUSIVE/selected=0`。PT-NEXT-011 的同一唯一筹码成本假设也已完成 137,924 条无 path error 的 paired path；增量为 `+0.4141 bps/日`、95% CI `[-0.4496,+1.2214]`、MDE `0.8638`，正式结论 `INCONCLUSIVE/selected=0`，累计点估计不被包装成 alpha。PT-NEXT-012 进一步用零 trial 描述性诊断确认可选块会显著改变动作、整体 OOF 响应仍接近零且 ENTRY/EXIT 表现不对称；PT-NEXT-013 已据此完成唯一 entry-only 假设，两个 co-primary 区间仍跨零，联合 `INCONCLUSIVE/selected=0`。PT-NEXT-014 再用与所有既有 action-value request 股票零重叠的 64 股做 cross-symbol held-out：显式停牌纠错后 `path_unknown=0`，但一只股票仍缺完整公司行动证据；两个主比较 97.5% 区间分别为 `[-1.2254,+0.9102]` 与 `[-7.1135,+13.0313] bps/日`，即使不考虑覆盖约束也均跨零，联合仍为 `INCONCLUSIVE/selected=0`。因此截至本版仍没有可称为 alpha 的支持证据，也不发布正式模型策略；下一研发方向不能继续沿同一 core/同一 policy 盲目加因子，应先基于现有跨股票阴性/不可分辨证据重新审视标签、动作价值估计和可执行政策的结构，同时继续以历史回放为主、prospective 为非阻塞旁证。production DDL/DML/dependency/restart/activation 均为 noop；BUG-1403 源码、用户重启后的生产加载、只读 identity/API 验收及 close-sync 均已完成，且不改变模型支持态。
