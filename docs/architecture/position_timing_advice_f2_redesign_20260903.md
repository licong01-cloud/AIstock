...
条目1～12为各历史版本交付事实，不代表本次重跑测试／运行态；其中真实账户authority要求仅适用于相应旧版本。条目13为v2.49本次纯文档设计复核。F-054～F-058的`DESIGN_VERIFIED`只验收文档合同，不代表未来代码、Oracle、分钟审计、模型或收益已交付；工程待办见§9.27。
...
结论：历史首发和模型／形态研究事实保留，仍无可发布的成本后择时alpha支持。PT-NEXT-021/022/023未获支持的事实不变；PT-NEXT-023在较高暴露下改善合成回撤，但未证明逐股联合目标或信号独立于仓位的风险管理价值。后续按§9.27完成逐股评价／被动现金基准、分钟执行敏感性与Oracle诊断，再选择一个恢复或轻量动作价值候选，最后在共同测试区间做连续回放；已有策略暂不淘汰，以机制证据决定有限重访。日频训练只读对齐QE冻结时间合同，分钟优先服务执行／风险测试；P2/P3、未来行情和其他模块不阻研发。新方向仅设计完成，不改DB、在线运行、重启或激活。
