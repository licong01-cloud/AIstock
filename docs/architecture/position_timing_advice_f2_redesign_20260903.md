...
> 日期：2026-09-09
> Feature tier：F2
> 状态：`FIRST_RELEASE_RUNTIME_VERIFIED_PT_NEXT_011_CHIP_COST_SINGLE_BLOCK_FROZEN`
...
- **已冻结 `PT-NEXT-011` 单一筹码成本信息块**：只比较 `CHIP_MEDIAN_COST_DISTANCE_LAG1_V1` 与 matched core；一个字段、一个假设、一个正式 request，不从筹码字段库或历史结果回选变体。结果无论正负都结束本次 family；只有 `SUPPORTED` 才允许另立运行接线任务。
...
### 9.14 进行中：PT-NEXT-011 单一筹码成本距离块

本项保持一个实施块完成设计、模块内源码、历史回放、证据回填和合入。先以不读 outcome 的 source-only 审计冻结 §6.12 的唯一字段、T-1 可见性、64 股人口与覆盖，再实现 candidate-local reader 和既有 single-block pipeline 的最小 profile 扩展；代码与设计形成干净提交后才生成不可变正式 request 并运行唯一 paired replay。收益结果只决定证据分类与未来是否值得另立运行接线任务，不阻塞本项工程合入，也不触发同 family 搜索。
...
| F-041 | §6.12 筹码 reader、单字段 feature contract、既有 single-block profile 与隔离边界 | `backend/tests/position_timing/test_action_value_chip.py`；`backend/tests/position_timing/test_action_value_incremental.py`；§9.14；§10.13；source-only coverage `108652/108634`、`outcomes_read=false`；正式历史 receipt 待本任务同块回填 | DESIGN_VERIFIED | none |
...
3. **禁止改变业务逻辑**：实现日频模型闭环、同股净超额目标、core-first、前向训练、连续回补和本地推断；PT-NEXT-006/007/008/011 只增加显式 optional 研究契约，不修改默认 core hash、共享 defaults、旧 v1 卡/receipt 或既有模块。行业编码只用于 PIT join；资金流只用 canonical CNY、连续全局窗口与 T-1 滞后；筹码块只用同源日 raw close/cost50 距离并整体 T-1；PT-NEXT-009 只更正停牌源身份，PT-NEXT-010 只把连续起点的已有持仓改为外生归一化库存。ATR 没有支持性下界；初始持仓修正后行业块完整区间为负并正式分类 `NEGATIVE`，资金流增量 CI `[-0.8132,+0.6273] bps/日` 仍跨零。既有三块 selected 均为 0、均未发布 serving policy；筹码块也不改变当日或未来 L1 正式卡。
