...
> 日期：2026-09-14
> Feature tier：F2
> 状态：`FIRST_RELEASE_RUNTIME_VERIFIED_PT_NEXT_019_FORMAL_REPLAY_INCONCLUSIVE_NOT_SERVING`
...
### 9.21 r4正式回放完成且不可分辨：PT-NEXT-018 形态择时与后续优化
...
| F-048 | PT-NEXT-018 | HARD | 单一离线request冻结R0四项原型比较与R0～R7/双特征集模型五项演进比较；新评价人口排除全部prior request与训练股，PIT/公司行动/停牌/逐腿费用/UNKNOWN及开发-训练-外层覆盖fail-closed。配股绑定独立typed authority与候选/基线共同账户政策，历史factor重述须由新不可变candidate关闭；不得在择时内手工补源。只写timing-owned research bundle，无API/UI/serving/registry/current/card/order/DB/runtime写入 |
...
| F-048 | `pattern_strategy.py`、`pattern_research.py`、`pattern_optimizer.py`、`pattern_model.py`、`pattern_rights_issue.py`；request v3与supersession lineage；§9.21与独立F1设计v2.1 | 五个`backend/tests/position_timing/test_pattern_*.py`；完整模块pytest；F1/F2 validator；最终request `2a8cdf...cb6f` source 696/696、三层64/64、九项`INCONCLUSIVE`、selected=0；bundle inspect/exact retry | PATTERN_TIMING_FORMAL_REPLAY_VERIFIED_INCONCLUSIVE | none |
...
4. **禁止私增门禁审批**：无样本/MDE、最低金额、HMM/事件、券商核验、sealed holdout 或人工审批阻断。因果/数量/身份错误只阻止对应无效计算或提醒；收益支持控制证据措辞与正式模型政策，不阻断研究、实验分析、工程合入和 L1 发布。PT-NEXT-012～018 均在单一实施块直接使用历史数据完成；PT-NEXT-018未等待最新交易日/HMM，r4最终64/64历史重放已结束。因子—公司行动预检只防止把来源错误或账户事件未定义的复权跳变算成收益，不能按收益、MDE或人为审批放行，也不得以换股/缩短历史伪造覆盖。配股authority与共同参与政策是正确计算所需输入身份，不是额外审签；九项`INCONCLUSIVE`只阻止宣称alpha和发布serving，不阻止源码合入、L1/L1a或其他独立研究。L4b-2 仍范围外。
...
7. **PT-NEXT-018 专项复核**：新增代码全部位于`backend/services/position_timing/`与直接测试；复用既有纯成交/成本/公司行动/停牌/模型函数但不改其默认值或入口。一个顶层request事前冻结两个family共9项正式比较；UNKNOWN或任一开发、训练、评价、外层覆盖不完整均不能进入SUPPORTED。r4 reader只消费明确candidate与typed authority，统一`NEVER_SUBSCRIBE`政策在收益前冻结；技术supersession保持相同人口、日期、阈值、比较和先验请求集合，只修复JSON身份及停牌跨企业行动日参考价。最终source 696/696、rights=3、三层64/64且无排除，九项区间全部跨0、selected=0，故只保留研究证据。没有抓取数据、读取或修改数据库、重建/激活candidate、换股或改变策略；旧研究、N0、timing registry/current、card/event/alert/order、数据库与运行态保持不变。
