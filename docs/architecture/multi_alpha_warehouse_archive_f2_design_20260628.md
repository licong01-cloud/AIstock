...
│  multi_alpha_leg    : 腿级**物化快照**(leg_id/因子集/模型/窗口/来源 seed 元数据) │
...
┌───────────────▼──────────────────────────────────────────────┐
│ 分析/UI 层                                                    │
│  UI 详情面板(每腿配置/来源/叠加)读数仓物化快照               │
│  数仓 SQL/视图: 跨实验统计、roster 演进、scheme 对比          │
└──────────────────────────────────────────────────────────────┘
```

### 2.2 关键设计原则
1. **腿级来源完整物化(用户决策)**:归档时把每腿的因子集 hash/因子清单、模型类型/family、训练窗口、freq/label_horizon、来源 seed run 的关键元数据**复制进 `multi_alpha_leg`**。seed run 即使被清理,roster 来源仍可从数仓完整复原。**不存裸指针**。
2. **叠加方式可复原**:`multi_alpha_scheme` 存 weighting_scheme + 静态 weights + per_window_weights(滚动权重全量),配合 normalize_method/walk_forward(run 级),完整描述"多腿如何叠加"。
3. **双链路 + 幂等**:实时 outbox(新 run)+ backfill(历史/补偿);两链路写同一套数仓表,以 `(run_id)` 幂等 upsert,重复归档不产生脏数据。
4. **避开 outbox 黑洞覆辙**(记忆 qe_archive_paper_outbox_bug):新增独立事件类型并**注册进 worker 白名单 + handler 真接生产**(非仅 tests 引用);macb 事件与 paper telemetry 不混写;消费失败显式 + 不可归档数据显式 skip(复用 `_claim_policy_skip_events` 模式),不留静默黑洞。
...
macb run succeeded/partial_failed/failed
  ...
  → qe_archive worker claim(白名单含新类型) → MultiAlphaCombineArchiveHandler
     → 读 macb 3 业务表 + 反查 seed_run_ids 对应 qe_archive.run 元数据(物化快照)
     → upsert qe_archive.multi_alpha_{run,leg,scheme,loo} + qe_archive.run 头
...
- macb 详情页(承接 `multi_alpha_combine_backtest_ui_reuse_design`):每个 loop(=窗口×topk run)可点开详情,展示:
  - **每腿**:leg_id、因子集(factor_names/hash)、模型类型、训练窗口、seed 来源(数量 + 来源 run）、provenance_complete 标识。
...
- 数据源:优先读数仓物化快照(历史可查);实时未归档时回退读业务表(标注"未归档")。
...
| ID | 设计条目 |
|---|---|
| F-001 | qe_archive macb 四表 schema(run/leg/scheme/loo)+ COMMENT |
...
| F-010 | UI 腿级详情面板（配置/来源/叠加/失败 reason）|
...
**Phase A — 数仓 schema + 物化归档(目标 2 核心,先保数据)**
1. DDL:qe_archive macb 四表 + COMMENT（F-001）；migration forward/rollback。
2. `partial_failed` 入业务表 status 约束 + reason.logical_status 回填（F-009 schema 部分）。
3. MultiAlphaCombineArchiveHandler：读业务表 + 反查 seed 物化腿快照（F-002/003/005）；幂等 upsert（F-007）；缺源显式 skip（F-008）。
4. outbox 事件 emit + 白名单注册（F-004）。
5. backfill macb 分支：补历史 28 run（F-006）；归档覆盖率报告（F-013）。
6. 隔离验证：evolution_loop/single_experiment/paper telemetry 零回归（F-014）。

**Phase B — UI 详情 + 分析面（目标 3）**
7. UI 腿级详情面板（F-010），读数仓物化快照。
8. 数仓跨实验分析 SQL/视图（F-011）。

**Phase C — 失败治理收口（目标 1 长期解）**
9. 对接远端派发 Phase 2 容量守卫跨来源统一（F-012，消除并发争抢失败）。
...
| L4 | UI E2E | 详情面板展示每腿配置/来源/叠加 + 失败 reason 截图;未归档回退标注 |
| L5 | 业务 oracle | 数仓 macb 指标 == 业务表 == UI 显示（CAGR/Sharpe/weights/marginal 逐值）；provenance_complete 真值核对（删 seed 后仍可复原）|
