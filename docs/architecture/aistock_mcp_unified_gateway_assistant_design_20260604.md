...
- 5a 后端切片（commit `24021029`）已完成实施内容 1、2、4：RA list/preflight/ReAct 目录源收敛到统一 manifest，`.mcp.json` canonical server_key 对齐，A2 只读 override 与 L2.5 external retrieval 自动只读已锁定；任务审计/UI/E2E 为 5b 范围，5a 不声明完整 Phase 5 完成。
- 5b / full Phase 5（commit `43da6fbc`，G1 run_id `research-assistant_20260604_111424_l4_mcp-gateway-phase5-assistant_1096055f_runner-validation__cf8153448f`）已完成实施内容 3、5 与完整 UI/E2E：任务审计写入 profile/tool/preflight/approval/evidence，`/research-assistant/mcp-tools` 展示统一 catalog source、manifest 工具数、risk/profile 分布、profile recommendation、preflight、approval pending、evidence refs 和 audit ledger；Playwright B-08、frontend lint/build 与 Validation Runner 均通过。
...
| --- | --- | --- | --- | --- |
| 全部 209 个工具进入 manifest（203 legacy + 6 catalog） | `backend/mcp/tool_manifest.py` | `pytest tests/mcp -q` 22 passed；self-check manifest_tool_count=209；G1 run_id `platform-mcp-gateway_20260604_064759_l2_mcp-gateway-manifest-quality_c3861e05_runner-validation__1ab0750fee` | PASS_R1 (`85ca5104`) | 无 |
| 默认 profile 为 lite | `.mcp.json`、`backend/mcp/profiles.py` | `python scripts/aistock_mcp_gateway_doctor.py --json` status=pass；default_profile=lite | PASS_R1_VERIFIED (`85ca5104`) | 本轮仅复验既有默认值 |
| validation 19 工具迁移 | `backend/mcp/modules/validation.py`、`backend/mcp/legacy_validation_adapter.py`、`backend/mcp/validation_issue_items.py` | inventory diff PASS；modules import 不再传递加载 `scripts.aistock_mcp_server` | PASS_R1 (`85ca5104`) | legacy BUG/GitHub wrappers 保留 lazy adapter 兼容入口 |
| QE 54 工具迁移 | `backend/mcp/modules/qe_*.py` | inventory diff PASS（26 experiment + 28 archive） | PASS_R1_VERIFIED (`85ca5104`) | 本轮未改 QE 业务模块，仅复验 inventory |
| 智能助手读取统一 catalog | `backend/services/research_assistant/mcp_catalog_sync.py`、`backend/services/research_assistant/service.py`、`backend/routers/research_assistant.py`、`frontend/src/app/research-assistant/mcp-tools/page.tsx`、RA API | `python -m nox -s mcp_gateway_phase5_assistant` 121 passed；frontend lint/build passed（仅既有 hook warnings）；Playwright B-08 1 passed；Validation Runner `valjob_20260604_111245_1096055f` return_code=0，run_id `research-assistant_20260604_111424_l4_mcp-gateway-phase5-assistant_1096055f_runner-validation__cf8153448f`；catalog=9 canonical servers/209 manifest tools，UI 显示 catalog source、risk/profile distribution、health/smoke 显式状态 | PASS_PHASE5_FULL (`43da6fbc`) | 5a 后端切片 + 5b UI/审计/E2E 全部完成；A3 stock_analysis 仍为后续方案 |
| 高风险工具 preflight/approval | `backend/mcp/tool_manifest.py`、`backend/services/research_assistant/mcp_catalog_sync.py`、`backend/services/research_assistant/service.py`、`backend/services/research_assistant/execution.py`、`backend/tests/research_assistant/test_phase5_mcp_audit.py`、`frontend/tests/research-assistant/phase5-mcp-gateway-ui.spec.ts` | `test_manifest_risk_no_write_as_readonly`、`test_external_research_l25_read_only_retrieval_stays_direct`、`test_phase5_mcp_audit.py`、Playwright B-08；RA preflight/action proposal 对 `assistant_usable=preflight_required` 保留 approval gate；UI 展示 approval pending/evidence/audit，external search/fetch 三工具 L2.5 read_only 自动执行，`external_research_save_evidence` 仍 preflight | PASS_PHASE5_FULL (`43da6fbc`) | 写/确认/长任务/外网写未放松；audit ledger 可还原 profile/tool/preflight/approval/evidence |
...
| Phase 6 resource monitor / client drift | `backend/mcp/gateway.py`, `scripts/aistock_mcp_gateway.py`, `scripts/aistock_mcp_gateway_doctor.py`, `tests/mcp/test_mcp_gateway_cli.py` | `python -m nox -s mcp_gateway_phase6_resource_monitor` PASS; startup summary schema `aistock_mcp_gateway_startup_summary_v1`; doctor `client_config_drift.status=pass`; process inventory reports live legacy child processes as warnings until old clients restart | PASS_PHASE6A_PENDING_COMMIT | Backend/script slice only; UI monitoring remains Phase 6b |
