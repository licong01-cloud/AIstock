...
| 智能助手读取统一 catalog | `backend/services/research_assistant/mcp_catalog_sync.py`、`backend/services/research_assistant/service.py`、RA API | `python -m pytest backend/tests/research_assistant/test_mcp_catalog_sync.py backend/tests/research_assistant/test_api.py -q -p no:cacheprovider`；catalog=14 servers/209 tools，含 6 个 `aistock-gateway-lite` catalog tools；preflight 输出 gateway manifest/profile/risk 元数据 | PASS_R2 | RA catalog 已从统一 manifest 派生，保留 RA-only confirmation/input-schema override |
