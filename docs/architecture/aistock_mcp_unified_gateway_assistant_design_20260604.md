...
- 5a 后端切片（commit `d8b55ed6`）已完成实施内容 1、2、4：RA list/preflight/ReAct 目录源收敛到统一 manifest，`.mcp.json` canonical server_key 对齐，A2 只读 override 与 L2.5 external retrieval 自动只读已锁定；任务审计/UI/E2E 为 5b 范围，5a 不声明完整 Phase 5 完成。
...
| 智能助手读取统一 catalog | `backend/services/research_assistant/mcp_catalog_sync.py`、`backend/services/research_assistant/service.py`、`backend/routers/research_assistant.py`、RA API | `python -m nox -s mcp_gateway_phase5_assistant` 119 passed；Validation Runner `valjob_20260604_093414_0985cde6` return_code=0，run_id `research-assistant_20260604_093524_l4_mcp-gateway-phase5-assistant_0985cde6_runner-validation__2d64d1d1ba`；catalog=9 canonical servers/209 manifest tools，preflight 输出 gateway manifest/profile/risk 元数据 | PASS_5A_BACKEND (`d8b55ed6`) | 5a 后端切片完成；UI、任务审计增强、Playwright B-08 和完整 Phase 5 completion 留给 5b |
| 高风险工具 preflight/approval | `backend/mcp/tool_manifest.py`、`backend/services/research_assistant/mcp_catalog_sync.py`、`backend/services/research_assistant/service.py`、`backend/services/research_assistant/execution.py` | `test_manifest_risk_no_write_as_readonly`、`test_external_research_l25_read_only_retrieval_stays_direct`、`test_ra_manifest_catalog_consumption.py`；RA preflight/action proposal 对 `assistant_usable=preflight_required` 保留 approval gate；external search/fetch 三工具 L2.5 read_only 自动执行，`external_research_save_evidence` 仍 preflight | PASS_5A_BACKEND (`d8b55ed6`) | 写/确认/长任务/外网写未放松；5b 补 UI approval pending/evidence 展示 |
