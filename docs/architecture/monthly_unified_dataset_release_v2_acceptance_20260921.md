...
| F-017 | `monthly_hmm_derive.py`、`monthly_hmm_consumer_probe.py`、`monthly_node_probe.py`、`monthly_node_probe_runner.py`、`monthly_official_adapters.py` | `test_monthly_hmm_consumer_probe.py`、`test_monthly_node_probe.py`、`test_monthly_node_probe_runner.py`：正式file-only producer适配、冻结model/config/script hash、新B/C绑定、控制器与节点分别调用正式frozen-input loader、无fit/no-DB、完整日期grid及远端身份闭合 | in_progress | 数据侧正式派生、登记、控制器预检和WSL/node1执行协议已实现；仍待正式模型authority配置、运行态组装及真实闭月节点readback后转为verified |
...
| F-025 | `monthly_local_validation.py`、`monthly_consumer_validation.py`、`monthly_consumer_registry.py`、`monthly_node_probe.py`、`monthly_node_probe_runner.py`、各consumer probes、`monthly_official_adapters.py` | 对应dataset-release测试：完整六池scope、不可变精确11 controller preflight注册、固定WSL/SSH命令、canonical stdin/stdout、v4 binding/node registration/manifest闭合、side-effect fail-closed | in_progress | 11项code-owned controller preflight与节点证明封装已实现，控制器PASS不能替代节点PASS；仍待运行态正式组装及真实三节点闭月readback，未达到verified |
