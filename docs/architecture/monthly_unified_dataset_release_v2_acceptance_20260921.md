...
| F-017 | `monthly_hmm_derive.py`、`monthly_hmm_consumer_probe.py`、`monthly_production.py`、`monthly_worker_runtime.py`、`monthly_node_probe.py`、`monthly_node_probe_runner.py`、`monthly_official_adapters.py` | `test_monthly_hmm_consumer_probe.py`、`test_monthly_production.py`、`test_monthly_worker_runtime_cli.py`、节点probe测试：正式file-only producer适配、外部冻结model/config/script hash authority、新B/C绑定、控制器与节点分别调用正式frozen-input loader、无fit/no-DB、完整日期grid及远端身份闭合 | in_progress | 数据侧正式派生、authority fail-closed loader、生产registry和WSL/node1执行协议已实现；仍待真实闭月节点readback后转为verified |
