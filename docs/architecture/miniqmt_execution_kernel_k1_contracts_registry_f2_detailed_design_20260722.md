...
>
> Feature tier：`F2`。文档状态：`design_ready`；实现状态：`K1-A implemented_verified + merged`（PR #2637，merge `e69e72dbdd6e5fd8d414721adaa86ccec7fafc2f`），`K1-B implemented_verified`（PR #2655，source merge 待用户授权），`K1-C not_started`，K1 overall `in_progress`；K2/K3/K4 均 `not_started`，产品 runtime 未切换。
>
...
3. `PluginProcessBindingsV2` 是 process-local sealed mapping：binding ID -> explicit callable。constructor 必须先复制 caller mapping，再以不可变 mapping view 保存 exact callable references；caller 后续修改原 dict 不得换掉已验证 binding。callable 只能由 composition root 代码引用，不能从任何 callable ref import；该 mapping 不进入任何 durable/canonical hash。每个 callable 的 `__module__ + __qualname__` 必须分别等于 descriptor 对应 callable ref，normalized `inspect.signature` 必须等于对应 signature hash，`inspect.getsourcefile` 的 repo-relative path/hash 必须与 `SourceAttributionV1.aistock_files` 闭合；只验证同 signature 而接受另一函数属于 `MINIQMT_PLUGIN_BINDING_INVALID`。signature normalization 固定为 parameter 的 `name/kind/required-or-canonical-JSON-default/annotation-qualified-name` ordered tuple 与 return annotation；禁止 `repr`、object address、PID 或解释器生成的临时 identity；
...
`PluginActiveOrderStateV1` exact fields：`local_vt_orderid,submit_command_id,broker_order_id|null,symbol,side,status,requested_price_decimal,requested_quantity,cumulative_filled_quantity,remaining_quantity,last_order_event_id|null,last_trade_event_id|null,mapping_sha256`；`mapping_sha256 = hash_hex_v1("miniqmt_plugin_active_order_state_v1", exact preceding fields)`，因此 local/broker identity、symbol、side、canonical price、requested/filled/remaining quantity 与 order/trade state lineage 全部进入同一 mapping closure。status 固定为 `PENDING_DISPATCH/SUBMITTED/PARTIALLY_FILLED/CANCEL_PENDING/CANCELLED/FILLED/REJECTED/OUTCOME_UNKNOWN`；`remaining_quantity = requested_quantity - cumulative_filled_quantity`，filled quantity 必须在 `[0,requested_quantity]`，所有 active remaining 之和不得超过 parent remaining；symbol/side 必须等于 frozen algo state。broker ID 只能由 durable Gateway/OMS receipt 提供，禁止从 local ID 猜测。inactive order 不留在 `active_orders`，但其历史继续由 command/child/order/trade facts 重建。
...
当前状态：`implemented_verified`。PR #2655 独立权威审核从 committed HEAD `85ee8f98` 重新建立 RED：首批审核矩阵 12 failed/1 passed，并分别证明 noncanonical/hash-correct snapshot、malformed descriptor 二次异常、input permutation failure receipt、truncation marker、active-order 价量/lineage、TWAP terminal/latest-view、behavior trace、legacy effective projection 等缺陷。补修后 registry direct 15 passed、current-three direct 33 passed、import-boundary 2 passed；关键模块 line/branch coverage 88.04%/73.10%；authoritative-review delta ownership 7/7 mapped，PR classifier 8/8 mapped；`miniqmt_execution_runtime_l2` 522 passed/1 skipped，`paper_v2_backend` 1048 passed/2 skipped/2 xfailed；三份 F2 validator、DESIGN-COMPLIANCE-001 和 PR #2655 required CI 闭合。`source_merge=pending_user_authorization`，产品 runtime 未切换。
...
当前状态：`not_started`。K1-B 只消费显式 supplied strict compatibility receipt；不生成、不默认、不伪造 receipt，也不声明 K1-C 完成。
...
| `F-055` | `backend/services/miniqmt_execution_runtime/plugin_registry.py` immutable catalog、descriptor/process binding、exact creation/restore key、canonical persisted snapshot readback、bounded aggregate receipt、supplied pinned receipt component closure 与 static route isolation | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py` = 15 passed，覆盖 input permutation/fresh-process byte identity、noncanonical/hash-correct readback、malformed descriptor aggregate continuation、failure-input permutation、missing/FAILED/component mismatch、duplicate/missing binding、256-item marker/hash closure、route isolation；PR #2655 required CI green | implemented_verified | none |
...
| `F-057` | `backend/execution_algos/vnpy_style/plugin_manifests.py` 三个 2.0.0 exact AISTOCK_DERIVED manifest/config/state/event/capability/source/executable behavior trace closure、strict active-order decimal/quantity/lineage codec、TWAP exchange-active/restart/EOD/zero-slice contract 与 observation-only legacy projection | `backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py` = 33 passed，覆盖 identity/schema/source/精确 delivery callbacks/价量字段/strict type/active-order mapping/state lineage/TWAP/BestLimit u53/current-core trace/legacy effective-conflict-drift-unknown matrix；PR #2655 required CI green | implemented_verified | none |
...
| `F-059` | §11 ownership/catalog/test plans and coverage | direct files = 48 passed，import-boundary = 2 passed；coverage line/branch = 88.04%/73.10%；authoritative-review delta ownership 7/7 mapped，PR classifier 8/8 mapped；`python -m nox -s miniqmt_execution_runtime_l2` = 522 passed/1 skipped；`python -m nox -s paper_v2_backend` = 1048 passed/2 skipped/2 xfailed | implemented_verified | none |
| `F-060` | §2、§10、§12-§13 state-separated rollout/rollback/gates；K1-B shadow-only，产品 runtime 零接线 | validation-receipt: 三份 F2 validator 8/8、18/18、60/60 与 DESIGN-COMPLIANCE-001 local PASS；production gates noop | design_ready | none |

## 16. DESIGN-COMPLIANCE-001 / 正式复核

| control | result | evidence |
| --- | --- | --- |
| no simplified/subset/POC | pass | K1-A writer/readback 与 K1-B 完整 catalog/current-three/shadow projection 均有直接证据；K1-C 明确 not_started，K1 overall 仍为 in_progress，没有把 K1-B 宣称为统一架构或 production runtime 完成 |
