...
>
> Feature tier：`F2`。文档状态：`implementation_verified`；实现状态：`K1-A implemented_verified + merged`（PR #2637，merge `e69e72dbdd6e5fd8d414721adaa86ccec7fafc2f`），`K1-B implemented_verified + merged`（PR #2655，final source HEAD `97f7a030a7eace0bacaa92892f40235116b60aaa`，merge `ae1035a1ab4916427f7b72443c6235d544eb4c8e`）；K1-C PR #2685 的 import-boundary 与 external vn.py DTO/enum authority 两项 P1 已完成 RED/GREEN、direct matrix、coverage、classifier/L2、F2 与 Linux required CI 验收，implementation HEAD `683cbd40f2baf1ef403520dedaecd23025c59bfd` 为 `implemented_verified`，CI run `30108224617` 的 MiniQMT/Paper/static/verdict 均 green；`source_merge=pending_user_authorization`，K1 overall `in_progress`；K2/K3/K4 均 `not_started`，`close_sync=not_applicable_feature`，产品 runtime 未切换，production DDL/DML/dependency/config/binding/broker/restart/runtime activation gates 均为 `noop`。
>
> 当前实现基线：PR #2685 base `main@7b642fd67d3209add4254f4f3b4f65ba6659ef35`，K1-C final-review repair 日期 2026-07-24。
>
...
AST 层必须解析 direct/alias/from/relative import、`builtins.__import__`、`importlib.import_module`、literal `getattr` chain、callable assignment/alias、`os.environ`/`getenv` 和 filesystem/process/thread/network calls；对 importlib/builtins/os/io/pathlib 等敏感 owner 的非 literal `getattr` 必须 fail loud。isolated-process 层在 target execution scope 内以 stack provenance/audit-equivalent guard 覆盖 helper-mediated 与 dynamically imported code，不得只检查 immediate caller frame。guard 在真实副作用发生前拒绝 builtins/io/pathlib/os/shutil 与 raw `_io.FileIO` 文件读写、创建、删除、移动，environment read/write，socket/DNS/HTTP，subprocess/os spawn，thread/process/executor/task；`open` audit 必须区分 importlib loader 读取与 target/helper 的直接文件操作，preload 发生在 guard activation 前，target scope 结束后恢复原对象。任何目标文件存在性、bytes 或目录状态必须在 direct tests 中证明未变化。
...
import-boundary receipt 另有 `PluginImportBoundaryReceiptV2`：除 `schema_version,status,checked_modules,ordered_failures` 外，固定保存 `observed_failure_count,retained_failure_count,failures_truncated,omitted_failure_count,omitted_failure_set_sha256,failure_set_sha256,receipt_sha256`。failure identity 使用 repo-relative `source_path`、`stage,module,line,column,reason,context_sha256`；isolated exception/stderr/invalid-receipt text 在进入 context/hash 前必须把 absolute source/root 替换为 repo-relative identity；按 stage/module/path/line/column/reason/context 排序，最多保留 64 项。超限必须保留唯一 `__failure_set__` marker，其 context 固定包含 `omitted_count,omitted_failure_set_sha256`; `failure_set_sha256 = hash_hex_v1("miniqmt_plugin_import_boundary_failure_set_v2", {retained_failures,omitted_count,omitted_failure_set_sha256})`，receipt hash 使用 `miniqmt_plugin_import_boundary_receipt_v2`。不同 temporary repo root 的同一 source 只能产生相同 repo-relative identity；PASSED 无 failure，FAILED 至少有真实 failure 或 truncation marker，不能 warning、previous receipt 或 fallback。
...
Final-review repair 当前为 `implemented_verified`：public import validator 已闭合 indirect/dynamic/alias escape、helper-mediated file/environment/network/process/thread/task side effect、raw FileIO 与 repo-relative bounded V2 evidence；Linux CI 先暴露 importlib source read stack 平台差异，稳定 failure carrier 随后精确证明外部 `jsonschema`/`pydantic` 内部 import 被深层 repo frame 误归因。最终 provenance 以首个具体 Python caller 为 authority，合法 loader/external dependency import 不误报，而 repo target/helper 的动态 forbidden import 仍 fail loud；write-capable C audit 与 direct/helper read/write nodes 保持。正式审核又发现 malformed isolated worker JSON 可触发父 validator 二次异常、静默 PASSED 或错误分类，现对 exact top-level/events/exception carrier shape 统一 fail-loud 为既有 receipt-invalid evidence。dual-upstream requirement/source/surface/receipt 已绑定 exact `vnpy_algotrading` 和 `vnpy core 4.0.0` bytes，core object/enum AST drift 进入 FAILED receipt 并阻止 catalog partial publication。当前 direct matrix 为 `49/57/60/55/39 = 260 passed`；implementation HEAD `683cbd40` 的 Linux CI run `30108224617` 为 MiniQMT L2 `674 passed/1 skipped`、Paper v2 `1042 passed/2 skipped/1 deselected`，static gate 与 CI verdict green。`source_merge=pending_user_authorization`；K1-B 只消费显式 supplied strict compatibility receipt，不生成、不默认、不伪造 receipt。
...
| --- | --- | --- | --- | --- |
| `F-053` | §3 target modules/dependency/import denylist；K1-A/B/C 只依赖 shadow contract/source modules，不连接 product runtime/network/DB/OMS/Gateway/broker；`PluginImportBoundaryReceiptV2` 使用 repo-relative bounded/truncated/hash-closed evidence | `backend/tests/miniqmt_execution_runtime/test_plugin_import_boundaries.py` = 57 passed；public validator 覆盖 direct/alias/from/relative/getattr/builtins/dynamic import、helper-mediated file/environment/network/process/thread、raw FileIO direct/helper read/write、bounded truncation、stable diagnostic carrier、isolated exception context/root-independent identity 与 malformed worker carrier fail-loud；首个具体 Python caller provenance 同时证明 external validation dependency 不误归因且 repo helper dynamic import 保留 exact forbidden module；Linux CI run `30108224617` MiniQMT L2 green | implemented_verified | none |
...
| `F-059` | §11 ownership/catalog/test plans and coverage | `backend/tests/miniqmt_execution_runtime/test_vnpy_compatibility_receipts.py` + `backend/tests/miniqmt_execution_runtime/test_plugin_import_boundaries.py` + shared contract/registry/current-three direct matrix = `49/57/60/55/39 = 260 passed`；coverage line/branch：import `87.47/76.06`、surface `89.61/81.34`、receipt `88.46/70.00`；repair delta/full PR classifier `20/29 files`、`unmapped_code_files=[]`，选择 MiniQMT/Paper；Linux CI run `30108224617`：MiniQMT `674 passed/1 skipped`、Paper `1042 passed/2 skipped/1 deselected`，static gate 与 verdict green；本地 L0/module registry PASS | implemented_verified | none |
| `F-060` | §2、§10、§12-§13 state-separated rollout/rollback/gates；K1-C shadow-only，产品 runtime 零接线 | artifact: `docs/architecture/miniqmt_execution_kernel_k1_contracts_registry_f2_detailed_design_20260722.md`、`docs/architecture/miniqmt_execution_kernel_vnpy_plugin_architecture_f2_design_20260722.md`、`docs/architecture/simulation_platform_unified_authoritative_blueprint_20260715.md`；三份 F2 validator 与 DESIGN-COMPLIANCE-001 本地执行，implementation HEAD `683cbd40` required CI green；K1-A/B merged，K1-C `source_merge=pending_user_authorization`，K1 overall `in_progress`，K2/K3/K4 `not_started`，`close_sync=not_applicable_feature`、production/runtime gates `noop` | implemented_verified | none |
