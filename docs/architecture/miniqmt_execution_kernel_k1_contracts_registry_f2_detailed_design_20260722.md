...
>
> Feature tier：`F2`。文档状态：`design_ready`；实现状态：`K1-A implemented_verified + merged`（PR #2637，merge `e69e72dbdd6e5fd8d414721adaa86ccec7fafc2f`），`K1-B implemented_verified`（PR #2655，source merge 待用户授权），`K1-C not_started`，K1 overall `in_progress`。
>
...
`PluginCatalogBuildFailureReceiptV1` 字段为 `schema_version,build_input_sha256,ordered_descriptor_keys,total_failure_count,failures_truncated,ordered_failures,omitted_failure_set_sha256,failure_set_sha256,receipt_sha256`；最多返回 256 项，若总数超过 256，保留排序后的前 255 项并以第 256 项 `SNAPSHOT_FREEZE/__failure_set__/MINIQMT_PLUGIN_REGISTRATION_CONFLICT` 记录 omitted count 与全部 omitted failure identities 的 hash。`failure_set_sha256 = hash_hex_v1("miniqmt_plugin_catalog_failure_set_v1", {total_failure_count,failures_truncated,ordered_failures,omitted_failure_set_sha256})`，`receipt_sha256 = hash_hex_v1("miniqmt_plugin_catalog_build_failure_receipt_v1", exact preceding fields)`。`build_input_sha256` 只覆盖 descriptors/creation bindings 的 bounded JSON-safe input evidence、process binding IDs 和 supplied receipt data，不含 callable。receipt 不含 wall clock、callable repr/地址或 Gateway catalog。exception 是该 receipt 的唯一返回载体，调用方不得以空 catalog、partial catalog 或上一个 catalog 假成功。
...
当前状态：`implemented_verified`。`plugin_registry.py`、`plugin_manifests.py`、legacy shadow projection、direct RED/GREEN、statement/branch coverage 88.90%/71.79%、ownership/classifier 8/8、模块 L2 508 passed/1 skipped、三份 F2 validator、DESIGN-COMPLIANCE-001 和 PR #2655 required CI 已闭合；`source_merge=pending_user_authorization`，产品 runtime 未切换。
...
| `F-055` | `backend/services/miniqmt_execution_runtime/plugin_registry.py` immutable catalog、descriptor/process binding、exact creation/restore key、bounded aggregate receipt、supplied pinned receipt component closure 与 static route isolation | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py` input permutation/fresh-process byte identity、missing/FAILED/component mismatch、duplicate/missing binding、256-item truncation、route isolation；K1-B direct matrix 34 passed；PR #2655 required CI green | implemented_verified | none |
...
| `F-057` | `backend/execution_algos/vnpy_style/plugin_manifests.py` 三个 2.0.0 exact manifest/config/state/event/capability/source/behavior closure、strict codec、TWAP exchange-active/restart/EOD contract 与 observation-only legacy projection | `backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py` identity/schema/source/价量字段/strict type/state lineage/TWAP/BestLimit u53/legacy conflict-drift-unknown matrix；K1-B direct matrix 34 passed；PR #2655 required CI green | implemented_verified | none |
...
| `F-059` | §11 ownership/catalog/test plans and coverage | `python -m pytest -q backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py` = 34 passed；coverage statement/branch 88.90%/71.79%；`python -m nox -s miniqmt_execution_runtime_l2` = 508 passed/1 skipped | design_ready | none |
