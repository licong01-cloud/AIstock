...
| `F-055` | `backend/services/miniqmt_execution_runtime/plugin_registry.py` immutable catalog、descriptor/process binding、exact creation/restore key、canonical persisted snapshot readback、bounded aggregate receipt、supplied pinned receipt component closure 与 static route isolation；builder/readback 共用同一 compatibility component authority | `backend/tests/miniqmt_execution_runtime/test_algo_plugin_registry.py` = 16 passed，覆盖 input permutation/fresh-process byte identity、noncanonical/hash-correct/incomplete component readback、malformed descriptor aggregate continuation、failure-input permutation、missing/FAILED/component mismatch、duplicate/missing binding、256-item marker/hash closure、route isolation；final-review incomplete receipt RED 1 failed，修复后 GREEN；PR #2655 required CI 以最终 HEAD 为准 | implemented_verified | none |
...
| `F-057` | `backend/execution_algos/vnpy_style/plugin_manifests.py` 三个 2.0.0 exact AISTOCK_DERIVED manifest/config/state/event/capability/source/executable behavior trace closure、strict active-order parent/local/broker/command/price/quantity/native-market-data/order-trade lineage codec、TWAP exchange-active/restart/EOD/zero-slice contract 与 observation-only legacy projection | `backend/tests/miniqmt_execution_runtime/test_current_three_plugin_manifests.py` = 36 passed，覆盖 identity/schema/source/精确 delivery callbacks/价量字段/strict type/active-order mapping/parent-child cumulative fill/same-hash-different-payload/native market lineage/state lineage/TWAP/BestLimit u53/current-core trace/legacy effective-conflict-drift-unknown matrix；final-review closure RED 2 failed，修复后 GREEN；PR #2655 required CI 以最终 HEAD 为准 | implemented_verified | none |
...
| `F-059` | §11 ownership/catalog/test plans and coverage | direct files = 52 passed，import-boundary = 2 passed；coverage line/branch = 88.17%/73.68%；final-review delta ownership 7/7 mapped，PR classifier 8/8 mapped；`python -m nox -s miniqmt_execution_runtime_l2` = 526 passed/1 skipped；`python -m nox -s paper_v2_backend` = 1048 passed/2 skipped/2 xfailed | implemented_verified | none |
