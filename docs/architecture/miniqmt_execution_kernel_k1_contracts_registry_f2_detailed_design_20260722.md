...
唯一 source authority 为 `vnpy/vnpy_algotrading@4133987530eb28f3538d1983545d81c4f83d7d59`。仓库必须保存 `backend/execution_algos/vnpy_compat/pinned_source/` 下六个原始 source bytes 与同一 commit 的 MIT `LICENSE`，并保存 `surface_contract.json` characterization artifact 以及 `source_manifest.json`：`schema_version,upstream_repo,upstream_commit,files[{path,sha256,size_bytes}],license_file{path,sha256},characterization_file{path,sha256},license,copyright,source_manifest_sha256`。`surface_contract.json` 固定 `schema_version,required_method_signatures,required_object_fields,required_enum_values,surface_contract_sha256`，domain 为 `miniqmt_vnpy_surface_contract_v1`；它独立于每个 plugin requirement，用于证明 external vn.py DTO/enum surface，不是 mock source 或 runtime façade。source manifest 的 hash domain 为 `miniqmt_vnpy_pinned_source_manifest_v1`；`source_lock_sha256` 仍严格使用 K1-B 已冻结的 `miniqmt_vnpy_compatibility_source_lock_v1` 与 requirement `source_files_and_hashes`，不得由 source manifest 另造第二 identity。fresh process readback 必须逐文件检查 exact path set、repo-relative path/no traversal、bytes SHA-256、size、commit/license/characterization hash，missing/extra/duplicate/decode/AST drift fail loud；CI 不联网、不安装、不动态 import upstream。

K1-C 的 `locked_surface.py` 公开 `load_pinned_source_manifest_v1()`、`extract_locked_surface_v1()` 和 `validate_pinned_source_v1()`：source manifest、AST method surface、structured object/enum requirements 与 characterization metadata 形成同一 immutable `LockedSurfaceV1`。四项 component/hash 的 exact formulas 依次为：
...
K1-B catalog consumption 只接受显式 supplied `VnpyCompatibilityReceiptV1`，并继续执行 §7.1 的 component/identity/status/hash closure。K1-C C2 只消费当前三份 manifest 的真实 generator 输出：PASSED receipt 进入 catalog；任一 pinned source/surface/characterization FAILED 进入现有 aggregate `PluginCatalogBuildFailureReceiptV1` 并阻止 zero/partial/previous catalog publication；per-route compatibility receipt 仍只影响 exact plugin/route。K1-C 不复制 catalog、不调用 Gateway、不接入 runtime。
...
当前 source 状态：checkpoint commit `1e23d595` 保存 source-isolated import-boundary；后续 C1/C2 已实现 repo-owned pinned source、locked surface、structured requirement/receipt、standard package import 与 K1-B real catalog seam，并完成本地 direct/coverage/classifier/F2 审核，状态为 `implemented_verified_local`、`source_merge=pending_pr`、required CI pending。K1-B 只消费显式 supplied strict compatibility receipt；不生成、不默认、不伪造 receipt。
...
| --- | --- | --- | --- | --- |
| `F-053` | §3 target modules/dependency/import denylist；K1-A/B/C 只依赖 shadow contract/source modules，不连接 product runtime/network/DB/OMS/Gateway/broker | `backend/tests/miniqmt_execution_runtime/test_plugin_import_boundaries.py` = 32 passed；source-isolated AST/process matrix覆盖 direct/alias/from/relative/dynamic import、DB/broker/network/wall-clock/random/UUID/environment/parallel owner；standard `import backend.execution_algos.vnpy_compat` 证明无算法注册、legacy/runtime/repository/Gateway/thread/process/network/write side effect，显式 registry export 保持旧 API | implemented_verified_local | none |
...
| `F-058` | §1.2、§9 repo-owned pinned requirement/source/surface/receipt lock；shared `compatibility_component_hashes_v1` 是 K1-B/C 唯一 component authority | `backend/tests/miniqmt_execution_runtime/test_vnpy_compatibility_receipts.py` = 24 passed：六个 exact source bytes/path set + MIT license + manifest/characterization readback，method kind/default/annotation/return/error、object/enum、missing/extra/duplicate/traversal/hash/decode/AST drift、PASSED/FAILED、bounded truncation、writer/readback/retry/fresh-process closure；三个真实 receipt 构建 catalog，任一 FAILED 保留 bounded evidence 并 zero-publication | implemented_verified_local | none |
| `F-059` | §11 ownership/catalog/test plans and coverage | `backend/tests/miniqmt_execution_runtime/test_vnpy_compatibility_receipts.py` / `test_plugin_import_boundaries.py` / `test_algo_plugin_registry.py` / `test_current_three_plugin_manifests.py` / `test_algo_plugin_contracts.py` = 24/32/55/39/60 passed；coverage `import_boundary` 86.62/74.58、`locked_surface` 85.77/71.95、`receipts` 87.50/70.00 line/branch；25 changed files mapped/unmapped/ambiguous=25/0/0；`python -m nox -s miniqmt_execution_runtime_l2` = 624 passed/1 skipped、`python -m nox -s paper_v2_backend` = 1050 passed/2 skipped/2 xfailed；L0/module registry PASS | implemented_verified_local | none |
| `F-060` | §2、§10、§12-§13 state-separated rollout/rollback/gates；K1-C shadow-only，产品 runtime 零接线 | validation-receipt: `python scripts/aistock_feature_workflow.py validate --design docs/architecture/miniqmt_execution_kernel_k1_contracts_registry_f2_detailed_design_20260722.md --tier F2` 与另两份蓝图结果 8/8、18/18、60/60；Feature Workflow pre-PR 与 DESIGN-COMPLIANCE-001 local PASS；K1-A/B merged，K1-C `source_merge=pending_pr`，K1 overall `in_progress`，K2/K3/K4 `not_started`，`close_sync=not_applicable_feature`、production/runtime gates `noop` | implemented_verified_local | none |

## 16. DESIGN-COMPLIANCE-001 / 正式复核

| control | result | evidence |
| --- | --- | --- |
| no simplified/subset/POC | pass | K1-A writer/readback、K1-B catalog/current-three/shadow projection 与 K1-C exact pinned source/surface/receipt/import/C2 seam 均有真实 direct evidence；K1-C 仅标 `implemented_verified_local` 且 required CI pending，K1 overall 仍为 in_progress，没有把 source implementation 宣称为 production runtime 完成 |
