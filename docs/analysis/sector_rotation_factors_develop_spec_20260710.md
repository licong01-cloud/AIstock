...
- 初版日期：2026-07-10
- 当前版本：v6.11（截至 2026-08-30 的进度与执行方案权威；策略演进最佳新策略包仍是唯一 P0。BUG-1191 / Issue #3793 的源码、运行态验证与 close-sync 已完成，但源码/运行态完成不等于数据候选签核或激活。canonical PIT v2 profile `qe_hmm_full_v2` 的最新 submission `dss_cdc7ee95f703cb1cbd8a4faf9e8cee40` 仍为 `BLOCKED_CONTRACT`，没有 run/release receipt，worker 虽健康空闲但 production activation 为 `not_requested`。因此 MA-E19R2 `qe_20260825_031740_2457` 仍是 9/12：2024H2～2025H2 九臂可作部分证据，2026H1 三臂没有有效 portfolio，完整 D1 仍为 `INCOMPLETE`。数据签核前只允许推进文件数据面的九臂语义等价审计、P0-D2 Sector Oracle 与 P0-D3 benchmark/Brinson 工具，不提交依赖新数据的正式实验；签核与独立激活后按语义等价结果选择仅补三臂或完整 12 臂，再执行 D1 synthesis、D2、D3，并由结果触发适应/门控、两层板块/lead-lag、右尾/meta-labeling、动态退出或受约束组合构建。不恢复历史补账、Archive/UI、批量物化、平台扩建或数据库数据面。）
...
> **历史标题解释：**下列 v6.1～v6.10 快照中遗留的“当前”“权威”“继续有效”只表示各自日期当时的原始记录，均不再表示 2026-08-30 的活动状态；活动进度、总账、顺序与门禁以 v6.11、第 2.5、9.9.2～9.9.4、15、17 节为准。v6.8 的分钟 TWAP 执行合同继续有效；v6.9 的 MA-E19A/B 9/12 状态已由 MA-E19R2 的新运行事实取代。
...
实时事实的最后核对时间为 2026-08-30：MA-E19R2 `qe_20260825_031740_2457` 仍是最新演进 task，Loop1～9 completed、Loop10～12 failed，且没有可用于 D1 完整裁决的 2026H1 portfolio。BUG-1191 / Issue #3793 已完成 source merge、运行态验证、close-sync 和 Issue 关闭；candidate、节点分发、active activation 与三臂/十二臂重跑仍是独立状态。回测数据集 CLI 的 canonical PIT v2 最新 submission `dss_cdc7ee95f703cb1cbd8a4faf9e8cee40` 为 `BLOCKED_CONTRACT`，没有 run/release receipt，worker 健康空闲也不能替代 candidate signoff。当前 root `main`、运行身份和其他 BUG 由各自工作流维护；本文不把 source merge、runtime activation、candidate signoff 或生产切换合并为一个状态。
...
| MA-E19 P0-D1 model-age/refit | `R2_PARTIAL_9_OF_12_WAITING_DATASET_SIGNOFF` | MA-E19R2 `qe_20260825_031740_2457`：Loop1～9 completed，Loop10～12 failed；BUG-1191 source/runtime/close-sync complete；dataset submission `dss_cdc7ee95f703cb1cbd8a4faf9e8cee40` blocked | 2024H2～2025H2 的结果仅为部分证据；2026H1 fixed/expanding/rolling 全缺，D1 仍 `INCOMPLETE`。先取得数据 signoff 并运行九臂语义等价审计：等价时只补三臂，不等价时完整重跑 12 臂；此前不晋级 DoubleAdapt/Proceed 或 learned gate。 |
...
| F-027 | v6.11 活动演进顺序 | dataset terminal receipt/catalog signoff/独立激活 -> 九臂语义等价裁决 -> 等价时只补 MA-E19R2 2026H1 三臂、否则完整 12 臂 -> D1 synthesis -> P0-D2 四格 sector oracle -> P0-D3 benchmark/Brinson -> 按结果触发适应/门控、两层板块/lead-lag、右尾/退出、横截面/正交或受约束板块主动权重。数据阻断期间只开发文件型等价/D2/D3 工具和新 Alpha 快筛，不占正式实验槽。 |
...
v6.11 当前执行顺序覆盖以下历史 Phase：MA-E19R2 9/12 为部分证据；BUG-1191 source/runtime/close-sync 已完成，但 dataset signoff 仍阻断正式实验。等待期间先交付九臂语义等价、P0-D2 sector oracle 和 P0-D3 benchmark/Brinson 的文件型工具；signoff/独立激活后依据等价审计仅补三臂或完整重跑 12 臂，再执行 D1 synthesis、D2、D3，并按结果选择 P1 分支。新 Alpha 文件快筛与概念 PR-A/PR-B 可以在互斥文件、CPU-only 且不占正式实验槽时并行；概念当前仍未实现。未来正式收益、风险、换手和成本评价统一使用 `execution_algo=TWAP`、`backtest_freq=1min`；分钟新信号因子因历史数据未就绪暂缓，但不允许 portfolio 回退日频。不恢复 MA-E14 阈值 overlay；F-014、通用 UI、历史补账/物化、监控/日志平台和底座完整化继续不实施。
...
| QE dataset | CANONICAL_PIT_V2_BLOCKED_CONTRACT_NO_SIGNOFF | profile `qe_hmm_full_v2` 最新 submission `dss_cdc7ee95f703cb1cbd8a4faf9e8cee40`：无 run/release receipt，worker healthy/IDLE，activation not requested。实验运行期禁止数据库访问；源码/runtime 状态不能替代 terminal receipt、catalog readback 或 candidate signoff。 |
