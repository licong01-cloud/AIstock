...
| F-014 长期趋势评价层 | `PHASE4_LOOP_ARCHIVE_UI_VERIFIED` | Phase 2 R8B 6/6 terminal/CAS；Loop4 2,004 metric + 6 artifact 与 exact replay；Phase 4 Loop DB 恢复/唯一操作、Archive 同 vintage 对比；mock 2/2 + live 只读 1/1 | Phase 1–3 与 Phase 4 Loop/Archive UI slice 已交付；其余 5 个 R8B、任务创建 profile/历史输入预览和 Phase 5 中断恢复/批量操作面仍待完成。 |
...
F-014 的可实施详细设计已固化在父设计与 `qe_long_trend_evaluation_phase2_compute_cas_f2_design_20260722.md`。它只读复用现有 Recorder/prediction pointer 和 `qe_archive.run` 父身份，不扩展通用 Prediction Store 或既有 Archive 通用 writer/schema；使用 feature/outcome 双快照、execution environment identity、extension-only 历史价格校验和右删失；逐信号/episode 明细进入 QE-only CAS Parquet。三张 additive `run_evaluation*` 表按恢复依赖交付：Phase 2 control/worker/CAS 与 Phase 3 metric/artifact/API/MCP 已完成，并由 R8B Loop4 `qelt_89331d…` 闭合一次 existing-CAS→DB→API/MCP 与 exact replay；Phase 4 已实现 Loop DB 恢复型长期趋势页签、唯一幂等入口、五期限/三 barrier/episode/execution/sector/artifact 展示，以及 Archive 同 outcome snapshot 的 run/model/seed/factor-set 对比。该 canary 写入 2,004 metric 和 6 artifact；其余 5 个 R8B、任务创建 profile/历史输入预览、Phase 5 中断恢复与批量历史重评仍待实施。R8B 6/6 已合计产出 12,024 项指标、13,241,712 行信号与 13,726 个 episode；每个 Loop 都是三个 family 完整计算、两个 family 带覆盖限制、一个 family 因无直接成交原因证据不可验证。所有可用结果继续用于研究，限制项进入补数、损失和互证分析，不改变 R8 原训练、回测、Archive 结果或任何研究方向。
...
2. **P0：F-014 Phase 2–4 与 R8–R11 真实重评**：Phase 2 已交付真实 worker/CAS，Phase 3 已合入三表/API/MCP并以 Loop4 完成 2,004 metric + 6 artifact 单 canary 物化及 exact replay，Phase 4 Loop/Archive UI slice 已完成 DB 恢复、唯一操作、同 vintage 对比及 mock/live 只读验证。下一源码任务为任务创建 profile、历史输入预览和 Phase 5 恢复 E2E；其他 5 个 R8B 和历史补算继续单独授权。每个已可计算指标族立即用于 R8–R11 的 MFE/MAE、time-to-hit、trend capture、false early-exit、post-exit opportunity 和回撤路径分析，平台完整度不控制其他实验。
...
- 与 Advisory Phase 8 对齐的 Phase 1 计算能力已覆盖 20–180 日、MFE/MAE、有序目标、time-to-hit、删失调整的 stage survival、右删失、episode capture/false-exit，以及信号→成交/退出阻断分层；Phase 2 control/worker/CAS、Phase 3 单 canary 明细表/API/MCP 与 Phase 4 Loop/Archive UI slice 已闭合，历史输入预览与扩展 E2E 仍 pending；
- F-014 详细设计：父设计 v1.11 与 Phase 2 从属设计，覆盖指标族独立状态、双快照、execution environment、理论机会与可成交性桥接、QE-only CAS/三表、API/MCP/UI、双端重启恢复和非 QE 零影响；当前为 `PHASE4_LOOP_ARCHIVE_UI_VERIFIED`，其余 5 个 R8B、Phase 4 余项与 Phase 5 状态单列，任一可计算指标族都可形成科研证据；
...
7. `[F014_PHASE4_LOOP_ARCHIVE_UI_VERIFIED]` Phase 1 core、Phase 2 control/worker/CAS、Phase 3 三表/API/MCP 与 Loop4 2,004 metric + 6 artifact/exact replay 均已完成；Phase 4 Loop DB 恢复/唯一操作和 Archive 同 vintage 对比已通过 mock 2/2 与当前 8001 live 只读 1/1。其余 5 个 R8B、任务创建 profile/历史输入预览、Phase 5 E2E、51 个 direct 与 51 个 rematerialize Loop继续按独立平台状态实现，不形成科研准入条件。
...
| F-014 | 本文 9.6、11.6、Phase G0-E；父设计 v1.11 与 Phase 2 从属设计；Advisory Phase 8 | R8B 6/6 worker/CAS；Loop4 2,004 metric + 6 artifact/API/MCP/exact replay；Phase 4 Loop/Archive UI、mock 2/2、live 只读 1/1；108 Loop资产审计 | PHASE4_LOOP_ARCHIVE_UI_VERIFIED | 单 canary 数据链与 Loop/Archive UI slice 已完成；其余 5 个 R8B、任务创建 profile/历史输入预览、Phase 5 E2E 与剩余批量重评继续交付。任何平台缺口不阻断研究。 |
