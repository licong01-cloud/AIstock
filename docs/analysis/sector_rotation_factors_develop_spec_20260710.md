...
| BUG-1014 WSL 主机响应与 qrun 隔离 | `RD_SOURCE_MERGED_AI_SAFE_SYNC_COMPLETE_PR_REFRESH_PENDING_RUNTIME_NOT_ACTIVE` | BUG-1014 / Issue #3266 / AIstock PR #3273；RD-Agent PR #18 / merge `da3270f8…`；任务 `qe_20260810_221723_14ab` 与 `qe_20260810_224522_b0d9` 的 Loop7 重叠；Windows/WSL 原生资源快照 | 根因不是图模型：GeneralPTNN LSTM 的 `n_jobs=2 + pin_memory + prefetch_factor=2` 使单 Loop 形成主进程加两个约 3 GB worker；跨 task 的 WSL 全局容量 2 允许两个本地 Loop 重叠，叠加既有 WSL swap 后触发高频换页。修复保持 batch/标签/模型/策略语义，改为 canonical `wsl2-5080` 全局 1 槽、拒绝会分槽的 `wsl/local` alias、本地 GeneralPTNN `n_jobs=0`/无 prefetch/不 pin、四个 CPU thread env 上限，并在 prepare/qrun/read/custom-command 前清除完整 `PG*`/DB、Bash/loader injection 与非控制面 secret；危险 `remote_env` 在值序列化前 fail closed。远端容量 4 与两节点并行不变。RD-Agent source merge 已完成，但 immutable release/deploy/用户重启/identity 尚未执行；AIstock 已安全同步最新主线，push/新 CI/合入及 backend-main 用户重启/identity 分别待完成。 |
...
| 项目 | 当前状态 | 说明 |
|---|---|---|
| source merge | BUG1014_RD_MERGED_AI_SAFE_SYNC_COMPLETE_PR_REFRESH_PENDING | BUG-1006 已合入并由用户重启加载；BUG-1014 的 RD-Agent PR #18 已以 `da3270f8…` 合入，AIstock PR #3273 的最终源码已在提交后安全同步实时 `origin/main`。AIstock push/新 head CI/mergeability/merge，以及 RD release/deploy/runtime、backend-main runtime、Loop 恢复和实验结论继续分别报告。 |
