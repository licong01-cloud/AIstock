...
- 初版日期：2026-07-10
- 当前版本：v5.18（F-014 任务不可变 profile 与历史输入预检正式审核修复、DEV migration 零残留验证完成；修复 commit `934bd422…` 已推送到 PR #2906，fresh CI、生产 DDL/runtime 待后续，2026-07-30）
...
| F-014 长期趋势评价层 | `PHASE4_TASK_PROFILE_INPUT_PREVIEW_REVIEW_REPAIRED_LOCAL_DEV_DDL_VERIFIED_PR_REFRESHED_CI_RUNNING_PROD_DDL_RUNTIME_PENDING` | 既有 Phase 2 R8B 6/6、Loop4 2,004+6、PR #2875 与运行态 API receipt 保持不变；本轮新增默认关闭且 task identity 不可修改的 `qe_long_trend_v1` 创建开关、执行器节点权威 root 解析、只读 dataset/Recorder/catalog 输入预检；正式审核已补齐缺失 order/trade 显式状态与 task summary profile projection；因 init mirror 触发 changed-file HIGH guardrail，AC 文案仅澄清既有显式 TWAP 分支，不改变执行逻辑；后端定向 92、Phase 2 122、Phase 3 105、QE read 14 passed，TypeScript/ESLint PASS、mock Playwright 3 passed/1 live skipped；现有 DEV migration forward/readback/reapply/guarded rollback/零残留通过；修复 commit `934bd422…` 已推送到 PR #2906 | PR #2906 fresh CI、生产 DDL、runtime/live browser 尚未完成；其余 5 个 R8B、Phase 5 与批量重评继续单列。缺输入不形成研究门禁。 |
...
| F-014 | 本文 9.6、11.6、Phase G0-E；父设计 v1.15 与 Phase 2 从属设计；Advisory Phase 8 | 既有 R8B 6/6、Loop4 2,004+6、PR #2875/runtime API receipt；本轮 task profile migration/create/builders/executor、历史输入 preview API/client/UI、正式审核补齐 order/trade 显式缺失与 task summary profile projection、后端/TS/ESLint/mock Playwright 定向证据；DEV migration 零残留 receipt | PHASE4_TASK_PROFILE_INPUT_PREVIEW_REVIEW_REPAIRED_LOCAL_DEV_DDL_VERIFIED_PR_REFRESHED_CI_RUNNING | PR #2906 已刷新至修复 commit `934bd422…`，但源码尚未合入/激活。PR CI、生产 DDL、live visual、其余 5 个 R8B、Phase 5 E2E 与批量重评继续交付。 |
