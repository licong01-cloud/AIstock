...
| BUG-1006 matched observation panel | `SOURCE_VALIDATED_PENDING_COMMIT_PUSH_CI` | BUG-1006 / Issue #3239；`qe_observation_panel_v1`；public in-memory composer、matched CE3/CE4-B、legacy no-contract 聚焦测试与 F2/nox 门禁 | 新 trial 可声明预先指定的参考因子交集；`observation_panel` 只属于生成期 data-loader contract，不进入策略 kwargs；缺配置因子、缺参考列或空面板均以稳定 reason code fail closed。未声明的旧任务继续允许已成功因子的历史 partial concat。该修复是 MA-E15 暴露的最小阻断 BUG，不扩展 UI、Archive、schema 或历史工程；运行态需源码合入后由用户重启。 |
...
| 项目 | 当前状态 | 说明 |
|---|---|---|
| source merge | BUG1006_SOURCE_VALIDATED_PENDING_COMMIT_PUSH_CI | MA-E14/MA-E15 已形成真实结果；本批修改 composer、聚焦测试、项目记忆与 v6.1 蓝图，聚焦测试、F2 和所需 nox session 已通过。commit/push/CI、merge、runtime activation 与后续 MA-E16 分别报告。 |
