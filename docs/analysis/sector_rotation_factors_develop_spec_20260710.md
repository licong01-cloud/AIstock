...
- 初版日期：2026-07-10
- 当前版本：v6.13（截至 2026-09-06 的策略实验与决策权威；2026-08-31 文件 release 已用于 MA-E19R3/D1B/D1C/D1D，策略演进最佳新策略包仍是唯一 P0。）
...
>
> 下一顺序：①对 rolling LSTM 做 seed314/2718 的 2026H1 matched 复验，再按结果扩展跨 vintage，避免单 seed 晋级；②利用已确认的 D2 sector ceiling，设计现实 soft top-down 与跨板块 lead-lag 的最小 matched canary；③针对 2026H1 Top50 仍为负，推进 NDCG@tail/LTR/quantile/trend-survival 候选生成，meta-label 只能在 recall 足够后处理 take/skip/size；④静态 blend 停止，不搜索测试期权重，只有严格 OOF 状态证据才允许 bounded gate；⑤D3 权重数据缺口由独立文件数据任务补齐但不阻断前四项。历史补账、Archive/UI、平台扩建和数据库数据面继续禁止。
...
> **历史标题解释：**下列 v6.1～v6.12 快照中遗留的“当前”“权威”“继续有效”只表示各自日期当时的原始记录，均不再表示 2026-09-06 的活动状态；活动进度、总账、顺序与门禁以 v6.13、第 2.5、9.9.2～9.9.4、15、17 节为准。v6.8 的分钟 TWAP 执行合同继续有效；v6.9～v6.12 的 MA-E19/数据阻断状态已由 v6.13 的真实运行事实取代。
...
| F-027 | v6.13 活动演进顺序 | MA-E19R3/D1B-D1D 与 D2 已完成；下一顺序为 rolling LSTM seed314/2718 2026H1 matched 复验 -> 按结果扩展跨 vintage/LOO -> 现实 soft top-down 与 cross-sector lead-lag canary -> NDCG@tail/LTR/quantile/trend-survival。固定静态 blend 停止；bounded gate 只允许严格 OOF。D3 benchmark sector weights 缺口不阻断上述实验。 |
...
v6.13 当前执行顺序覆盖以下历史 Phase：MA-E19R3 12/12、D1B/D1C/D1D 与 D2 已形成真实文件型结果；rolling LSTM 为单 seed 候选腿，固定静态 blend 已被四窗结果拒绝。下一步先做多 seed matched 复验，再推进现实两层板块/lead-lag 与右尾候选生成；D3 缺冻结 benchmark sector weights，保持 fail closed 且不阻断策略演进。新 Alpha 文件快筛与概念线可以在互斥文件、CPU-only 且不占正式实验槽时并行。正式收益、风险、换手和成本评价统一使用 `execution_algo=TWAP`、`backtest_freq=1min`；不允许 portfolio 回退日频。不恢复 MA-E14 阈值 overlay；F-014 平台、通用 UI、历史补账/物化、监控/日志平台和底座完整化继续不实施。
...
### Phase G0-I：v6.13 MA-E19 结果触发型演进
...
8. `[NO_PLATFORM_EXPANSION]` 上述 trial 只复用现有文件数据面、WSL 最多 2 槽、结果面和最小自然输出；不新增 UI、Archive、schema、历史物化、数据库数据面、在线补齐或基础设施完整化任务。
...
v6.13 解释规则：F-001～F-023 的历史 `PLANNED/OPEN/PENDING` 只保留设计和证据状态，不代表当前排期。F-024～F-042 的角色、窗口、数据、TWAP、D1/D2/D3、右尾与 OOF 约束继续有效；当前事实为 D1R/D1B-D1D 和 D2 已完成，D3 Brinson 因冻结 benchmark sector weights 缺失而 `NOT_COMPUTABLE`。实验状态以第 2.5/17 节为准，不把候选腿、oracle 或日频 companion 冒充已部署策略。历史补账、通用 UI、监控/日志平台和平台完整化继续终止。
...
| strategy evolution priority | LSTM_MULTI_SEED_THEN_REAL_SECTOR_LAYER_THEN_RIGHT_TAIL | 唯一 P0 是最佳新策略包演进。先验证 rolling LSTM 多 seed，再执行现实 soft top-down/lead-lag 与右尾目标；只在严格 OOF 证据下研究条件 gate。D3 数据缺口、历史补账、UI/Archive 和平台工程均不得阻断。 |
