...
- 初版日期：2026-07-10
- 当前版本：v5.20（F-014 任务不可变 profile 与历史输入预检已随 PR #2906 合入，生产 DDL 已应用并经重连回读验证；runtime/live/Phase 5 待后续，2026-07-30）
...
| F-014 长期趋势评价层 | `PHASE4_TASK_PROFILE_INPUT_PREVIEW_SOURCE_CI_MERGED_ROOT_SYNCED_DEV_VERIFIED_PROD_DDL_APPLIED_VERIFIED_RUNTIME_LIVE_PHASE5_PENDING` | 既有 Phase 2 R8B 6/6、Loop4 2,004+6、PR #2875 与运行态 API receipt 保持不变；task profile/input preview 正式审核已补齐 order/trade 显式状态与 task summary profile projection；后端定向 92、Phase 2 122、Phase 3 105、QE read 14、mock Playwright 3 passed/1 live skipped；PR #2906 required checks 15/15，通过 squash merge `bba48911342a…` 合入，root 已同步；DEV migration 重跑后零残留；生产 `aistock` 已应用 migration，重连确认 nullable `text`、精确 comment、114 条既有 task 全为 `NULL`，DML 0 行 | runtime activation、real Recorder preview、live browser、其余 5 个 R8B、Phase 5 与批量重评尚未完成；缺输入不形成研究门禁。source worktree/branch 保留待独立清理授权。 |
...
F-014 的可实施详细设计已更新至父设计 v1.17，并继续引用 `qe_long_trend_evaluation_phase2_compute_cas_f2_design_20260722.md`。它只读复用现有 Recorder/prediction pointer 和 `qe_archive.run` 父身份，不扩展通用 Prediction Store 或既有 Archive 通用 writer/schema；使用 feature/outcome 双快照、execution environment identity、extension-only 历史价格校验和右删失；逐信号/episode 明细进入 QE-only CAS Parquet。Phase 2 control/worker/CAS 与 Phase 3 metric/artifact/API/MCP 已完成，并由 R8B Loop4 `qelt_89331d…` 闭合 existing-CAS→DB→API/MCP 与 exact replay。PR #2875 第三审已关闭 Loop/Archive UI slice 阻断；PR #2906 已把默认关闭且 task identity 不可修改的注册 profile、node-owned root 解析和独立只读 historical preview 合入 `main`。新增 migration 已在现有 DEV 重跑并恢复零残留，生产 DDL 已应用且通过重连 readback；runtime activation、live browser 与真实 Recorder 预检仍待后续；其余 5 个 R8B、Phase 5 中断恢复与批量历史重评仍待实施。R8B 6/6 已合计产出 12,024 项指标、13,241,712 行信号与 13,726 个 episode；所有可用结果继续用于研究。
...
2. **P0：F-014 Phase 2–4 与 R8–R11 真实重评**：Phase 2 worker/CAS、Phase 3 三表/API/MCP与 Loop4 2,004 metric + 6 artifact/exact replay 已完成；PR #2875 Loop/Archive UI slice 已合入并完成后端运行态回读。任务创建 profile 与历史输入预检已通过 PR #2906 合入 `bba48911342a…` 且 root sync 完成，DEV migration 零残留与生产 DDL/readback 门禁均已闭合；下一步分别处理运行态激活/live visual 和 Phase 5 恢复 E2E；其他 5 个 R8B 和历史补算继续单独授权。
...
- F-014 详细设计：父设计已更新至 v1.17，Phase 2 从属设计保持有效；当前为 `PHASE4_TASK_PROFILE_INPUT_PREVIEW_SOURCE_CI_MERGED_ROOT_SYNCED_DEV_VERIFIED_PROD_DDL_APPLIED_VERIFIED_RUNTIME_LIVE_PHASE5_PENDING`，其余 5 个 R8B、运行时激活、前端 live 可视验收与 Phase 5 状态单列；
...
| F-014 | 本文 9.6、11.6、Phase G0-E；父设计 v1.17 与 Phase 2 从属设计；Advisory Phase 8 | 既有 R8B 6/6、Loop4 2,004+6、PR #2875/runtime API receipt；task profile migration/create/builders/executor、historical preview API/client/UI、正式审核与 PR #2906 required checks 15/15；DEV migration 零残留；merge `bba48911342a…`；生产 migration apply + reconnect readback | PHASE4_TASK_PROFILE_INPUT_PREVIEW_SOURCE_CI_MERGED_ROOT_SYNCED_DEV_VERIFIED_PROD_DDL_APPLIED_VERIFIED | 源码与生产 DDL 已交付但运行态尚未激活。live visual、其余 5 个 R8B、Phase 5 E2E 与批量重评继续交付。 |
