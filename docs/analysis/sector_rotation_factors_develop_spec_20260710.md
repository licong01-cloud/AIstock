...
- 初版日期：2026-07-10
- 当前版本：v5.0（R8 完整收口、共同成熟区间复核、近期衰减与 R9S 策略转换实验，2026-07-15）
...
v5.0 收口 R8A 12/12 与 R8B 6/6 的最终结果，并从归档逐日 IC/净值曲线补充共同成熟区间和 2024H2/2025/2026H1 分段。原始 RankIC 中 h120 最高，但在全部 18 个 Loop 共同成熟区间 `2024-07-01..2025-12-24` 上，LGBM h60 的 RankIC `0.12303` 高于 h120 的 `0.12115`；h120 因没有 2026 年成熟标签，不能把原始高值解释为对近期行情更有效。R8 同时显示 2026H1 的绝对收益与近期成熟 RankIC 普遍衰减，因此下一步优先复用现有预测和模型做策略转换、种子集成、板块/近期归因与因子角色实验。v5.0 还记录两项当前基础架构事实：`score_weighted_topk_v2` 已可执行 `n_drop` 对照并支持 backtest-only 模型复用；时序 `step_len` 仍由 composer 固定为 20，`runtime_flags.sequence_step_len` 仅是元数据，在代码支持真实可变 `TSDatasetH.step_len` 前不得把 40/60 写入运行卡并冒充已执行。该缺口不限制其他 QE 研究并行推进。
...
| QE 历史文件资产关联回填 | `COMPLETED_IDEMPOTENT` | 扫描 1,018 个归档运行；新增关联 304 个运行/912 条资产索引；原已关联 192；幂等 dry-run 待写 0 | 522 个运行的原始归档文件已不存在，保留为 `MISSING` 而不伪造；R8 的 18/18 个 Loop 均有 3 个文件资产、3 个 raw payload、完整配置和 `reproducibility_level=full`。 |
...
| F-014 长期趋势评价层 | `DESIGN_READY_AMENDED_CODE_PENDING` | `docs/architecture/qe_long_trend_evaluation_f2_design_20260714.md` | QE-only 设计已补齐长期捕获和可成交性桥接；evaluator、QE CAS/三表、API/MCP/UI、DDL 和实际 R6/R8 评价可并行实现。每个指标族独立发布证据状态，平台缺口不阻断研究。 |
| F-014 指标族证据 | `INDEPENDENT_FAMILY_STATUS_PENDING` | 第 9.6 节；F-018/F-020 | signal path、position episode、portfolio、order fill、execution cause 与 platform surface 分别记录 `AVAILABLE/PARTIAL/MISSING/NOT_VERIFIABLE`；任一可算结果立即用于分析，缺失项同步形成获取与互证计划。 |
| R8A 长周期 LGBM 对照 | `COMPLETED_12_OF_12` | `qe_20260715_101942_d873`；h30/h40/h60/h120 均 3/3 完成 | 原始 RankIC `0.09824/0.10290/0.11119/0.12115`，共同成熟区间为 `0.11581/0.11869/0.12303/0.12115`；h60 是当前同区间最强排序期限，h120 更适合作为慢趋势状态探针。绝对 CAGR 从 h30 `62.93%` 降至 h120 `39.64%`，h120 年化换手反升至 `20.28`，明确暴露策略转换不匹配。 |
| R8B LSTM 长周期对照 | `COMPLETED_6_OF_6` | `qe_20260715_104922_001d`；h40、h60 均 3/3 完成 | h40/h60 平均 RankIC、绝对 CAGR、Sharpe、绝对最大回撤、年化换手为 `0.10092/61.88%/1.6848/-18.41%/18.47` 与 `0.10011/61.19%/1.7655/-18.48%/15.98`。LSTM 的 RankIC 不高于同期限 LGBM，但收益转化、回撤和换手更好，继续作为 Type-B 第一主模型。 |
| R8 共同成熟与近期分段 | `COMPLETED_EXISTING_ARTIFACT_ANALYSIS` | 18 个 Loop 的逐日 IC 与 `cumulative_portfolio_with_cost`；共同成熟区间 `2024-07-01..2025-12-24` | 2026H1 绝对收益：LGBM h30/h40/h60/h120 为 `+3.46%/-1.92%/-7.10%/-11.11%`，LSTM h40/h60 为 `+0.99%/-2.10%`。时间衰减是真实研究信号；“科技抱团导致”仍需板块持仓和未捕获右尾事件归因，不做未经证据的因果命名。 |
| R9S 策略转换对照 | `RUNNING_24_LOOPS_TWO_TASKS` | 第 9.6.2 节；LSTM `qe_20260715_190925_2cd9`、LGBM `qe_20260715_190941_b667` | 两个 task 已由 QE 标准执行器启动并完成配置回读：均为 `backtest_only=true`、`force_full_train=false`、G14-FP、Top50，`n_drop=max_n_drop=1/3`；WSL LSTM h40/h60 共 12 Loop，远端 LGBM h60/h120 共 12 Loop。 |
| R8B2 可变 step_len | `RESEARCH_PLANNED_CONFIG_GAP_IDENTIFIED` | `config_composer.py` 当前对 TimeSeries `model_step_len=20`；运行卡的 `sequence_step_len` 仅归档 | 先补真实 `TSDatasetH.step_len` 配置能力再运行 20/40/60 对照；不得用元数据冒充执行值。该程序缺口不影响 R9S、因子角色、TCN、F-014、两层和关系研究。 |
...
| 当前 QE `factor_data`（2018-08-01 至 2026-06-30） | 23 列，含 `l2_code_id` | 123 个数据列，含 `l2_code_id=int16` | WSL 副本已被 R6/R7/R8B 实际读取，同身份远端副本已被 R8A 12/12 实际读取；满足当前 QE 研究，不外推为非 QE 运行时 readiness。 |
...
R8A task `qe_20260715_101942_d873` 已完成 12/12，名称为 `R8A-CPU Type-B G14-FP 长周期标签 h30-h120 4horizon x 3seed LGBM`。h30/h40/h60/h120 均为三种子成功，固定运行于 `rdagent-node1`、并行度 4；任务使用 `filtered_pool_20260630`、G14-FP 14 因子、Alpha158 off、`score_weighted_topk_v2`、Top50、`V25_1_SMALL_CAP`、同一数据 split、费用和 QE-only ST PIT。12 个 Loop 均已进入 QE 数仓，且每个 Loop 有 3 个文件资产、3 个 raw payload、完整配置与 `reproducibility_level=full`。

R8A 的最终均值如下。Calmar 不使用 benchmark fallback 相关口径，而按 absolute CAGR / absolute max drawdown 重新计算：

| 标签 | RankIC 均值 ± seed sd | absolute CAGR | absolute Sharpe | absolute max drawdown | absolute Calmar | 年化换手 |
|---|---:|---:|---:|---:|---:|---:|
| h30 | `0.09824 ± 0.00058` | 62.93% | 1.7788 | -20.19% | 3.12 | 19.33 |
| h40 | `0.10290 ± 0.00062` | 55.53% | 1.6557 | -20.73% | 2.68 | 18.87 |
| h60 | `0.11119 ± 0.00036` | 53.11% | 1.6396 | -21.39% | 2.48 | 17.15 |
| h120 | `0.12115 ± 0.00053` | 39.64% | 1.3105 | -20.91% | 1.90 | 20.28 |

原始 RankIC 随期限上升，但不同期限的最后可成熟日不同。把 R8A/R8B 全部 18 个 Loop 对齐到共同成熟区间 `2024-07-01..2025-12-24` 后，R8A h30/h40/h60/h120 RankIC 变为 `0.11581/0.11869/0.12303/0.12115`：h60 高于 h120，说明 h120 的原始最高值部分来自缺少 2026 年低表现样本。h120 仍有较高的正 RankIC 日期比例，可继续作为慢趋势状态探针；当前 Top50 策略下其年化换手反而高于 h60，且收益转化明显变弱，不能把长标签直接等同于长持仓。

#### 9.6.2 R8B LSTM 主模型证据与其他模型并行顺序

R8B 不读取 R8A 中途结果，而是提前预注册 h40/h60 两个业务优先期限作为并行 canary；每个期限使用 seeds `123/314/2718`，首轮固定 `step_len=20`，共 6 个 `gpu_parallel_standard` Loop。这样可在 R8A 完成前利用独立 GPU 节点缩短等待时间，同时仍能在事后按同期限、同种子比较 LGBM 与 LSTM 的模型归纳偏置。R8B 按原卡归档，不回写改变已完成 Loop。

R8B 已完成，task 为 `qe_20260715_104922_001d`，名称为 `R8B-GPU Type-B G14-FP LSTM h40-h60 2horizon x 3seed canary`。模型固定 `__seed_LSTM_10D_hs64_d02__`，使用 `TimeSeries`/`TSDatasetH`、`step_len=20`；G14-FP 14 因子、数据快照、股票池、split、Alpha158、策略、Top50、执行算法、费用和 QE-only ST PIT 与 R8A 完全一致。6 个 Loop 分配到 `wsl2-5080`，并行度 2，h40 与 h60 均 3/3 成功。h40/h60 的平均 RankIC、absolute CAGR、absolute Sharpe、absolute max drawdown、absolute Calmar、年化换手分别为 `0.10092/61.88%/1.6848/-18.41%/3.39/18.47` 与 `0.10011/61.19%/1.7655/-18.48%/3.31/15.98`。相对同期限 LGBM，LSTM h40/h60 的 CAGR 高 `6.35/8.09` 个百分点，回撤改善 `2.32/2.91` 个百分点；h60 Sharpe 高 `0.126`、年化换手低 `1.17`。LSTM 的 RankIC 未形成单向优势，但收益转化、风险调整后表现和换手更适合作为当前长期腿主模型。

全部 18 个 R8 Loop 的共同成熟和时间分段进一步显示：2026H1 absolute portfolio return 中，LGBM h30/h40/h60/h120 为 `+3.46%/-1.92%/-7.10%/-11.11%`，LSTM h40/h60 为 `+0.99%/-2.10%`。各期限在 2024H2 和 2025 年明显更强；最近 126 个自身可成熟信号日中，LGBM h40/h60 RankIC 为 `0.01966/0.04933`，LSTM 为 `0.02504/0.05799`。这证明近期衰减需要单独研究；当前数据尚未把衰减因果定位到科技抱团，下一步使用板块持仓、板块收益、未捕获右尾事件和 F-014 regime 切片继续归因。

R8 的模型归因也提示任务角色发生变化。同为 LightGBM gain，显式板块相关因子的合计权重从 h20 约 `33.6%` 降到 h60/h120 约 `20.0%/19.6%`，`Price_Deviation_Historical_High` 从 `20.9%` 升至 h120 `37.3%`，`RevenueYOY_Momentum` 从 `7.7%` 升至 `16.9%`；因此 h120 更像一般慢趋势/基本面状态，而不是纯板块轮动腿。LSTM 的 prediction-correlation 归因中板块相关因子在 h40/h60 约占 `52.9%/52.0%`，F/P 两因子约占 `13.8%/16.4%`；该证据支持长期因子角色消融，但不同模型的 gain 与 correlation 不能直接互相排序，也不能替代正式消融。

##### 9.6.2.1 R9S backtest-only 策略转换实验卡

R9S 直接复用 R8 已训练模型和预测，隔离“模型预测”与“持仓转换”的差异。首批只改变 `n_drop`，不同时改变 `hold_thresh`、TopK、模型、标签、因子、费用或执行算法：

| 项目 | R9S-LSTM | R9S-LGBM |
|---|---|---|
| task/status | `qe_20260715_190925_2cd9` / `RUNNING` | `qe_20260715_190941_b667` / `RUNNING` |
| 来源 | R8B Loops 1–6 | R8A Loops 7–12 |
| 模型/期限 | LSTM h40/h60，各 3 seeds | LGBM h60/h120，各 3 seeds |
| 新变量 | `n_drop=max_n_drop=1/3`；既有 R8 默认值 5 作为不重跑基线 | `n_drop=max_n_drop=1/3`；既有 R8 默认值 5 作为不重跑基线 |
| 固定项 | Top50、`enable_dynamic_ndrop=true`、`min_n_drop=0`、G14-FP、2026-06-30 数据、split、Alpha158 off、V25、费用、ST PIT | 同左 |
| 执行 | `backtest_only=true`、`force_full_train=false`，`wsl2-5080` 并行 2，12 个新 Loop | `backtest_only=true`、`force_full_train=false`，`rdagent-node1` 并行 4，12 个新 Loop |
| 主比较 | absolute CAGR/Sharpe/max drawdown/Calmar、换手、持仓数量和 2026H1 分段 | 同左 |
| 后续补充 | F-014 position episode、持仓生存、false exit、MFE/MAE、板块暴露 | 同左 |

当前 V2 策略目录默认 `n_drop=5`、`max_n_drop=5`、`enable_dynamic_ndrop=true`、`min_n_drop=0`，实际动态替换上限由 `max_n_drop` 消费。因此 R9S 同时设置 `n_drop` 与 `max_n_drop`，避免只修改兼容字段却没有改变执行行为。选择 1/3 而不是一次加入更多策略参数，是为了让首批结果能明确回答降低每日替换上限是否改善长期信号的收益转化。该实验不声称已经覆盖决策周期、排名缓冲和趋势失效退出；这些仍是并行研究方向，未来可在现有策略能力或新增 QE-only 策略支持下另立 trial。

##### 9.6.2.2 R8B2 step_len 基础架构事实

R8B2 的研究假设仍是 `step_len=20/40/60 × label h40/h60`，但当前 `config_composer.py` 对有源码的 TimeSeries/`TSDatasetH` 模型直接设置 `model_step_len=20` 并写入 YAML；运行卡的 `runtime_flags.sequence_step_len` 不参与 composer。由此，当前直接创建 40/60 任务会实际继续使用 20。v5.0 将其记录为 QE 基础架构补充任务：未来必须把可执行 `step_len` 纳入 loop schema、API/MCP、composer、execution manifest 和归档回读，并验证生成 YAML 的 `dataset.kwargs.step_len` 与实验卡一致。该事实不暂停 LSTM、TCN、R9S、因子角色、F-014、两层或关系研究。
...
1. LSTM：Type-B 长期趋势第一主模型。当前先执行 R9S 的 backtest-only 策略转换、G13-F/G14-FP/G15-FPL 因子角色、跨种子集成和 R8C 长期腿组合；R8B2 在补齐真实可变 `step_len` 后执行 `20/40/60` 对照，R8M 首个共享/迁移表示优先采用 LSTM encoder。
...
截至 2026-07-15，R2–R7 与历史失败 task 的 Loop 级证据已补账，R8A 12/12、R8B 6/6 和历史文件资产关联回填均已完成；共同成熟区间和 2026H1 分段已进入研究总账。以下仅按当前信息增益/成本排列，不是依赖链或研究许可条件；任一方向可在 QE 范围内并行，资源不足时调整并发和先后，不停止研究路线：

1. **P0：R9S 策略转换 backtest-only 对照**：复用 R8B LSTM h40/h60 与 R8A LGBM h60/h120 三种子模型，运行 `n_drop=max_n_drop=1/3`，以既有默认值 5 为不重跑基线；优先解释降低替换上限能否改善收益转化、回撤、换手、持仓生存和 2026H1。
2. **P0：R8 现有制品的种子集成、共同成熟和近期/板块归因**：建立 h40/h60 LSTM ensemble 与 h60/h120 LGBM ensemble，报告共同日期、2024H2/2025/2026H1、板块持仓/收益、未捕获右尾事件和模型/因子归因；不把“近期弱”直接命名为特定市场原因。
3. **P0：LSTM 长期限因子角色**：G13-F 收益转化、G14-FP 均衡、G15-FPL 排序增强分别执行 h40/h60 三种子对照；R8B 的 G14-FP 结果直接复用，不重复训练。
4. **P0：F-014 QE-only 实现和 R8 历史重评**：计算、可成交性、CAS/三表、API/MCP/UI、历史补算和 E2E 并行；每个指标族完成即发布，不控制其他实验是否开始。
5. **P1：R8B2 可变 step_len 基础架构与实验**：先让 loop schema、API/MCP、composer、manifest 和归档回读真实支持 `20/40/60`，随后执行 h40/h60 三种子矩阵；不得只改 `runtime_flags`。
6. **P1：TCN 长期限并行对照**：以 G14-FP h40/h60 三种子为首个同口径矩阵，再比较 G13-F/G15-FPL；R1 成功证据使其成为 LSTM 主线旁的高价值结构对照，而非等待或替代 LSTM。
7. **P1：R8C 多任务角色组合**：h120 LGBM 作为慢趋势状态、h60 LSTM ensemble 作为长期主腿、h20 LGBM 或 h40 LSTM 作为入场时机；比较 portfolio fusion、风险预算、软条件化和 LOO，不重复 R7 的简单同标签等权 prediction-fusion。
8. **P1：四格 oracle、真实 hard/soft 两层与板块层评分**：oracle 和 reality 同时推进，输出连续上界、现实损失、板块 Recall 与板块内排序归因，不产生 `GO/STOP`。
9. **P2：R8M 与迁移对照**：以 LSTM encoder 为首个主实现，比较独立训练、共享 head、冻结迁移、全量微调、transfer matrix、LOO 和梯度冲突；PCGrad/动态权重作为可并行对照，不等待单一指标裁决。
...
12. **P2/P3：关系与状态模型**：GAT h40/h60、portfolio fusion、HIST-industry、动态多关系、MASTER/IGMTF/TRA、DoubleAdapt、概念 PIT/代理实验和超图均保持开放；当前二值行业边、时间 Transformer 或某个种子结果不外推到其他结构。
13. **P3：ALSTM/GRU2/Transformer 等当前弱或失败配置复核**：先保存现有弱/失败证据；可按 wiring 修复、任务定义或结构变化另立新卡，不使用 `KILL` 或方向淘汰状态。
...
- R8A/R8B/R9S/R8C 实验卡：R8A LGBM 长周期标签扫描 `qe_20260715_101942_d873` 已 12/12 完成，R8B LSTM canary `qe_20260715_104922_001d` 已 6/6 完成；R9S 已启动 LSTM `qe_20260715_190925_2cd9` 与 LGBM `qe_20260715_190941_b667` 两个 backtest-only task，复用其模型做 `n_drop=1/3` 策略转换；R8C 和更多腿可并行设计/执行，并持续吸收 F-014 新增指标族；
- 与 Advisory Phase 8 对齐的 20–180 日、MFE/MAE、有序目标、time-to-hit、生存、右删失、捕获率、假退出，以及信号→成交/退出阻断分层报告仍 pending；
- F-014 详细设计：`docs/architecture/qe_long_trend_evaluation_f2_design_20260714.md`，覆盖指标族独立状态、双快照、理论机会与可成交性桥接、QE-only CAS/三表、API/MCP/UI、重启恢复和非 QE 零影响；当前为 `DESIGN_READY_AMENDED`，任一指标族完成即形成科研证据，平台交付状态单列；
...
3. `[R8A_R8B_COMPLETED_AND_ARCHIVED]` R8A 已完成 12/12，R8B 已完成 6/6；18 个 Loop 均进入数仓并关联 3 个文件资产。共同成熟区间与 2026H1 分段已经补账，LSTM 明确为 Type-B 第一主模型，h60 是当前 LGBM 共同区间最强排序期限，h120 保留慢趋势状态角色。
4. `[R9S_RUNNING_TWO_TASKS]` LSTM `qe_20260715_190925_2cd9` 复用 R8B h40/h60，LGBM `qe_20260715_190941_b667` 复用 R8A h60/h120；两者均以三种子执行 `n_drop=max_n_drop=1/3` backtest-only，对照既有默认值 5。启动后回读确认 24 个 Loop 全部为 G14-FP、Alpha158 off、正确节点和来源引用；随后并行推进 LSTM 因子角色、种子集成、R8C、TCN、LTR 和正交模态。
5. `[DESIGN_READY_AMENDED]` 按 F-014 三工作流实现 QE-only evaluator；指标族结果完成即分析，平台交付状态单列，不阻断 R8B2、R8M、R8C 或期限研究。
6. `[DESIGN_READY_RUN_PENDING]` 四格 oracle、soft-gating 与真实 hard/soft 两层模型并行；不输出 go/stop。
7. `[PLANNED]` 按第 9.6.3 节建立独立 R8M，wiring、transfer、多种子和 F-014 各指标族可并行，不预设共享表示优于独立训练。
8. `[PLANNED]` 并行执行关系模型、R8C/更多腿和交叉数据研究；若当前 trial 未转化为可成交的 60–180 日右尾捕获，分析因子、标签、模型容量、腿数、成本和执行损失并形成新实验。
...
| L4 | GATs/LGBM、融合/两层 oracle 与真实模型、长期趋势、R8M、PIT 关系模型、成本容量、拥挤/尾部、DSR/PBO 与多种子业务流 | R2–R7 与历史失败 task 的成功 Loop 已补账，R8A 12/12、R8B 6/6 及共同成熟/近期分段已完成；R9S、F-014、oracle、R8M、两层、capacity/DSR/PBO、关系模型和 Loop 总账继续按 F-010、F-013–F-021 记录证据。 |
...
| F-014 | 本文 9.6、11.6、Phase G0-E；`qe_long_trend_evaluation_f2_design_20260714.md`；Advisory Phase 8 | 20–180 日标签基础架构；R8A/R8B receipt；共同成熟/近期分段；signal path、position episode、portfolio、order fill、execution cause、sector regime；QE-only CAS/三表/API/MCP/UI | APPROVED_BY_USER: DESIGN_AMENDED_NO_RESEARCH_GATE | 30–180D 标签与 maturity purge 已实现；R8A 12/12、R8B 6/6 和现有曲线分段已完成。各指标族独立计算/发布科研证据；缺失项形成数据行动计划，平台链完成度单列。 |
...
| F-017 | 本文 9.9、Phase G0-F、14.3 | 模型资源分类、cache/recorder 隔离、并行制品、backtest-only、combine-backtest 与 restart recovery | APPROVED_BY_USER: PARTIAL_RUNTIME_VERIFIED | R6、R7A/R7B 证明组合路径可运行；R8A 已按远端 CPU 4 完成 12/12，R8B 已按标准 GPU 模型 2 完成 6/6，且 18 个 Loop 文件资产可查询。R9S 继续验证跨节点 backtest-only 并发、归档和后端重启后的状态恢复。 |
...
- v5.0 文档与实验 rollout：继承 v4.7 的无研究门禁原则与 v4.8/v4.9 的 Loop 级补账和 LSTM 主线；补齐 R8A 12/12、R8 共同成熟/2026H1 分段、历史文件资产关联回填、R9S backtest-only 实验卡和 R8B2 step_len 基础架构事实。R9S 已以 LSTM `qe_20260715_190925_2cd9`、LGBM `qe_20260715_190941_b667` 启动 24 个回放 Loop；本次文档变更不修改代码、DDL、数据集或非 QE 运行时，实验只写 QE task/Loop/结果并复用已训练模型。
...
| 项目 | 当前状态 | 说明 |
|---|---|---|
| source merge | V5_0_DOC_BRANCH_PENDING_MERGE | 历史代码已在 main；v5.0 蓝图更新在独立文档分支提交后进入合入流程，实验 task 状态与文档合并状态分别报告。 |
...
| QE research writes | R8_ARCHIVE_COMPLETE_R9S_TASK_WRITES_ACTIVE | 官方因子指标/相关性和 R8 QE archive 已持久化；历史资产关联回填幂等完成。R9S 两个 task 正在写 QE task/Loop/结果，不写非 QE 表。 |
...
| QE experiment | R6_R7_R8A_R8B_COMPLETED_R9S_RUNNING | R8A 12/12、R8B 6/6 已完成；R9S-LSTM `qe_20260715_190925_2cd9` 与 R9S-LGBM `qe_20260715_190941_b667` 已启动并通过配置回读。LSTM 因子角色、两层、关系、长期 evaluator 和数据获取可并行。 |
