...
- 初版日期：2026-07-10
- 当前版本：v5.18（F-014 任务不可变 profile 与历史输入预检正式审核修复、DEV migration 零残留验证完成；PR #2906 已打开，修复 commit/push、CI、生产 DDL/runtime 待后续，2026-07-30）
...
| F-014 长期趋势评价层 | `PHASE4_TASK_PROFILE_INPUT_PREVIEW_REVIEW_REPAIRED_LOCAL_DEV_DDL_VERIFIED_PR_OPEN_REFRESH_CI_PENDING_PROD_DDL_RUNTIME_PENDING` | 既有 Phase 2 R8B 6/6、Loop4 2,004+6、PR #2875 与运行态 API receipt 保持不变；本轮新增默认关闭且 task identity 不可修改的 `qe_long_trend_v1` 创建开关、执行器节点权威 root 解析、只读 dataset/Recorder/catalog 输入预检；正式审核已在当前工作树补齐缺失 order/trade 显式状态与 task summary profile projection；因 init mirror 触发 changed-file HIGH guardrail，AC 文案仅澄清既有显式 TWAP 分支，不改变执行逻辑；后端定向 92、Phase 2 122、Phase 3 105、QE read 14 passed，TypeScript/ESLint PASS、mock Playwright 3 passed/1 live skipped；现有 DEV migration forward/readback/reapply/guarded rollback/零残留通过；PR #2906 已打开 | 正式审核修复尚未 commit/push；PR #2906 refresh/CI、生产 DDL、runtime/live browser 尚未完成；其余 5 个 R8B、Phase 5 与批量重评继续单列。缺输入不形成研究门禁。 |
...
2. **P0：F-014 Phase 2–4 与 R8–R11 真实重评**：Phase 2 worker/CAS、Phase 3 三表/API/MCP与 Loop4 2,004 metric + 6 artifact/exact replay 已完成；PR #2875 Loop/Archive UI slice 已合入并完成后端运行态回读。任务创建 profile 与历史输入预检已完成正式审核修复和本地验证，DEV migration 零残留门禁已闭合；下一步是提交/推送审核修复、刷新 PR #2906、重跑 CI，再分别处理生产 DDL 授权/执行/回读、运行态激活/live visual 和 Phase 5 恢复 E2E；其他 5 个 R8B 和历史补算继续单独授权。
...
| F-014 | 本文 9.6、11.6、Phase G0-E；父设计 v1.15 与 Phase 2 从属设计；Advisory Phase 8 | 既有 R8B 6/6、Loop4 2,004+6、PR #2875/runtime API receipt；本轮 task profile migration/create/builders/executor、历史输入 preview API/client/UI、正式审核补齐 order/trade 显式缺失与 task summary profile projection、后端/TS/ESLint/mock Playwright 定向证据；DEV migration 零残留 receipt | PHASE4_TASK_PROFILE_INPUT_PREVIEW_REVIEW_REPAIRED_LOCAL_DEV_DDL_VERIFIED_PR_OPEN_REFRESH_PENDING | PR #2906 已打开但审核修复尚未 commit/push；新增源码尚未合入/激活。PR CI、生产 DDL、live visual、其余 5 个 R8B、Phase 5 E2E 与批量重评继续交付。 |
...
- v5.18 F-014 Phase 4 follow-up source：新增 nullable task profile migration 三件套与 init mirror；标准/fork/strategy/custom 新 task identity 使用同一注册 profile，既有 task 禁止变化；四条 builder 与 executor 在节点权威 root 上激活 normal postprocess；独立只读预检列出 dataset/Recorder/artifact 可用性与 data action，UI 明示 technical readiness 非研究门禁并始终保留创建入口。PR #2906 已打开；正式审核已在当前工作树补齐正常 catalog 下 order/trade 显式缺失行和 summary task profile projection。因 init mirror 触发 changed-file HIGH guardrail，AC 文案仅澄清既有显式 TWAP 分支，不改变执行代码、默认值或配置。聚焦后端更新为 92 passed；修复 commit/push、PR CI、生产 DDL、运行时、live browser、其余 R8B 和 Phase 5 均未执行。
