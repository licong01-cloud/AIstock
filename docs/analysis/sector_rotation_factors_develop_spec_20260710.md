...
| F-014 长期趋势评价层 | `PHASE4_TASK_PROFILE_INPUT_PREVIEW_SOURCE_LOCAL_DEV_DDL_VERIFIED_PR_PROD_DDL_RUNTIME_PENDING` | 既有 Phase 2 R8B 6/6、Loop4 2,004+6、PR #2875 与运行态 API receipt 保持不变；本轮新增默认关闭且 task identity 不可修改的 `qe_long_trend_v1` 创建开关、执行器节点权威 root 解析、只读 dataset/Recorder/catalog 输入预检；后端定向 91 passed、TypeScript/ESLint PASS、mock Playwright 3 passed/1 live skipped；现有 DEV migration forward/readback/reapply/guarded rollback/零残留通过 | 本轮 PR、生产 DDL、runtime/live browser 尚未完成；其余 5 个 R8B、Phase 5 与批量重评继续单列。缺输入不形成研究门禁。 |
