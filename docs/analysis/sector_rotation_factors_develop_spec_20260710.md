...
- 初版日期：2026-07-10
- 当前版本：v6.10（截至 2026-08-25 的进度与执行方案权威；策略演进最佳新策略包仍是唯一 P0。MA-E19R2 `qe_20260825_031740_2457` 已完成 2024H2～2025H2 的 9/12 个分钟 `TWAP` arm，2026H1 三臂因 BUG-1191 / Issue #3793 揭示的分钟 Bin `instruments/all.txt` 截止 2026-04-28、而物理 1min feature/calendar 已到 2026-06-30 的交叉文件覆盖不一致而 fail closed。该缺陷是 universe 元数据缺陷，不是模型或 5,092 个分钟 feature 大面积缺失；完整 D1 裁决仍为 `INCOMPLETE`。高价值顺序调整为 BUG-1191 candidate-only 修复与签核 → 用户确认的 candidate 激活 → 完成 MA-E19R2 2026H1 三臂 → D1 synthesis → sector oracle → benchmark/Brinson → 结果触发的适应、两层板块/lead-lag、右尾/meta-labeling、动态退出或受约束组合构建。受影响历史实验只做最小研究解释修正：明确窗口越过 2026-04-28 的 V25/V25_1 策略执行指标待分钟 universe 修复后代表性重跑，模型/prediction/label/IC/RankIC 不自动失效；不恢复历史补账、Archive/UI 或批量物化。canonical PIT v2、概念板块、零数据库数据面和 maintenance-only 基础设施边界继续独立有效。）
...
> **历史标题解释：**下列 v6.1～v6.9 快照中遗留的“当前”“权威”“继续有效”只表示各自日期当时的原始记录，均不再表示 2026-08-25 的活动状态；活动进度、总账、顺序与门禁以 v6.10、第 2.5、9.9.2～9.9.4、15、17 节为准。v6.8 的分钟 TWAP 执行合同继续有效；v6.9 的 MA-E19A/B 9/12 状态已由 MA-E19R2 的新运行事实取代。
...
### 2.5 当前执行总账（截至 2026-08-25）

本表是阅读本文时判断“已完成/待执行/仅设计”的首要入口。历史 Gate-0 receipt 不因后续进度而删除，但当前状态以本表、对应实验 task/run 和第 15 节验收矩阵为准。

实时事实的最后核对时间为 2026-08-25：MA-E19R2 `qe_20260825_031740_2457` 是最新演进 task，Loop1～9 completed、Loop10～12 failed，且没有可用于 D1 完整裁决的 2026H1 portfolio。三臂统一失败于分钟 universe 覆盖 guard；BUG-1191 / Issue #3793 已登记，源码、PR、candidate、节点分发、active activation 和三臂重跑保持独立状态。回测数据集 CLI 的 canonical PIT v2 状态继续由其 durable workflow 独立维护，不能用本次 v1 分钟 universe 修复冒充 v2 signoff。当前 root `main`、运行身份和其他 BUG 也由各自工作流维护；本文不把 source merge、runtime activation、candidate signoff 或生产切换合并为一个状态。
...
| canonical PIT v2 候选 | `SOURCE_FIX_MERGED_DURABLE_SUBMISSION_STILL_BLOCKED` | profile `qe_hmm_full_v2`；submission `dss_d41b1818feb7c81b0124b193b74b0349` 仍为 `BLOCKED_CONTRACT`；resolution attempt `dsra_402c713cc20548d7be7c15a6eed56bb6`；source-freeze `BLOCKED_SOURCE_AUDIT_INCOMPLETE`；BUG-1157 / Issue #3700 open；PR #3706 merge `4fb89674…` | source 修复与数据状态分开：依赖 canonical PIT v2 的新正式实验、sample activation 与生产切换继续暂缓，直到 worker-scheduler 身份、repair attempt、candidate signoff 分别完成。既有 2026-06-30 文件快照不被改写；禁止删异常行、放宽 builder、回退幸存者池、绕过 terminal evidence 或在 QE 数据面访问数据库。 |
...
| 分钟线 TWAP 执行口径 | `MA_E19R2_PARTIAL_9_OF_12_UNIVERSE_METADATA_BLOCKED` | v6.8/v6.10；MA-E19R2 九个成功 arm；`execution_algo=TWAP`、`backtest_freq=1min`、`rdagent-node1`；BUG-1191 | 正式 portfolio 评价仍统一使用 1min `NestedExecutor` 与 `TailTWAPWithLimitStrategy`；`CLOSE_PRICE` 仅为诊断 companion。确定性重跑已前进到 9/12，但 2026H1 被过期 `all.txt` 正确阻断；candidate 修复与三臂完成前，禁止把部分结果写成完整收益验证。 |
...
| MA-E19 P0-D1 model-age/refit | `R2_PARTIAL_9_OF_12_BLOCKED_BY_MINUTE_UNIVERSE_METADATA` | MA-E19R2 `qe_20260825_031740_2457`：Loop1～9 completed，Loop10～12 failed；物理 1min feature/calendar 到 2026-06-30，4,922 条 `all.txt` span 截止 2026-04-28 | 2024H2～2025H2 的新结果可作为确定性修复后的部分证据；2026H1 fixed/expanding/rolling 全缺，D1 仍 `INCOMPLETE`。先完成 BUG-1191 candidate/激活，再完成三臂；此前不晋级 DoubleAdapt/Proceed 或 learned gate。 |
...
截至 2026-08-25 的活动权威由 v6.10 覆盖：MA-E19R2 已形成 9/12 的 P0-D1 确定性重跑部分证据，BUG-1191 的分钟 universe candidate 修复、独立激活授权与 2026H1 三臂完成是当前第一执行工作包；D1 synthesis、D2 sector oracle 与 D3 benchmark/Brinson 随后执行。所有后续方向采用结果触发而非论文热度排序；新横截面专家可在不占用正式实验资源时完成文件数据、公式和快速筛选设计，但在 canonical PIT v2 candidate signoff 前不提交依赖该数据的新正式 QE run。
...
**P0-D1 v6.10 进度修正。**MA-E19R2 `qe_20260825_031740_2457` 已在 BUG-1133 修复身份后完成 12 arm 中的 9 arm，覆盖 2024H2～2025H2 的 fixed/expanding/rolling；2026H1 三臂统一由分钟 universe 元数据覆盖 guard 阻断。该失败不是 Alpha 负结果，也不要求重跑已完成九臂。下一任务固定为 BUG-1191 candidate-only 修复、覆盖/hash/双节点签核、用户独立确认激活，再以现有 checkpoint/attempt 一次提交语义完成 Loop10～12 或等价新身份三臂。固定 CE3、seed 123、h20、21 日 purge、Top50/n_drop1、分钟 TWAP、成本、股票池和数据 identity，保存 prediction/order/strategy identity。只有 12/12 完整矩阵可计算后，才决定是否增加 LSTM cadence、多 seed 或适应模型。
...
21. `docs/analysis/ma_e19_p0_triad_and_alpha_execution_plan_20260824.md` v1.1 是 v6.10 的统一执行任务卡：它只能细化本文既有优先级、输入、输出、触发与退出条件，不能改变本文的策略唯一 P0、零数据库、分钟 TWAP、资源上限、maintenance-only 和用户进程控制边界；二者冲突时以本文 v6.10 当前权威段、2.5、9.9.2～9.9.4、15、17 为准。

#### 9.9.4 v6.9 DESIGN-COMPLIANCE-001 文档审核

- `no_simplified_delivery`：当前总账逐项保留 MA-E01～MA-E16，并新增 MA-E19A/B 两个真实 task、全部 12 arm 的 completed/failed 状态、九臂指标与 2026H1 三臂缺口；不把 9/12、BUG 修复、执行方案或部分 TWAP canary冒充 D1 完成。统一执行方案覆盖 D1R、D2、D3、结果触发 P1、新 Alpha 卡、分钟因子延期和明确结束标识，不以 POC/子集替代完整矩阵。
- `no_silent_error`：BUG-1133 的同分排序非确定性、三臂失败、单 seed、portfolio 不可裁决、canonical PIT v2 durable source-audit 阻断，以及 BUG-1157 仅 source merged、runtime/repair/signoff 尚未完成的差异均显式记录；概念实现未开始与分钟历史数据缺口同样保留。新增诊断继续把 signal/portfolio/execution、staleness/regime、absolute/active、sector/within-sector 分栏；缺 prediction/order identity、分钟数据、benchmark/PIT 权重、matured label、OOF prediction 或 taxonomy 时为 `NOT_COMPUTABLE`，禁止 DB 回退、补零和口径替换。
- `no_business_semantic_drift`：裸 h20 主标签、CE3、seed 123、21 日 purge、既有模型、QE-only 文件数据面、Top50/n_drop1、费用与分钟 TWAP 均不改写；MA-E19R 重新运行完整矩阵是为恢复确定性证据，不重解释或覆盖旧 run。benchmark/Brinson、tail/meta/gate、受约束组合和概念线均另立身份，不能利用测试窗口选方向或把风险改善反推为 Alpha。
- `no_unrequested_gate_or_approval`：PIT v2 阻断和 D1→D2→D3 顺序是数据/信息依赖，不是新增人工审批；任何已有、PIT 可证明的文件证据仍可用于规划和低成本只读分析。继续不恢复 P0-1～P0-4、F-014、UI、Archive、BUG-852、历史完整度或平台建设门禁，也不产生进程控制、DDL/DML、依赖安装、candidate activation、生产切换或合入权限。
...
| F-027 | v6.10 活动演进顺序 | BUG-1191 candidate-only 修复/签核 -> 用户确认的合入与激活 -> MA-E19R2 2026H1 三臂 -> D1 synthesis -> P0-D2 四格 sector oracle -> P0-D3 benchmark/Brinson -> 按结果触发适应/门控、两层板块/lead-lag、右尾/退出、横截面/正交或受约束板块主动权重。已完成九臂不重复训练；新 Alpha 文件快筛和概念 candidate 可在不占正式实验槽时并行。 |
...
### Phase G0-I：v6.10 MA-E19R2、分钟 universe 修复、P0 三联诊断与结果触发型演进

1. `[MA_E19R2_PARTIAL_9_OF_12_BUG1191_BLOCKED]` execution parity / model age：MA-E19R2 已在 BUG-1133 修复身份上完成 2024H2～2025H2 九臂；2026H1 三臂因分钟 Bin `all.txt` 与物理 feature/calendar 覆盖不一致而 fail closed。先完成 BUG-1191 的源码、候选验证、用户确认的合入/激活，再只完成缺失三臂；已完成九臂不重复训练。node1 全局只允许 1 个重训练/回测 arm，WSL 不占本批正式槽。
2. `[PENDING_AFTER_D1R]` D1 synthesis：核对四个 vintage 的 IC/RankIC、Top-K/右尾、CAGR/Sharpe/MDD/Calmar、换手/成本、prediction/order identity 与 determinism replicate；分离可由新近成熟数据恢复的成分和 calendar-regime 剩余弱化。只有完整矩阵支持恢复时，才设计 LSTM cadence 或 DoubleAdapt/Proceed canary。
3. `[PENDING_AFTER_D1R]` D2 sector oracle：运行 reality/oracle sector × reality/oracle stock 四格、hard/soft 两层、L2 主 taxonomy 与可用时的 PIT L1 敏感性；保存 sector recall、within-sector rank、右尾捕获、TWAP 成本/成交、主动暴露和不可部署标记。
4. `[PENDING_AFTER_D1R]` D3 benchmark/Brinson：对 D1R 与当前主要腿同时保留 absolute/active 口径；冻结 benchmark 与等权可投资池，计算 beta、tracking error、IR 与 allocation/selection/interaction。benchmark/sector weight 输入缺失时 `NOT_COMPUTABLE`，不建设 DB/历史回填链路。
5. `[RESULT_TRIGGERED_NOT_PRECOMMITTED]` 若 D1 恢复稳定，比较简单重训与 DoubleAdapt/Proceed；若 D2 板块上界显著，推进现实两层、跨板块 lead-lag 与动态关系；若右尾召回仍低，推进 LTR/NDCG/tail/hazard；若候选已召回但转化/退出弱，再推进 meta-labeling；若 D2+D3 证明 allocation 主导，再运行受约束主动板块权重与 risk-budget/HRP 对照。TRA/MIGA gate 只在现有腿出现条件互补后运行。
6. `[ALPHA_DESIGN_PARALLEL_NO_FORMAL_RUN]` 在数据阻断期间，完成 participation gap、leadership exhaustion、residual cohesion break、dynamic residual flow relation 与有 PIT 文件才允许的 fundamental diffusion 任务卡和日频/H5/Parquet 快筛方案；概念线保持 `DESIGN_READY_NOT_STARTED`。分钟级新信号因子保持 `DEFERRED_DATA_NOT_READY`。
7. `[NO_PLATFORM_EXPANSION]` 上述 trial 复用现有文件数据面、双节点调度、结果面和最小自然输出；不新增 UI、Archive、schema、历史物化、数据库数据面、在线补齐或基础设施完整化任务。
...
| F-040 | 本文 v6.8/v6.10 权威、2.5.3、9.9.3、12～14、Phase G0-I；MA-E19R2 task detail；`config_composer.py`、`qrun_limit_minute.py`、`tail_twap_strategy.py` | validation-receipt: 九个成功 arm 均为 `TWAP/1min`、Top50/n_drop1、node1、文件数据面；2026H1 三臂由交叉文件 coverage guard 拒绝；完整 config 仍需绑定 prediction/order、费用、涨跌停/停牌、交易单位和确定性证据 | APPROVED_BY_USER: PARTIAL_MINUTE_TWAP_9_OF_12_BUG1191_BLOCKED | candidate 激活与三臂完成前不得宣称完整收益已验证 |
...
| 项目 | 当前状态 | 说明 |
|---|---|---|
| source merge | BUG_1191_SOURCE_IN_PROGRESS_NOT_MERGED | BUG-1191 / Issue #3793 已登记；源码、测试、审核、PR、合入、candidate 和激活分别记账。BUG-1133 已 verified/closed；canonical PIT v2/BUG-1157 状态保持独立。 |
| QE dataset | V1_MINUTE_FEATURES_TO_20260630_UNIVERSE_METADATA_TO_20260428_CANDIDATE_FIX_PENDING | v1 分钟 feature/calendar 本体覆盖 2026-06-30，但 active `all.txt` 大部分截止 2026-04-28；BUG-1191 candidate 与 active activation 未完成。canonical PIT v2 signoff 继续独立，实验运行期禁止数据库访问。 |
...
| candidate snapshot | BUG_1191_MINUTE_UNIVERSE_CANDIDATE_NOT_BUILT_NOT_ACTIVATED | BUG-1191 只允许产生新 immutable candidate 并做覆盖/hash/双节点签核；未获独立授权前不得覆盖 active Bin 或把源码修复写成数据已生效。 |
