...
- Latest integration commit before this checkpoint commit: `714ef8a merge(qe): phase 2 asset ledger foundation`
...
The latest integration gates after Phase 2 asset ledger:
...
- Dev DB migration smoke not yet run for the full governance migration stack.
...
2. Run:
   ...
   - `git log --oneline -12`
3. Confirm integration branch is still clean and at or after `714ef8a`.
4. Do not merge to `main`.
5. If continuing governance, first run dev/test DB migration smoke for:
...
6. Then run dev-port smoke on a real governance-enabled dev backend/frontend, not production `8001`.
7. Coordinate with Claude Code before touching Paper v2 / vn.py / trading_core files.
