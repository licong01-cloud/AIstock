...
- Immediately after the canonical root fast-forward, and before close-sync or cleanup, the single merge-aftercare owner runs change-scoped `install-client --apply` plus `verify-clients --workflow-only` for only the lanes changed under `.codex/**` or `.claude/**`. `merge-finalizer --sync-root --apply` performs this automatically; a successful no-drift result is a no-op and no second authorization or client restart is required.
