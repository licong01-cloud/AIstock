...
5. For a BUG with `backend_restart_required=true`, source PRs use `Refs`, the GitHub Issue remains open, and `finish-batch`/`close-sync-batch` are forbidden. Compatible non-restart BUGs from one shared source PR may use `close-sync-batch`; independently merged non-runtime BUGs may use `close-sync-aggregate` only when at least two are already ready, with one exact `--source-pr BUG-ID=PR-URL` mapping each, and must never wait merely to accumulate a batch. `restart-plan` only expands the existing repo runbook and catalog target. After the user restarts, run `post-restart-verify --bug-id BUG-XXX --target <target> --expected-identity <merge-sha>` and pass its complete digest-bound receipt to `close-sync --bug-id BUG-XXX --pr-url <source-pr> --validation-evidence "<command> -> passed" --post-restart-receipt <receipt> --create-registry-worktree --apply --create-pr`.
