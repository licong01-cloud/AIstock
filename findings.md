...
# Findings: PT-NEXT-023 R8-only Core/Tactical Timing Research (2026-09-19)

- Live `origin/main` at task start is `120da3fac01a14ea22eaf3790ecddfe58c41ac47`; dedicated worktree/branch created from that exact commit.
- PT-NEXT-022 already implements R8 P1, T1/T2, independent CNY 10m accounts, componentized fees, T+1, suspension/limit checks, WSL parallel replay, immutable bundle, and exact retry.
- Formal PT-NEXT-022 evidence: P1 T1/T2 both materially trail same-stock BUY_AND_HOLD; P2/P3 remain typed unavailable because strict financial PIT was not delivered.
- R8 contains daily_basic and bak_basic snapshots. The latter cannot be relabeled as strict quarterly/revision-aware financial PIT; any use needs a new proxy identity, lag, coverage audit, and UNKNOWN semantics.
- The highest-value new research question is whether persistent core exposure plus bounded tactical trims fixes the cash-drag/late-reentry structure, not whether more fixed thresholds can be searched.
- `fundamental_timing._replay_one` currently gives both T1/T2 the same full risk exit and two-close trend exit; T2's 70/30 name only affects a single R0 trim. Therefore its core is not persistent and the earlier interpretation is confirmed by code.
- The existing benchmark hard-codes two policies, three screens, six hypotheses, and P1-only replay. Extending it in place would change PT-NEXT-022 request/contract hashes and sealed semantics; PT-NEXT-023 needs a new module/folder/schema while reusing pure loaders/execution helpers.
- `fundamental_screen.open_r8_candidate_identity` validates only `daily_basic.h5`. PT-NEXT-023 needs an additive, fail-closed `bak_basic.h5` reference/reader if proxy screens are included; the old identity and P1 behavior must remain unchanged.
- Existing report already retains per-fill authority/status, daily exposure, terminal excess, pool views, and coverage. New attribution can be derived during the new report without altering the old artifact format.
- R8 physical proxy fields are confirmed: `bak_basic.h5` has 7,923,740 rows and 15 `bb_*` values including revenue/profit YoY, margins, EPS/BVPS, assets, PE and holder count; `daily_basic.h5` has 8,124,082 rows and 16 `db_*` valuation/liquidity/share/market-cap values. Both are immutable inventory-bound files.
- Formal bundle `52218b...` shows pooled T1 total return 5.75% and T2 5.43% versus same-stock BH 142.03%; full risk exits and trend exits each generated tens of thousands of filled sells, while re-entry attempts also suffered many price-guard blocks. This supports testing persistent core exposure and explicit attribution.
- Baseline per-account mean invested-session fraction is only about 42.3% and mean exposure about 40.5%, while conditional exposure when invested is about 95%. Thus the primary structural issue is long flat-cash intervals, not undersized buys while invested.
- Baseline timing fees average roughly CNY 118k (T1) and CNY 130k (T2) per paired account versus about CNY 1.49k for BH. T2 adds turnover without improving aggregate return, so the new strategy must bound tactical legs and report fee drag explicitly.
- `execute()` already supports legal partial sells but BUY always spends all available cash. A true core/tactical policy can still be implemented without changing shared execution by: initial full investment, tactical partial sells, and recovery buys from only the account's released cash. Persistent core is enforced by never using ordinary trend/risk signals to sell below the frozen core fraction; no new buy-fraction API is required.
- Source-only distribution checks support the frozen nested scopes without sparse-tail engineering: U1 retains about 58.8% of U0 physical rows and U2 about 31.9%; these counts were frozen before any new policy outcome was read.
- Synthetic account tests confirmed shared initial BUY equivalence, S1 recovery from its own cash, S2 two legal 15% stages, risk reduction to rather than through the 80/70 floor, and retry after a directionally blocked sell.
## PT-NEXT-023 implementation review update (2026-09-19)

- Synthetic and real-R8 replay exposed a board-lot boundary artifact: after a core-floor reduction, a sub-lot tactical remainder caused repeated `PARTIAL_QUANTITY_UNAVAILABLE` events. The planner now checks execution-day factor and legal sell quantity before emitting a partial sale; `000002.SZ` changed from 218 fill-attempt rows (202 unavailable) to 6 filled rows and zero unavailable rows, with zero floor violations.
- A real R8 smoke selected `000002.SZ`: U0 enrolled for 110 eligible sessions; U1/U2 correctly remained `ENROLLMENT_UNKNOWN` rather than being backfilled; both frozen policies completed through the real replay path.
- The report builder now tolerates a zero-enrollment screen without assuming terminal or attribution columns exist, preserving explicit empty coverage instead of failing during reporting.
