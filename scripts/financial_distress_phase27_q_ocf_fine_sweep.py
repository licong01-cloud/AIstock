"""Phase-27 research-only fine sweep for q_ocf_to_sales financial distress score-down.
...
DEFAULT_OUTPUT_DIR = Path("reports/event_signal/financial_distress_phase27_q_ocf_fine_sweep")
DEFAULT_DOC_PATH = Path("docs/analysis/event_signal_financial_distress_phase27_q_ocf_fine_sweep_result_20260511.md")
DEFAULT_ROOT_ENV = Path("F:/Dev/AIstock/.env")
BENCHMARK_RULE = "loss_to_market_cap_ge_50pct_mv_lt_10bn"
REPORT_VERSION = "financial_distress_phase27_q_ocf_fine_sweep_v1_20260511"


@dataclass(frozen=True)
class Phase27Summary:
    output_json: str
    output_md: str
...
def run_phase27(args: argparse.Namespace) -> Phase27Summary:
    load_dotenv(DEFAULT_ROOT_ENV, override=False)
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    rule_keys = ["indicator_decline_q_ocf_to_sales_lt_0_mv_ge_10bn"]
    ...
    output_json = output_dir / "financial_distress_phase27_q_ocf_fine_sweep.json"
...
def main(argv: Optional[list[str]] = None) -> int:
    summary = run_phase27(parse_args(argv))
