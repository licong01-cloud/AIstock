...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch Advisory model-first training in an explicit WSL worktree.")
...
def main() -> int:
    ...
    if not request.is_file():
        raise FileNotFoundError(f"training request does not exist: {request}")
    command = " && ".join(
        [
            ...
            (
                "python scripts/wsl/advisory_model_train.py --request "
                f"{shlex.quote(args.request_wsl)}"
            ),
        ]
    )
