"""Ingest Tushare daily_basic (stock daily indicators) with init/incremental and per-day batches.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest Tushare daily_basic into TimescaleDB")
    ...
    parser.add_argument(
        "--truncate",
        action="store_true",
        help="Truncate market.daily_basic before init (destructive)",
    )
    parser.add_argument(
        "--bulk-session-tune",
        action="store_true",
...
def _get_max_trade_date(conn) -> Optional[dt.date]:
    with conn.cursor() as cur:
        cur.execute("SELECT max(trade_date) FROM market.daily_basic")
        row = cur.fetchone()
...
def _fetch_daily_basic_for_date(pro, trade_date: dt.date) -> List[Dict[str, Any]]:
...
def main() -> None:
    ...
    with psycopg2.connect(**DB_CFG) as conn:
        ...
        if mode == "init":
            ...
            if args.truncate:
                with conn.cursor() as cur:
                    cur.execute("TRUNCATE TABLE market.daily_basic")
                print("[WARN] TRUNCATE market.daily_basic executed before full ingestion")
            ...
            if start_date > end_date:
                print("[INFO] daily_basic up to date; nothing to do")
                return
        else:
        ...
        _log(conn, job_id, "info", f"start tushare daily_basic ingestion {mode} {start_date} -> {end_date}")
