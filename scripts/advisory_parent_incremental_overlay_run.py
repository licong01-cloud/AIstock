...
from backend.services.advisory_model_first.parent_incremental_overlay_pipeline import (  # noqa: E402
...
class AdvisoryParentOverlayArgumentError(ValueError):
    pass


class _TypedArgumentParser(argparse.ArgumentParser):
    def error(self, message: str) -> NoReturn:
        raise AdvisoryParentOverlayArgumentError(message)


def _parser() -> argparse.ArgumentParser:
    parser = _TypedArgumentParser(description="Development-only Advisory N3 parent incremental overlay")
    commands = parser.add_subparsers(dest="command", required=True)

    prepare = commands.add_parser("prepare")
    prepare.add_argument("--parent-bundle", required=True)
...
def _run(args: argparse.Namespace) -> dict[str, Any]:
    if args.command == "prepare":
        request = prepare_parent_overlay_request(
    ...
    if args.command == "run":
        return run_parent_incremental_overlay(args.request)
    if args.command == "inspect":
        return inspect_parent_incremental_overlay_bundle(args.bundle)
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        result = exc.as_dict()
        exit_code = 1
    except (AdvisoryParentOverlayArgumentError, ValidationError, ValueError, KeyError) as exc:
        result = {
            "status": "failed",
            "reason_code": "ADVISORY_N3_PARENT_OVERLAY_REQUEST_INVALID",
            "message": str(exc),
        ...
        result = {
            "status": "failed",
            "reason_code": "ADVISORY_N3_PARENT_OVERLAY_UNEXPECTED_FAILURE",
            "message": str(exc),
            "context": {"error_type": type(exc).__name__},
        }
        exit_code = 1
