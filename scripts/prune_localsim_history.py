...
from scripts.localsim_history_prune_core import (  # noqa: E402
...
def _install_dev_fixture(cur: Any, request: LocalSimPruneRequest) -> None:
    if request != LocalSimPruneRequest.build(FIXTURE_ACCOUNT_ID, FIXTURE_PACKAGE_ID, FIXTURE_ANCHOR_RUN_ID):
        raise LocalSimPruneSafetyError(
            "--dev-fixture requires exact successor account, package, and anchor identities"
        )
    ...
    cur.execute(
        ...
        INSERT INTO paper_v2.simulation_account_v1(
            account_id,account_hash,schema_version,account_name,broker_backend,package_id,
            manifest_sha256,admission_receipt_id,initial_capital,account_config_json,status,created_by
        ) VALUES (%s,%s,'simulation_account_v1','BUG-1223 successor fixture','local_sim',%s,%s,
                  'bug1223_fixture_admission',500000,%s,'ACTIVE','bug1223_fixture')
        """,
        (
            FIXTURE_ACCOUNT_ID,
            "7" * 64,
            ...
            psycopg2.extras.Json(
                {
                    ...
                    "lineage_source_legacy_account_id": portfolio_id,
                }
            ),
        ),
    )
    cur.execute(
        ...
        INSERT INTO paper_v2.simulation_ledger_scope_v1(
            ledger_scope_id,ledger_scope_hash,schema_version,scope_kind,source_identity,
            native_account_id,created_by
        ) VALUES (%s,%s,'simulation_ledger_scope_v1','LEGACY_PORTFOLIO',%s,NULL,'bug1223_fixture')
        """,
        (portfolio_id, "8" * 64, portfolio_id),
    )
    cur.execute(
        """
    ...
    cur.execute(
        ...
        INSERT INTO paper_v2.legacy_localsim_account_lineage_v1(
            lineage_id,lineage_hash,schema_version,legacy_account_id,account_id,release_id,binding_id,
            ledger_scope_id,economic_facts_sha256,status,created_by
        ) VALUES ('lslineage_bug1223_fixture',%s,'legacy_localsim_account_lineage_v1',%s,%s,%s,%s,%s,%s,
                  'ACTIVE','bug1223_fixture')
        """,
        ("9" * 64, portfolio_id, FIXTURE_ACCOUNT_ID, release_id, binding_id, portfolio_id, "a" * 64),
    )
...
def _verify_dev_position_authority(conn: Any, request: LocalSimPruneRequest) -> dict[str, Any]:
    with conn.cursor() as cur:
        cur.execute(
            """
            SELECT symbol,quantity FROM paper_v2.positions
            WHERE portfolio_id=%s AND run_id=%s ORDER BY symbol
            """,
            ("paper_bug1223_fixture", request.anchor_run_id),
        )
        observed = {str(symbol): int(quantity) for symbol, quantity in cur.fetchall()}
...
def main() -> int:
    ...
    request = LocalSimPruneRequest.build(args.simulation_account_id, args.package_id, args.anchor_run_id)
    ...
    if args.apply:
        ...
        exact_authorization = (
            "AUTHORIZE_LOCALSIM_HISTORY_PRUNE:production:"
            f"{request.simulation_account_id}:{request.package_id}:{request.anchor_run_id}"
        )
        if not args.confirm_production or args.authorization != exact_authorization:
            raise LocalSimPruneSafetyError(
                "production apply requires --confirm-production and exact account/package/anchor authorization"
            )
    ...
    try:
        ...
        if args.validate_rollback:
            with conn.cursor() as cur:
                ...
                payload = {
                    "schema_version": "aistock_localsim_history_prune_receipt_v2",
                    "target": "dev",
