"""Apply config summary panel + clone button to evolution page.tsx (v2)"""
import sys
...
if "Copy" not in content.split('from "lucide-react"')[0]:
    ...
    checks.append("[SKIP] Copy import already exists")
...
# Insert after the second }, []); (RDAgent URL params effect, line 251-252)
...
# === 5. Add config summary IIFE in task table ===
# The exact pattern from the original file at lines 1172-1175:
#                       </tr>
#                     );
#                   })}
#                 </tbody>
...
new_tr_end = """                      </tr>
                      ...
                      {isActive && (() => {
                        ...
                        return (
                        <tr>
                          <td colSpan={5} style={{ padding: "0 16px 10px 16px", backgroundColor: "#eff6ff", borderBottom: "2px solid #bfdbfe" }}>
                            <div style={{ display: "flex", alignItems: "flex-start", gap: "12px", padding: "8px 12px", backgroundColor: "#f8faff", borderRadius: "8px", border: "1px solid #e0e7ff" }}>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", alignItems: "center", marginBottom: "6px" }}>
                                  ...
                                  {mId && <CB l="模型" v={mId.length > 16 ? mId.slice(0, 16) + "..." : mId} c="#059669" />}
...
if "配置摘要面板" not in content:
    content = content.replace(old_tr_end, new_tr_end, 1)
