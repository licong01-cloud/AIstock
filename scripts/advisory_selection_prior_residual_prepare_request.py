...
from backend.services.advisory_model_first.selection_prior_residual_contracts import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prepare one exact Advisory P0-J Stage A selection-prior-residual output-constraint request."
    )
    ...
    p0i_loaded = load_grouped_rank_bundle(
        p0i_root,
        expected_bundle_id=p0i_root.name,
        load_boosters=False,
    )
    p0f_manifest = p0f_loaded["manifest"]
    p0g_manifest = p0g_loaded["manifest"]
    p0h_manifest = p0h_loaded["manifest"]
    p0i_manifest = p0i_loaded["manifest"]
    ...
    if (
        p0i_manifest.get("experiment_status") != "NEGATIVE_STOP_INCOMPLETE_CPCV"
        or p0i_manifest.get("model_available") is not False
    ):
        parser.error("P0-I reference is not the frozen evidence-only negative bundle")
    ...
    request = build_frozen_selection_prior_residual_request(
        ...
        p0i_evidence_reference=P0ISelectionPriorResidualEvidenceReferenceV1(
            bundle_root=args.p0i_bundle_root_wsl,
            bundle_id=p0i_root.name,
            manifest_file_sha256=_sha256(p0i_root / "manifest.json"),
        ),
...
def _reference(
    *,
    ...
    if role == "P0G_V1_REFERENCE":
        ...
        boost_rounds = int(winner["final_boost_rounds"])
    return ExactSelectionPriorResidualReferenceV1(
        role=role,
...
def _verify_shared_identity(
    ...
    mismatches = [
        key
        for key in keys
        if p0f.get(key) != p0g.get(key)
        or p0f.get(key) != p0h.get(key)
        or p0f.get(key) != p0i.get(key)
    ]
    if mismatches:
        parser.error(f"P0-F/P0-G/P0-H/P0-I exact reference identities differ: {mismatches}")
