...
from backend.services.advisory_model_first.dual_head_output_constraint_contracts import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Prepare one exact Advisory P0-H Stage A dual-head output-constraint request."
    )
    ...
    _verify_shared_identity(parser, p0f_manifest, p0g_manifest)
    ...
    request = build_frozen_dual_head_request(
...
def _reference(
    *,
    ...
    objective = (
        ...
        else str(winner["training_objective"])
    )
    return ExactDualHeadReferenceV1(
        role=role,
        ...
        winner_boost_rounds=int(winner["final_boost_rounds"]),
    )
...
def _verify_shared_identity(
    ...
    mismatches = [key for key in keys if p0f.get(key) != p0g.get(key)]
    if mismatches:
        parser.error(f"P0-F/P0-G exact reference identities differ: {mismatches}")
