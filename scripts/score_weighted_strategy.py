...
class ScoreWeightedTopkStrategy(TopkDropoutStrategy):
    ...
    def _can_sell_under_hold_thresh(self, stock_id, trade_start_time):
        """Preserve Qlib's minimum holding-period contract for custom rebalances."""
        ...
        try:
            hold_count = self.trade_position.get_stock_count(
                stock_id,
                bar=time_per_step,
            )
        except TypeError:
            # Older Qlib Position implementations do not accept ``bar``.
            hold_count = self.trade_position.get_stock_count(stock_id)
    ...
    def _apply_hold_thresh_to_rebalance(
        ...
        """Filter normal exits and cap buys to the slots actually released."""
        eligible_sells = []
        blocked_sells = []
        ...
        retained_count = sum(
            stock_id not in eligible_sell_set for stock_id in current_holdings
        )
    ...
    def generate_trade_decision(self, execute_result=None):
        ...
        actual_sells, actual_buys, hold_blocked_sells = (
            self._apply_hold_thresh_to_rebalance(
                requested_sells,
                actual_buys,
                current_holdings,
                trade_start_time,
