...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch Advisory M3 outcome training in WSL.")
...
def main() -> int:
    ...
    if not request.is_file():
        raise FileNotFoundError(f"outcome training request does not exist: {request}")
    command = " && ".join(
        [
            ...
            f"python scripts/wsl/advisory_outcome_train.py --request {shlex.quote(args.request_wsl)}",
        ]
    )
