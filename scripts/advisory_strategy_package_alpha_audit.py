...
from backend.services.advisory_model_first.alpha_signal_audit_pipeline import (  # noqa: E402
...
class AdvisoryAlphaAuditArgumentError(ValueError):
    pass


class _TypedArgumentParser(argparse.ArgumentParser):
    def error(self, message: str) -> None:
        raise AdvisoryAlphaAuditArgumentError(message)


def _parser() -> argparse.ArgumentParser:
    parser = _TypedArgumentParser(description="Development-only current StrategyPackage three-arm alpha signal audit")
...
def _run(args: argparse.Namespace) -> dict[str, Any]:
    if args.command == "prepare":
        request = prepare_three_arm_alpha_audit_request(
        ...
        return {
            ...
            "arm_ids": [item.arm_id for item in request.arms],
            "planned_trial_count": 0,
            "sealed_holdout_accessed": False,
            "output_path": Path(args.output).resolve().as_posix(),
        }
    if args.command == "run":
        return run_three_arm_alpha_audit(args.request)
    if args.command == "inspect":
        return inspect_three_arm_alpha_audit_bundle(args.bundle)
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        result = exc.as_dict()
        exit_code = 1
    except (AdvisoryAlphaAuditArgumentError, ValidationError, ValueError, KeyError) as exc:
        result = {
            "status": "failed",
        ...
        result = {
            "status": "failed",
            "reason_code": "ADVISORY_ALPHA_AUDIT_UNEXPECTED_FAILURE",
            "message": str(exc),
            "context": {"error_type": type(exc).__name__},
        }
        exit_code = 1
