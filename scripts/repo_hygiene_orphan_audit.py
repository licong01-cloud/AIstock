...
AUDIT_EXTENSIONS = CODE_EXTENSIONS | DOC_EXTENSIONS | DATA_EXTENSIONS | {".yaml", ".yml", ".toml", ".sql", ".ps1", ".sh", ".bat"}
...
def tracked_files(root: Path) -> list[str]:
    return [item for item in git_lines(root, "ls-files") if not is_excluded(item)]


def discovered_files_and_empty_dirs(root: Path) -> tuple[list[str], list[str]]:
    files: set[str] = set()
...
def is_excluded(path: str) -> bool:
    p = norm(path)
    return p == ".git" or any(p.startswith(prefix) for prefix in EXCLUDED_PREFIXES) or "/__pycache__/" in p
...
def category_for(path: str, *, ref_count: int, entrypoint: bool, protected: str | None) -> tuple[str, str, str, str, bool]:
    p = norm(path)
    lower = p.lower()
    ...
    if suffix in DATA_EXTENSIONS:
        if ref_count == 0 and not entrypoint:
            if p.startswith("docs/analysis/") or p.startswith("docs/archive/") or p.startswith("tests/aistock_validation/history/"):
                return "legacy_artifact", "P3", "archive", "historical evidence artifact without detected references", True
            if any(fnmatch.fnmatch(name, pattern) or fnmatch.fnmatch(lower, pattern) for pattern in TEMP_PATTERNS):
                if not p.startswith("tests/") and not p.startswith("docs/") and not p.startswith("configs/"):
                    return "generated_or_temp_artifact", "P4", "delete_candidate", "temporary artifact outside evidence directories", True
            if not p.startswith("docs/") and not p.startswith("tests/") and not p.startswith("configs/"):
                return "generated_or_temp_artifact", "P4", "delete_candidate", "unreferenced data artifact outside protected directories", True
    if any(fnmatch.fnmatch(name, pattern) or fnmatch.fnmatch(lower, pattern) for pattern in BACKFILL_PATTERNS):
        if ref_count == 0 and not entrypoint:
            return "legacy_backfill_or_migration_like", "P1", "review", "backfill/migration-like file may be one-time but can retain audit value", True
        return "backfill_or_migration_entry", "P0", "keep", "backfill/migration-like file has references or entrypoint hints", False
    if any(fnmatch.fnmatch(name, pattern) or fnmatch.fnmatch(lower, pattern) for pattern in DEBUG_PATTERNS):
        if ref_count == 0 and not entrypoint:
            return "legacy_debug_or_smoke", "P1", "review", "debug/smoke/manual-looking file lacks references and entrypoint hints", True
        return "debug_or_smoke_referenced", "P1", "review", "debug/smoke/manual-looking file should be reviewed before cleanup", True
    if suffix in CODE_EXTENSIONS and ref_count == 0 and not entrypoint:
        if p.startswith("scripts/"):
            return "manual_script_or_orphan_candidate", "P1", "review", "script has no detected references; may be manual CLI entrypoint", True
...
def git_last_commit_map(root: Path) -> dict[str, str]:
    proc = subprocess.run(
        ["git", "-C", str(root), "log", "--name-only", "--pretty=format:COMMIT %h %cs"],
        text=True,
        encoding="utf-8",
        errors="replace",
        capture_output=True,
...
def row_for(root: Path, path: str, refs: dict[str, set[str]], last_commit: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "git_last_commit": (last_commit or {}).get(path) or git_last_commit(root, path),
...
def build_audit(root: Path = ROOT, *, run_id: str | None = None, max_rows: int | None = None) -> dict[str, Any]:
    discovered_files, empty_dirs = discovered_files_and_empty_dirs(root)
...
def main(argv: list[str] | None = None) -> int:
    ...
    markdown_output = Path(args.markdown_output) if args.markdown_output else output_dir / "repo-hygiene-orphan-audit.md"
    csv_output = Path(args.csv_output) if args.csv_output else output_dir / "repo-hygiene-orphan-audit.csv"
    payload = build_audit(root, run_id=args.run_id, max_rows=args.max_rows)
