"""Phase-31 non-q_ocf structured financial-distress screen.

This research-only script pivots away from broad q_ocf intersections and tests
expectation-miss, profit deterioration, margin, leverage, and liquidity splits.
...
DEFAULT_OUTPUT_DIR = Path("reports/event_signal/financial_distress_phase31_non_qocf_structured_screen")
...
PHASE19_BASELINE_RULE = "indicator_large_decline_mv_10_30bn"
LOSS_MV_BASELINE_RULE = "loss_to_market_cap_ge_50pct_mv_lt_10bn"
REPORT_VERSION = "financial_distress_phase31_non_qocf_structured_screen_v1_20260511"


@dataclass(frozen=True)
class Phase31Summary:
    output_json: str
    output_md: str
...
def _run_overlay_sweep(args: argparse.Namespace, *, rule_keys: Sequence[str]) -> Path:
    ...
    rule_by_key = {rule.rule_key: rule for rule in PHASE31_RESEARCH_RULES}
...
def _build_markdown(payload: Mapping[str, Any]) -> str:
    ...
    phase_decision = "WSL_TRUE_QE_CANDIDATE_FOUND" if true_candidates else "NO_WSL_TRUE_QE_RERUN"
    phase_decision_note = (
        "one or more rows pass the cheap gate; run one-loop WSL true QE before policy work"
        if true_candidates
        else "direct downside may exist, but cheap overlay precision/effect remains below the true-rerun gate"
    )
    candidate_note = (
        "non-zero means prepare WSL one-loop smoke for the top row"
        if true_candidates
        else "0 means do not spend WSL true-rerun budget in this phase"
    )
    outcome_rows = [
        [
            "true-QE candidates",
    ...
    return "\n".join(
        [
            ...
            * _fixed_width_table(
            ...
            (
                "- Phase 31 produces at least one cheap candidate; run one-loop WSL true QE before any signal policy work."
                if true_candidates
                else "- Phase 31 does not produce a WSL true-QE candidate; keep these rules as watchlist/direct-event research only."
            ),
...
def run_phase31(args: argparse.Namespace) -> Phase31Summary:
    load_dotenv(DEFAULT_ROOT_ENV, override=False)
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    rule_keys = [rule.rule_key for rule in PHASE31_RESEARCH_RULES]
    ...
    payload = {
        ...
        "parameters": {
            ...
            "phase19_baseline_rule": PHASE19_BASELINE_RULE,
            "loss_mv_baseline_rule": LOSS_MV_BASELINE_RULE,
        },
        "artifacts": {
    ...
    output_json = output_dir / "financial_distress_phase31_non_qocf_structured_screen.json"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    summary = run_phase31(parse_args(argv))
