"""Run the approved P2-3 market-relative jump spike against read-only inputs."""
...
from backend.services.hmm_risk.market_relative_jump_spike import (  # noqa: E402
    DEVELOPMENT_END,
    DEVELOPMENT_START,
    JumpSpikeError,
...
def _load_request(path: Path) -> dict[str, Any]:
    try:
        ...
        raise JumpSpikeError(
            "hmm_risk_jump_input_identity_mismatch",
            "request JSON cannot be read canonically",
            stage="input",
            evidence={"exception_type": type(exc).__name__},
        ) from exc
    if not isinstance(value, dict):
        raise JumpSpikeError("hmm_risk_jump_input_identity_mismatch", "request must be an object", stage="input")
    return value
...
def _loader_request(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(source, dict):
        raise JumpSpikeError("hmm_risk_jump_input_identity_mismatch", "request source is missing", stage="input")
    if str(source.get("source_end") or "") != DEVELOPMENT_END.isoformat():
        raise JumpSpikeError(
            "hmm_risk_jump_holdout_access_forbidden",
            "P2-3 source_end must stop at the development boundary",
            stage="input",
        )
    try:
        ...
        raise JumpSpikeError(
            "hmm_risk_jump_input_identity_mismatch",
            "source_start must be an ISO date",
            stage="input",
        ) from exc
    ...
    family = {
        ...
        "train_end": DEVELOPMENT_END.isoformat(),
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        report = report_for_write(run_p2_3_spike(inputs, request, producer_commit=producer_commit), failure=False)
        ...
        try:
            ...
            sys.stderr.write(
                f"P2-3 spike failed and failure receipt could not be written: {type(write_exc).__name__}: {write_exc}\n"
            )
            return 2
        sys.stderr.write(f"P2-3 spike failed: {report['failure_reason_code']}; receipt={path}\n")
        return 1
