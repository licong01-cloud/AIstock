...
def main() -> int:
    parser = argparse.ArgumentParser(description="Launch Advisory M5C calibration in WSL.")
    ...
    if not request.is_file():
        raise FileNotFoundError(f"price-range calibration request does not exist: {request}")
    command = " && ".join((
