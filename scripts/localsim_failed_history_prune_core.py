...
class LocalSimPruneRequest:
    ...
    @classmethod
    def build(cls, package_id: str, anchor_run_id: str) -> "LocalSimPruneRequest":
        ...
        return cls(package_id=package, anchor_run_id=anchor)
...
def _anchor(cur: Any, request: LocalSimPruneRequest) -> dict[str, Any]:
    columns = (
        "run_id",
        "trade_date",
        "strategy_id",
    ...
    cur.execute(
        ...
        WHERE broker_backend='local_sim' AND package_id=%s
...
def _candidate_runs(cur: Any, request: LocalSimPruneRequest, *, anchor_date: date) -> list[dict[str, Any]]:
    columns = (
        "run_id",
        "trade_date",
        "strategy_id",
    ...
    rows = _fetch_dicts(
        ...
        WHERE broker_backend='local_sim' AND package_id=%s AND trade_date < %s
          AND status=ANY(%s)
        ORDER BY trade_date,run_id
        """,
        (request.package_id, anchor_date, list(DELETABLE_STATUSES)),
        columns,
    )
    for row in rows:
...
def build_prune_plan(cur: Any, request: LocalSimPruneRequest) -> dict[str, Any]:
    anchor = _anchor(cur, request)
    candidates = _candidate_runs(cur, request, anchor_date=anchor["trade_date"])
    ...
    plan: dict[str, Any] = {
        "schema_version": "aistock_localsim_failed_history_prune_plan_v1",
        "database_identity": _database_identity(cur),
        "request": {"package_id": request.package_id, "anchor_run_id": request.anchor_run_id},
        "anchor": {
