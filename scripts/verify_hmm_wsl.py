...
NEW_COEFF_PATH = "/mnt/f/Dev/AIstock/backend/data/hmm_models/b2d5bcc6-8463-4156-bf1a-e1392a00279a/2026-04-27/coefficients_preset_A_2026-01-26_2026-04-24.json"
...
def load_qlib_data(qlib_path: str, start_date: str, end_date: str) -> pd.DataFrame:
    """从 Qlib 加载股票数据."""
    import qlib
...
def calculate_portfolio_metrics(returns_df: pd.DataFrame, return_col: str = 'pct_chg', top_n: int = 50) -> Dict:
    ...
    # 确定分组列名
    date_col = 'datetime' if 'datetime' in returns_df.columns else 'date'

    for trade_date, group in returns_df.groupby(date_col):
        # 按调整后收益排序，选择 top_n
...
def compare_versions(old_coeff_path: str, new_coeff_path: str, qlib_path: str) -> None:
    """对比两个版本的 HMM 系数效果."""

    print("="*100)
    print("HMM 系数效果验证 - WSL 环境")
    print("="*100)
    ...
    print("  2. 未考虑交易成本和滑点")
    print("  3. 测试时间段较短 (21天)，建议生成全量系数后再次验证")
    print("  4. 实际效果可能因策略不同而异")
