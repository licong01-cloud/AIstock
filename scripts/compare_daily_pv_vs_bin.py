...
SNAPSHOT_PROVIDER = "F:/Dev/AIstock/qlib_snapshots/qlib_export_20251209"
BIN_PROVIDER = "F:/Dev/AIstock/qlib_bin/qlib_bin_20251209"
SYMBOL = "000001.SZ"
FIELDS = ["$close", "$volume", "$amount"]
FREQ = "day"
OUTPUT_MD = Path("F:/Dev/AIstock/docs/daily_pv_vs_bin_compare.md")


def load_features(provider_uri: str) -> pd.DataFrame:
    qlib.init(
        provider_uri=provider_uri,
        mount_path=provider_uri,
        auto_mount=False,
        redis_port=-1,
    )
...
def main() -> None:
    print("加载 daily_pv.h5 (snapshot provider)...")
    snapshot_df = load_features(SNAPSHOT_PROVIDER)
    print("加载 bin provider...")
    bin_df = load_features(BIN_PROVIDER)
    ...
    with OUTPUT_MD.open("w", encoding="utf-8") as f:
        f.write("# daily_pv.h5 与 bin 数据对比\n\n")
        f.write(f"- 股票: {SYMBOL}\n")
        f.write(f"- 字段: {', '.join(FIELDS)}\n")
        f.write(f"- 频率: {FREQ}\n\n")

        f.write("## 日期范围与记录数\n\n")
        f.write("### daily_pv.h5 (snapshot provider)\n")
