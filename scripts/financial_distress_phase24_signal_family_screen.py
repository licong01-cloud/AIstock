"""Phase-24 research-only screen for structured financial distress signal families.
...
DEFAULT_OUTPUT_DIR = Path("reports/event_signal/financial_distress_phase24_signal_family_screen")
DEFAULT_DOC_PATH = Path("docs/analysis/event_signal_financial_distress_phase24_signal_family_screen_result_20260510.md")
DEFAULT_ROOT_ENV = Path("F:/Dev/AIstock/.env")
BENCHMARK_RULE = "loss_to_market_cap_ge_50pct_mv_lt_10bn"
REPORT_VERSION = "financial_distress_phase24_signal_family_screen_v1_20260510"


@dataclass(frozen=True)
class Phase24Summary:
    output_json: str
    output_md: str
...
def _build_markdown(payload: Mapping[str, Any]) -> str:
    ...
    rule_family_rows = [
        [
            "expectation_miss",
            "forecast >=50% growth but actual misses; test gap>=50/100 and actual source",
            "direct event downside + cheap overlay top50 interaction",
        ],
        [
            "profit_revenue_deterioration",
            "indicator decline with sharper yoy, revenue divergence, both-down, or negative margin",
            "screen all market-cap buckets; no small-cap-only assumption",
        ],
        [
            "cashflow_leverage_liquidity",
            "OCF decline, debt/assets>=70%, current ratio<1, combined financial quality stress",
            "research-only score-down; not hard ban/forced sell",
        ],
    ]
    return "\n".join(
        [
...
def run_phase24(args: argparse.Namespace) -> Phase24Summary:
    load_dotenv(DEFAULT_ROOT_ENV, override=False)
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    rule_keys = [rule.rule_key for rule in PHASE24_RESEARCH_RULES]
    ...
    output_json = output_dir / "financial_distress_phase24_signal_family_screen.json"
...
def main(argv: Optional[list[str]] = None) -> int:
    summary = run_phase24(parse_args(argv))
