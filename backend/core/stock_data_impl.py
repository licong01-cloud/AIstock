...
class StockDataFetcher:
        ...
        try:
            ...
            try:
                ...
                if self.data_source_manager.tushare_available and info["name"] == "未知":
                    print("[Tushare] 尝试获取基本信息（tushare）...")
                    try:
                        ...
                        if df is not None and not df.empty:
                            ...
                            print("[Tushare] ✅ 成功获取部分信息")
                    except Exception as te:
                        print(f"[Tushare] ❌ 获取失败: {te}")

            try:
                print("[数据源管理器] 尝试获取最近交易数据...")
            ...
            if info["pe_ratio"] == "N/A":
                try:
                    with network_optimizer.apply():
                        pe_data = ak.stock_zh_valuation_baidu(
            ...
            if info["pb_ratio"] == "N/A":
                try:
                    with network_optimizer.apply():
                        pb_data = ak.stock_zh_valuation_baidu(
                ...
                try:
                    ...
                    if hist_df is not None and not hist_df.empty:
                        ...
                        if len(hist_df) > 1:
                            ...
                            change_pct = (
                                (latest["收盘"] - prev_close) / prev_close
                            ) * 100
                            info["change_percent"] = round(change_pct, 2)
                except Exception as e:
    ...
    def _get_us_stock_info(self, symbol):
        ...
        try:
            ...
            try:
                ...
                if pe_ratio == "N/A" or pe_ratio is None or (
                ...
                if pb_ratio == "N/A" or pb_ratio is None or (
                    isinstance(pb_ratio, float) and np.isnan(pb_ratio)
                ):
                    pb_ratio = "N/A"
    ...
    def _get_chinese_financial_data(self, symbol):
        ...
        try:
            ...
            try:
                with network_optimizer.apply():
                    balance_sheet = ak.stock_financial_abstract_ths(
                        symbol=symbol, indicator="资产负债表"
                    )
                if balance_sheet is not None and not balance_sheet.empty:
                    financial_data["balance_sheet"] = balance_sheet.head(8).to_dict(
                        "records"
                    )
            except Exception as e:
                print(f"获取资产负债表失败: {e}")
            ...
            try:
                with network_optimizer.apply():
                    income_statement = ak.stock_financial_abstract_ths(
                        symbol=symbol, indicator="利润表"
                    )
                if income_statement is not None and not income_statement.empty:
                    financial_data["income_statement"] = income_statement.head(
                        8
                    ).to_dict("records")
            except Exception as e:
                print(f"获取利润表失败: {e}")
            ...
            try:
                with network_optimizer.apply():
                    cash_flow = ak.stock_financial_abstract_ths(
                        symbol=symbol, indicator="现金流量表"
                    )
                if cash_flow is not None and not cash_flow.empty:
                    financial_data["cash_flow"] = cash_flow.head(8).to_dict(
                        "records"
                    )
            except Exception as e:
                print(f"获取现金流量表失败: {e}")
            ...
            try:
                ...
                if financial_abstract is not None and not financial_abstract.empty:
                    ...
                    # 筛选出包含关键指标的行
                    indicator_rows = financial_abstract[
                        financial_abstract["指标"].isin(key_indicators)
                    ]
                    ...
                    if not indicator_rows.empty:
                        ...
                        date_columns = [
                            col
                            for col in financial_abstract.columns
                            if col not in ["选项", "指标"]
                        ]
                        if date_columns:
    ...
    def _get_hk_financial_data(self, symbol):
        ...
        try:
            ...
            try:
                with network_optimizer.apply():
                    financial_indicator = ak.stock_hk_financial_indicator_em(
                        symbol=hk_code
                    )
                ...
                if financial_indicator is not None and not financial_indicator.empty:
                    ...
                    financial_data["financial_ratios"] = {
                        "基本每股收益": self._safe_convert(
                            indicator_dict.get("基本每股收益(元)", "N/A")
                        ),
                        "每股派息": self._safe_convert(
                            indicator_dict.get("每股派息(元)", "N/A")
                        ),
                        "每股净资产": self._safe_convert(
                            indicator_dict.get("每股净资产(元)", "N/A")
                        ),
                        "每股股息TTM": self._safe_convert(
                            indicator_dict.get("每股股息TTM(港元)", "N/A")
                        ),
                        "派息比率": self._safe_convert(
                            indicator_dict.get("派息比率(%)", "N/A")
                        ),
                        "每股经营现金流": self._safe_convert(
                            indicator_dict.get("每股经营现金流(元)", "N/A")
                        ),
                        "股息率TTM": self._safe_convert(
                            indicator_dict.get("股息率TTM(%)", "N/A")
                        ),
                        "总市值": self._safe_convert(
                            indicator_dict.get("总市值(港元)", "N/A")
                        ),
                        "港股市值": self._safe_convert(
                            indicator_dict.get("港股市值(港元)", "N/A")
                        ),
                        "营业总收入": self._safe_convert(
                            indicator_dict.get("营业总收入", "N/A")
                        ),
                        "营业收入环比增长": self._safe_convert(
                            indicator_dict.get("营业总收入滚动环比增长(%)", "N/A")
                        ),
                        "销售净利率": self._safe_convert(
                            indicator_dict.get("销售净利率(%)", "N/A")
                        ),
                        "净利润": self._safe_convert(
                            indicator_dict.get("净利润", "N/A")
                        ),
                        "净利润环比增长": self._safe_convert(
                            indicator_dict.get("净利润滚动环比增长(%)", "N/A")
                        ),
                        "ROE股东权益回报率": self._safe_convert(
                            indicator_dict.get("股东权益回报率(%)", "N/A")
                        ),
                        "市盈率": self._safe_convert(
                            indicator_dict.get("市盈率", "N/A")
                        ),
                        "市净率": self._safe_convert(
                            indicator_dict.get("市净率", "N/A")
                        ),
                        "ROA总资产回报率": self._safe_convert(
                            indicator_dict.get("总资产回报率(%)", "N/A")
                        ),
                        "法定股本": self._safe_convert(
                            indicator_dict.get("法定股本(股)", "N/A")
                        ),
                        "已发行股本": self._safe_convert(
                            indicator_dict.get("已发行股本(股)", "N/A")
                        ),
                        "每手股": self._safe_convert(
                            indicator_dict.get("每手股", "N/A")
                        ),
                    }

                    print(f"✅ 成功获取港股 {hk_code} 的财务指标")
                    print(
                        f"   ROE: {financial_data['financial_ratios']['ROE股东权益回报率']}"
                    )
                    print(
                        f"   市盈率: {financial_data['financial_ratios']['市盈率']}"
                    )
                    print(
                        f"   市净率: {financial_data['financial_ratios']['市净率']}"
                    )
                else:
    ...
    def _get_us_financial_data(self, symbol):
        ...
        try:
            ...
            try:
                ...
                if balance_sheet is not None and not balance_sheet.empty:
                    financial_data["balance_sheet"] = balance_sheet.iloc[
                        :, :4
                    ].to_dict("index")
            except Exception as e:
                print(f"获取资产负债表失败: {e}")
            ...
            try:
                ...
                if income_stmt is not None and not income_stmt.empty:
                    financial_data["income_statement"] = income_stmt.iloc[
                        :, :4
                    ].to_dict("index")
            except Exception as e:
                print(f"获取利润表失败: {e}")
            ...
            try:
                ...
                if cash_flow is not None and not cash_flow.empty:
                    financial_data["cash_flow"] = cash_flow.iloc[:, :4].to_dict(
                        "index"
                    )
            except Exception as e:
                print(f"获取现金流量表失败: {e}")
