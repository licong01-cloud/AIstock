-- BUG-1004: close the durable MiniQMT event CHECK authority over LEGACY_V1,
-- P1-D quote evidence, and the complete KERNEL_V2 routing surface.
-- Operator-applied only. Application startup must never execute this DDL.
...
BEGIN
    ...
    IF table_oid IS NULL THEN
        RAISE EXCEPTION 'BUG-1004 preflight: qmt_strategy.execution_runtime_event does not exist';
    END IF;
    ...
    IF type_def IS NULL OR source_def IS NULL OR composite_def IS NULL THEN
        RAISE EXCEPTION 'BUG-1004 preflight: all three named event CHECK constraints must exist exactly once';
    END IF;
    ...
    IF type_shape<>'' OR source_shape<>'' THEN
        RAISE EXCEPTION 'BUG-1004 preflight: event type/source CHECK expression drift';
    END IF;
    ...
    IF (type_values=quote_types AND source_values<>quote_sources)
       OR (type_values=target_types AND source_values<>target_sources)
       OR (source_values=quote_sources AND type_values<>quote_types)
       OR (source_values=target_sources AND type_values<>target_types) THEN
        RAISE EXCEPTION 'BUG-1004 preflight: event type/source constraints are at different migration states';
    END IF;
    IF type_values<>quote_types AND type_values<>target_types THEN
        RAISE EXCEPTION 'BUG-1004 preflight: event type allowlist drift';
    END IF;
    IF source_values<>quote_sources AND source_values<>target_sources THEN
        RAISE EXCEPTION 'BUG-1004 preflight: event source allowlist drift';
    END IF;
    IF normalized_composite<>normalized_old_composite AND normalized_composite<>normalized_target_composite THEN
        RAISE EXCEPTION 'BUG-1004 preflight: KERNEL_V2 composite constraint drift';
    END IF;
    IF (type_values=quote_types AND normalized_composite<>normalized_old_composite)
       OR (type_values=target_types AND normalized_composite<>normalized_target_composite) THEN
        RAISE EXCEPTION 'BUG-1004 preflight: allowlist/composite constraints are at different migration states';
    END IF;
    IF EXISTS (
        SELECT 1 FROM qmt_strategy.execution_runtime_event
         WHERE event_type<>ALL(target_types) OR source<>ALL(target_sources)
    ) THEN
        RAISE EXCEPTION 'BUG-1004 preflight: existing rows contain values outside target allowlists';
    END IF;
    ...
    source_shape := regexp_replace(source_shape,'CHECK|SOURCE|IN|ANY|ARRAY|TEXT|VARCHAR|CHARACTER|[()\[\],=[:space:]]','','g');
    IF type_shape<>'' OR source_shape<>'' THEN
        RAISE EXCEPTION 'BUG-1004 apply: event type/source CHECK expression drift after lock';
    END IF;
    normalized_composite := regexp_replace(regexp_replace(upper(composite_def),'::TEXT','','g'),'[()[:space:]]','','g');

    IF type_values=target_types AND source_values=target_sources AND normalized_composite=normalized_target_composite THEN
        ...
        RAISE NOTICE 'BUG-1004 apply no-op: exact KERNEL_V2 event CHECK authority already present and validated';
        RETURN;
    END IF;
    IF type_values<>quote_types OR source_values<>quote_sources OR normalized_composite<>normalized_old_composite THEN
        RAISE EXCEPTION 'BUG-1004 apply: event CHECK authority changed after lock';
    END IF;
    ...
    SELECT coalesce(array_agg(m[1] ORDER BY m[1]),ARRAY[]::text[]) INTO source_values
      FROM regexp_matches(source_def,'''([^'']+)''','g') AS m;
    normalized_composite := regexp_replace(regexp_replace(upper(composite_def),'::TEXT','','g'),'[()[:space:]]','','g');
    IF type_values<>target_types OR source_values<>target_sources OR normalized_composite<>normalized_target_composite
       OR NOT type_validated OR NOT source_validated OR NOT composite_validated THEN
        RAISE EXCEPTION 'BUG-1004 apply: independent post-DDL event CHECK readback drift';
    END IF;
END
$$;
