...
-- BUG-462 forward migration: LocalSim fill replay must be idempotent.
-- Rollback migration (manual, post-review only):
--   ALTER TABLE paper_v2.cash_ledger DROP CONSTRAINT IF EXISTS cash_ledger_run_fill_unique;
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
...
COMMENT ON CONSTRAINT cash_ledger_run_fill_unique ON paper_v2.cash_ledger IS
    'BUG-462 LocalSim idempotency guard: one cash ledger row per run_id/fill_id so fill replay cannot double-count cash.';
