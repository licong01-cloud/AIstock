...
@pytest.mark.parametrize(
    ...
    [
        "",
        ".",
        "relative/qe_workspace",
        "/",
        "//host/qe_workspace",
        "/mnt/f/qe_workspace/../other",
        r"C:\qe_workspace",
    ],
)
def test_preflight_qe_node_rejects_non_absolute_api_workspace_root(
    monkeypatch,
    api_workspace_base,
):
    node_row = {
    ...
    class FakeClient:
        ...
        async def get_workspace_config(self):
            return {"workspace_base": api_workspace_base}
    ...
    with pytest.raises(QENodePreflightError) as exc_info:
        asyncio.run(preflight_qe_node("wsl2-5080"))

    assert exc_info.value.error_code == "QE_NODE_WORKSPACE_ROOT_INVALID"


def test_preflight_qe_node_rejects_api_and_configured_workspace_root_mismatch(monkeypatch):
    node_row = {
        "node_id": "wsl2-5080",
        "api_base_url": "http://127.0.0.1:9000",
        "status": "online",
    ...
    class FakeClient:
        ...
        async def get_workspace_config(self):
            return {"workspace_base": "/tmp/other-qe-workspace"}
    ...
    assert exc_info.value.context == {
        "node_id": "wsl2-5080",
        "configured_workspace_base": "/mnt/f/Dev/RD-Agent-main/qe_workspace",
        "api_workspace_base": "/tmp/other-qe-workspace",
    }


def test_preflight_qe_node_normalizes_matching_trailing_slash(monkeypatch):
    node_row = {
        "node_id": "wsl2-5080",
        "api_base_url": "http://127.0.0.1:9000",
        "status": "online",
        "workspace_base": "/mnt/f/qe_workspace",
        "factor_data_dir": "/mnt/x/factors",
        "qlib_data_path": "/mnt/x/day",
        "qlib_minute_path": "/mnt/x/minute",
        "qlib_rdagent_root": "/mnt/f/Dev/RD-Agent-main",
    ...
    class FakeClient:
        ...
        async def get_workspace_config(self):
            return {"workspace_base": "/mnt/f/qe_workspace/"}
    ...
    node = asyncio.run(preflight_qe_node("wsl2-5080"))
