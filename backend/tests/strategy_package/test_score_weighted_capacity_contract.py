...
@pytest.mark.xfail(
    CAPACITY_STRATEGY_ID not in SCORE_WEIGHTED_V2_IDS,
    reason=(
        "Agent B capacity StrategyPackage contract is not present yet; "
        "new strategy_id must join the score_weighted_v2 family with its own capacity default."
    ),
    strict=True,
)
def test_capacity_strategy_id_uses_new_default_without_mutating_legacy_default() -> None:
    ...
    assert SCORE_WEIGHTED_DEFAULTS["max_single_order_value"] == 5_000_000.0
    assert contract["portfolio_strategy"]["strategy_family"] == "score_weighted_topk_v2"
