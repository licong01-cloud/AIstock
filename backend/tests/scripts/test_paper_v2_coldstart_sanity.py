...
def _args(**overrides: Any) -> argparse.Namespace:
    data = {
        "mode": "prod",
        "confirm_prod": TOKEN,
        "operator_confirmation": f"{TOKEN} target=prod",
...
@pytest.mark.parametrize(
    ...
    [
        (["--mode", "prod", "--operator-confirmation", f"{TOKEN} target=prod"], True, True, "confirm-prod"),
        (["--mode", "prod", "--confirm-prod", TOKEN, "--operator-confirmation", f"{TOKEN} target=prod"], False, True, ENV_FLAG),
        (["--mode", "prod", "--confirm-prod", TOKEN, "--operator-confirmation", f"{TOKEN} target=prod"], True, False, "mutex"),
...
def test_prod_rejects_trading_hours_before_connect(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert sanity.main(["--json", "--mode", "prod", "--confirm-prod", TOKEN, "--operator-confirmation", f"{TOKEN} target=prod"]) == 2
    payload = json.loads(capsys.readouterr().out)
...
def test_prod_rejects_wrong_db_target_before_connect(monkeypatch: pytest.MonkeyPatch, overrides: dict[str, Any], needle: str) -> None:
    ...
    with pytest.raises(sanity.ColdStartSanityError, match=needle):
        sanity._require_prod_guards(args, sanity._target_from_args(args), now=_dt())
...
def test_preflight_backend_down_stops_before_db(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    report = sanity.run_prod(_args(), sanity._target_from_args(_args()), sanity.SentinelOrder("sanity-test"))
...
def test_preflight_db_unreachable_is_no_go(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert sanity.main(["--json", "--mode", "prod", "--confirm-prod", TOKEN, "--operator-confirmation", f"{TOKEN} target=prod", "--package-id", "pkg_1"]) == 2
...
def test_sentinel_payload_is_exact() -> None:
    payload = sanity.SentinelOrder(run_id="sanity-test").payload()
...
def test_poll_fill_passes_complete_row() -> None:
    check, fill = sanity._poll_fill(FakeConnection(), sanity.SentinelOrder("sanity-test"), timeout_seconds=1, poll_seconds=0.001)
...
def test_poll_fill_rejects_missing_market_context() -> None:
    ...
    check, _ = sanity._poll_fill(conn, sanity.SentinelOrder("sanity-test"), timeout_seconds=1, poll_seconds=0.001)
...
def test_poll_fill_rejects_missing_timestamps() -> None:
    ...
    if check["status"] == "PASS":
        ...
        malformed[9] = None
        malformed[10] = None
        check, _ = sanity._poll_fill(FakeConnection(fill_row=tuple(malformed)), sanity.SentinelOrder("sanity-test"), timeout_seconds=1, poll_seconds=0.001)
    assert check["status"] == "FAIL"
...
def test_outbox_requires_telemetry_routing() -> None:
    check, event = sanity._check_outbox(FakeConnection(outbox_rows=[_outbox_row("archive")]), sanity.SentinelOrder("sanity-test"))
    assert check["status"] == "FAIL"
...
def test_outbox_allows_pending_or_sent() -> None:
    for status in ("pending", "sent"):
        check, event = sanity._check_outbox(FakeConnection(outbox_rows=[_outbox_row("telemetry", status)]), sanity.SentinelOrder("sanity-test"))
        assert check["status"] == "PASS"
...
def test_audit_chain_requires_evidence() -> None:
    ...
    check = sanity._check_audit_chain(conn, sanity.SentinelOrder("sanity-test"), {"created_at": _dt(1)}, require_ledger=True)
    assert check["status"] == "FAIL"
...
def test_audit_chain_rejects_bad_timestamp_order() -> None:
    ...
    check = sanity._check_audit_chain(conn, sanity.SentinelOrder("sanity-test"), {"created_at": _dt(2)}, require_ledger=True)
    assert check["status"] == "FAIL"
...
def test_cleanup_commits_each_table() -> None:
    conn = FakeConnection()
    check = sanity._cleanup_sentinel(conn, sanity.SentinelOrder("sanity-test"))
    assert check["status"] == "PASS"
...
def test_prod_main_success_exit_zero(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    rc = sanity.main(["--json", "--mode", "prod", "--confirm-prod", TOKEN, "--operator-confirmation", f"{TOKEN} target=prod", "--package-id", "pkg_1", "--run-id", "sanity-test"])
    payload = json.loads(capsys.readouterr().out)
    assert rc == 0
