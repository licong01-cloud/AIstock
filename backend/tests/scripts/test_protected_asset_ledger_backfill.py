...
import scripts.protected_asset_ledger_backfill as backfill
...
class FakeConnection:
    def __init__(self, *, select_only: bool = True) -> None:
        ...
        self.rows = [("pkg_a", 1, 0, 1, 0), ("pkg_b", 0, 1, 0, 1)]
...
def test_dry_run_rejects_non_dev_or_non_5433_before_connect(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(backfill.ProtectedAssetLedgerBackfillError, match="target_db=dev"):
    ...
    with pytest.raises(backfill.ProtectedAssetLedgerBackfillError, match="port 5433"):
        backfill.run_dry_run_preview(target=target, limit=10)
...
def test_dry_run_rejects_non_aistock_dev_dbname_before_connect(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(backfill.ProtectedAssetLedgerBackfillError, match="aistock_dev"):
        backfill.run_dry_run_preview(target=target, limit=10)
