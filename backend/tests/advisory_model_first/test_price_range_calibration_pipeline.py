...
def _request_v2(tmp_path: Path):
    ...
    return build_frozen_daily_price_envelope_calibration_request(
        **values,
...
def test_daily_price_envelope_calibration_keeps_binary_fields_retired(
    tmp_path: Path, monkeypatch
) -> None:
    request = _request_v2(tmp_path)
    request_path = tmp_path / "daily-request.json"
    request.write_json(request_path)
    features = pd.DataFrame(
        {
            "decision_as_of_trade_date": ["2026-01-02", "2026-01-03"],
    ...
    monkeypatch.setattr(pipeline, "_verify_environment", lambda _request: None)
    monkeypatch.setattr(
        pipeline, "read_daily_price_envelope_bundle_manifest", lambda *_a, **_k: {}
    )
    monkeypatch.setattr(
        pipeline,
        "_validate_daily_request",
        lambda *_a, **_k: {"validation": 0, "test": 0},
    )
    monkeypatch.setattr(pipeline, "_validate_descriptor", lambda _descriptor: None)
    monkeypatch.setattr(pipeline, "_read_bound_parquet", lambda _descriptor: features)
    monkeypatch.setattr(
        ...
        else pipeline.json.loads(path.read_text()),
    )
    monkeypatch.setattr(pipeline, "_read_split_labels", read_split)
    monkeypatch.setattr(
        pipeline, "_prepare_matrix_from_schema", lambda frame, **_k: frame
    )
    monkeypatch.setattr(
        pipeline,
        "_load_parent_quantile_models",
        lambda _root: {
            name: object()
    ...
    monkeypatch.setattr(
        pipeline,
        "_predict_triplet",
        lambda _models, matrix: (
            np.full(len(matrix), -0.01),
            np.full(len(matrix), 0.01),
            np.full(len(matrix), 0.02),
        ),
    )
    ...
    monkeypatch.setattr(
        pipeline, "publish_calibrated_daily_price_envelope_bundle", publish
    )
    monkeypatch.setattr(pipeline, "_peak_rss_bytes", lambda: 100)
    monkeypatch.setattr(
        pipeline.importlib.metadata, "version", lambda _name: "test-version"
    )

    receipt = pipeline.run_daily_price_envelope_calibration_pipeline(request_path)

    assert order == ["validation", "test_after_spec"]
    predictions = captured["calibrated_test_predictions"]
    assert isinstance(predictions, pd.DataFrame)
    assert "entry_executable_calibration_state" not in predictions.columns
    assert receipt["price_range_binding_activated"] is False
    assert receipt["activation_recommended"] is False
    assert (
        receipt["metrics"]["activation_decision_basis"]
