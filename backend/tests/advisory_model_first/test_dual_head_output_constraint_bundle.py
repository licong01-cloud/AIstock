...
def _publish(tmp_path: Path, resource: int, *, incomplete: bool = False):
    ...
    published = publish_dual_head_bundle(
        request=request,
        ...
        winner_receipt={"family_id": "FAMILY_DUAL_HEAD_CORE", "seed": 20260813},
...
def test_bundle_is_immutable_and_exact_retry_reuses_identity(tmp_path: Path) -> None:
    ...
    assert find_dual_head_bundle_for_request(request)[0] == first_id
    ...
    with pytest.raises(AdvisoryModelFirstError, match="differs from manifest"):
        load_dual_head_bundle(path, expected_bundle_id=first_id, load_boosters=False)
...
def test_incomplete_bundle_is_evidence_only_and_has_no_models(tmp_path: Path) -> None:
    ...
    load_dual_head_bundle(path, expected_bundle_id=bundle_id, load_boosters=False)
    with pytest.raises(AdvisoryModelFirstError, match="no loadable models"):
        load_dual_head_bundle(path, expected_bundle_id=bundle_id, load_boosters=True)
