...
def test_shared_feature_builder_keeps_legacy_drop_date_default_and_supports_meta_candidate_drop() -> None:
    ...
    meta_source = (
        Path(__file__).resolve().parents[2] / "services" / "advisory_model_first" / "meta_label_features.py"
    ).read_text(encoding="utf-8")
    assert '"preserve_exact" if v2 else "drop_candidate"' in meta_source
    assert 'feature_schema_version: str = "advisory_feature_schema_v1"' in meta_source
