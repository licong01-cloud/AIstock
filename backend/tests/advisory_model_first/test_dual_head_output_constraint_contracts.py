...
from backend.services.advisory_model_first.dual_head_output_constraint_contracts import (
    ...
    build_frozen_dual_head_request,
)
...
def _request(**overrides):
    values = {
        ...
        "family_specs": approved_dual_head_families(),
    ...
    return build_frozen_dual_head_request(**values)


def test_dual_head_request_round_trip_and_dynamic_fields() -> None:
    ...
    loaded = FrozenAdvisoryDualHeadTrainingRequestV1.model_validate_json(first.model_dump_json())
    assert loaded.functional_payload() == first.functional_payload()
    assert json.loads(first.model_dump_json())["experiment_lineage"][-1] == "P0-H-v1"


def test_dual_head_request_freezes_roster_coverage_and_physical_units() -> None:
    request = _request()
    ...
    assert request.shadow_price_multipliers == DUAL_HEAD_SHADOW_PRICE_MULTIPLIERS
    assert request.liability_clip_min == 2 / (5 * 20)
    assert request.liability_clip_max == 2 / 5


def test_dual_head_request_rejects_roster_reference_unit_and_cutoff_drift() -> None:
    with pytest.raises(ValueError, match="family order"):
        _request(family_specs=tuple(reversed(approved_dual_head_families())))
