...
from backend.services.advisory_model_first.grouped_rank_output_constraint_training import (
    ...
    add_grouped_return_relevance,
    add_liability_target,
    build_inner_fold_specs,
    combine_grouped_rank_predictions,
    grouped_rank_feature_names,
    normalize_grouped_return_scores,
...
def test_grouped_relevance_is_matured_only_with_average_ties_and_fixed_range() -> None:
    labels = _labels()
    first_date = labels["decision_as_of_trade_date"].min()
    tied = (labels["decision_as_of_trade_date"] == first_date) & labels["selection_rank"].isin([1, 2])
    labels.loc[tied, "net_excess_return_bps"] = 100.0
    adjusted = add_grouped_return_relevance(labels)
    matured = adjusted[adjusted["label_status"] == "MATURED"]
    assert matured[RETURN_RELEVANCE_COLUMN].between(0, 4).all()
    assert adjusted.loc[tied, RETURN_RELEVANCE_COLUMN].nunique() == 1
    assert adjusted.loc[adjusted["label_status"] != "MATURED", RETURN_RELEVANCE_COLUMN].isna().all()


def test_same_date_percentile_uses_only_raw_group_scores_and_preserves_ties() -> None:
    rows = _features(periods=1)[
        ["decision_as_of_trade_date", "target_trade_date", "instrument", "selection_effective_rank"]
    ]
    raw = np.arange(20, dtype=float)
    raw[10] = raw[11]
    percentile = normalize_grouped_return_scores(rows, raw)
    assert percentile.between(0.0, 1.0).all()
    assert percentile.iloc[10] == percentile.iloc[11]
...
def test_oof_price_scale_selection_and_exact_top20_priority_are_deterministic() -> None:
    ...
    second = scale.candidates_score_per_fraction[1]
    ...
    assert selected.shadow_price_score_per_fraction == second
    ranked = combine_grouped_rank_predictions(predictions, shadow_price=second)
    ...
    assert set(ranked["entry_priority_score_kind"]) == {
        "GROUPED_RANK_OUTPUT_CONSTRAINED_PERCENTILE_V1"
    }
    with pytest.raises(AdvisoryModelFirstError):
        combine_grouped_rank_predictions(predictions.iloc[:-1], shadow_price=second)


def test_grouped_rank_features_never_include_future_labels() -> None:
    forbidden = {"holding_trading_days", "net_excess_return_bps", LIABILITY_TARGET_COLUMN}
    for family in approved_grouped_rank_families():
        assert forbidden.isdisjoint(grouped_rank_feature_names(family))
...
def test_categorical_vocabulary_is_train_only_and_unseen_sets_missing_indicator() -> None:
    ...
    merged, matrix, vocabulary, _ = _training_matrix(
        features,
        labels,
        approved_grouped_rank_families()[0],
...
def test_lightgbm_inner_oof_scores_all_eligible_top20() -> None:
    ...
    result = train_grouped_rank_oof(
        features=features,
        labels=labels,
        folds=folds,
        family=approved_grouped_rank_families()[0],
        seed=20260813,
    )
