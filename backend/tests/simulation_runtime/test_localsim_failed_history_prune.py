...
from scripts.localsim_failed_history_prune_core import (
    ANCHOR_STATUS,
    DELETABLE_STATUSES,
...
def test_request_requires_explicit_package_and_anchor_identities() -> None:
    request = LocalSimPruneRequest.build("pkg_current", "simrun_current")
    ...
    with pytest.raises(LocalSimPruneSafetyError, match="pkg_"):
        LocalSimPruneRequest.build("current", "simrun_current")
    with pytest.raises(LocalSimPruneSafetyError, match="simrun_"):
        LocalSimPruneRequest.build("pkg_current", "current")
