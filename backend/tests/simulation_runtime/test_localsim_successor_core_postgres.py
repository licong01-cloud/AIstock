...
def test_successor_schema_and_repository_are_idempotent_atomic_and_readable_on_dev_postgres() -> None:
    ...
    try:
        account, release, binding = service.create_account(
            account_name=f"test_int_localsim_successor_{marker}",
            package_id=package_id,
            manifest_sha256=manifest_sha256,
            admission_receipt_id=f"test_int_admission_{marker}",
            initial_capital=1_000_000.0,
            runtime_profile_id="test_int_localsim_runtime",
            runtime_profile_version_id=f"test_int_runtime_{marker}",
            runtime_profile_sha256="a" * 64,
            daily_strategy_profile_version_id=f"test_int_daily_{marker}",
        ...
        assert duplicate[1].release_id == release_id
        assert duplicate[2].binding_id == binding_id
        ...
        with psycopg2.connect(**dsn) as read_conn:
            with read_conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                ...
                assert dict(cur.fetchone()) == {"status": "CAUGHT_UP", "completed_trade_date": days[-1]}
    finally:
