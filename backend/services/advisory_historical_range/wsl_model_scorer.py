...
REASON_WSL_INFERENCE_UNAVAILABLE = "ADVISORY_COMPARISON_MODEL_INFERENCE_ENVIRONMENT_UNAVAILABLE"
...
class WslFrozenFeatureMatrixScorer:
    ...
    def __call__(
        ...
        for column in CATEGORICAL_FEATURE_COLUMNS:
            ...
            categorical_vocabulary[column] = [int(value) for value in matrix[column].cat.categories]
        ...
        model_relative_paths = [
            str(path.relative_to(bundle.bundle_path.resolve())).replace("\\", "/") for path in model_paths
        ]
        ...
        repo_root = Path(self.repo_root or Path(__file__).resolve().parents[3]).resolve()
        ...
        with tempfile.TemporaryDirectory(prefix="advisory_historical_wsl_score_") as temporary_root:
        ...
        try:
            ...
            booster_feature_names = [tuple(row) for row in result["booster_feature_names"]]
    ...
    def _command(self, *, helper_path: str, request_path: str, output_path: str) -> list[str]:
        distro = self.distro or os.getenv("QLIB_WSL_DISTRO") or "Ubuntu"
        conda_sh = self.conda_sh or os.getenv("QLIB_WSL_CONDA_SH") or "~/miniconda3/etc/profile.d/conda.sh"
...
def _deferred_model_paths(bundle: LoadedAdvisoryModelBundle) -> tuple[Path, ...]:
    ...
    members = (bundle.booster,) if schema_version == "advisory_model_bundle_v1" else bundle.boosters
    if not members or any(not isinstance(member, DeferredLightgbmBooster) for member in members):
        raise AdvisoryModelFirstError(
