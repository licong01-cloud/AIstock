...
def publish_grouped_rank_bundle(
    *,
    ...
    if (return_booster is None) != (liability_booster is None):
        raise _bundle_error("grouped-rank bundle must publish both models or neither")
    root = Path(request.output_root).resolve()
    target_root = root / "grouped_rank_output_constraint_bundles"
    target_root.mkdir(parents=True, exist_ok=True)
    temp = Path(tempfile.mkdtemp(prefix="advisory_grouped_rank_", dir=root))
    try:
        if return_booster is not None and liability_booster is not None:
            return_booster.save_model(str(temp / RETURN_MODEL_FILENAME))
        ...
        _write_json(temp / "grouped_rank_feature_schema.json", dict(feature_schema))
        ...
        _write_json(temp / "grouped_rank_transform_receipt.json", dict(transform_receipt))
        ...
        model_available = return_booster is not None
        payload = {
            "schema_version": "advisory_grouped_rank_output_constraint_bundle_v1",
            "stage": "P0_I_V1_STAGE_A_OFFLINE",
            ...
            "model_files": (
                {"return": RETURN_MODEL_FILENAME, "liability": LIABILITY_MODEL_FILENAME}
                if model_available
                else {}
            ),
            "activated": False,
            ...
            "model_role": "offline_grouped_rank_output_constrained_policy_utility_v1",
        ...
        if target.exists():
            existing = load_grouped_rank_bundle(target, expected_bundle_id=bundle_id, load_boosters=False)
            existing_identity = {
                key: value
            ...
            if existing_identity != payload:
                raise _bundle_error("existing grouped-rank bundle differs from its content identity")
            return bundle_id, target, existing["manifest"]
        os.replace(temp, target)
        load_grouped_rank_bundle(target, expected_bundle_id=bundle_id, load_boosters=False)
...
def find_grouped_rank_bundle_for_request(
    request: FrozenAdvisoryGroupedRankTrainingRequestV1,
) -> tuple[str, Path, dict[str, Any]] | None:
    target_root = Path(request.output_root).resolve() / "grouped_rank_output_constraint_bundles"
    ...
    for candidate in sorted(target_root.iterdir()):
        ...
        try:
            frozen = FrozenAdvisoryGroupedRankTrainingRequestV1.model_validate_json(
                (candidate / "training_request.json").read_text(encoding="utf-8")
            )
        except (OSError, ValueError) as exc:
            raise _bundle_error("grouped-rank bundle request cannot be read") from exc
        if frozen.functional_payload() != request.functional_payload():
            raise _bundle_error("grouped-rank bundle request differs from exact retry request")
        matches.append((str(manifest["bundle_id"]), candidate, loaded["manifest"]))
    if len(matches) > 1:
        raise AdvisoryModelFirstError(
            "multiple grouped-rank bundles claim the same request",
            reason_code="ADVISORY_GROUPED_RANK_RETRY_MISMATCH",
        )
    return matches[0] if matches else None


def load_grouped_rank_bundle(
    ...
    *,
    ...
    try:
        ...
        raise _bundle_error("grouped-rank manifest cannot be read") from exc
    ...
    if root.name != expected_bundle_id or manifest.get("bundle_id") != expected_bundle_id or actual != expected_bundle_id:
        raise _bundle_error("grouped-rank bundle identity is invalid")
    for name, descriptor in (manifest.get("files") or {}).items():
        path = (root / name).resolve()
        try:
            path.relative_to(root)
        except ValueError as exc:
            raise _bundle_error("grouped-rank bundle file escapes its root") from exc
        if (
            not path.is_file()
            or path.stat().st_size != int(descriptor["size_bytes"])
            or _sha256(path) != descriptor["sha256"]
        ):
            raise _bundle_error("grouped-rank bundle file differs from manifest", filename=name)
    result: dict[str, Any] = {"manifest": manifest, "bundle_path": root}
    if load_boosters:
        if not manifest.get("model_available"):
            raise _bundle_error("incomplete grouped-rank experiment has no loadable models")
        try:
            import lightgbm as lgb

            result["return_booster"] = lgb.Booster(model_file=str(root / RETURN_MODEL_FILENAME))
            result["liability_booster"] = lgb.Booster(model_file=str(root / LIABILITY_MODEL_FILENAME))
        except Exception as exc:
            raise _bundle_error("grouped-rank models cannot be loaded") from exc
    return result
