...
def publish_daily_price_envelope_bundle(
    *,
    model_root: str | Path,
    request: FrozenAdvisoryPriceRangeTrainingRequestV2,
    split: OutcomeDateSplit,
    training: "PriceRangeTrainingResult",
    environment_report: Mapping[str, Any],
    resource_report: Mapping[str, Any],
) -> tuple[str, Path, dict[str, Any]]:
    if tuple(sorted(training.models)) != tuple(sorted(PRICE_RANGE_QUANTILE_MODEL_NAMES)):
        raise AdvisoryModelFirstError(
            "daily price envelope bundle requires the exact three quantile heads",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
            context={"model_names": sorted(training.models)},
        )
    root = Path(model_root).resolve()
    bundles_root = root / "price_range_bundles"
    bundles_root.mkdir(parents=True, exist_ok=True)
    temporary = Path(tempfile.mkdtemp(prefix=".daily-price-envelope-", dir=bundles_root))
    try:
        models_root = temporary / "models"
        models_root.mkdir()
        ...
        _write_json(
            temporary / "label_policy.json",
            _expected_daily_price_envelope_label_policy(),
        )
        _write_json(temporary / "split.json", split.as_dict())
        _write_json(temporary / "metrics.json", training.metrics)
        _write_json(
        ...
        manifest_payload = {
            ...
            "feature_schema_hash": request.feature_schema_hash,
            "label_policy_version": request.label_policy_version,
            "entry_gap_condition": request.entry_gap_condition,
            "quantiles": list(PRICE_RANGE_QUANTILES),
        ...
        manifest = {**manifest_payload, "price_range_bundle_id": bundle_id}
        _write_json(temporary / "manifest.json", manifest)
        _validate_daily_price_envelope_bundle(temporary, expected_bundle_id=bundle_id)
        target = bundles_root / bundle_id
        if target.exists():
            existing = _validate_daily_price_envelope_bundle(
                target, expected_bundle_id=bundle_id
            )
            if existing != manifest:
                raise AdvisoryModelFirstError(
                    "existing daily price envelope bundle has different content",
                    reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
                    context={"price_range_bundle_id": bundle_id},
                )
            shutil.rmtree(temporary)
        ...
        if temporary.exists():
            try:
                ...
                raise AdvisoryModelFirstError(
                    "incomplete daily price envelope bundle could not be removed",
                    reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
                    context={
                        "temporary_path": str(temporary),
                        "source_error_type": type(exc).__name__,
...
def read_daily_price_envelope_bundle_manifest(
    bundle_path: str | Path,
    *,
    expected_bundle_id: str,
) -> dict[str, Any]:
    return _validate_daily_price_envelope_bundle(
        Path(bundle_path).resolve(), expected_bundle_id=expected_bundle_id
    )


def _validate_daily_price_envelope_bundle(
    bundle_path: Path,
    *,
    expected_bundle_id: str,
) -> dict[str, Any]:
    ...
    if not manifest_path.is_file():
        raise AdvisoryModelFirstError(
            "daily price envelope manifest is missing",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
        )
    try:
        ...
        raise AdvisoryModelFirstError(
            "daily price envelope manifest cannot be read",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
            context={"error_type": type(exc).__name__},
        ) from exc
    if not isinstance(manifest, dict):
        raise AdvisoryModelFirstError(
            "daily price envelope manifest is not an object",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
        )
    payload = {
    ...
    if manifest.get("price_range_bundle_id") != expected_bundle_id or actual_id != expected_bundle_id:
        raise AdvisoryModelFirstError(
            "daily price envelope canonical identity is invalid",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
    ...
    required = {
        ...
        *{f"models/{name}.txt" for name in PRICE_RANGE_QUANTILE_MODEL_NAMES},
    }
    files = manifest.get("files")
    if not isinstance(files, dict) or set(files) != required:
        raise AdvisoryModelFirstError(
            "daily price envelope members differ from the exact contract",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
            context={
                "missing_files": sorted(required - set(files or {})),
                "unexpected_files": sorted(set(files or {}) - required),
            },
        )
    for name, descriptor in files.items():
        ...
        if (
            ...
            raise AdvisoryModelFirstError(
                "daily price envelope member is missing or corrupt",
                reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
    ...
    try:
        ...
        raise AdvisoryModelFirstError(
            "daily price envelope semantic members cannot be read",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
            context={"error_type": type(exc).__name__},
        ) from exc
    expected_manifest = {
        ...
        "feature_schema_version": request.feature_schema_version,
        "feature_schema_hash": request.feature_schema_hash,
        "label_policy_version": DAILY_PRICE_ENVELOPE_LABEL_POLICY_VERSION,
        "entry_gap_condition": DAILY_PRICE_ENVELOPE_ENTRY_GAP_CONDITION,
        "quantiles": list(PRICE_RANGE_QUANTILES),
        "model_names": list(PRICE_RANGE_QUANTILE_MODEL_NAMES),
        "repository_commit": request.repository_commit,
    ...
    if (
        ...
        or not isinstance(feature_schema, dict)
        ...
        or label_policy != _expected_daily_price_envelope_label_policy()
    ):
        raise AdvisoryModelFirstError(
            "daily price envelope semantic identities are inconsistent",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
        )
    _validate_split(split)
    _validate_daily_price_envelope_metrics(metrics)
    _validate_daily_price_envelope_training_log(training_log, request=request)
    _validate_daily_price_envelope_test_predictions(
        predictions, split=split, metrics=metrics
    )
    return manifest
...
def _expected_daily_price_envelope_label_policy() -> dict[str, Any]:
    return {
        "schema_version": DAILY_PRICE_ENVELOPE_LABEL_POLICY_VERSION,
        "entry_session": "next_trading_day_open",
        "target": "target_open/decision_close-1",
        "authoritative_suspension_semantics": "NOT_APPLICABLE",
        "missing_market_row_semantics": "UNAVAILABLE_NOT_NEGATIVE",
        "one_price_limit_up_semantics": "AVAILABLE_IF_OPEN_OBSERVED",
        "entry_gap_condition": DAILY_PRICE_ENVELOPE_ENTRY_GAP_CONDITION,
        "quantiles": list(PRICE_RANGE_QUANTILES),
        "binary_head": "RETIRED_NON_IDENTIFIABLE",
    }
...
def _validate_daily_price_envelope_metrics(value: Any) -> None:
    distribution = value.get("entry_gap_distribution") if isinstance(value, dict) else None
    if (
        not isinstance(value, dict)
        or value.get("model_count") != len(PRICE_RANGE_QUANTILE_MODEL_NAMES)
        or value.get("status") != "EXPERIMENTAL_SHADOW"
        or value.get("calibration_state") != "UNCALIBRATED"
        or not isinstance(value.get("heads"), dict)
        or set(value["heads"]) != set(PRICE_RANGE_QUANTILE_MODEL_NAMES)
        or not isinstance(value.get("test_row_count"), int)
        or value["test_row_count"] <= 0
        or value.get("test_date_count") != 80
        or not isinstance(distribution, dict)
        or distribution.get("condition")
        != DAILY_PRICE_ENVELOPE_ENTRY_GAP_CONDITION
    ):
        raise AdvisoryModelFirstError(
            "daily price envelope metrics differ from the three-head contract",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
        )
    for name in PRICE_RANGE_QUANTILE_MODEL_NAMES:
        head = value["heads"].get(name)
        if (
            not isinstance(head, dict)
            or head.get("condition") != DAILY_PRICE_ENVELOPE_ENTRY_GAP_CONDITION
            or not isinstance(head.get("row_count"), int)
            or head["row_count"] <= 0
            or not isinstance(head.get("best_iteration"), int)
            or head["best_iteration"] <= 0
            ...
            raise AdvisoryModelFirstError(
                "daily price envelope head metric is invalid",
                reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
                context={"head": name},
            )


def _validate_daily_price_envelope_training_log(
    value: Any,
    *,
    request: FrozenAdvisoryPriceRangeTrainingRequestV2,
) -> None:
    ...
    if (
        ...
        or set(evaluation_history) != set(PRICE_RANGE_QUANTILE_MODEL_NAMES)
    ):
        raise AdvisoryModelFirstError(
            "daily price envelope training environment or resource receipt is invalid",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
        )


def _validate_daily_price_envelope_test_predictions(
    ...
    *,
    ...
    forbidden = {"entry_executable", "entry_executable_probability"} & set(
        predictions.columns
    )
    if (
        missing
        ...
        or not predictions["entry_gap_condition"].eq(
            DAILY_PRICE_ENVELOPE_ENTRY_GAP_CONDITION
        ).all()
    ):
        raise AdvisoryModelFirstError(
            "daily price envelope predictions have an invalid identity or schema",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
            context={"missing_columns": missing, "forbidden_columns": sorted(forbidden)},
        )
    numeric = predictions.loc[:, PRICE_RANGE_QUANTILE_MODEL_NAMES].apply(
    ...
    if (
        ...
        raise AdvisoryModelFirstError(
            "daily price envelope predictions violate quantile semantics",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
        )
    ...
    status = predictions["entry_gap_label_status"].astype(str)
    ...
    if (
        ...
        or not set(status.unique()).issubset(
            {"AVAILABLE", "NOT_APPLICABLE", "UNAVAILABLE"}
        )
        or observed_gap.loc[available].isna().any()
        or not np.isfinite(observed_gap.loc[available].to_numpy(dtype=float)).all()
        or observed_gap.loc[~available].notna().any()
        or ranks.isna().any()
        or ranks.le(0).any()
        or ranks.gt(20).any()
        ...
        raise AdvisoryModelFirstError(
            "daily price envelope labels or parent ranking fields are inconsistent",
            reason_code="ADVISORY_PRICE_RANGE_BUNDLE_INVALID",
        )
