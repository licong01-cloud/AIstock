...
class MetaLabelProgress:
    ...
    def add(self, stage: str, started: float, **details: Any) -> None:
        ...
        if peak > self.limit:
            raise AdvisoryModelFirstError(
                "meta-label training exceeded RSS limit",
    ...
    def report(self) -> dict[str, Any]:
        return {
            "schema_version": "advisory_meta_label_resource_v1",
...
def run_meta_label_pipeline(request_path: str | Path) -> dict[str, Any]:
    request = FrozenAdvisoryMetaLabelTrainingRequestV1.model_validate_json(
    ...
    if _sha256(manifest_path) != request.policy_dataset_manifest_file_sha256:
        raise AdvisoryModelFirstError(
            "P0-C manifest hash differs from request", reason_code="ADVISORY_META_LABEL_SOURCE_INVALID"
        )
    for key in (
        "program_id",
        "binding_version_id",
        ...
        if manifest.get(key) != getattr(request, key):
            raise AdvisoryModelFirstError(
                "P0-C identity differs from meta-label request",
                reason_code="ADVISORY_META_LABEL_SOURCE_INVALID",
                context={"field": key},
            )
    reference = _load_reference_challenger(request)
    ...
    if existing is not None:
        ...
        reference_comparison_path = bundle_path / "reference_challenger_comparison.json"
        reference_comparison = (
            json.loads(reference_comparison_path.read_text(encoding="utf-8"))
            if reference_comparison_path.is_file()
            else None
        )
        return {
    ...
    if len(paths) != 28:
        raise AdvisoryModelFirstError(
            "meta-label requires all 28 READY CPCV paths",
            reason_code="ADVISORY_META_LABEL_SOURCE_INVALID",
    ...
    cutoff = (
        pd.Timestamp(rankings.loc[rankings["is_candidate_decision"], "decision_as_of_trade_date"].max())
        .date()
        .isoformat()
    )
    ...
    for path in paths:
        ...
        selection_priority = rankings.loc[
            ...
            & rankings["decision_as_of_trade_date"].isin(pd.to_datetime(path["validation_dates"]))
        ...
        baseline_metrics, _, _ = evaluate_meta_label_validation_blocks(
            rankings=rankings,
            predictions=selection_priority,
            ...
            cost_policy=cost,
            request_id=f"{request.request_id}_selection_{path['path_id']}",
        )
        baseline_by_path[path["path_id"]] = baseline_metrics[request.primary_metric]
        validation_features = feature_result.features[
            pd.to_datetime(feature_result.features["decision_as_of_trade_date"])
            .dt.normalize()
            .isin(pd.to_datetime(path["validation_dates"]))
        ].copy()
        ...
        hmm_metrics, _, _ = evaluate_meta_label_validation_blocks(
            rankings=rankings,
            predictions=hmm_priority,
            validation_blocks=validation_blocks,
            block_by_date=cpcv["block_by_date"],
            daily=candidate_daily,
            benchmark_daily=benchmark,
            ...
            policy_sha256=request.shadow_policy_sha256,
            cost_policy=cost,
            request_id=f"{request.request_id}_hmm_{path['path_id']}",
        )
        ...
        random_metrics, _, _ = evaluate_meta_label_validation_blocks(
            rankings=rankings,
            predictions=random_priority,
            validation_blocks=validation_blocks,
            block_by_date=cpcv["block_by_date"],
            daily=candidate_daily,
            benchmark_daily=benchmark,
        ...
        for family in request.family_specs:
            for seed in request.seed_roster:
                ...
                result = train_meta_label_trial(
                    features=feature_result.features,
                    labels=labels,
                    train_dates=pd.to_datetime(path["train_dates"]),
                    validation_dates=pd.to_datetime(path["validation_dates"]),
                    family=family,
                    seed=seed,
                    outcome_weighting=request.outcome_weighting,
                )
                policy_metrics, _, _ = evaluate_meta_label_validation_blocks(
                    rankings=rankings,
                    ...
                    policy_sha256=request.shadow_policy_sha256,
                    cost_policy=cost,
                    request_id=f"{request.request_id}_{trial_id}_{path['path_id']}",
                )
                ...
                trial_rows.append(
                    {
                        ...
                        **{f"policy_{key}": value for key, value in policy_metrics.items() if key != "block_metrics"},
                        "selection_baseline_mean_daily_net_excess_return_bps": baseline_metrics[request.primary_metric],
    ...
    block_scores = raw_blocks.groupby(["trial_id", "family_id", "seed", "block_id"], as_index=False)[
    ...
    summary = trial_metrics.groupby(["family_id", "seed"], as_index=False).agg(
        ...
        mean_policy_lift_bps=("policy_lift_bps", "mean"),
        path_win_rate=("policy_lift_bps", lambda values: float((values > 0).mean())),
    ...
    progress.add(
        ...
        started,
        trial_path_count=len(trial_metrics),
        winner_family=winner.family_id,
    ...
    winner_receipt = {
        "schema_version": "advisory_meta_label_winner_v1",
        ...
        "path_win_rate": float(winner.path_win_rate),
        "mean_roc_auc": float(winner.mean_roc_auc),
        "mean_brier": float(winner.mean_brier),
        "final_boost_rounds": final_rounds,
        "tie_break": request.tie_break,
    }
    if request.outcome_weighting is not None:
        winner_receipt["training_objective"] = "OUTCOME_MAGNITUDE_WEIGHTED_BINARY_V1"
        winner_receipt["outcome_weighting"] = final.outcome_weighting_receipt
    reference_comparison = (
    ...
    baseline = {
        "schema_version": "advisory_meta_label_baselines_v1",
        "selection_path_primary_metric": baseline_by_path,
        "selection_mean_primary_metric": float(np.mean(list(baseline_by_path.values()))),
        "hmm_top5_path_primary_metric": hmm_baseline_by_path,
        "hmm_top5_mean_primary_metric": float(np.mean(list(hmm_baseline_by_path.values()))),
        "random_top5_path_primary_metric": random_baseline_by_path,
        "random_top5_mean_primary_metric": float(np.mean(list(random_baseline_by_path.values()))),
        "candidate20_equal_path_primary_metric": candidate20_by_path,
        "candidate20_equal_mean_primary_metric": float(np.mean(list(candidate20_by_path.values()))),
        "candidate20_equal_semantics": "CANDIDATE_COUNTERFACTUAL_MEAN_NOT_FIVE_SLOT_PORTFOLIO",
        "m5a_status": "HISTORICAL_REFERENCE_NOT_USED_FOR_SELECTION_OR_MATCHED_POLICY",
    }
    resource = progress.report()
    bundle_id, bundle_path, bundle_manifest = publish_meta_label_bundle(
        request=request,
        ...
        reference_challenger_comparison=reference_comparison,
        training_log={
            "environment": environment,
            "schema_receipt": schema_receipt.__dict__,
            "trial_summary": summary.to_dict("records"),
            "outcome_weighting": request.outcome_weighting.model_dump(mode="json")
            if request.outcome_weighting is not None
            else None,
            "reference_comparison": reference_comparison,
        },
...
def _load_reference_challenger(
    request: FrozenAdvisoryMetaLabelTrainingRequestV1,
) -> dict[str, Any] | None:
    if request.reference_meta_label_bundle_root is None:
        return None
    root = Path(request.reference_meta_label_bundle_root).resolve()
    manifest_path = root / "manifest.json"
    if (
        request.reference_meta_label_bundle_id is None
        or request.reference_meta_label_manifest_file_sha256 is None
        or not manifest_path.is_file()
        or _sha256(manifest_path) != request.reference_meta_label_manifest_file_sha256
    ):
        raise AdvisoryModelFirstError(
            "meta-label reference manifest differs from the frozen request",
            reason_code="ADVISORY_META_LABEL_REFERENCE_INVALID",
        )
    loaded = load_meta_label_bundle(
        root,
        expected_bundle_id=request.reference_meta_label_bundle_id,
        load_booster=False,
    )
    ...
    if mismatches:
        raise AdvisoryModelFirstError(
            "meta-label reference identity differs from the frozen request",
            reason_code="ADVISORY_META_LABEL_REFERENCE_INVALID",
            context={"mismatches": mismatches},
        )
    try:
        winner = json.loads((root / "winner_receipt.json").read_text(encoding="utf-8"))
        trial_metrics = pd.read_parquet(root / "cpcv_trial_metrics.parquet")
    except (OSError, ValueError, KeyError) as exc:
        raise AdvisoryModelFirstError(
            "meta-label reference evidence cannot be read",
            reason_code="ADVISORY_META_LABEL_REFERENCE_INVALID",
        ) from exc
    return {
        "manifest": manifest,
...
def _compare_reference_challenger(
    *,
    request: FrozenAdvisoryMetaLabelTrainingRequestV1,
    ...
    metric_column = f"policy_{request.primary_metric}"
    ...
    ][["path_id", metric_column]].rename(columns={metric_column: "reference_primary_metric_bps"})
    candidate_rows = trial_metrics[
        (trial_metrics["family_id"] == winner_family_id) & (trial_metrics["seed"].astype(int) == winner_seed)
    ][["path_id", metric_column]].rename(columns={metric_column: "candidate_primary_metric_bps"})
    if (
        len(reference_rows) != 28
        or len(candidate_rows) != 28
        or reference_rows["path_id"].duplicated().any()
        or candidate_rows["path_id"].duplicated().any()
        or set(reference_rows["path_id"]) != set(candidate_rows["path_id"])
    ):
        raise AdvisoryModelFirstError(
            "meta-label reference comparison does not contain the exact 28 paths",
            reason_code="ADVISORY_META_LABEL_REFERENCE_INVALID",
        )
    ...
    return {
        ...
        "primary_metric": request.primary_metric,
        "candidate_family_id": winner_family_id,
        "candidate_seed": winner_seed,
        "reference_bundle_id": request.reference_meta_label_bundle_id,
        "reference_family_id": str(reference_winner["family_id"]),
        "reference_seed": int(reference_winner["seed"]),
        "path_count": len(paired),
        "candidate_mean_primary_metric_bps": float(paired["candidate_primary_metric_bps"].mean()),
        "reference_mean_primary_metric_bps": float(paired["reference_primary_metric_bps"].mean()),
        "candidate_minus_reference_mean_primary_metric_bps": float(lifts.mean()),
        "candidate_path_win_rate": float((lifts > 0.0).mean()),
        "tie_count": int((lifts == 0.0).sum()),
        "research_improvement": bool(lifts.mean() > 0.0 and (lifts > 0.0).mean() > 0.5),
...
def _verify_environment(request: FrozenAdvisoryMetaLabelTrainingRequestV1) -> dict[str, Any]:
    if (
        ...
        raise AdvisoryModelFirstError(
            "meta-label training requires WSL rdagent-gpu", reason_code="ADVISORY_MODEL_TRAINING_REQUIRES_WSL"
        )
    ...
    if commit != request.repository_commit:
        raise AdvisoryModelFirstError(
            "meta-label repository commit mismatch",
