...
def publish_calibrated_daily_price_envelope_bundle(
    *,
    ...
    parent_root = Path(request.parent_bundle_root).resolve()
    parent = read_daily_price_envelope_bundle_manifest(
        parent_root, expected_bundle_id=request.parent_price_range_bundle_id
    )
    _validate_daily_parent(request, parent_root, parent)
    spec_path = Path(calibration_spec_path).resolve()
    spec = _read_json(spec_path)
    _validate_daily_spec(spec, request=request)
    bundles_root = Path(request.output_root).resolve() / "price_range_bundles"
    bundles_root.mkdir(parents=True, exist_ok=True)
    temporary = Path(
        tempfile.mkdtemp(prefix=".daily-price-envelope-calibration-", dir=bundles_root)
    )
    try:
        (temporary / "models").mkdir()
        ...
        _copy_exact(spec_path, temporary / "calibration_spec.json")
        _write_json(
            temporary / "calibration_request.json", request.model_dump(mode="json")
        )
        _write_json(temporary / "metrics.json", dict(metrics))
        _write_json(temporary / "training_log.json", dict(calibration_log))
        calibrated_test_predictions.to_parquet(
        ...
        payload = {
            **{key: parent[key] for key in inherited},
            ...
            "calibration_policy_version": CALIBRATION_POLICY_VERSION,
            "calibration_method": CALIBRATION_METHOD,
            "calibration_spec_sha256": sha256_file(
                temporary / "calibration_spec.json"
            ),
            "repository_commit": request.repository_commit,
            "files": files,
        }
        ...
        validate_calibrated_daily_price_envelope_bundle(
            temporary, expected_bundle_id=bundle_id
        )
        target = bundles_root / bundle_id
        if target.exists():
            existing = validate_calibrated_daily_price_envelope_bundle(
                target, expected_bundle_id=bundle_id
            )
            if existing != manifest:
                raise bundle_error(
                    "existing calibrated daily price envelope has different content"
                )
            shutil.rmtree(temporary)
            return bundle_id, target, existing
        os.replace(temporary, target)
        readback = validate_calibrated_daily_price_envelope_bundle(
            target, expected_bundle_id=bundle_id
        )
        if readback != manifest:
            raise bundle_error(
                "calibrated daily price envelope differs after publication"
            )
        return bundle_id, target, manifest
    except Exception:
        if temporary.exists():
...
def validate_calibrated_daily_price_envelope_bundle(
    ...
    *,
    ...
    if (
        manifest.get("price_range_bundle_id") != expected_bundle_id
        or canonical_json_sha256(payload) != expected_bundle_id
        or manifest.get("schema_version") != "advisory_price_range_bundle_v4"
        or manifest.get("output_schema_version")
        != "advisory_daily_price_envelope_v1"
        or manifest.get("objective_contract") != "RISK_MANAGED_ADVISORY"
        or manifest.get("calibration_state") != "CALIBRATED_INTERVAL"
        or manifest.get("entry_gap_calibration_state") != "CALIBRATED"
        or manifest.get("entry_admission_model_status")
        != "RETIRED_NON_IDENTIFIABLE"
        or manifest.get("calibration_method") != CALIBRATION_METHOD
        or manifest.get("model_names") != list(PRICE_RANGE_QUANTILE_MODEL_NAMES)
        or manifest.get("model_count") != len(PRICE_RANGE_QUANTILE_MODEL_NAMES)
    ):
        raise bundle_error("calibrated daily price envelope manifest is invalid")
    files = manifest.get("files")
    required = {
        "training_request.json",
        ...
        "training_log.json",
        "test_predictions.parquet",
        "calibrated_test_predictions.parquet",
        *{f"models/{name}.txt" for name in PRICE_RANGE_QUANTILE_MODEL_NAMES},
    }
    if not isinstance(files, dict) or set(files) != required:
        raise bundle_error(
            "calibrated daily price envelope member set differs from contract"
        )
    for name, descriptor in files.items():
        path = _member(root, name)
        if (
            ...
            raise bundle_error(
                "calibrated daily price envelope member is missing or corrupt",
                filename=name,
            )
    ...
    if (
        request.request_id != manifest.get("request_id")
        or request.request_sha256 != manifest.get("request_sha256")
        or request.parent_price_range_bundle_id
        != manifest.get("parent_price_range_bundle_id")
        or request.parent_price_range_request_id
        != manifest.get("parent_price_range_request_id")
        or request.parent_price_range_request_sha256
        != manifest.get("parent_price_range_request_sha256")
        or request.parent_price_range_manifest_file_sha256
        != manifest.get("parent_price_range_manifest_file_sha256")
        or sha256_file(root / "calibration_spec.json")
        != manifest.get("calibration_spec_sha256")
    ):
        raise bundle_error(
            "calibrated daily price envelope request differs from manifest"
        )
    _validate_daily_spec(_read_json(root / "calibration_spec.json"), request=request)
    predictions = pd.read_parquet(root / "calibrated_test_predictions.parquet")
    required_columns = {
    ...
    forbidden = {"entry_executable", "entry_executable_calibration_state"} & set(
        predictions
    )
    if required_columns - set(predictions) or forbidden or predictions.empty:
        raise bundle_error(
            "calibrated daily price envelope predictions are incomplete"
        )
    keys = ["decision_as_of_trade_date", "target_trade_date", "instrument"]
    if predictions[keys].isna().any().any() or predictions.duplicated(keys).any():
        raise bundle_error(
            "calibrated daily price envelope prediction identities are invalid"
        )
    if set(predictions["entry_gap_calibration_state"].astype(str)) != {"CALIBRATED"}:
        raise bundle_error(
            "calibrated daily price envelope prediction state is invalid"
        )
    numeric_columns = sorted(
        required_columns - {*keys, "entry_gap_calibration_state"}
    )
    numeric = predictions[numeric_columns]
    if not all(math.isfinite(float(value)) for value in numeric.to_numpy().ravel()):
        raise bundle_error(
            "calibrated daily price envelope predictions contain nonfinite values"
        )
    spec = _read_json(root / "calibration_spec.json")
    delta = float(spec["delta"])
    raw_q10 = predictions["entry_gap_raw_q10"].astype(float)
    ...
    if not (
        ...
        raise bundle_error("calibrated daily price envelope formula is invalid")
    metrics = _read_json(root / "metrics.json")
    test_metrics = metrics.get("test") if isinstance(metrics, dict) else None
    if (
        not isinstance(test_metrics, dict)
        or test_metrics.get("row_count") != len(predictions)
        or metrics.get("activation_recommended") is not False
        or metrics.get("activation_decision_basis")
        != "FRESH_CONFIRMATION_REQUIRED"
    ):
        raise bundle_error(
            "calibrated daily price envelope metrics or activation boundary are invalid"
        )
    return manifest


def _validate_daily_parent(
    request: FrozenAdvisoryPriceRangeCalibrationRequestV2,
    root: Path,
    ...
    if (
        ...
        raise AdvisoryModelFirstError(
            "daily price envelope calibration parent identity mismatch",
...
def _validate_daily_spec(spec: Mapping[str, Any], *, request) -> None:
    delta = spec.get("delta")
    row_count = spec.get("row_count")
    rank = spec.get("finite_sample_rank")
    validation_metrics = spec.get("validation_metrics")
    ...
    if (
        ...
        != "advisory_daily_price_envelope_calibration_spec_v1"
        or spec.get("request_id") != request.request_id
        or spec.get("request_sha256") != request.request_sha256
        or spec.get("calibration_policy_version") != CALIBRATION_POLICY_VERSION
        or spec.get("method") != CALIBRATION_METHOD
        ...
        or not math.isfinite(float(delta))
        or float(delta) < 0
        or spec.get("entry_admission_model_status")
        != "RETIRED_NON_IDENTIFIABLE"
    ):
        raise bundle_error("daily price envelope calibration spec is invalid")
