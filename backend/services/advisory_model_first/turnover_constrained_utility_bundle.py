...
MODEL_FILENAME = "turnover_constrained_utility_model.txt"


def publish_turnover_constrained_utility_bundle(
    *,
    ...
    constraint_receipt: Mapping[str, Any],
    ...
    target_root = root / "turnover_constrained_utility_bundles"
    target_root.mkdir(parents=True, exist_ok=True)
    temp = Path(tempfile.mkdtemp(prefix="advisory_turnover_utility_", dir=root))
    try:
        if booster is not None:
            booster.save_model(str(temp / MODEL_FILENAME))
        ...
        _write_json(temp / "constraint_receipt.json", dict(constraint_receipt))
        _write_json(temp / "transform_receipt.json", dict(transform_receipt))
        ...
        payload = {
            "schema_version": "advisory_turnover_constrained_utility_bundle_v1",
            "stage": "P0_G_V1_STAGE_A_OFFLINE",
            "experiment_status": advancement_receipt["experiment_status"],
            "stage_b_eligible": bool(advancement_receipt["advanced_to_stage_b"]),
            "runtime_eligible": False,
            "model_available": booster is not None,
            "model_file": MODEL_FILENAME if booster is not None else None,
            "activated": False,
        ...
        if target.exists():
            existing = load_turnover_constrained_utility_bundle(
                target,
            ...
            if existing_identity != payload:
                raise _bundle_error("existing turnover utility bundle differs from its content identity")
            return bundle_id, target, existing["manifest"]
        os.replace(temp, target)
        load_turnover_constrained_utility_bundle(target, expected_bundle_id=bundle_id, load_booster=False)
...
def find_turnover_constrained_utility_bundle_for_request(
    request: FrozenAdvisoryTurnoverConstrainedUtilityTrainingRequestV1,
) -> tuple[str, Path, dict[str, Any]] | None:
    target_root = Path(request.output_root).resolve() / "turnover_constrained_utility_bundles"
    ...
    for candidate in sorted(target_root.iterdir()):
        ...
        try:
            frozen = FrozenAdvisoryTurnoverConstrainedUtilityTrainingRequestV1.model_validate_json(
                (candidate / "training_request.json").read_text(encoding="utf-8")
            )
        except (OSError, ValueError) as exc:
            raise _bundle_error("turnover utility bundle request cannot be read") from exc
        if frozen.functional_payload() != request.functional_payload():
            raise _bundle_error("turnover utility bundle request differs from exact retry request")
        matches.append((str(manifest["bundle_id"]), candidate, loaded["manifest"]))
    if len(matches) > 1:
        raise AdvisoryModelFirstError(
            "multiple turnover utility bundles claim the same request",
            reason_code="ADVISORY_TURNOVER_UTILITY_RETRY_MISMATCH",
        )
    return matches[0] if matches else None


def load_turnover_constrained_utility_bundle(
    ...
    *,
    ...
    try:
        ...
        raise _bundle_error("turnover utility manifest cannot be read") from exc
    ...
    if root.name != expected_bundle_id or manifest.get("bundle_id") != expected_bundle_id or actual != expected_bundle_id:
        raise _bundle_error("turnover utility bundle identity is invalid")
    for name, descriptor in (manifest.get("files") or {}).items():
        path = (root / name).resolve()
        try:
            path.relative_to(root)
        except ValueError as exc:
            raise _bundle_error("turnover utility bundle file escapes its root") from exc
        if (
            not path.is_file()
            or path.stat().st_size != int(descriptor["size_bytes"])
            or _sha256(path) != descriptor["sha256"]
        ):
            raise _bundle_error("turnover utility bundle file differs from manifest", filename=name)
    result: dict[str, Any] = {"manifest": manifest, "bundle_path": root}
    if load_booster:
        if not manifest.get("model_available"):
            raise _bundle_error("incomplete turnover utility experiment has no loadable model")
        try:
            import lightgbm as lgb

            result["booster"] = lgb.Booster(model_file=str(root / MODEL_FILENAME))
        except Exception as exc:
            raise _bundle_error("turnover utility model cannot be loaded") from exc
    return result
