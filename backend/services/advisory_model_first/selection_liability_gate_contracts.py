...
SELECTION_LIABILITY_GATE_SEEDS = (20260813, 20260817, 20260823)
SELECTION_LIABILITY_GATE_HOLDING_DAYS = (1, 2, 3, 5, 10, 20)
...
SELECTION_LIABILITY_GATE_LINEAGE = (
    "P0-D-v2",
    "P0-H-v1",
    "P0-I-v1",
    "P0-J-v1",
...
class SelectionLiabilityGateFamilySpecV1(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    family_id: Literal[
        "FAMILY_SELECTION_LIABILITY_GATE_CORE",
        "FAMILY_SELECTION_LIABILITY_GATE_CORE_HMM",
    ]
    include_hmm: bool
    ...
    def validate_family(self) -> "SelectionLiabilityGateFamilySpecV1":
        if self.include_hmm != self.family_id.endswith("_HMM"):
            raise ValueError("family_id and include_hmm disagree")
        return self


class ExactP0DSelectionLiabilityGateReferenceV1(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
...
class SelectionLiabilityGateEvidenceReferenceV1(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    role: Literal["P0H_V1_EVIDENCE", "P0I_V1_EVIDENCE", "P0J_V1_EVIDENCE"]
    bundle_root: str
    bundle_id: str = Field(pattern=r"^[0-9a-f]{64}$")
    manifest_file_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    expected_experiment_status: Literal[
    ...
    @model_validator(mode="after")
    def validate_role(self) -> "SelectionLiabilityGateEvidenceReferenceV1":
        expected = {
            "P0H_V1_EVIDENCE": ("NEGATIVE_STOP_NOT_ADVANCED", True),
            "P0I_V1_EVIDENCE": ("NEGATIVE_STOP_INCOMPLETE_CPCV", False),
            "P0J_V1_EVIDENCE": ("NEGATIVE_STOP_INCOMPLETE_CPCV", False),
        }[self.role]
        if (self.expected_experiment_status, self.expected_model_available) != expected:
            raise ValueError("evidence role and expected state disagree")
        return self


class FrozenAdvisorySelectionLiabilityGateTrainingRequestV1(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    schema_version: Literal["frozen_advisory_selection_liability_gate_training_request_v1"] = (
    ...
    family_specs: tuple[SelectionLiabilityGateFamilySpecV1, ...]
    seed_roster: tuple[int, ...] = SELECTION_LIABILITY_GATE_SEEDS
    exact_p0d_reference: ExactP0DSelectionLiabilityGateReferenceV1
    p0h_evidence_reference: SelectionLiabilityGateEvidenceReferenceV1
    p0i_evidence_reference: SelectionLiabilityGateEvidenceReferenceV1
    p0j_evidence_reference: SelectionLiabilityGateEvidenceReferenceV1
    minimum_expected_holding_days: tuple[int, ...] = SELECTION_LIABILITY_GATE_HOLDING_DAYS
    maximum_liability_thresholds: tuple[float, ...] = SELECTION_LIABILITY_GATE_THRESHOLDS
    ...
    experiment_lineage: tuple[str, ...] = SELECTION_LIABILITY_GATE_LINEAGE
    ...
    model_role: Literal["OFFLINE_SELECTION_PRESERVING_LIABILITY_GATE_V1"] = (
        "OFFLINE_SELECTION_PRESERVING_LIABILITY_GATE_V1"
    )
    ...
    execution_mode: Literal["SEQUENTIAL_LIABILITY_GATE_MAX_8GB_V1"] = (
        "SEQUENTIAL_LIABILITY_GATE_MAX_8GB_V1"
    )
    resource_max_rss_bytes: Literal[8589934592] = 8589934592

    @model_validator(mode="after")
    def validate_identity(self) -> "FrozenAdvisorySelectionLiabilityGateTrainingRequestV1":
        if tuple(item.family_id for item in self.family_specs) != (
            "FAMILY_SELECTION_LIABILITY_GATE_CORE",
            "FAMILY_SELECTION_LIABILITY_GATE_CORE_HMM",
        ):
            raise ValueError("selection-liability-gate request has an invalid family order")
        if self.seed_roster != SELECTION_LIABILITY_GATE_SEEDS:
            raise ValueError("selection-liability-gate seed roster differs from the approved roster")
        if self.minimum_expected_holding_days != SELECTION_LIABILITY_GATE_HOLDING_DAYS:
            raise ValueError("selection-liability-gate holding-day roster differs from the physical roster")
        if self.maximum_liability_thresholds != SELECTION_LIABILITY_GATE_THRESHOLDS:
            raise ValueError("selection-liability-gate threshold roster differs from the physical roster")
        physical = tuple(
            self.turnover_action_count / (self.target_count * days)
            for days in self.minimum_expected_holding_days
        )
        if physical != self.maximum_liability_thresholds:
            raise ValueError("holding-day and maximum-liability rosters disagree")
        if self.experiment_lineage != SELECTION_LIABILITY_GATE_LINEAGE:
            raise ValueError("selection-liability-gate lineage is invalid")
        ...
        if (
            self.p0h_evidence_reference.role,
            self.p0i_evidence_reference.role,
            self.p0j_evidence_reference.role,
        ) != ("P0H_V1_EVIDENCE", "P0I_V1_EVIDENCE", "P0J_V1_EVIDENCE"):
            raise ValueError("P0-H/P0-I/P0-J evidence roles are invalid")
        ...
        if decision > observation or observation > information:
            raise ValueError("selection-liability-gate information cutoffs are inconsistent")
        ...
        if self.request_id != f"advselgatereq_{expected[:24]}":
...
def build_frozen_selection_liability_gate_request(
    **values: Any,
    ...
    for field in (
        "seed_roster",
        "experiment_lineage",
        "minimum_expected_holding_days",
        "maximum_liability_thresholds",
    ):
        if field in values:
            values[field] = tuple(values[field])
    values["family_specs"] = tuple(
        item
        if isinstance(item, SelectionLiabilityGateFamilySpecV1)
        else SelectionLiabilityGateFamilySpecV1.model_validate(item)
        for item in values.get("family_specs", approved_selection_liability_gate_families())
    )
    ...
    seed = FrozenAdvisorySelectionLiabilityGateTrainingRequestV1.model_construct(
        request_id="pending",
        request_sha256="0" * 64,
    ...
    return FrozenAdvisorySelectionLiabilityGateTrainingRequestV1(
        request_id=f"advselgatereq_{digest[:24]}",
...
def approved_selection_liability_gate_families() -> tuple[SelectionLiabilityGateFamilySpecV1, ...]:
    return (
        SelectionLiabilityGateFamilySpecV1(
            family_id="FAMILY_SELECTION_LIABILITY_GATE_CORE",
            include_hmm=False,
        ),
        SelectionLiabilityGateFamilySpecV1(
            family_id="FAMILY_SELECTION_LIABILITY_GATE_CORE_HMM",
            include_hmm=True,
        ),
    )
