...
from backend.services.advisory_model_first.selection_prior_residual_contracts import (
    ExactSelectionPriorResidualReferenceV1,
...
from backend.services.advisory_model_first.selection_prior_residual_training import (
    ...
    selection_prior_residual_candidate_metrics,
    eligible_constraint_dates,
    fit_final_selection_prior_residual_models,
    fit_oof_price_scale,
    score_final_selection_prior_residual_models,
...
ARM_ID = "ARM_P0J_V1_SELECTION_PRIOR_RESIDUAL_OUTPUT_CONSTRAINED_UTILITY"


def run_selection_prior_residual_pipeline(request_path: str | Path) -> dict[str, Any]:
    try:
        request = FrozenAdvisorySelectionPriorResidualTrainingRequestV1.model_validate_json(
            Path(request_path).read_text(encoding="utf-8")
        )
    except (OSError, ValueError) as exc:
        raise AdvisoryModelFirstError(
            "selection-prior-residual frozen request cannot be read or validated",
            reason_code="ADVISORY_SELECTION_PRIOR_RESIDUAL_REQUEST_INVALID",
        ) from exc
    ...
    p0i_evidence = _load_p0i_evidence(request, request.p0i_evidence_reference)
    existing = find_selection_prior_residual_bundle_for_request(request)
    ...
    for path in paths:
        ...
        for family in request.family_specs:
            for seed in request.seed_roster:
                ...
                try:
                    oof = train_selection_prior_residual_oof(
                        features=feature_result.features,
                        labels=labels,
                        folds=folds,
                        family=family,
                    ...
                    def evaluate_price(price: float) -> float:
                        priorities = combine_selection_prior_residual_predictions(oof.predictions, shadow_price=price)
                    ...
                    residual_rounds = max(1, int(np.median(oof.residual_best_iterations)))
                    liability_rounds = max(1, int(np.median(oof.liability_best_iterations)))
                    models = fit_final_selection_prior_residual_models(
                        features=feature_result.features,
                        labels=labels,
                        train_dates=train_dates,
                        family=family,
                        seed=seed,
                        residual_boost_rounds=residual_rounds,
                        liability_boost_rounds=liability_rounds,
                        reliability=oof.reliability,
                    )
                    validation_predictions = score_final_selection_prior_residual_models(
                        features=feature_result.features,
                        models=models,
                    ...
                    combined = combine_selection_prior_residual_predictions(
                        validation_predictions,
                        shadow_price=price_selection.shadow_price_bps_per_fraction,
                    )
                    diagnostic_rows = _attach_labels(combined, labels)
                    candidate_metrics = selection_prior_residual_candidate_metrics(diagnostic_rows)
                ...
                constraint_receipts.append(
                    {
                        ...
                        "return_scale_bps": scale.return_scale_bps,
                        "liability_scale": scale.liability_scale,
                        "base_price_bps_per_fraction": scale.base_price_bps_per_fraction,
                        "residual_boost_rounds": residual_rounds,
                        "liability_boost_rounds": liability_rounds,
                        "inner_folds": list(oof.fold_receipts),
                        "residual_reliability": oof.reliability.as_dict(),
                ...
                trial_rows.append(
                    {
                        ...
                        **_episode_metrics(policy_daily, policy_episodes, target_count=policy.target_count),
                        ...
                        "shadow_price_bps_per_fraction": price_selection.shadow_price_bps_per_fraction,
                        "p0d_oof_turnover_budget": price_selection.p0d_oof_turnover_budget,
                        "p0j_oof_turnover": price_selection.p0j_oof_turnover,
                        "oof_constraint_slack": price_selection.constraint_slack,
                        "residual_best_iteration": residual_rounds,
                        "liability_best_iteration": liability_rounds,
                        "residual_reliability_status": oof.reliability.status,
                    }
                )
    ...
    constraint_payload = {
        "schema_version": "advisory_selection_prior_residual_inner_oof_constraint_v1",
    ...
    if len(trial_metrics) != request.expected_outer_trial_path_count:
        raise AdvisoryModelFirstError(
            "selection-prior-residual outer trial roster is incomplete",
            reason_code="ADVISORY_SELECTION_PRIOR_RESIDUAL_OUTER_ROSTER_INVALID",
    ...
    final_oof = train_selection_prior_residual_oof(
        features=feature_result.features,
        labels=labels,
        folds=final_folds,
        family=family,
    ...
    def evaluate_final_price(price: float) -> float:
        priorities = combine_selection_prior_residual_predictions(final_oof.predictions, shadow_price=price)
        metrics = _evaluate_constraint_blocks(
            rankings=rankings,
            entry_priorities=priorities,
            calibration_dates=eligible_dates,
    ...
    final_residual_rounds = max(1, int(np.median(final_oof.residual_best_iterations)))
    final_liability_rounds = max(1, int(np.median(final_oof.liability_best_iterations)))
    final_models = fit_final_selection_prior_residual_models(
        features=feature_result.features,
        labels=labels,
        train_dates=all_dates,
        family=family,
        seed=int(winner.seed),
        residual_boost_rounds=final_residual_rounds,
        liability_boost_rounds=final_liability_rounds,
        reliability=final_oof.reliability,
    )
    progress.add(
        "trials_and_final",
        started,
    ...
    feature_schema.update(
        {
            ...
            "entry_priority_score_kind": "SELECTION_PRIOR_RESIDUAL_OUTPUT_CONSTRAINED_UTILITY_BPS",
        }
    )
    winner_receipt = {
        "schema_version": "advisory_selection_prior_residual_winner_v1",
        "arm_id": ARM_ID,
        ...
        "final_residual_boost_rounds": final_residual_rounds,
        "final_liability_boost_rounds": final_liability_rounds,
        "final_shadow_price_bps_per_fraction": final_price.shadow_price_bps_per_fraction,
        "advancement_status": advancement["experiment_status"],
    }
    constraint_payload["final"] = {
        "return_scale_bps": final_scale.return_scale_bps,
        "liability_scale": final_scale.liability_scale,
        "base_price_bps_per_fraction": final_scale.base_price_bps_per_fraction,
        "residual_boost_rounds": final_residual_rounds,
        "liability_boost_rounds": final_liability_rounds,
        "inner_folds": list(final_oof.fold_receipts),
        "residual_reliability": final_oof.reliability.as_dict(),
        **final_price.__dict__,
    }
    baseline = {
        "schema_version": "advisory_selection_prior_residual_baselines_v1",
    ...
    references = {
        "schema_version": "advisory_selection_prior_residual_reference_comparison_v1",
        ...
        "p0i_v1_evidence": {
            "bundle_id": request.p0i_evidence_reference.bundle_id,
            "experiment_status": request.p0i_evidence_reference.experiment_status,
            "model_available": request.p0i_evidence_reference.model_available,
        },
        "p0f_is_advancement_gate": False,
        "p0g_is_advancement_gate": False,
        "p0h_is_advancement_gate": False,
        "p0i_is_advancement_gate": False,
        ...
        "p0i_bundle_id": request.p0i_evidence_reference.bundle_id,
    }
    diagnostics = {
        "schema_version": "advisory_selection_prior_residual_candidate_diagnostics_v1",
    ...
    bundle_id, bundle_path, manifest = publish_selection_prior_residual_bundle(
        request=request,
        residual_booster=final_models.residual_booster,
        ...
        selection_prior_receipt=final_models.selection_prior.as_dict(),
        residual_reliability_receipt=final_models.reliability.as_dict(),
        transform_receipt={
            "schema_version": "advisory_selection_prior_residual_transform_v1",
            "residual": {
                "location": final_models.residual_transform.location_bps,
                "scale": final_models.residual_transform.scale_bps,
            },
            "liability": {
            ...
            "shadow_price_bps_per_fraction": final_price.shadow_price_bps_per_fraction,
    ...
    *,
    ...
    if not parts:
        raise AdvisoryModelFirstError(
            ...
            reason_code="ADVISORY_SELECTION_PRIOR_RESIDUAL_P0D_OOF_INVALID",
        )
...
def _verify_prediction_dates(predictions: pd.DataFrame, expected_dates: Sequence[pd.Timestamp]) -> None:
    ...
    if actual != expected or counts.empty or not counts.eq(20).all():
        raise AdvisoryModelFirstError(
            "selection-prior-residual/P0-D OOF prediction dates differ from exact calibration dates",
            reason_code="ADVISORY_SELECTION_PRIOR_RESIDUAL_OOF_DATE_MISMATCH",
        )
...
def _load_reference(
    ...
    reference: ExactSelectionPriorResidualReferenceV1,
) -> dict[str, Any]:
    ...
    if not manifest_path.is_file() or _sha256(manifest_path) != reference.manifest_file_sha256:
        raise AdvisoryModelFirstError(
            "selection-prior-residual reference manifest differs from request",
            reason_code="ADVISORY_SELECTION_PRIOR_RESIDUAL_REFERENCE_MISMATCH",
    ...
    if mismatches:
        raise AdvisoryModelFirstError(
            "selection-prior-residual reference identity differs from request",
            reason_code="ADVISORY_SELECTION_PRIOR_RESIDUAL_REFERENCE_MISMATCH",
    ...
    if not winner or (
        ...
        raise AdvisoryModelFirstError(
            "selection-prior-residual reference winner identity differs from request",
            reason_code="ADVISORY_SELECTION_PRIOR_RESIDUAL_REFERENCE_MISMATCH",
            context={"role": reference.role},
        )
    return {
...
def _load_p0i_evidence(
    request: FrozenAdvisorySelectionPriorResidualTrainingRequestV1,
    reference: P0ISelectionPriorResidualEvidenceReferenceV1,
) -> dict[str, Any]:
    root = Path(reference.bundle_root).resolve()
    manifest_path = root / "manifest.json"
    if not manifest_path.is_file() or _sha256(manifest_path) != reference.manifest_file_sha256:
        raise AdvisoryModelFirstError(
            "selection-prior-residual P0-I evidence manifest differs from request",
            reason_code="ADVISORY_SELECTION_PRIOR_RESIDUAL_REFERENCE_MISMATCH",
            context={"role": reference.role},
        )
    loaded = load_grouped_rank_bundle(
        root,
        expected_bundle_id=reference.bundle_id,
        load_boosters=False,
    )
    expected = {
        ...
        "shadow_policy_sha256": request.shadow_policy_sha256,
        "feature_schema_hash": request.feature_schema_hash,
        "experiment_status": reference.experiment_status,
        "model_available": reference.model_available,
    }
    mismatches = {
        key: {"expected": value, "actual": loaded["manifest"].get(key)}
        for key, value in expected.items()
    ...
    if mismatches:
        raise AdvisoryModelFirstError(
            "selection-prior-residual P0-I evidence identity differs from request",
            reason_code="ADVISORY_SELECTION_PRIOR_RESIDUAL_REFERENCE_MISMATCH",
            context={"role": reference.role, "mismatches": mismatches},
        )
    return {"root": root, "loaded": loaded}
...
def _reference_winner_rows(
    ...
    specification: ExactSelectionPriorResidualReferenceV1,
    ...
    if len(selected) != len(expected_paths) or set(selected["path_id"]) != expected_paths:
        raise AdvisoryModelFirstError(
            "selection-prior-residual reference does not contain exact 28 winner paths",
            reason_code="ADVISORY_SELECTION_PRIOR_RESIDUAL_REFERENCE_MISMATCH",
...
def _verify_label_status_identity(
    ...
    if (
        ...
        raise AdvisoryModelFirstError(
            "selection-prior-residual label status identity differs from frozen P0-C",
...
def _verify_cpcv_identity(
    ...
    if (
        ...
        raise AdvisoryModelFirstError(
            "selection-prior-residual CPCV identity differs from frozen P0-C",
            reason_code="ADVISORY_SELECTION_PRIOR_RESIDUAL_OUTER_ROSTER_INVALID",
        )
...
def _diagnostic_summary(rows: pd.DataFrame) -> dict[str, Any]:
    columns = [
        ...
        "residual_mae",
        "residual_rmse",
        "residual_daily_spearman_mean",
        "residual_daily_spearman_null_count",
        "residual_alpha",
        "liability_mae",
        "liability_rmse",
        ...
        "selection_top5_overlap_fraction",
    ...
    *,
    ...
    bundle_id, bundle_path, manifest = publish_selection_prior_residual_bundle(
        ...
        selection_prior_receipt={
            ...
            "status": "NOT_AVAILABLE_INCOMPLETE_CPCV",
        },
        residual_reliability_receipt={
            "schema_version": "advisory_selection_prior_residual_reliability_v1",
            "status": "NOT_AVAILABLE_INCOMPLETE_CPCV",
        },
        transform_receipt={"schema_version": "advisory_selection_prior_residual_transform_v1", "status": "INCOMPLETE"},
        trial_metrics=trial_metrics,
        block_scores=block_scores,
        candidate_diagnostics={"schema_version": "advisory_selection_prior_residual_candidate_diagnostics_v1", "status": "INCOMPLETE"},
        pbo_receipt={"schema_version": "advisory_policy_pbo_v1", "status": "NOT_COMPUTABLE"},
        winner_receipt={"schema_version": "advisory_selection_prior_residual_winner_v1", "status": "INCOMPLETE"},
        baseline_comparison={
            "schema_version": "advisory_selection_prior_residual_baselines_v1",
            "selection_path_metrics": baseline_by_path,
        },
        reference_comparison={
            ...
            "p0i_bundle_id": references[4]["loaded"]["manifest"]["bundle_id"],
        },
        advancement_receipt=advancement,
        training_log={
            "environment": environment,
