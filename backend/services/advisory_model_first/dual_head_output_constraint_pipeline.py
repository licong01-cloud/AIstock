...
from backend.services.advisory_model_first.dual_head_output_constraint_bundle import (
    find_dual_head_bundle_for_request,
    publish_dual_head_bundle,
)
from backend.services.advisory_model_first.dual_head_output_constraint_contracts import (
    ExactDualHeadReferenceV1,
    FrozenAdvisoryDualHeadTrainingRequestV1,
)
from backend.services.advisory_model_first.dual_head_output_constraint_training import (
    ...
    dual_head_candidate_metrics,
    eligible_constraint_dates,
    fit_final_dual_head_models,
    fit_oof_price_scale,
    score_final_dual_head_models,
...
def run_dual_head_output_constraint_pipeline(request_path: str | Path) -> dict[str, Any]:
    try:
        request = FrozenAdvisoryDualHeadTrainingRequestV1.model_validate_json(
            Path(request_path).read_text(encoding="utf-8")
        )
    except (OSError, ValueError) as exc:
        raise AdvisoryModelFirstError(
            "dual-head frozen request cannot be read or validated",
            reason_code="ADVISORY_DUAL_HEAD_REQUEST_INVALID",
        ) from exc
    ...
    existing = find_dual_head_bundle_for_request(request)
            ...
            for seed in request.seed_roster:
                ...
                try:
                    ...
                    def evaluate_price(price: float) -> float:
                        priorities = combine_dual_head_predictions(oof.predictions, shadow_price=price)
                    ...
                    models = fit_final_dual_head_models(
                        features=feature_result.features,
                        labels=labels,
                    ...
                    validation_predictions = score_final_dual_head_models(
                        features=feature_result.features,
                        models=models,
                ...
                trial_rows.append(
                    {
                        ...
                        **_episode_metrics(policy_daily, policy_episodes, target_count=policy.target_count),
                        ...
                        "p0h_oof_turnover": price_selection.p0h_oof_turnover,
    ...
    constraint_payload = {
        "schema_version": "advisory_dual_head_inner_oof_constraint_v1",
    ...
    if path_failure is not None:
        return _publish_incomplete(
            ...
            references=(p0d_reference, p0f_reference, p0g_reference),
    ...
    if len(trial_metrics) != request.expected_outer_trial_path_count:
        raise AdvisoryModelFirstError(
            "dual-head outer trial roster is incomplete",
            reason_code="ADVISORY_DUAL_HEAD_OUTER_ROSTER_INVALID",
    ...
    def evaluate_final_price(price: float) -> float:
        priorities = combine_dual_head_predictions(final_oof.predictions, shadow_price=price)
        metrics = _evaluate_constraint_blocks(
            rankings=rankings,
            entry_priorities=priorities,
            calibration_dates=eligible_dates,
    ...
    final_models = fit_final_dual_head_models(
        features=feature_result.features,
        labels=labels,
    ...
    feature_schema.update(
        {
            ...
            "entry_priority_score_kind": "DUAL_HEAD_OUTPUT_CONSTRAINED_UTILITY_BPS",
        }
    )
    winner_receipt = {
        "schema_version": "advisory_dual_head_winner_v1",
        "arm_id": ARM_ID,
    ...
    baseline = {
        "schema_version": "advisory_dual_head_baselines_v1",
    ...
    references = {
        "schema_version": "advisory_dual_head_reference_comparison_v1",
    ...
    diagnostics = {
        "schema_version": "advisory_dual_head_candidate_diagnostics_v1",
    ...
    bundle_id, bundle_path, manifest = publish_dual_head_bundle(
        request=request,
        ...
        transform_receipt={
            "schema_version": "advisory_dual_head_transform_v1",
            "return": {
                "location": final_models.return_transform.location_bps,
                "scale": final_models.return_transform.scale_bps,
            },
            "liability": {
    ...
    *,
    ...
    if not parts:
        raise AdvisoryModelFirstError(
            ...
            reason_code="ADVISORY_DUAL_HEAD_P0D_OOF_INVALID",
        )
...
def _verify_prediction_dates(predictions: pd.DataFrame, expected_dates: Sequence[pd.Timestamp]) -> None:
    ...
    if actual != expected or counts.empty or not counts.eq(20).all():
        raise AdvisoryModelFirstError(
            "dual-head/P0-D OOF prediction dates differ from exact calibration dates",
            reason_code="ADVISORY_DUAL_HEAD_OOF_DATE_MISMATCH",
        )
...
def _load_reference(
    request: FrozenAdvisoryDualHeadTrainingRequestV1,
    reference: ExactDualHeadReferenceV1,
) -> dict[str, Any]:
    ...
    if not manifest_path.is_file() or _sha256(manifest_path) != reference.manifest_file_sha256:
        raise AdvisoryModelFirstError(
            "dual-head reference manifest differs from request",
            reason_code="ADVISORY_DUAL_HEAD_REFERENCE_MISMATCH",
    ...
    if mismatches:
        raise AdvisoryModelFirstError(
            "dual-head reference identity differs from request",
            reason_code="ADVISORY_DUAL_HEAD_REFERENCE_MISMATCH",
    ...
    if not winner or (
        ...
        raise AdvisoryModelFirstError(
            "dual-head reference winner identity differs from request",
            reason_code="ADVISORY_DUAL_HEAD_REFERENCE_MISMATCH",
            context={"role": reference.role},
        )
    return {
...
def _reference_winner_rows(
    ...
    specification: ExactDualHeadReferenceV1,
    ...
    if len(selected) != len(expected_paths) or set(selected["path_id"]) != expected_paths:
        raise AdvisoryModelFirstError(
            "dual-head reference does not contain exact 28 winner paths",
            reason_code="ADVISORY_DUAL_HEAD_REFERENCE_MISMATCH",
...
def _verify_label_status_identity(
    ...
    if (
        ...
        raise AdvisoryModelFirstError(
            "dual-head label status identity differs from frozen P0-C",
            reason_code="ADVISORY_DUAL_HEAD_LABEL_INVALID",
...
def _verify_cpcv_identity(
    ...
    if (
        ...
        raise AdvisoryModelFirstError(
            "dual-head CPCV identity differs from frozen P0-C",
            reason_code="ADVISORY_DUAL_HEAD_OUTER_ROSTER_INVALID",
        )
...
def _publish_incomplete(
    *,
    ...
    references: tuple[dict[str, Any], dict[str, Any], dict[str, Any]],
    ...
    bundle_id, bundle_path, manifest = publish_dual_head_bundle(
        ...
        transform_receipt={"schema_version": "advisory_dual_head_transform_v1", "status": "INCOMPLETE"},
        trial_metrics=trial_metrics,
        block_scores=block_scores,
        candidate_diagnostics={"schema_version": "advisory_dual_head_candidate_diagnostics_v1", "status": "INCOMPLETE"},
        pbo_receipt={"schema_version": "advisory_policy_pbo_v1", "status": "NOT_COMPUTABLE"},
        winner_receipt={"schema_version": "advisory_dual_head_winner_v1", "status": "INCOMPLETE"},
        baseline_comparison={
            "schema_version": "advisory_dual_head_baselines_v1",
            "selection_path_metrics": baseline_by_path,
        },
