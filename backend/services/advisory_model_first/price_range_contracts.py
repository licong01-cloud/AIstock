...
PRICE_RANGE_QUANTILE_MODEL_NAMES = PRICE_RANGE_MODEL_NAMES[1:]
ENTRY_GAP_CONDITION = "ENTRY_EXECUTABLE"
DAILY_PRICE_ENVELOPE_LABEL_POLICY_VERSION = "advisory_price_range_label_policy_v2"
DAILY_PRICE_ENVELOPE_ENTRY_GAP_CONDITION = "NEXT_TRADING_DAY_VALID_OPEN"
...
def build_frozen_price_range_training_request(
    **values: object,
    ...
    for artifact_field in ("candidates_artifact", "features_artifact"):
        if artifact_field in values:
            values[artifact_field] = PriceRangeInputArtifactV1.model_validate(
                values[artifact_field]
            )
...
class FrozenAdvisoryPriceRangeTrainingRequestV2(BaseModel):
    """Frozen three-quantile request for AdvisoryDailyPriceEnvelopeV1."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    schema_version: Literal["frozen_advisory_price_range_training_request_v2"] = (
        "frozen_advisory_price_range_training_request_v2"
    )
    request_id: str
    request_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    created_at: str
    ...
    candidate_semantics_id: str
    entry_gap_condition: Literal["NEXT_TRADING_DAY_VALID_OPEN"] = (
    ...
    decision_date_end: str = "2026-03-10"
    data_cutoff: str = "2026-06-30"
    trainer_seed: int = 20260914
    resource_max_rss_bytes: int = Field(default=8 * 1024**3, gt=0, le=8 * 1024**3)

    @model_validator(mode="after")
    def validate_identity(self) -> "FrozenAdvisoryPriceRangeTrainingRequestV2":
        if self.quantiles != PRICE_RANGE_QUANTILES:
            raise ValueError(f"quantiles must equal {PRICE_RANGE_QUANTILES}")
        expected = canonical_json_sha256(self.functional_payload())
        if expected != self.request_sha256:
...
def build_frozen_daily_price_envelope_training_request(
    **values: object,
    ...
    seed = FrozenAdvisoryPriceRangeTrainingRequestV2.model_construct(
        schema_version="frozen_advisory_price_range_training_request_v2",
        request_id="pending",
        request_sha256="0" * 64,
        created_at=created_at,
        **values,
    ...
    digest = canonical_json_sha256(seed.functional_payload())
    return FrozenAdvisoryPriceRangeTrainingRequestV2(
        schema_version="frozen_advisory_price_range_training_request_v2",
        request_id=f"advprreq_{digest[:24]}",
        request_sha256=digest,
        created_at=created_at,
