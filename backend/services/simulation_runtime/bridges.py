...
class LocalSimExecutionBridge:
    ...
    @staticmethod
    def _export_execution_snapshot(
        *, broker: BrokerBackend, handles: tuple[OrderHandle, ...]
    ) -> LocalSimExecutionSnapshot | None:
...
class MiniQMTExecutionBridge:
        ...
        *,
        ...
        build = self._runtime_client.build_managed_vnpy_order_requests(
            ...
            quote_provider=_quote_provider(
                quote_by_symbol=quote_by_symbol or {}, price_by_symbol=price_by_symbol or {}
            ),
    ...
    def submit_plan(self, **kwargs: Any) -> MiniQMTRuntimeManagedBatchSubmitResult:
        ...
        if mode != "SIM":
            raise LiveApprovalRequiredError(
                ...
                context={"mode": mode, "reason_code": "MINIQMT_EVENT_LOOP_LIVE_FORBIDDEN"},
            )
        plan = kwargs.get("plan")
        binding = kwargs.get("binding")
        raise RuntimeConfigInvalidError(
            "MiniQMT SIM compiler submit route is retired; use event_loop(A) only",
            context={
                "reason_code": "MINIQMT_SIM_COMPILER_ROUTE_RETIRED",
                "stage": "MINIQMT_COMPILER_SUBMIT_REJECTED",
                "plan_id": getattr(plan, "plan_id", None),
                "binding_id": getattr(binding, "binding_id", None),
                "strategy_id": getattr(binding, "strategy_id", None),
                "broker_backend": getattr(getattr(binding, "broker_backend", None), "value", None),
                "runtime_kind": MiniQMTExecutionRuntimeKind.COMPILER.value,
                "allowed_runtime_kind": MiniQMTExecutionRuntimeKind.EVENT_LOOP.value,
                "broker_called": False,
                "submitted_intents": 0,
                "failed_intents": len(getattr(plan, "intents", ()) or ()),
            },
        )
    ...
    def _account_group_id(*, plan: ExecutionPlan, binding: SimulationReleaseBinding) -> str:
        return str(
            plan.account_group_id or binding.account_group_id or binding.broker_account_id or binding.strategy_id
        )
...
def _vnpy_policy_context_from_plan(plan: ExecutionPlan) -> dict[str, Any] | None:
    ...
    inferred_algo_code = _infer_vnpy_algo_code_from_policy_ids(
        plan=plan, policy_container=policy_container, payload=payload
    )
    ...
    return {
        ...
        "policy_sha256": str(
            payload.get("policy_sha256") or policy_container.get("sha256") or plan.execution_policy_sha256
        ),
        "algo_code": algo_code,
...
def _policy_error_context(
    *,
    ...
    context: dict[str, Any] = {
        ...
        "payload_policy_id": payload.get("validated_execution_policy_id")
        or payload.get("policy_id")
