"""Shared MiniQMT vn.py-style execution adapter.

The module keeps vnpy_algotrading-derived algorithm semantics in one place for
Paper v2 single-strategy execution and MiniQMT virtual-strategy bridge
execution. It reuses AIstock vn.py-style cores without importing vn.py
EventEngine, MainEngine, gateway, or UI runtime objects.
"""
...
from .miniqmt_order_state import (
...
class MiniQMTChildOrderHandle:
    """Adapter-neutral handle for one MiniQMT child order."""
    ...
    def model_dump(self, mode: str = "python") -> dict[str, Any]:  # noqa: ARG002
        return {
            ...
            "native_order_id": self.native_order_id,
            "native_context": dict(self.native_context),
        }


@dataclass(frozen=True)
class MiniQMTOrderStatus:
    """Adapter-neutral broker order status snapshot."""

    handle_id: str
    state: str
    filled_quantity: int = 0
    avg_fill_price: Decimal | float | int | str | None = None
    ...
    def model_dump(self, mode: str = "python") -> dict[str, Any]:
        avg_fill_price: Any = self.avg_fill_price
        if isinstance(avg_fill_price, Decimal):
            avg_fill_price = float(avg_fill_price) if mode == "json" else avg_fill_price
        last_event_at: Any = self.last_event_at
        if isinstance(last_event_at, datetime) and mode == "json":
...
@dataclass(frozen=True)
class MiniQMTCancelResult:
    accepted: bool
    reason: str | None = None
    raw: dict[str, Any] = field(default_factory=dict)

    def to_dict(self) -> dict[str, Any]:
        return {"accepted": bool(self.accepted), "reason": self.reason, "raw": dict(self.raw)}


@dataclass(frozen=True)
class MiniQMTChildOrderRequest:
...
@dataclass(frozen=True)
class MiniQMTChildOrderSubmitResult:
    handle: MiniQMTChildOrderHandle | None
    status: MiniQMTOrderStatus | None = None
...
class MiniQMTChildOrderSubmitter(Protocol):
    """Boundary implemented by Paper v2 and MiniQMT virtual-ledger adapters."""
...
class UnifiedMiniQMTVnpyExecutionAdapter:
    """Route one parent OrderIntent through a selected vn.py-style asset."""

    def __init__(
        self,
        *,
        submitter: MiniQMTChildOrderSubmitter,
        policy_context: dict[str, Any],
                    ...
                    actions.extend(
                        core.update_order(
                            VnpyOrderUpdate(
                                ...
                                price=_status_avg_price(child.status),
    ...
    def _submit_child_action(
        ...
        *,
        ...
        request = MiniQMTChildOrderRequest(
        ...
        try:
            submitted = self.submitter.submit_child(request)
            return MiniQMTAlgoChildOrder(
                vt_orderid=action.vt_orderid or child_intent.intent_id,
                handle=submitted.handle,
                intent=child_intent,
                submitted_at=submitted_at,
                native_context=dict(submitted.native_context or {}),
                status=submitted.status,
                trades=[dict(row) for row in submitted.trades],
                submit_error=submitted.submit_error,
            )
        ...
        except Exception as exc:  # noqa: BLE001 - broker/submitter diagnostics must be preserved
            error = {
    ...
    def _cancel_matching_child(self, children: list[MiniQMTAlgoChildOrder], action: VnpyAction) -> None:
        for child in children:
            ...
            if child.status is not None and not _status_is_active(child.status):
                continue
            try:
                ack = self.submitter.cancel_child(child.handle, action=action, reason=action.reason or "vnpy_cancel")
                child.native_context = {**dict(child.native_context), "cancel_ack": ack.to_dict()}
                queried = self.submitter.query_order(child.handle)
                if queried is not None:
                    child.status = queried
                    child.trades = [dict(row) for row in self.submitter.query_trades(child.handle)]
            except TradingCoreError as exc:
                child.native_context = {**dict(child.native_context), "cancel_error": exc.to_dict()}
            except Exception as exc:  # noqa: BLE001 - preserve broker raw diagnostics
...
def _status_is_active(status: MiniQMTOrderStatus | None) -> bool:
    if status is None:
        return False
    return status.state in {"pending", "partial_filled"}


def _status_avg_price(status: MiniQMTOrderStatus | None) -> float | None:
    if status is None or status.avg_fill_price in {None, ""}:
        return None
    try:
        parsed = float(status.avg_fill_price)
    except (TypeError, ValueError):
        return None
    return parsed if parsed > 0 else None


def _status_raw(status: MiniQMTOrderStatus | None) -> dict[str, Any]:
    if status is None:
        return {}
