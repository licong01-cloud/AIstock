...
class TraceDeliveryEvent(BaseModel):
    ...
    @classmethod
    def from_request(cls, request: TraceDeliveryEventRequest, *, event_at: datetime) -> "TraceDeliveryEvent":
        payload = {**request.request_payload(), "event_at": event_at.astimezone(timezone.utc)}
...
class TraceOutboxRecord(BaseModel):
    ...
    def _identity_matches(self) -> "TraceOutboxRecord":
        ...
        if _canonicalize(binding) != _canonicalize(self.binding.model_dump(mode="json")):
...
class TraceAdmissionValidator(Protocol):
    ...
    def validate(
        self, *, envelope: StageTraceEnvelope, binding: TraceCaptureBinding, conn: Any | None = None
    ) -> None: ...
...
class RejectingTraceAdmissionValidator:
    ...
    def validate(
        ...
        raise SourceLedgerError(
            ...
            context={"trace_outbox_id": envelope.trace_outbox_id, "capture_batch_id": binding.capture_batch_id},
        )
...
class TraceDispatchFailureHandler(Protocol):
    ...
    def __call__(
        self, *, envelope: StageTraceEnvelope, binding: TraceCaptureBinding, reason_code: str, error: Exception
    ) -> None: ...
...
class BoundedTraceOutboxDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        self._queue: Queue[tuple[StageTraceEnvelope, TraceCaptureBinding] | None] = Queue(maxsize=max_pending)
        self._thread = Thread(target=self._run, name="advisory-trace-outbox", daemon=True)
        self._thread.start()

    def append(self, envelope: StageTraceEnvelope, *, binding: TraceCaptureBinding) -> None:
...
class InMemoryTraceOutboxRepository:
    ...
    def append(self, envelope: StageTraceEnvelope, *, binding: TraceCaptureBinding) -> TraceOutboxRecord:
        ...
        if existing is not None:
            if existing.envelope == envelope and existing.binding == binding:
                return existing
            raise SourceLedgerError(REASON_TRACE_OUTBOX_CONFLICT, "same trace hash has conflicting immutable payload")
    ...
    def append_delivery(self, request: TraceDeliveryEventRequest) -> TraceDeliveryEvent:
        if request.trace_outbox_id not in self._events_by_outbox:
            raise SourceLedgerError(REASON_TRACE_OUTBOX_RECORD_INVALID, "trace outbox record does not exist")
        ...
        if request.delivery_event_no <= len(chain):
            ...
            if existing.request.request_hash == request.request_hash:
                return existing
            raise SourceLedgerError(REASON_TRACE_OUTBOX_CONFLICT, "same delivery event sequence has a different request")
        if request.delivery_event_no != len(chain) + 1:
            raise SourceLedgerError(REASON_TRACE_OUTBOX_CHAIN_INVALID, "delivery event number must be the next sequence number")
        if chain:
            predecessor = chain[-1]
            if predecessor.delivery_event_hash != request.predecessor_event_hash:
                raise SourceLedgerError(REASON_TRACE_OUTBOX_CHAIN_INVALID, "delivery predecessor does not match prior event")
            if predecessor.request.event_type is TraceDeliveryEventType.OBSERVATION_WRITTEN:
                raise SourceLedgerError(REASON_TRACE_OUTBOX_CHAIN_INVALID, "written delivery event is terminal")
...
class PostgresTraceOutboxRepository:
        ...
        with self._conn_factory() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                ...
                if existing is not None:
                    ...
                    if record.envelope == envelope and record.binding == binding:
                        return record
                    raise SourceLedgerError(REASON_TRACE_OUTBOX_CONFLICT, "same trace hash has conflicting immutable payload")
                cur.execute(
                    """
                ...
                # above do not mutate state and therefore need no fresh lease.
                self._admission_validator.validate(envelope=envelope, binding=binding, conn=conn)
    ...
    def append_delivery(self, request: TraceDeliveryEventRequest) -> TraceDeliveryEvent:
        with self._conn_factory() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute("SELECT pg_advisory_xact_lock(hashtext(%s))", (request.trace_outbox_id,))
                cur.execute(
                ...
                cur.execute(
                    """
                    SELECT * FROM app.advisory_selection_stage_trace_delivery_event
                    WHERE trace_outbox_id = %s AND delivery_event_no = %s
                    FOR UPDATE
                    """,
                    (request.trace_outbox_id, request.delivery_event_no),
                )
                existing = cur.fetchone()
                if existing is not None:
                    event = _delivery_from_row(dict(existing))
                    if event.request.request_hash == request.request_hash:
                        return event
                    raise SourceLedgerError(REASON_TRACE_OUTBOX_CONFLICT, "same delivery sequence has a different request")
                cur.execute("SELECT clock_timestamp() AS event_at")
                event = TraceDeliveryEvent.from_request(request, event_at=cur.fetchone()["event_at"])
                try:
                    cur.execute(
                        """
                    ...
                    raise SourceLedgerError(REASON_TRACE_OUTBOX_CHAIN_INVALID, "database rejected trace delivery chain") from exc
...
class TraceCaptureReconciler:
    ...
    def __init__(self, *, outbox: TraceOutboxIdentityLookup, gap_handler: TraceCaptureGapHandler) -> None:
        self._outbox = outbox
...
def _validate_envelope_binding(*, envelope: StageTraceEnvelope, binding: TraceCaptureBinding) -> None:
    ...
    if _canonicalize(embedded) != _canonicalize(binding.model_dump(mode="json")):
        raise SourceLedgerError(REASON_TRACE_OUTBOX_RECORD_INVALID, "trace envelope binding does not match append binding")
    natural_key = _natural_key(envelope, binding)
    if not all(natural_key):
        raise SourceLedgerError(REASON_TRACE_OUTBOX_RECORD_INVALID, "trace envelope selection identity is incomplete")
...
def _record_from_row(row: dict[str, Any]) -> TraceOutboxRecord:
    ...
    try:
        ...
        raise SourceLedgerError(REASON_TRACE_OUTBOX_RECORD_INVALID, "persisted trace outbox row is invalid") from exc
    if (
        ...
        raise SourceLedgerError(REASON_TRACE_OUTBOX_RECORD_INVALID, "persisted trace outbox columns do not match envelope binding")
    return record
...
def _delivery_from_row(row: dict[str, Any]) -> TraceDeliveryEvent:
    request = TraceDeliveryEventRequest(
        ...
        predecessor_event_hash=str(row["predecessor_event_hash"]) if row["predecessor_event_hash"] else None,
    ...
    if event.delivery_event_id != str(row["delivery_event_id"]) or event.delivery_event_hash != str(row["delivery_event_hash"]):
        raise SourceLedgerError(REASON_TRACE_OUTBOX_RECORD_INVALID, "persisted delivery event does not match canonical hash")
    if request.request_hash != str(row["delivery_request_hash"]):
        raise SourceLedgerError(REASON_TRACE_OUTBOX_RECORD_INVALID, "persisted delivery request hash is invalid")
    return event
