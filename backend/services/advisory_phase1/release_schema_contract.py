...
def canonicalize(value: Any, *, key: str | None = None) -> Any:
    ...
    if isinstance(value, dict):
        return {str(item_key): canonicalize(item_value) for item_key, item_value in sorted(value.items(), key=lambda item: str(item[0]))}
...
class ManagedMigration(StrictModel):
    ...
    def _validate_file_hash(cls, value: str) -> str:
        return _sha256(value, field_name="file_sha256")
...
class ReleaseSchemaContract(StrictModel):
    ...
    def _validate_contract(self) -> "ReleaseSchemaContract":
        ...
        parent = (self.partition_contract.schema, self.partition_contract.parent_relation)
    ...
    def canonical_payload(self) -> dict[str, Any]:
        return self.model_dump(mode="python", exclude={"contract_content_hash"})
...
class PendingDdlOperation(StrictModel):
    ...
    def _validate_operation(self) -> "PendingDdlOperation":
        if self.kind == "MIGRATION":
            if self.migration_order is None or any(value is not None for value in (self.partition_name, self.lower_bound, self.upper_bound)):
                raise ValueError("migration operation must contain only migration_order")
        elif self.migration_order is not None or self.partition_name is None or self.lower_bound is None or self.upper_bound is None:
...
class ReleaseSchemaPlanRequest(StrictModel):
    ...
    @field_validator("contract_content_hash", "ddl_session_policy_hash", "capacity_request_hash", "capacity_receipt_hash")
    @classmethod
...
class ReleaseSchemaPlan(StrictModel):
    ...
    @field_validator("contract_content_hash", "ddl_session_policy_hash", "pre_catalog_fingerprint", "expected_final_catalog_fingerprint", "plan_content_hash")
    @classmethod
    ...
    def _validate_plan(self) -> "ReleaseSchemaPlan":
        ...
        expected_downstream = self.managed_schema_status is ManagedSchemaStatus.COMPATIBLE and self.prerequisite_status is PrerequisiteStatus.COMPATIBLE
    ...
    def canonical_payload(self) -> dict[str, Any]:
        return self.model_dump(mode="python", exclude={"plan_content_hash"})
...
class ReleaseSchemaReceipt(StrictModel):
    schema_version: Literal[RECEIPT_SCHEMA_VERSION]
    ...
    @field_validator("request_content_hash", "plan_content_hash", "contract_content_hash", "pre_catalog_fingerprint", "post_catalog_fingerprint", "receipt_content_hash")
    @classmethod
    def _validate_optional_hash(cls, value: str | None, info: Any) -> str | None:
        return _sha256(value, field_name=info.field_name) if value is not None else None
    ...
    def _validate_receipt(self) -> "ReleaseSchemaReceipt":
        ...
        if self.pre_catalog_evidence is not None and self.pre_catalog_fingerprint != self.pre_catalog_evidence.total_sha256:
        ...
        if self.post_catalog_evidence is not None and self.post_catalog_fingerprint != self.post_catalog_evidence.total_sha256:
    ...
    def canonical_payload(self) -> dict[str, Any]:
        return self.model_dump(mode="python", exclude={"receipt_content_hash"})
...
DEFAULT_RELEASE_SCHEMA_REGISTRY = RELEASE_SCHEMA_REGISTRY_ROOT / "advisory_phase1_dataset_foundation_v1.json"
...
def load_release_schema_contract(path: Path | None = None) -> ReleaseSchemaContract:
    ...
    try:
        ...
        raise ReleaseSchemaContractError(REASON_CONTRACT_INVALID, "registry path escapes the repository registry root") from exc
    try:
        payload = json.loads(registry_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise ReleaseSchemaContractError(REASON_CONTRACT_INVALID, f"unable to load release schema registry: {type(exc).__name__}") from exc
    if not isinstance(payload, dict):
        raise ReleaseSchemaContractError(REASON_CONTRACT_INVALID, "release schema registry must contain one JSON object")
    ...
    if supplied_hash != canonical_json_sha256(hash_payload):
        raise ReleaseSchemaContractError(REASON_CONTRACT_HASH_MISMATCH, "release schema registry hash does not match content")
    try:
