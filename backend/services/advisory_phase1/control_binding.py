...
class ControlBindingRequest(BaseModel):
    ...
    governance_scope_hash: str | None = Field(default=None, min_length=64, max_length=64)
    ...
    predecessor_binding_event_hash: str | None = Field(default=None, min_length=64, max_length=64)
    ...
    def _validate_shape(self) -> "ControlBindingRequest":
        if self.control_type is ControlType.TRACE_CAPTURE and not self.admission_scope_set_hash:
            raise ValueError("TRACE_CAPTURE binding requires admission_scope_set_hash")
        if self.binding_event_revision_no == 1 and self.predecessor_binding_event_hash is not None:
            raise ValueError("first control binding revision cannot have a predecessor")
        if self.binding_event_revision_no > 1 and self.predecessor_binding_event_hash is None:
            raise ValueError("non-first control binding revision requires predecessor_binding_event_hash")
        if _canonical_json_sha256(self.config_payload) != self.config_or_store_backend_hash:
            raise ValueError("config_or_store_backend_hash does not match config_payload")
        return self
...
class ControlBindingEvent(BaseModel):
    ...
    @classmethod
    def from_request(cls, request: ControlBindingRequest, *, bound_at: datetime) -> "ControlBindingEvent":
        payload = {**request.request_payload(), "bound_at": bound_at.astimezone(timezone.utc)}
    ...
    def append(self, request: ControlBindingRequest) -> ControlBindingEvent:
        ...
        if request.binding_event_revision_no <= len(chain):
            ...
            if existing.request.append_request_hash == request.append_request_hash:
                return existing
            raise SourceLedgerError(REASON_CONTROL_BINDING_CONFLICT, "same control binding revision has different content")
        if request.binding_event_revision_no != len(chain) + 1:
            raise SourceLedgerError(REASON_CONTROL_BINDING_CHAIN_INVALID, "control binding revision is not the next sequence")
        ...
        if chain:
            ...
            if request.predecessor_binding_event_hash != predecessor.binding_event_hash:
                raise SourceLedgerError(REASON_CONTROL_BINDING_CHAIN_INVALID, "control binding predecessor does not match")
            if (
                predecessor.request.config_or_store_backend_hash == request.config_or_store_backend_hash
                and predecessor.request.enabled == request.enabled
                and predecessor.request.config_source == request.config_source
            ):
                raise SourceLedgerError(REASON_CONTROL_BINDING_CONFLICT, "new control binding revision cannot repeat predecessor content")
        chain.append(event)
        return event

    def current(self, *, control_type: ControlType, environment: str, admission_scope_set_hash: str | None, governance_scope_hash: str | None) -> ControlBindingEvent:
        ...
        if not chain:
            raise SourceLedgerError(REASON_CONTROL_BINDING_UNAVAILABLE, "control binding chain is not configured")
        return chain[-1]
...
class PostgresControlBindingRepository:
    ...
    def append(self, request: ControlBindingRequest) -> ControlBindingEvent:
        with self._conn_factory() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute("SELECT pg_advisory_xact_lock(hashtext(%s))", (request.binding_chain_key,))
                cur.execute(
                    """
                    SELECT * FROM app.advisory_phase1_control_binding_event
                ...
                event = ControlBindingEvent.from_request(request, bound_at=cur.fetchone()["bound_at"])
                try:
                    cur.execute(
                        """
                    ...
                    raise SourceLedgerError(REASON_CONTROL_BINDING_CHAIN_INVALID, "database rejected control binding chain") from exc
    ...
    def get_by_hash(self, binding_event_hash: str) -> ControlBindingEvent:
        with self._conn_factory() as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                cur.execute(
                    "SELECT * FROM app.advisory_phase1_control_binding_event WHERE binding_event_hash = %s",
                    (binding_event_hash,),
                )
                row = cur.fetchone()
        if row is None:
            raise SourceLedgerError(REASON_CONTROL_BINDING_UNAVAILABLE, "control binding event does not exist")
...
def _event_from_row(row: dict[str, Any]) -> ControlBindingEvent:
    request = ControlBindingRequest(
        ...
        admission_scope_set_hash=str(row["admission_scope_set_hash"]) if row["admission_scope_set_hash"] else None,
        governance_scope_hash=str(row["governance_scope_hash"]) if row["governance_scope_hash"] else None,
        ...
        predecessor_binding_event_hash=str(row["predecessor_binding_event_hash"]) if row["predecessor_binding_event_hash"] else None,
    ...
    if event.binding_event_id != str(row["binding_event_id"]) or event.binding_event_hash != str(row["binding_event_hash"]):
        raise SourceLedgerError(REASON_CONTROL_BINDING_CONFLICT, "persisted control binding event does not match canonical hash")
    if request.binding_chain_key != str(row["binding_chain_key"]) or request.append_request_hash != str(row["append_request_hash"]):
        raise SourceLedgerError(REASON_CONTROL_BINDING_CONFLICT, "persisted control binding hashes are invalid")
    return event
