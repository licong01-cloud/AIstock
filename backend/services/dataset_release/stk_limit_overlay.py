"""Bounded missing-only ``stk_limit`` overlay for immutable candidates."""
...
STK_LIMIT_RULE_OVERLAY_SCHEMA = "dataset_release_stk_limit_rule_overlay_v1"
...
def build_stk_limit_rule_overlay(
    *,
    ...
    """Derive exact missing PIT keys while retaining only one code partition."""
    ...
    for code in codes:
        ...
        database_dates = {_row_date(row, "trade_date", "stk_limit") for row in code_limits}
            ...
            if day in missing_dates:
                if point is None or latest_adj is None or latest_adj_date is None:
                    ...
                    try:
                        ...
                        overlay_rows.append(derived.as_source_row())
