...
def is_filtered_stock_pool(stock_pool_path: str | None) -> bool:
    if not stock_pool_path:
        return False
    try:
        _safe_stock_pool_filename(str(stock_pool_path))
    except RuntimeError:
        return False
    return True
...
def _safe_stock_pool_filename(stock_pool_path: str) -> str:
    ...
    if ".." in normalized.split("/"):
        raise RuntimeError("stock_pool path must not contain parent-directory segments")
    filename = normalized.rsplit("/", 1)[-1]
    if not filename.endswith(".txt"):
        filename = f"{filename}.txt"
    if not _FILTERED_POOL_RE.fullmatch(filename) or Path(filename).name != filename:
        raise RuntimeError("stock_pool must be a filtered_pool_* or index_pool__* file name")
    return filename
...
def _read_stock_pool_payload(stock_pool_path: str) -> tuple[Path, str, str, str]:
    ...
    if not local_path.exists() or not local_path.is_file():
        raise RuntimeError(
            f"local filtered stock_pool cache file is missing; generate the pool before submitting QE: {local_path}"
        )
...
def read_stock_pool_snapshot(stock_pool_path: str) -> StockPoolSnapshot:
    ...
    for line_number, raw_line in enumerate(content.splitlines(), start=1):
        ...
        parts = line.split("\t")
        if len(parts) != 3:
            raise RuntimeError(f"stock_pool row {line_number} must contain symbol, start date, and end date")
...
def _remote_instruments_dir(node: dict[str, Any]) -> str:
    ...
    if not qlib_data.startswith("/") or "\x00" in qlib_data or not _LINUX_ABS_PATH_RE.fullmatch(qlib_data):
        raise RuntimeError(f"node {node_id} has invalid qlib_data_path for stock_pool install: {qlib_data!r}")
...
def build_stock_pool_install_command(*, filename: str, remote_instruments_dir: str, expected_sha256: str) -> str:
    ...
    return " && ".join(
        [
            ...
            (f"[ \"$actual_sha\" = {sha_q} ] || {{ echo 'stock_pool source checksum mismatch' >&2; exit 1; }}"),
            f"mkdir -p {dir_q}",
            f"cp -f {src_q} {dest_q}",
            f"final_sha=\"$(sha256sum {dest_q} | cut -d ' ' -f 1)\"",
            (f"[ \"$final_sha\" = {sha_q} ] || {{ echo 'stock_pool installed checksum mismatch' >&2; exit 1; }}"),
        ]
    )
...
def sync_all_filtered_pools_to_remote_node(node: dict[str, Any]) -> dict[str, Any]:
    ...
    files = sorted({*root.glob("filtered_pool_*.txt"), *root.glob("index_pool__*.txt")}) if root.exists() else []
