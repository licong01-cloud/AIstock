...
This module does not compute factor values or create live snapshots. Official
...
class FactorValuePipeline:
    ...
    def get_computable_factors(
        ...
        """Return official-cache computable factors backed by catalog code_text.

        Official factor cache generation uses the original catalog code_text
        and offline backtest data. It must not depend on non-official
        live/simulation transformation status or transformed-file artifacts.
        """
        conditions = [
            "is_available = true",
            "code_text IS NOT NULL",
            "length(trim(code_text)) > 0",
        ]
        params: list = []
        ...
        sql = f"""
            SELECT factor_name, source, factor_type
        ...
        for row in rows:
            ...
            data["code_source"] = "code_text"
            data["code_exists"] = True
            data["cache_source"] = "official_offline_backtest_factor_data"
            results.append(data)
