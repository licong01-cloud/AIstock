...
class MetaModelCombiner:
    ...
    def _ic_weighted(
        ...
        if actual_returns is None:
            ...
            for g_name, g_pred in aligned.items():
                ...
                # Use last lookback_days unique dates
                ...
                try:
                    ...
                    weights[g_name] = max(avg_ic, 0.0)  # negative IC → zero weight
                except Exception as e:
                    logger.warning(f"IC computation failed for {g_name}: {e}")
                    weights[g_name] = 0.0
    ...
    def _ols_ridge(
        ...
        if len(common_idx) < 30:
            logger.warning("Not enough data for OLS, falling back to IC-weighted")
            return self._ic_weighted(group_predictions, actual_returns)
    ...
    def _stacking(
        ...
        try:
            ...
            logger.warning("lightgbm not available, falling back to OLS")
            return self._ols_ridge(group_predictions, actual_returns)
        ...
        if len(common_idx) < 100:
            logger.warning("Not enough data for stacking, falling back to OLS")
            return self._ols_ridge(group_predictions, actual_returns)
    ...
    def _weighted_sum(
        ...
        if combined is None:
            # All weights zero — return first group as fallback
            first = next(iter(aligned.values()))
            return first.to_frame("score")
