...
def list_monitor_tasks(enabled_only: bool = False) -> List[Dict[str, Any]]:
...
def create_monitor_task(task: Dict[str, Any]) -> Dict[str, Any]:
...
def update_monitor_task(stock_code: str, body: Dict[str, Any]) -> Dict[str, Any]:
    if not stock_code:
        raise HTTPException(status_code=400, detail="股票代码不能为空")
...
def delete_monitor_task(task_id: int) -> Dict[str, Any]:
...
def analyze_stock(payload: Dict[str, Any]) -> Dict[str, Any]:
