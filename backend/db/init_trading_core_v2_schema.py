...
DDL: list[str] = [
    ...
    """,
    "COMMENT ON TABLE strategy_pkg.selection_score_artifact IS 'Immutable ranked raw-score artifact for one StrategyPackage inference input context; v2 rows carry an immutable evidence header.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.artifact_id IS 'Stable artifact row identifier; v2 retries return the original row instead of replacing it.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.package_id IS 'StrategyPackage that produced the raw ranked scores.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.manifest_sha256 IS 'Frozen StrategyPackage manifest hash used for inference.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.trade_date IS 'Requested inference trade date; the effective score date is recorded in v2 input-context evidence.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.data_source IS 'Authoritative market data source label used for inference.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.runtime_config_hash IS 'Score-production runtime hash; v2 includes a fixed artifact contract marker but excludes capture mode.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.scores_json IS 'Canonical ordered raw score rows before HMM, risk, and tradability stages.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.artifact_sha256 IS 'SHA256 hash of canonical scores_json; retained for legacy compatibility.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.score_count IS 'Count of raw scored rows stored in scores_json.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.universe_count IS 'Actual authoritative inference input universe count, not a copy of score_count.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.top_score_symbol IS 'Highest-ranked raw score symbol when score_count is positive.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.status IS 'Artifact producer status; SUCCEEDED may represent a proven valid no-candidate raw outcome.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.error_json IS 'Structured producer error context for failed artifacts.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.metadata IS 'Diagnostic and provider metadata; immutable v2 semantic identity is carried by explicit hash columns.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.artifact_contract_version IS 'NULL for legacy rows; selection_score_artifact_v2 enables immutable prospective evidence semantics.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.artifact_payload_sha256 IS 'SHA256 hash of the v2 canonical artifact header; unique for non-legacy rows.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.artifact_input_context_hash IS 'SHA256 hash of raw score/cutoff/calendar input identity without Program or capture context.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.source_revision_set_hash IS 'SHA256 hash of source read receipts used by the v2 raw inference artifact.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.asset_closure_hash IS 'SHA256 hash of frozen package/model/factor asset closure used by the v2 artifact.';",
    "COMMENT ON COLUMN strategy_pkg.selection_score_artifact.created_at IS 'Artifact creation timestamp; not part of immutable semantic hashes.';",
    """
    ...
    """,
    "COMMENT ON TABLE selection.daily_selection_evidence IS 'Immutable broker-neutral DailySelectionEvidence generated by the shared StrategyPackageSelectionService before target, rebalance, execution or broker logic. Legacy v1 remains readable; v2 carries prospective evidence lineage.';",
    ...
    "COMMENT ON COLUMN selection.daily_selection_evidence.evidence_payload_json IS 'Canonical JSON payload. v1 remains readable; daily_selection_evidence_v2 carries typed prospective decision clock, config, source, asset and stage lineage; must not contain self evidence id/hash or broker/account/capital/order fields.';",
    ...
    "CREATE UNIQUE INDEX IF NOT EXISTS ux_strategy_pkg_selection_artifact_v2_payload ON strategy_pkg.selection_score_artifact(artifact_payload_sha256) WHERE artifact_payload_sha256 IS NOT NULL",
    ...
    "CREATE UNIQUE INDEX IF NOT EXISTS idx_paper_v2_broker_account_binding_active_legacy_account ON paper_v2.broker_account_binding(broker_backend, broker_mode, broker_account_id) WHERE binding_status = 'ACTIVE' AND allocation_mode <> 'account_group_slots'",
    "CREATE UNIQUE INDEX IF NOT EXISTS idx_paper_v2_broker_account_binding_active_account_slot ON paper_v2.broker_account_binding(broker_backend, broker_mode, broker_account_id, account_group_id, strategy_slot_id) WHERE binding_status = 'ACTIVE' AND allocation_mode = 'account_group_slots'",
    "CREATE UNIQUE INDEX IF NOT EXISTS idx_paper_v2_broker_account_binding_active_portfolio ON paper_v2.broker_account_binding(portfolio_id) WHERE binding_status = 'ACTIVE'",
    "CREATE INDEX IF NOT EXISTS idx_paper_v2_broker_account_binding_account_group ON paper_v2.broker_account_binding(broker_backend, broker_mode, broker_account_id, account_group_id, updated_at DESC) WHERE binding_status = 'ACTIVE' AND allocation_mode = 'account_group_slots'",
    ...
    "CREATE INDEX IF NOT EXISTS idx_paper_v2_simulation_release_binding_account_slot ON paper_v2.simulation_release_binding(broker_backend, account_group_id, strategy_slot_id, approval_state) WHERE account_group_id IS NOT NULL OR strategy_slot_id IS NOT NULL",
    ...
    "CREATE INDEX IF NOT EXISTS idx_paper_v2_simulation_daily_run_account_slot_date ON paper_v2.simulation_daily_run(broker_backend, account_group_id, strategy_slot_id, trade_date DESC) WHERE account_group_id IS NOT NULL OR strategy_slot_id IS NOT NULL",
