...
def resolve_modules(
    *,
    ...
    """Resolve Phase 1 modules and reject future-only profiles explicitly."""
    ...
    if explicit_modules is not None:
        ...
        future = [name for name in explicit_modules if name not in _PHASE1_MODULES]
        if future:
            raise ValueError(f"Modules {future!r} are future-only in Phase 1")
        if explicit_modules != ["research"]:
            raise ValueError("Phase 1 only allows --modules=research")
    ...
    if selected in FUTURE_PROFILES:
        raise ValueError(f"Profile {selected!r} is future-only in Phase 1")
    raise ValueError(f"Unknown MCP profile {selected!r}; allowed Phase 1 profile: 'research'")
