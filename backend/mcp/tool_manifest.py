...
MODULE_TOOL_NAMES: dict[str, tuple[str, ...]] = {
    "advisory": (
        "advisory_list_programs",
    ...
    "simulation_runtime_monitoring": (
        "simulation_runtime_monitoring_cutover_readiness",
    ...
    "qmt_broker_monitoring": (
        "qmt_broker_monitoring_get_status",
    ...
    "selection_center": (
        "selection_center_list_selectable_packages",
...
MODULE_PROFILE_TAGS: dict[str, tuple[str, ...]] = {
    ...
    "strategy_packages": ("strategy_package_ops", "simulation_ops", "simulation_stable", "full"),
    "selection_center": ("selection_advisory", "simulation_ops", "simulation_stable", "full"),
    "advisory": ("selection_advisory", "simulation_ops", "simulation_stable", "full"),
    "simulation_runtime_monitoring": ("simulation_runtime_monitor", "simulation_ops", "simulation_stable", "full"),
    "qmt_broker_monitoring": ("simulation_runtime_monitor", "simulation_ops", "simulation_stable", "full"),
...
TOOL_METADATA_OVERRIDES: dict[str, ToolMetadataOverride] = {
    ...
    "simulation_runtime_monitoring_scheduler_status": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only GET evidence: simulation_runtime_monitoring exposes GET /scheduler/status",
    ),
    "simulation_runtime_monitoring_scheduler_verification": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only GET evidence: simulation_runtime_monitoring exposes GET /scheduler/verification-status",
    ),
    "simulation_runtime_monitoring_list_bindings": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only GET evidence: simulation_runtime_monitoring exposes GET /localsim/accounts/{account_id}/bindings",
    ),
    ...
    "qlib_monthly_release_plan": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only preview: delegates to the unified monthly release plan API and does not persist an operation",
    ),
    "qlib_monthly_release_status": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only durable operation status from the unified monthly release API",
    ),
    "qlib_monthly_release_receipts": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only bounded receipt index from the unified monthly release API",
    ),
    "qlib_monthly_release_submit_confirmed": ToolMetadataOverride(
        risk_level="write_confirmed",
        assistant_usable="preflight_required",
        requires_confirmation=True,
        reason="confirmed durable monthly release submission; data work remains owned by the registered worker",
    ),
    "qlib_monthly_release_resume_confirmed": ToolMetadataOverride(
        risk_level="write_confirmed",
        assistant_usable="preflight_required",
        requires_confirmation=True,
        reason="confirmed state transition for a durable unified monthly release",
    ),
    "qlib_monthly_release_cancel_confirmed": ToolMetadataOverride(
        risk_level="write_confirmed",
        assistant_usable="preflight_required",
        requires_confirmation=True,
        reason="confirmed cooperative cancellation request for a durable unified monthly release",
    ),
    "qlib_monthly_release_activate_confirmed": ToolMetadataOverride(
        risk_level="production_adjacent",
        assistant_usable="preflight_required",
        requires_confirmation=True,
        reason="confirmed active-profile CAS requiring a separately issued dataset action authorization",
    ),
    "qlib_monthly_release_rollback_confirmed": ToolMetadataOverride(
        risk_level="production_adjacent",
        assistant_usable="preflight_required",
        requires_confirmation=True,
        reason="confirmed active-profile rollback CAS requiring a separately issued dataset action authorization",
    ),
    ...
    "multi_alpha_combine_backtest_archive_detail_get": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only GET evidence: backend/mcp/modules/qe_archive.py reads immutable multi-alpha Archive detail including recovery child/attempt snapshots",
    ),
    ...
    "multi_alpha_combine_backtest_controls_get": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only GET control capabilities exposes durable state and evidence only",
    ),
    "multi_alpha_combine_backtest_children_list": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only GET returns durable child identities and statuses",
    ),
    "multi_alpha_combine_backtest_child_get": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only GET returns one child and append-only attempt evidence",
    ),
    "multi_alpha_combine_backtest_child_attempts_list": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only GET lists one durable child attempt lineage",
    ),
    "multi_alpha_combine_backtest_commands_list": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only GET lists durable command ledger entries",
    ),
    "multi_alpha_combine_backtest_command_get": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="read-only GET returns one durable command state",
    ),
    "multi_alpha_combine_backtest_recovery_preview": ToolMetadataOverride(
        risk_level="read_only",
        assistant_usable="direct_or_catalog",
        requires_confirmation=False,
        reason="POST preview only computes frozen recovery topology and evidence without a durable mutation",
    ),
    "multi_alpha_combine_backtest_pause": ToolMetadataOverride(
        risk_level="production_adjacent",
        assistant_usable="preflight_required",
        requires_confirmation=False,
        reason="QE-only durable pause command mutation uses required idempotency and no feature-specific confirmation",
    ),
    "multi_alpha_combine_backtest_resume": ToolMetadataOverride(
        risk_level="production_adjacent",
        assistant_usable="preflight_required",
        requires_confirmation=False,
        reason="QE-only durable resume command mutation uses required idempotency and no feature-specific confirmation",
    ),
    "multi_alpha_combine_backtest_cancel": ToolMetadataOverride(
        risk_level="production_adjacent",
        assistant_usable="preflight_required",
        requires_confirmation=False,
        reason="QE-only durable cancel intent uses required idempotency and async typed receipt reconciliation",
    ),
    "multi_alpha_combine_backtest_stop": ToolMetadataOverride(
        risk_level="production_adjacent",
        assistant_usable="preflight_required",
        requires_confirmation=False,
        reason="QE-only stop is the durable cancel alias and has no feature-specific confirmation",
    ),
    "multi_alpha_combine_backtest_reconcile": ToolMetadataOverride(
        risk_level="production_adjacent",
        assistant_usable="preflight_required",
        requires_confirmation=False,
        reason="QE-only reconcile writes an idempotent durable observation command only",
    ),
    "multi_alpha_combine_backtest_attempt_cancel": ToolMetadataOverride(
        risk_level="production_adjacent",
        assistant_usable="preflight_required",
        requires_confirmation=False,
        reason="QE-only exact-attempt cancel uses required idempotency and no feature-specific confirmation",
    ),
    "multi_alpha_combine_backtest_child_recovery_execute": ToolMetadataOverride(
        risk_level="production_adjacent",
        assistant_usable="preflight_required",
        requires_confirmation=False,
        reason="QE-only child recovery persists the previewed immutable scope with idempotency and no approval state",
    ),
...
def _backend_endpoint_for(tool_name: str, module: str) -> str:
    ...
    if module == "simulation_runtime_monitoring":
...
def validate_manifest(entries: Iterable[ToolManifestEntry] = TOOL_MANIFEST) -> list[str]:
    ...
    for tool_name, override in sorted(TOOL_METADATA_OVERRIDES.items()):
        if (
            tool_name not in MODULE_TOOL_NAMES.get("catalog", ())
            and tool_name not in entry_names
            and entries is TOOL_MANIFEST
        ):
    ...
    for entry in entry_list:
        ...
        if (
            _has_side_effect_name_token(entry.tool_name)
            and entry.risk_level == "read_only"
            and not _has_read_only_override_exemption(entry.tool_name)
        ):
            errors.append(
                f"side-effect-looking tool is read_only without explicit read-only override: {entry.tool_name}"
            )
    return errors
