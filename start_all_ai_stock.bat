...
if %errorlevel%==0 (
  wt -w 0 new-tab --title "TDX Go Backend" cmd /k "chcp 65001>nul & cd /d %AIROOT%\tdx-api-main\web & set TDX_HTTP_PORT=19080 & go run ." ^
    ; new-tab --title "AIstock Backend" cmd /k "chcp 65001>nul & cd /d %AIROOT% & call conda activate AIstock & uvicorn backend.main:app --host 0.0.0.0 --port 8001" ^
...
start "AIstock Backend" cmd /k "chcp 65001>nul & cd /d %AIROOT% & call conda activate AIstock & uvicorn backend.main:app --host 0.0.0.0 --port 8001"
