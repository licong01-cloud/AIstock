...
# Task Plan: PT-NEXT-023 R8-only Core/Tactical Timing Research (2026-09-19)

## Goal
Without adding data sources or changing online behavior, diagnose PT-NEXT-022's timing shortfall and test a frozen R8-only core/tactical policy plus bounded screening proxies against same-stock BUY_AND_HOLD under the existing independent cash-account and A-share execution contract.

## Current Phase
Phase 2: pure implementation and deterministic benchmark.

## Phases
- [x] Phase 1: write F1 design, freeze hypotheses/screens/policies, validate design.
- [ ] Phase 2: implement attribution, proxy screens, and core/tactical replay only inside position_timing plus direct tests.
- [ ] Phase 3: run targeted tests, 1-vs-N determinism, and multi-round code/design review.
- [ ] Phase 4: run WSL R8 prepare -> run -> inspect -> exact retry and analyze results.
- [ ] Phase 5: update design/result evidence, run F1/F2 validation and local merge gate, then prepare commit/PR.

## Frozen Boundaries
- R8-only; no new provider, DB query/write, download, candidate/profile activation, or service control.
- Do not alter PT-NEXT-022 immutable artifacts or online L1/L1a behavior.
- Do not modify QE, HMM, Selection, Advisory, Paper, MiniQMT, or local_data.
- Strict P2/P3 remain unavailable; bak_basic use, if accepted by source audit, is named an exploratory daily-snapshot proxy and never a financial PIT substitute.
- No parameter sweep. New economic hypotheses are frozen before outcomes are read.
- Backend restart remains user-owned and is not expected for this offline task.

## Decisions Made
| Decision | Rationale |
|----------|-----------|
| F1 independent research namespace | Existing PT-NEXT-022 evidence is immutable and must remain reproducible |
| Diagnose exposure/exit/re-entry attribution before interpreting new results | Prevents threshold tuning from replacing causal failure analysis |
| True core exposure is distinct from T2's post-entry 70/30 holdings split | Current T2 can fully exit and remain cash, so it is not a persistent core overlay |
| Screens and timing contribution are reported separately | A better selected universe does not prove timing beats same-stock holding |

## Errors Encountered
| Error | Attempt | Resolution |
|-------|---------|------------|
| Existing repository planning files contain many prior task sections | 1 | Append a uniquely named PT-NEXT-023 section and avoid rewriting prior entries |
| Bash heredoc syntax was sent to PowerShell for a read-only HDF probe | 1 | Use a PowerShell here-string piped to `python -`; do not repeat the unsupported redirection |
| Baseline attribution probe requested nonexistent `mean_timing_exposure`/`mean_hold_exposure` columns | 1 | Use the actual sealed stock columns `average_exposure`, `conditional_exposure`, and `invested_session_fraction`; retain all earlier successful output |
...
| First full WSL chunk stopped on `000505.SZ` with a floor gap of `-1e-22` virtual units | 1 | Preserve fail-closed behavior for real crossings, but normalize Decimal arithmetic noise only within a frozen `1e-12` virtual-unit tolerance; add tests for both the noise case and a real violation |
| First inline WSL locator attempt lost the Python `-c` argument through nested quoting | 1 | Pipe a PowerShell here-string to native WSL Python stdin; no source/data/artifact changed by the failed command |
### PT-NEXT-023 execution log update (2026-09-19)

- [x] Harden zero-enrollment report generation.
- [x] Run compile, Ruff, F1 validator and combined direct-regression suite.
- [x] Exercise a genuinely enrolled symbol against physical R8 inputs.
- [x] Remove repeated sub-board-lot tactical sell attempts and freeze the regression assertion.
- [ ] Complete implementation review, immutable commit, native-WSL deterministic verification and full formal replay.
