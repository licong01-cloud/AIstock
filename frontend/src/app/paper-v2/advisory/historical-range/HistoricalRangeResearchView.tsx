  ...
  return (
    <div className="ahr-shell" data-testid="historical-range-view">
      ...
      <section className="ahr-section">
        ...
        <div className="ahr-table-wrap"><table className="ahr-table"><thead><tr><th>Batch</th><th>日期</th><th>Programs</th><th>状态</th><th>当前命令</th><th>日进度</th><th>恢复项</th><th>创建时间</th><th>操作</th></tr></thead><tbody>
          {model.batches.map((batch) => <tr key={String(batch.batch_id)} className={model.selectedBatch?.batch_id === batch.batch_id ? "is-selected" : ""}>
            <td><code>{String(batch.batch_id)}</code></td><td>{String(batch.start_trade_date)} → {String(batch.end_trade_date)}</td><td>{String(batch.program_count)}</td><td><span className={statusClass(batch.status)}>{String(batch.status)}</span></td><td>{batch.current_operation_status ? <><small>{String(batch.current_operation_type)}</small><span className={statusClass(batch.current_operation_status)}>{String(batch.current_operation_status)}</span></> : "-"}</td><td>{String(batch.successful_day_count || 0)} / {String(batch.planned_day_count || 0)}</td><td>{String(batch.recoverable_program_count || 0)}</td><td>{String(batch.created_at || "-")}</td><td><button className="ahr-row-button" title="查看" aria-label="查看任务" onClick={() => void model.selectBatch(String(batch.batch_id))} type="button"><Eye size={16} /></button></td>
          </tr>)}
          {!model.batches.length && !model.loading ? <tr><td colSpan={9}><div className="ahr-state">尚无历史验证 batch；合法空列表不是请求失败。</div></td></tr> : null}
        </tbody></table></div>
      ...
      {selectedBatch ? <section className="ahr-section ahr-detail" data-testid="historical-range-detail">
        ...
        {model.activeOperation ? <div className="ahr-operation"><span>本次命令</span><code>{value(model.activeOperation, "operation_id")}</code><strong className={statusClass(model.activeOperation.status)}>{value(model.activeOperation, "status")}</strong>{model.activeOperation.result_status ? <span>Receipt: <strong>{String(model.activeOperation.result_status)}</strong></span> : null}{record(model.activeOperation.snapshot)?.snapshot_id ? <span>Dataset snapshot: <strong>{String(record(model.activeOperation.snapshot)?.snapshot_id)}</strong> / {String(record(model.activeOperation.snapshot)?.status)}</span> : null}{record(model.activeOperation.bridge_receipt)?.result_status === "VALID_EMPTY" ? <span>Dataset bridge: <strong>VALID_EMPTY</strong>（权威空结果 receipt）</span> : null}{model.activeOperation.result_ref ? <span>证据: {structuredValue(record(model.activeOperation.result_ref)?.semantic_content_hash || model.activeOperation.result_ref)}</span> : null}{model.activeOperation.error_json ? <div className="ahr-operation-error"><strong>{String(record(model.activeOperation.error_json)?.reason_code || "命令失败")}</strong><span>{String(record(model.activeOperation.error_json)?.message || "失败 receipt 已持久化；batch 主状态不会覆盖此错误。")}</span>{record(model.activeOperation.error_json)?.context ? <span>{structuredValue(record(model.activeOperation.error_json)?.context)}</span> : null}</div> : null}</div> : null}
      ...
      {model.selectedRun ? <section className="ahr-section">
        ...
        <div className="ahr-summary"><h3>Summary 与 Dataset</h3>{model.summaries.length ? model.summaries.map((summary) => { const metrics = nestedRows(summary, "summary_json", "metrics"); const unavailable = nestedRows(summary, "summary_json", "unavailable_metrics"); return <div className="ahr-summary-card" key={String(summary.summary_id)}><strong>Summary v{String(summary.summary_version)}</strong><span>Outcome set {String(summary.covered_outcome_set_hash)}</span><div className="ahr-summary-metrics">{metrics.map((item, index) => <span key={`${metricText(item)}:${index}`}>{metricText(item)}</span>)}{unavailable.map((item, index) => <span className="is-unavailable" key={`unavailable:${index}`}>{metricText(item)}</span>)}</div></div>; }) : <div className="ahr-state">暂无 Summary。Dataset 的 VALID_EMPTY 会作为有效 receipt 显示，不会伪装成失败。</div>}{model.summaryPage.has_more ? <button className="pv2-button" onClick={() => void model.loadMoreSummaries()} type="button">加载更多 Summary</button> : null}</div>
