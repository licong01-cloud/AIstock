...
type Props = {
  ...
  onTabChange: (tab: "original" | "non_official") => void;
  onClose: () => void;
};
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b">
          ...
          <div className="flex items-center gap-3">
            <div className="flex rounded-lg border overflow-hidden">
              ...
              <button
                onClick={() => onTabChange("non_official")}
                className={`px-3 py-1.5 text-sm font-medium transition-colors ${
                  tab === "non_official"
        ...
        <div className="flex-1 overflow-auto p-4 flex flex-col gap-2">
          ...
          {tab === "non_official" && data.non_official_code_path && (
            <div className="flex items-center gap-2 px-3 py-2 bg-green-50 border border-green-200 rounded-lg">
              <span className="text-xs text-green-600 font-medium shrink-0">📄 非官方改造文件路径:</span>
              <span className="text-xs font-mono text-green-800 truncate" title={data.non_official_code_path}>
                {data.non_official_code_path}
              </span>
          ...
          {tab === "non_official" && data._transformed_code_error && (
            <div className="px-3 py-2 bg-red-50 border border-red-200 rounded-lg">
          ...
          <pre className="text-xs font-mono bg-gray-900 text-gray-100 p-4 rounded-lg overflow-auto whitespace-pre-wrap min-h-32 flex-1">
            {tab === "original"
              ...
              : data.transformed_code_text || "（尚未改造或改造失败，非官方改造文件不存在）"}
          </pre>
        </div>
      </div>
    </div>
